  ...
  mlambda(fmlambda), sruth(fsruth), mthetac(fmthetac)
{
  ...
  betta  = DynLinArr< double >(qe);
  betta2 = DynLinArr< double >(qe);
  ...
  for(ne=0; ne<qe; ne++)
  {
    ...
    betta[ne] = lorbetta(gamma_1);
    betta2[ne] = betta[ne] * betta[ne];
  ...
  for(ne=1; ne<qe; ne++)
  {
    double d = (eLoss[ne] * betta[ne] - eLoss[ne-1] * betta[ne-1] )/
  ...
  {
    eLoss[ne] = (eLoss[n_deriv_min-1] * betta[n_deriv_min-1] + deriv_min * 
      (hmd->energy_mesh->get_ec(ne) - 
       hmd->energy_mesh->get_ec(n_deriv_min-1)))/betta[ne];
  }
...
(double energy, double nscat) const
{
