...
#endif
{
  ...
  double coefpa = double(particle_charge * particle_charge) /
                  (FSCON * betta2 * M_PI);
  for (ne = 0; ne < qe; ne++) {
    r0 = 1.0 + hmd->epsi1[ne];
  ...
  for(ne=0; ne<qe; ne++)
  {
    ...
    for(na=0; na<qa; na++)
    {
      ...
      for(ns=0; ns<qs; ns++)
      {
        ...
        //     <<" r1="<<r1<<" r2="<<r2
        //     <<" frezer[na][ns][ne]="<<frezer[na][ns][ne] << std::endl; 
        //adda[na][ns][ne] = r1 + r2 + frezer[na][ns][ne];
    ...
    addaC[ne] = s;
    //mcout<<"ne="<<ne<<" ec="<<ec<<" addaC[ne]="<<addaC[ne] << std::endl; 
#ifndef EXCLUDE_A_VALUES
    addaC_a[ne] = s_a;
#endif
  }
    ...
    if(l > 4) {
      ...
      for (ne = 0; ne < qe; ne++) {
        Ifile << std::setw(12) << hmd->energy_mesh->get_ec(ne)
              << std::setw(12) << log1C[ne]
              << std::setw(12) << log2C[ne]
              << std::setw(12) << chereC[ne]
              << std::setw(12) << addaC[ne]
              << std::setw(12) << chereCangle[ne]
              << std::setw(12) << Rreser[ne]
#ifdef DEBUG_EnTransfCS
          ...
          if (l > 5) {
            ...
            for (ne = 0; ne < qe; ne++) {
              ...
              //    << std::setw(12) << rezer[na][ns][ne]
                    << std::setw(12) << frezer[na][ns][ne]
                    << std::setw(12) << adda[na][ns][ne]
#ifndef EXCLUDE_A_VALUES
...
{
  mfunname("std::ostream& operator << (std::ostream& file, const EnTransfCSType& f)");
  if(f.etcs.get() == NULL) {
