  ...
  s_print_listing(fs_print_listing)
{
  ...
  check_econd11( fabs(length_vel-mparticle::speed_of_light)/
		 (length_vel+mparticle::speed_of_light),
		 > 1.0e-10 , mcerr);
}
...
void HeedPhoton::physics(void)
{
  ...
  if(s_photon_absorbed == 0)  // still not absorbed
  {
	...
	for(na=0; na<qa; na++)
	{
	  ...
	  for(ns=0; ns<qs; ns++)
	  {
	    cs[nst] = hmd->apacs[na]->get_ICS(ns, energy) * 
	      at_weight_quan;
	...
	if(xleng*cm < nextpos.prange)
	{
	  ...
	  nextpos.ptloc = nextpos.pt; nextpos.tid.up_absref(&nextpos.ptloc);
	}
      }
    }
  }
...
void HeedPhoton::physics_after_new_speed(void)
{
  ...
  //if(s_photon_absorbed == 1)
  {
    const absvol* av = currpos.G_lavol(); // get least address of volume
    //const EnTransfCSType* etcst = dynamic_cast< const EnTransfCSType*  >(av);
    //check_econd11(etcst ,  ==  NULL, mcerr); 
    //const HeedMatterDef* hmd =  etcst->etcs->hmd;
    ...
    hmd->apacs[na_absorbing]->get_escape_particles
      (ns_absorbing, energy, el_energy, ph_energy);
    if(s_print_listing == 1)
    {
      mcout<<"The condition:\n";
	...
	vel.random_sfer_vec();
      ...
      double mod_v = betta * mparticle::speed_of_light;
      vel = vel * mod_v;
      //Iprint(mcout, currpos.pt);
      if(s_print_listing == 1)
      {
	...
	Iprint4n(mcout, el_energy[nel], gam_1, betta, mod_v);
      ...
      ac.pass(new HeedDeltaElectron(currpos.tid.eid[0].amvol.getver(), 
				    currpos.pt, 
				    vel,
				    currpos.time,
				    particle_number) );
	...
	Iprint2n(mcout, el_energy[nph], vel);
      ...
      ActivePtr< gparticle > ac;
      ac.pass(new  HeedPhoton(currpos.tid.eid[0].amvol.getver(), 
			      currpos.pt,
			      vel,
			      currpos.time,
			      particle_number,
