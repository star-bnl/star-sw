...
int sign_nonlinear_interpolation(double e1, double cs1, 
				 double e2, double cs2, double threshold)
{
  ...
  // normal case:
  if(cs2 >= cs1 || cs2 <= 0 
     || e1 < 300.0e-6 || e1 < 1.5 * threshold )
    //if(cs2 >= cs1 || cs2 <= 0)  // for debug
    return 0;
  else
  {
    double pw = log(cs1/cs2) / log(e1/e2);
    //Iprintn(mcout, pw);
    //if( pw >= -1.0 || pw < 5.0)  old wrong, always true
    if( pw >= -1.0 ||  // good case for linear interpolation 
	pw < -5.0)     // unclear odd case, power would be dangerous
      return 0;        // linear
    else
      return 1;        // non linear
  }
  return 0; // to make sure
#endif
}
...
{
  double res;
  if(sign_nonlinear_interpolation(xp1, yp1,
				  xp2, yp2, xmin) == 1)
    res = t_integ_power_2point<double>
      (xp1, yp1, xp2, yp2,   
       x1, x2);
  else
    res = t_integ_straight_2point<double>
      (xp1, yp1, xp2, yp2,   
       x1, x2,
       0,  
       1);
...
double my_val_fun( double xp1, double yp1, double xp2, double yp2, 
		   double xmin, double xmax, double x)
{
  ...
  if(sign_nonlinear_interpolation(xp1, yp1,
				  xp2, yp2, xmin) == 1)
  {
    //mcout<<"NON linear interp\n"; 
    res = t_value_power_2point<double>
      (xp1, yp1, xp2, yp2,   
       x);
  }
  else
  {
    //mcout<<"linear interp\n"; 
    res = t_value_straight_2point<double>
      (xp1, yp1, xp2, yp2,   
       x,
       1);
  }
...
double glin_integ_ar(DynLinArr< double > x, DynLinArr< double > y, 
...
// fit table by a straight line and integrate the area below it.
{
  ...
  double s;
  s = t_integ_generic_point_ar< double, DynLinArr< double >,
      PointCoorMesh< double, DynLinArr< double > >  >
      (pcmd, y, &my_integr_fun, x1, x2, 
       1, threshold, 0, DBL_MAX);
  /*
  // compare with old version
  double old_res = old_glin_integ_ar(x, y, 
				 q, x1, x2,
				 threshold );
  if(!apeq(s, old_res))
  {
...
/*
double glin_val_ar(DynLinArr< double > x, DynLinArr< double > y, 
		   long q, double ex,
		     double threshold )
// fit table by a straight line and integrate the area below it.
{
  ...
  PointCoorMesh< double, DynLinArr< double > > pcmd( q, &x);
  double s;
  s = t_value_generic_point_ar< double, DynLinArr< double >,
      PointCoorMesh< double, DynLinArr< double > >  >
...
void PhotoAbsCS::print(ostream& file, int l) const
{
  if(l > 0)
    Ifile<<"PhotoAbsCS: name="<<name
	 <<" Z = "<<Z<<" threshold = "<<threshold<<endl;
}
...
double OveragePhotoAbsCS::get_integral_CS(double energy1, double energy2) const
{
  mfunname("double OveragePhotoAbsCS::get_integral_CS(double energy1, double energy2) const");
...
void OveragePhotoAbsCS::print(ostream& file, int l) const
{
  mfunname("void PhotoAbsCS::print(ostream& file, int l) const");
  Ifile<<"OveragePhotoAbsCS: width = "<<width
       <<" step="<<step<<" max_q_step="<<max_q_step<<'\n';
  indn.n+=2;
...
HydrogenPhotoAbsCS::HydrogenPhotoAbsCS(void): PhotoAbsCS("H", 1, 15.43e-6)
{;}
...
double HydrogenPhotoAbsCS::get_CS(double energy) const
{
  ...
  else
  {
	...
	0.0535*(pow (100.0e-6/energy  , 3.228) );
    }
    else
    {
      return 0.0;
...
double HydrogenPhotoAbsCS::get_integral_CS(double energy1, double energy2) 
const
{
...
void HydrogenPhotoAbsCS::scale(double fact)
{
  mfunnamep("void HydrogenPhotoAbsCS::scale(double fact)");
  funnw.ehdr(mcout);
  mcerr<<"For hydrogen it is not provided since all parameters are constant\n";
  spexit(mcerr);
}



void HydrogenPhotoAbsCS::print(ostream& file, int l) const
{
  if(l > 0)
    Ifile<<"HydrogenPhotoAbsCS: name="<<name
	 <<" Z = "<<Z<<" threshold = "<<threshold<<endl;
}

    
SimpleTablePhotoAbsCS::SimpleTablePhotoAbsCS
(const String& fname, int fZ, double fthreshold, const String& ffile_name): 
  ...
  file_name(ffile_name)
{
  ...
  while( 1 );
  if(q != ener.get_qel() )
  {            // reducing the size if necessary
    ener.put_qel(q);
    cs.put_qel(q);
  }
...
SimpleTablePhotoAbsCS::SimpleTablePhotoAbsCS
(const SimpleTablePhotoAbsCS& total, 
 const SimpleTablePhotoAbsCS& part, 
 double emax_repl)
{
  mfunname("SimpleTablePhotoAbsCS::SimpleTablePhotoAbsCS (const SimpleTablePhotoAbsCS& total,...)");
...
double SimpleTablePhotoAbsCS::get_CS(double energy) const
{
  ...
  else
  {
	...
	{  // linear case
	  //mcout<<"linear\n";
	  //Iprintn(mcout, cs[i1] + (energy - ener[i1])*(cs[i2] - cs[i1])/(ener[i2] - ener[i1]) );
	  return cs[i1] + (energy - 
			   ener[i1])*(cs[i2] - cs[i1])/(ener[i2] - ener[i1]);
	}
	else
...
double SimpleTablePhotoAbsCS::get_integral_CS(double energy1, double energy2 )
const
{
  mfunname("double SimpleTablePhotoAbsCS::get_integral_CS(...)");
  //mcout<<"SimpleTablePhotoAbsCS::get_integral_CS started\n";
  ...
  else
  {
    ...
    if(energy1 < energy21)
    {
      ...
      s = glin_integ_ar(ener, cs, 
			ener.get_qel(), energy1, energy21, threshold);
    }
    //print(mcout, 3);
...
void SimpleTablePhotoAbsCS::print(ostream& file, int l) const
{
  if(l>0)
  {
    Ifile<<"SimpleTablePhotoAbsCS: name="<<name
	 <<" Z = "<<Z<<endl;
    Ifile<<" threshold = "<<threshold
	 <<" file_name="<<file_name<<endl;
    if(l>1)
    {
      indn.n+=2;
      long n;
      for( n=0; n<ener.get_qel(); n++)
      {
	Ifile<<"n="<<n<<" ener="<<ener[n]<<" cs="<<cs[n]<<endl;
      }
      indn.n-=2;
    }
  }
...
PhenoPhotoAbsCS::PhenoPhotoAbsCS(void): PhotoAbsCS("none",0, 0.0), power(0.0)
{;}
  ...
  PhotoAbsCS(fname, fZ, fthreshold), power(fpower)
{
  ...
  check_econd11a( power , <= 2 , 
		  " cannot be so, otherwise the integral is infinite", 
		  mcerr);
  factor = pow( threshold , power-1.0) * Thomas_sum_rule_const_Mb * Z * 
    (power-1.0);
}
...
void PhenoPhotoAbsCS::print(ostream& file, int l) const
{
  if(l>0)
  {
    Ifile<<"PhenoPhotoAbsCS: name="<<name<<" Z = "<<Z<<endl;
    Ifile<<" threshold = "<<threshold
  ...
  PhotoAbsCS(fname, fZ, 0.0) 
{
  ...
  name = fname;
#ifdef USE_STLSTRING
  ifstream BT_file(name.c_str());
#else
  ifstream BT_file(name);
#endif
  if( !BT_file )
  {
    funnw.ehdr(mcerr);
    mcerr<<"cannot open file "<<name<<endl;
    spexit(mcerr);
  }
  int i = 0;
  ...
  funnw.ehdr(mcerr);
  mcerr<<"there is no element Z="<<fZ<<" in file "<<threshold_file_name<<endl;
  spexit(mcerr);
 mark1:
 ...
 DynLinArr< double >& fphoton_energy) const
{
  ...
  return ir;
      /*
      if(channel_prob_dens.get_qel() == 2)
      {
	if(rn < channel_prob[0])
	{
	  felectron_energy = electron_energy[0];
	  fphoton_energy = photon_energy[0];
	}
	...
	{
	  felectron_energy = electron_energy[1];
	  fphoton_energy = photon_energy[1];
	}
      }
      else
      {
	double r = chisran(rn, channel_prob);
	long nch = long(r);
	felectron_energy = electron_energy[nch];
	fphoton_energy = photon_energy[nch];
      }
    }
    return 1;
  }
...
void AtomicSecondaryProducts::print(ostream& file, int l) const
{
  if(l > 0)
  {
...
AtomPhotoAbsCS::AtomPhotoAbsCS(void): name("none"), Z(0), qshell(0) {;}

double AtomPhotoAbsCS::get_TICS
(double energy,
 double factual_minimal_threshold) const
{
  mfunname("double AtomPhotoAbsCS::get_TICS(double energy, double factual_minimal_threshold) const");
...
(double energy1, double energy2,
 double factual_minimal_threshold) const
{
  mfunname("double AtomPhotoAbsCS::get_integral_TICS(double energy1, double energy2, double factual_minimal_threshold) const");
...
(int nshell, 
 ...
 double factual_minimal_threshold) const
{
  mfunname("double AtomPhotoAbsCS::get_TICS(int nshell, double energy, double factual_minimal_threshold) const");
  
  if(s_ignore_shell[nshell] == 0)
  {
    if(factual_minimal_threshold <= energy) 
      // All what is absorbed, should ionize
      return get_integral_ACS(nshell, 
			      energy);
    else
      return 0.0;
  }
...
(int nshell, 
 ...
 double factual_minimal_threshold) const
{
  mfunname("double AtomPhotoAbsCS::get_integral_TICS(int nshell, double energy1, double energy2, double factual_minimal_threshold) const");
  ...
  if(s_ignore_shell[nshell] == 0)
  {
    ...
    return 
      get_integral_ACS(nshell, 
		       factual_minimal_threshold, energy2);
  }
  else
    return 0.0;
}
...
void AtomPhotoAbsCS::print(ostream& file, int l) const
{
  mfunnamep("void AtomPhotoAbsCS::print(ostream& file, int l) const");
  if(l > 0)
  {
    Ifile<<"AtomPhotoAbsCS(l="<<l<<"): name="<<name
	 <<" Z = "<<Z<<" qshell = "<<qshell<<endl;
...
double AtomPhotoAbsCS::get_I_min(void) const
{
  ...
  double st = DBL_MAX;
  for(n=0; n<qshell; n++)
  {  // currently the minimal shell is the last, but to avoid this assumption
    // we check all
					  ...
					  DynLinArr< double >& ph_energy) const
{
  ...
  else
  {
    ...
    double en = energy - get_threshold(nshell);  // energy of photo-electron
    ...
    //if(is == 0 && main_n > 0)
    if(is == 0) // generate default channel
    {
      if(main_n > 0)
      {
...
#endif
	if(main_n_largest - main_n >= 2) // at least K, l, M shells exists
	  // In this case we use more advanced scheme
	{
	  ...
	  double e_temp = 0.0;
	  if( (e_temp = 
	       get_threshold(nshell) - hdist - 2.0*get_threshold(n_choosen)) 
		    ...
		    
	  {
	    ...
	    {
	      el_energy[2] = e_temp;
	      //el_energy[2] = get_threshold(n_choosen) - 2.0 * get_threshold(n_min);
	    }
	    else
	...
	else   // for if(main_n_largest - main_n >= 2) 
	{
	  ...
	  double en1 = get_threshold(nshell) - hdist - 
	    2.0 * get_threshold(n_min);
	  if(en1 >= 0.0)
	  {
...
SimpleAtomPhotoAbsCS::SimpleAtomPhotoAbsCS(void): 
  AtomPhotoAbsCS() {;}

SimpleAtomPhotoAbsCS::SimpleAtomPhotoAbsCS(int fZ, const String& ffile_name):
  file_name(ffile_name)
{
  mfunnamep("SimpleAtomPhotoAbsCS::SimpleAtomPhotoAbsCS(int fZ, const String& ffile_name)");
  ...
  while( findmark(file, "#") == 1)
  {
	...
	if(fl[nshell] > 0)
	{
	  ...
	  asp[nshell].add_channel(fl[nshell], felectron_energy,
				  fphoton_energy);
	}
      }
      return;
  ...
  funnw.ehdr(mcerr);
  mcerr<<"there is no element Z="<<fZ<<" in file "<<file_name<<endl;
  spexit(mcerr);
}

SimpleAtomPhotoAbsCS::SimpleAtomPhotoAbsCS(int fZ, const PhotoAbsCS& facs)
{
  mfunname("SimpleAtomPhotoAbsCS::SimpleAtomPhotoAbsCS(int fZ, const PhotoAbsCS& facs)");
...
double SimpleAtomPhotoAbsCS::get_integral_ACS
...
double SimpleAtomPhotoAbsCS::get_integral_ACS
(int nshell, double energy1, double energy2) const
{
  mfunname(
  "double SimpleAtomPhotoAbsCS::get_integral_ACS(int nshell, double energy1, double energy2)");
  check_econd21(nshell , < 0 || , > qshell , mcerr);
  if(s_ignore_shell[nshell] == 0)
  {
...
double SimpleAtomPhotoAbsCS::get_integral_ICS
(double energy1, double energy2) const
{
  mfunname
    ("double SimpleAtomPhotoAbsCS::get_integral_ICS(double energy1, double energy2) const");
  int n;
  double s = 0.0;
  for(n=0; n<qshell; n++)
  {
...
{
  mfunname(
  "double SimpleAtomPhotoAbsCS::get_integral_ICS(int nshell, double energy1, double energy2)");
  check_econd21(nshell , < 0 || , > qshell , mcerr);
  if(s_ignore_shell[nshell] == 0)
  {
    return acs[nshell]->get_integral_CS(energy1, energy2);
...
void SimpleAtomPhotoAbsCS::print(ostream& file, int l) const
{
  if(l>0)
  {
...
ExAtomPhotoAbsCS::ExAtomPhotoAbsCS(void): 
  AtomPhotoAbsCS() {;}
...
{
  mfunnamep("SimpleAtomPhotoAbsCS::SimpleAtomPhotoAbsCS(int fZ, const String& fname, , const String& fthreshold_file_name, const String& fsimple_table_file_name, double fminimal_threshold)");
  //Imcout<<"ExAtomPhotoAbsCS::ExAtomPhotoAbsCS is run for fZ="<<fZ<<endl;
  check_econd11(fZ, < 1 , mcerr);
#ifdef USE_STLSTRING
  ifstream threshold_file(threshold_file_name.c_str());
#else
  ifstream threshold_file(threshold_file_name);
#endif
  if( !threshold_file )
  {
    funnw.ehdr(mcerr);
    mcerr<<"cannot open file "<<threshold_file_name<<endl;
    spexit(mcerr);
  }
  ...
  */
  ...
  if(ener[0] < thr[qshell-1])
  {
    long ne;
    for(ne=0; 
	ne<ener.get_qel() && (ener[ne] < thr[qshell-1] || 
  ...
  do    // Actually this is a loop by the group of thresholds
  {
    ...
    if(s_more == 1)
    {
      ...
      long ne;
      for(ne=nce; ne<ener.get_qel(); ne++)
      {   // finding energy larger than the next threshold
	if(thr[nt] <= ener[ne])
	{
	  nce_next = ne; s_spes = 0; break;
	}
	else
	{// At the following condition energy could be less then threshold,
    ...
    {
      double extrap_CS=save_left_CS * 
	pow(ener[nce_next-1] , 2.75) / pow(ener[ne] , 2.75);
  ...
  for(ns=0; ns<nt2; ns++)
  {
    acs[ns].pass( new PhenoPhotoAbsCS(shell_name[ns], Zshell[ns],
				thr[ns]) );
  }
  ...
  for(ns=nt2; ns<qshell; ns++)
  {
    ...
    //				thr[ns],
    //				ener, SCS[ns]) );
    SimpleTablePhotoAbsCS* adr  = 
      new SimpleTablePhotoAbsCS(shell_name[ns], Zshell[ns],
				thr[ns],
				ener, SCS[ns]);
  ...
  if(pred_integ > integ)
  {
    ...
    {
      // add excitation
      exener[0] = low_boundary_of_excitations * acs[qshell-1]->get_threshold();
...
{
  mfunnamep
  ("ExAtomPhotoAbsCS::ExAtomPhotoAbsCS(int fZ, const String& fname, const String& fBT_file_name, int id, double fminimal_threshold)");
  check_econd11(fZ, < 1 , mcerr);
  check_econd21(id, < 1 || , > 2 , mcerr);
  
  name = fname;
#ifdef USE_STLSTRING
  ifstream BT_file(BT_file_name.c_str());
#else
  ifstream BT_file(BT_file_name);
#endif
  if( !BT_file )
  {
    funnw.ehdr(mcerr);
    mcerr<<"cannot open file "<<BT_file_name<<endl;
    spexit(mcerr);
  }
  ...
  while( (i = findmark(BT_file, "Z =")) == 1 )
  {
    ...
    qshell++;
    acs.put_qel(qshell);
    acs[qshell-1].pass( new SimpleTablePhotoAbsCS(shellname, 
						  0,  // unknown here
						  thr,
						  fener, 
						  fcs) );
...
{
  mfunnamep
  ("ExAtomPhotoAbsCS::ExAtomPhotoAbsCS(int fZ, const String& fname, const String& fFitBT_file_name, int id, int id1, double fminimal_threshold)");
  check_econd11(fZ, < 1 , mcerr);
  check_econd21(id, < 1 || , > 2 , mcerr);
  Z = fZ;
  name = fname;
#ifdef USE_STLSTRING
  ifstream BT_file(fFitBT_file_name.c_str());
#else
  ifstream BT_file(fFitBT_file_name);
#endif
  if( !BT_file )
  {
    funnw.ehdr(mcerr);
...
#endif
	acs[nshell].pass( new SimpleTablePhotoAbsCS(shellname, 
						    0,  // unknown here
						    threshold,
						    l, E0, yw, ya, P, sigma));
	//Iprintn(mcout, nshell); 
  ...
  funnw.ehdr(mcerr);
  mcerr<<"there is no element Z="<<fZ<<" in file "<<fFitBT_file_name<<endl;
  spexit(mcerr);
 mark1:
  if(id == 2)
  {
...
(int fZ, 
 const String& fname,   // just name of this atom
 const String& fFitBT_file_name,
 // const String& fthreshold_file_name, // minimal potential corresponding to:
 const String& fsimple_table_file_name,
 double emax_repl,
 ...
 double fminimal_threshold)
{
  ...
  name = fname;
  int s_no_scale = 1;
  *this = ExAtomPhotoAbsCS(fZ, 
			   fname,   // just name of this atom
			   fFitBT_file_name,
			   id, s_no_scale,
			   fminimal_threshold);
  ...
  SimpleTablePhotoAbsCS* first_shell =  dynamic_cast< SimpleTablePhotoAbsCS* >
     (apacs);
   //mcout<<"first_shell:\n";
  ...
  // corrected.
   ...
   //dynamic_cast< SimpleTablePhotoAbsCS* >
     // (&(acs[nsmin]->getver()));  // assumes the first is the last
...
double ExAtomPhotoAbsCS::get_ICS(double energy) const
{
  ...
  //mcout<<"ExAtomPhotoAbsCS::get_ICS(double energy) is started\n"; 
  //Iprintn(mcout, energy);
  int n;
  double s = 0.0;
  for(n=0; n<qshell; n++)
...
double ExAtomPhotoAbsCS::get_integral_ICS
(double energy1, double energy2) const 
{  
  mfunname(
...
double ExAtomPhotoAbsCS::get_ICS(int nshell, double energy) const
{
  ...
  {
    double shift = 0.0;
    double t = acs[nshell]->get_threshold();
    if(minimal_threshold > 0.0)
    {
      if(t < minimal_threshold) shift = minimal_threshold - t;
    }
...
{
  mfunname(
  "double ExAtomPhotoAbsCS::get_integral_ICS(int nshell, double energy1, double energy2)");
  check_econd21(nshell , < 0 || , > qshell , mcerr);
  if(s_ignore_shell[nshell] == 0)
  {
    double shift = 0.0;
...
double ExAtomPhotoAbsCS::get_integral_ACS
(double energy1, double energy2) const
{
  mfunname
    ("double ExAtomPhotoAbsCS::get_integral_ACS(double energy1, double energy2) const");
  //mcout<<" ExAtomPhotoAbsCS::get_integral_ACS is started\n";
  int n;
  double s = 0.0;
  for(n=0; n<qshell; n++)
  {
	...
	if(t < minimal_threshold) shift = minimal_threshold - t;
      ...
      double tt;
      s += (tt = acs[n]->get_integral_CS(energy1 - shift, energy2 - shift));
      //Iprint3n(mcout, n, shift, tt);
    }
  }
...
{
  mfunname(
  "double ExAtomPhotoAbsCS::get_integral_ACS(int nshell, double energy1, double energy2)");
  check_econd21(nshell , < 0 || , > qshell , mcerr);
  if(s_ignore_shell[nshell] == 0)
  {
    double shift = 0.0;
...
void ExAtomPhotoAbsCS::print(ostream& file, int l) const
{
  if(l>0)
  {
    Ifile<<"ExAtomPhotoAbsCS(l="<<l<<"): name="<<name
	 <<" Z = "<<Z<<" qshell = "<<qshell<<endl;
    indn.n+=2;
    Ifile<<"threshold_file_name="<<threshold_file_name<<'\n'; 
    Ifile<<"simple_table_file_name="<<simple_table_file_name<<'\n';  
    Ifile<<"BT_file_name="<<BT_file_name 
	 <<endl;
    Ifile<<"Thomas_sum_rule_const_Mb * Z = "
	 <<Thomas_sum_rule_const_Mb * Z<<'\n';
						 ...
						 long fmax_q_step)
{
  ...
  //mcout<<"ExAtomPhotoAbsCS::replace_shells_by_overage:\n";
  long n;
  for(n=0; n<qshell; n++)
  {
    //Iprintn(mcout, n);
    ...
    PhotoAbsCS* a = new OveragePhotoAbsCS(acs[n].getver(), fwidth,
					fstep, fmax_q_step);
...
MolecPhotoAbsCS::MolecPhotoAbsCS
(const AtomPhotoAbsCS& fatom1, int fqatom_ps1,
 ...
 double fW, double fF): W(fW), F(fF)
{
  ...
  if(W == 0.0)
  {
    ...
    W = coef_I_to_W * (qatom_ps[0] * atom[0]->get_Z() * atom[0]->get_I_min() + 
      ...
      (qatom_ps[0] * atom[0]->get_Z()  + 
       qatom_ps[1] * atom[1]->get_Z() );
#else
...
{
  mfunname("double MolecPhotoAbsCS::get_integral_ACS(double energy1, double energy2) const");
  long q = qatom_ps.get_qel();
  long n;
  double s = 0.0;
  for(n=0; n<q; n++)
...
{
  mfunname("double MolecPhotoAbsCS::get_integral_ICS(double energy1, double energy2) const");
  long q = qatom_ps.get_qel();
  long n;
  double s = 0.0;
  for(n=0; n<q; n++)
...
void MolecPhotoAbsCS::print(ostream& file, int l) const 
{
...
PhotoAbsorptionCS::PhotoAbsorptionCS(const String& ffile_name, 
		    PassivePtr< EnergyMesh > fenergy_mesh)
{
  mfunname("PhotoAbsorptionCS::PhotoAbsorptionCS(const String& ffile_name, PassivePtr< EnergyMesh > fenergy_mesh)");
