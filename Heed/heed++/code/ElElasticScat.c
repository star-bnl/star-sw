                                                ...
                                                double angle)
{
  ...
  double beta2 = (2.0 * energy / ELMAS + pow(energy/ELMAS, 2.0)) / 
                 pow(gamma, 2.0);
...
double ElElasticScat::get_CS(long Z, double energy, double angle, int s_interp)
{
  ...
  double c = (f1 * pow(2, 2.0) - f2 * pow(z1, 2.0)) /
             (f2 * z1           - f1 * z2);
  double k = f1 / (z1 * (z1 + c));
  double r = k * Z * (Z + c);
  if (r < 0.0) r = 0.0;
...
/*
double ElElasticScatLowSigma::get_mean(long Z, double energy, double nscat) const
{
