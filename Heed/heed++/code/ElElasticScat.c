...
#include "wcpplib/graphics/histdef.h" // here USE_HBOOK is defined
#endif
...
ElElasticScat::ElElasticScat(const String& file_name):atom(0)
{
  ...
  while( findmark(file, "$") == 1)
  {
	...
	{
	  funnw.ehdr(mcerr);
	  mcerr<<"error at reading D, Z="<<Z<<" nc="<<nc<<" ne="<<ne<<'\n';
	  spexit(mcerr);
	}
      }
    }
...
double ElElasticScat::get_CS_for_presented_atom
(long na, double energy, double angle)
{
  mfunnamep("double ElElasticScat::get_CS_for_presented_atom(long na, double energy, double angle)");
  long ne;
  double enKeV = energy * 1000.0;
  //Iprintn(mcout, enKeV);
  double gamma = 1.0 + energy / ELMAS;  
  double beta2 = ( 2.0 * energy / ELMAS
		   + pow(energy/ELMAS, 2.0) ) /
    pow(gamma, 2.0);
  double gamma_betta2_t = gamma * beta2;
  double coe = atom[na].Z/(FSCON*FSCON)/gamma_betta2_t;
  if(enKeV < energy_mesh[0])
  {
    double r;
    for(ne=0; ne<qe; ne++)  // looking for valid data
    {
    ...
    {
      double r;
      for(ne=qe-1; ne>=0; ne--)  // looking for valid data
      {
	r = atom[na].data[ne].CS(angle);
	if( r >= 0.0) break;
      }
      ...
      r = r * coe*coe;
      return r;
    //double coe = atom[na].Z/(FSCON*FSCON)/gamma_betta2[0];
	...
	if(energy_mesh[ne] > enKeV)
	{
      ...
      double cs[2];
      //double coe[2]; 
      long ne_left = ne-1;  // starting points
      long ne_right = ne;
      for(ne=ne_left; ne>=0; ne--) // looking for valid data
      {
	...
	if( cs[0] >= 0.0) break;
      ...
      //Iprintn(mcout, cs[0]);
      //coe[0] = atom[na].Z/(FSCON*FSCON)/gamma_betta2[ne-1];
      //cs[0] = cs[0] * coe[0] * coe[0];
      for(ne=ne_right; ne<qe; ne++) // looking for valid data
      {
	...
	if( cs[1] >= 0.0) break;
      ...
      //cs[1] = atom[na].data[ne].CS(angle);
      //Iprintn(mcout, cs[1]);
      //coe[1] = atom[na].Z/(FSCON*FSCON)/gamma_betta2[ne];
...
double ElElasticScat::get_CS(long Z, double energy, double angle, int s_interp)
{
  mfunname("double ElElasticScat::get_CS(long Z, double energy, double angle, int s_interp)");
  ...
  check_econd11a(Z_left, == -100, " havenot found previous atom", mcerr); 
  check_econd11a(Z_right, == 10000, " havenot found next atom", mcerr); 
  ...
  double c = (f1 * pow(z2, 2.0) - f2 * pow(z1, 2.0) ) /
    (f2 * z1    - f1 * z2    );
  double k = f1 / (z1 * ( z1 + c ) );
  double r = k * Z * (Z + c);
  if(r < 0.0) r = 0.0;
...
double ElElasticScat::get_CS_Rutherford(long Z, double energy, double angle)
{
  ...
  double r = 1/4. *
    Z * Z *ELRAD * ELRAD * ELMAS * ELMAS /
    (momentum2 * gamma_betta2_t * pow( sin(angle/2.0), 4.0 ) ) /
    ( pow( 5.07E10, 2.0) ) * 1.0e16;
  */

  double gamma_1 =  energy / ELMAS;
  double betta2 = lorbetta2(gamma_1);
  double momentum2 = energy * energy + 2.0 * ELMAS * energy;
  double r = 1/4. *
    Z * Z *ELRAD * ELRAD * ELMAS * ELMAS /
    (momentum2 * betta2 * pow( sin(angle/2.0), 4.0 ) ) /
    ( pow( 5.07E10, 2.0) ) * 1.0e16;
...
void ElElasticScat::fill_hist(void)
{
  ...
  for(na=0; na<qa; na++)
  {
	...
	double beta2 = ( 2.0 * energyMeV / ELMAS
			 + pow(energyMeV/ELMAS, 2.0) ) /
	  pow(gamma, 2.0);
	double gamma_betta2_t = gamma * beta2;
	double coe = atom[na].Z/(FSCON*FSCON)/gamma_betta2_t;
	...
	double t;
	cor_hist.ac(na,ne).fill
	  (angle, 0.0, (t = get_CS(atom[na].Z, energyMeV, anglerad) ));
	corpol_hist.ac(na,ne).fill
	  (angle, 0.0, 
	   2.0 * M_PI * sin(anglerad) * t);
	corpola_hist.ac(na,ne).fill
	  (angle, 0.0, 
	   2.0 * M_PI * sin(anglerad) * t / 
	...
	{
	  int_hist.ac(na,ne).fill
	    (angle, 0.0, get_CS(atom[na].Z, energyMeV, angle/180.0 * M_PI, 1 ));
	}
	rut_hist.ac(na,ne).fill
	  (angle, 0.0, (t = get_CS_Rutherford
	   (atom[na].Z, energyMeV, angle/180.0 * M_PI) ));
	rutpol_hist.ac(na,ne).fill
	  (angle, 0.0, 2.0 * M_PI * sin(anglerad) * t);
	s_rut += 2.0 * M_PI * sin(anglerad) * t;
      ...
      path_length_cor_hist[na].fill
      	(ne, 0.0, 
      	 1.0 / (coef *angle_step * s_cor * 1.e-20*meter2) / cm);
      path_length_rut_hist[na].fill
	(ne, 0.0, 
	 1.0 / (coef *angle_step * s_rut * 1.e-20*meter2) / cm);
    }
  }
}
...
{
  mfunnamep("double ElElasticScat::fill_hist_low_scat(const String& file_name, const String& file_name_dist)");
  int s_write_dist = 0;
  ...
  */
  ...
  long za;
  ofile<<"this file is created by function void ElElasticScat::fill_hist_low_scat(void)\n";
  ofile<<"This is new format, in which the mean of 1 - cos(theta) is inserted\n";
  ofile<<" format:\nnumber of atoms maximal number of interactions,\nthen loop by atoms:\nZ of atom\n";
  //ofile<<"number of energies\n"
  ofile<<"ne energy_mesh[ne]  (mean of 1 - cos(theta)) (sqrt(mean of (1-coef)^2)\n";
  //ofile<<"number of interactions, mean cos of scattering angle, sigma of cos of scattering angle(thinking that mean is zero)\n";
  ...
  for(za=1; za<=zmax; za++)
  {
	...
	cs[nan] = s;
      ...
      PointsRan angular_points_ran(angular_mesh_c, cs,
	...
	//ang_hist.ac(na,ne,nq).init();
      ...
      String name;
	//name = "ang_" + long_to_String(atom[na].Z) + '_' + 
      name = "mean_" + long_to_String(za) + '_' + 
	  long_to_String(ne);
	...
	(name, quan[qquan-1]+1, -0.5, quan[qquan-1]+0.5);
      //mean_hist.ac(za-1,ne) = histdef
      //	(name, quan[qquan-1], 0.0, quan[qquan-1]);
      mean_hist.ac(za-1,ne).init();
      name = "sigma_" + long_to_String(za) + '_' + 
	  long_to_String(ne);
	...
	if(disp[ncs] < 0.4)
	{
      ...
      ofile<<ne<<' '<<setw(12)<<energy_mesh[ne]
	   <<' '<<setw(12)<<mean_coef
	   <<' '<<setw(12)<<coef<<'\n';
    }
  }
  if(s_write_dist == 1)
  {
	...
	for(nq=0; nq<qquan; nq++)
	{
	  ...
	  long q = hi.get_qel();
	  ofile<<"\n# "
	       <<setw(5)<<za<<' '<<setw(5)<<ne<<' '<<setw(5)
	       <<nq<<' '<<setw(5)<<q
	       <<' '<<setw(15)<<mea_ang_hist.ac(za-1,ne,nq)
	       <<' '<<setw(15)<<sig_ang_hist.ac(za-1,ne,nq)
	       <<'\n';
	  long n;
	  for(n=0; n<q; n++)
	  {
...
void ElElasticScat::print(ostream& file, int l) const
{
  if(l <= 0) return;
  Ifile<<"ElElasticScat(l="<<l<<"): qe="<<qe
       <<" atom.get_gel()="<<atom.get_qel()<<endl;
  if(l <= 1) return;
  indn.n += 2;
  long qa =atom.get_qel(); 
  long na;
  ...
  for(ne=0; ne<qe; ne++)
  {
    file<<setw(12)<<energy_mesh[ne];
  }
  file<<endl;
  Ifile<<"gamma_betta2=";
  for(ne=0; ne<qe; ne++)
  {
    file<<setw(12)<<gamma_betta2[ne];
  }
  file<<endl;
  ...
  for(na=0; na<qa; na++)
  {
    ...
    for(n=0; n<4; n++)
    {
      ...
      for(ne=0; ne<qe; ne++)
      {
	file<<setw(12)<<atom[na].data[ne].A[n];
      }
      file<<endl;
    }
    for(n=0; n<7; n++)
    {
      ...
      for(ne=0; ne<qe; ne++)
      {
	file<<setw(12)<<atom[na].data[ne].C[n];
      }
      file<<endl;
    }
    Ifile<<"B     ";
    for(ne=0; ne<qe; ne++)
    {
      file<<setw(12)<<atom[na].data[ne].B;
    }
    file<<endl;
    /*
    for(n=0; n<2; n++)
    {
      ...
      for(ne=0; ne<qe; ne++)
      {
	file<<setw(12)<<atom[na].data[ne].D[n];
      }
      file<<endl;
    }
    */
...
(ElElasticScat* fees, const String& file_name):
  ees(fees)
{
...
/*
double ElElasticScatLowSigma::get_mean
(long Z, double energy, double nscat) const
{
  mfunname("double ElElasticScatLowSigma::get_mean(...)");
  check_econd21(Z , < 1 || , > qat , mcerr);
  check_econd21(nscat , < 0 || , > qscat , mcerr);
  ...
  //Iprintn(mcout, n1);
  //Iprintn(mcout, n2);
  double v1 = nscat * mean[Z-1][n1];
  double v2 = nscat * mean[Z-1][n2];
  //Iprintn(mcout, v1);
  //Iprintn(mcout, v2);
  double r = v1 + (v2 - v1)/
    (ees->get_energy_mesh(n2) - ees->get_energy_mesh(n1)) *
    (energyKeV - ees->get_energy_mesh(n1));
  return r;
}

double ElElasticScatLowSigma::get_sigma
(long Z, double energy, double nscat) const
{
  mfunname("double ElElasticScatLowSigma::get_sigma(...)");
  check_econd21(Z , < 1 || , > qat , mcerr);
  check_econd21(nscat , < 0 || , > qscat , mcerr);
  double energyKeV = energy * 1000.0;
  if(energyKeV < ees->get_energy_mesh(0)) energyKeV = ees->get_energy_mesh(0); 
  ...
  //Iprintn(mcout, n1);
  //Iprintn(mcout, n2);
  double v1 = nscat * coef[Z-1][n1];
  double v2 = nscat * coef[Z-1][n2];
  //Iprintn(mcout, v1);
  //Iprintn(mcout, v2);
  double r = v1 + (v2 - v1)/
    (ees->get_energy_mesh(n2) - ees->get_energy_mesh(n1)) *
  ...
  ees(fees)
{
  ...
  for(nat=0; nat<qat; nat++)
  {
	...
	check_econd12(fnscat, != , nscat, mcerr);
 	check_econd11(mean, <  0, mcerr);
 	check_econd11(mean, <  0, mcerr);
	sigma[nat][ne][nscat] = fsigma;
      }
    }
  }
}
...
double ElElasticScatLowSigma::get_sigma
(long Z, double energy, long nscat) const
{
  mfunname("double ElElasticScatLowSigma::get_sigma(...)");
  check_econd21(Z , < 1 || , > qat , mcerr);
  check_econd21(nscat , < 1 || , > qscat , mcerr);
  double energyKeV = energy * 1000.0;
  if(energyKeV < ees->get_energy_mesh(0)) energyKeV = ees->get_energy_mesh(0); 
  if(energyKeV > ees->get_energy_mesh(ees->get_qe()-1)) 
    energyKeV = ees->get_energy_mesh(ees->get_qe()-1);
  ...
  Iprintn(mcout, n1);
  Iprintn(mcout, n2);
  double v1 = sigma[Z-1][n1][nscat-1];
  double v2 = sigma[Z-1][n2][nscat-1];
  Iprintn(mcout, v1);
  Iprintn(mcout, v2);
  double r = v1 + (v2 - v1)/
    (ees->get_energy_mesh(n2) - ees->get_energy_mesh(n1)) *
