...
void HeedDeltaElectron::physics_after_new_speed(void) {
  ...
  const HeedDeltaElectronCSType* hmecst = 
    dynamic_cast< const HeedDeltaElectronCSType*  >(av);
  if (s_print_listing == 1) mcout<<"physics_after_new_speed: started"<< std::endl;
  if (hmecst == NULL) return;
  ...
  */
  ...
  if (s_print_listing == 1) Iprint3nf(mcout, prev_kin_energy/eV, curr_kin_energy/eV, Eloss/eV);
  if (curr_kin_energy <= 0.0) {
    if (s_print_listing == 1) {
    ...
    if (Eloss > 0.0) {
      if (Eloss >= necessary_energy) {
        ...
        if (necessary_energy <= 0.0) {
          ...
          necessary_energy = hdecs->pairprod->get_eloss(prev_kin_energy / eV) * eV;
#else 
...
#ifdef USE_ADJUSTED_W
          necessary_energy = eV * hdecs->pairprod->get_eloss(curr_kin_energy_for_cond/eV);
#else	    
          necessary_energy = hdecs->pairprod->get_eloss() * eV; 
#endif
          if (s_print_listing == 1) {
            Iprintnf(mcout, Eloss_left/eV); 
            Iprint2nf(mcout, curr_kin_energy_for_cond/eV, necessary_energy/eV);
          } 
        }
    ...
    if (q_low_path_length > 0) {
      ...
      if (q_low_path_length < max_q_low_path_length_for_direct) {
        ...
        for (long nscat = 0; nscat < q_low_path_length; ++nscat) {
          ...
          if (s_print_listing == 1) Iprint2nf(mcout, theta_rot, theta_rot/M_PI *180.0);
          vec dir = currpos.dir;
          //Iprint(mcout, dir);
          basis temp(dir, "temp");
          vec vturn;
