...
void HeedParticle_BGM::physics(void)
{
  ...
  else
  {
	...
	PointCoorMesh< double, DynLinArr< double > > 
	  pcm( aetcs_bgm->mesh->q, &(aetcs_bgm->mesh->x) );
	...
	//	       "bg="<<bg<<" n1="<<n1<<" n2="<<n2<<" b1="<<b1<<" b2="<<b2<<" aetcs_bgm->mesh="<<aetcs_bgm->mesh<<'\n',
	//	       mcerr);
...
void HeedParticle_BGM::print(ostream& file, int l) const 
{
  if(l >=0 )
  {  
    Ifile<<"HeedParticle_BGM (l="<<l<<"): particle_number="<<particle_number
	 <<" type=";
    print_notation(file);
    file<<endl;
    if(l == 1) return;
