...
public:
  ...
  point Gpt(int n) const 
    { // there is no funname line, check directly
      ...
      if(n>=qpt)
      {
	mcerr<<"error in polyline:Gpt(int n): n>qpt: n="<<n
	     <<" qpt="<<qpt<<'\n';
	spexit(mcerr);
      }
      return pt[n];
  ...
  straight Gsl(int n) const 
    {
      if(n>=qsl)
      {
	mcerr<<"error in polyline:Gpt(int n): n>qsl: n="<<n
	     <<" qsl="<<qsl<<'\n';
	spexit(mcerr);
      }
      return sl[n];
...
public:
  ...
  int cross(const straight& fsl, 
	    point *pc, int& qpc, polyline *pl, int& qpl, vfloat prec) const ;
...
protected:
  ...
  void polyline_del(void)
    { 
      ...
      if(sl!=NULL) {delete[] sl; sl = NULL; } 
      if(aref!=NULL) {delete[] aref; aref = NULL; }
    }
public:
  polyline(void) 
  { point ptl; polyline_init(&ptl,0);}
...
public:
  ...
  vfloat perpos(const point& fpt) const    // perpendicular position, 
    ...
    {    // it is assumed that the point is inside
      vfloat r = sl[0].distance(fpt); return r;
    }
