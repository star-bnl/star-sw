...
int absvol::find_embed_vol(const point& fpt, const vec& dir, 
  ...
  for (int n = 0; n < atid->qeid; ++n) atid->eid[n].amvol->m_print(mcout,0);
...
int manip_absvol::m_find_embed_vol(const point& fpt, 
  ...
  for (int n = 0; n < atid->qeid; ++n) atid->eid[n].print(mcout, 0);
...
sh_manip_absvol::sh_manip_absvol(sh_manip_absvol& f) : absref(f), manip_absvol(f), csys(f.csys) {}
sh_manip_absvol::sh_manip_absvol(const sh_manip_absvol& f) : absref(f), manip_absvol(f), csys(f.csys) {}
