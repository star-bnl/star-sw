...
vfloat polyline::distance(const point& fpt, point& fcpt) const 
{
  ...
  for(n=0; n<qsl; n++)
  {
    ...
    if(check_par( v1 , v2 , 0.01 ) == -1 ) 
    {        // anti-parallel vectors, point inside borders
      if(sldist < mx) { mx=sldist; fcpt=cpt; }
    }
    else
    {
      if( (sldist=length(fpt-pt[n])) < mx) { mx=sldist; fcpt=pt[n]; }
      if( (sldist=length(fpt-pt[n+1])) < mx) { mx=sldist; fcpt=pt[n+1]; }
    }
  }
  return mx;
}

int cross4pllines(const polyline pl[4], vfloat precision, 
		  straight& sl, point ptc[4][2])
{
  pvecerror(
  ...
  polyline_pl(), s_convex(1)
{
  ...
  for( n=0; n<fqsl-1; n++)
	...
	if( check_par(fsl[n].Gdir(), fsl[m].Gdir(), 0 )!=0) //1 par, -1 antipar
	{
	  mcerr<<"error in polyline_init(straight* fsl, int fqsl):\n"
...
{ 
  ...
  polyline_del(); 
  polyline_init(fpl.pt, fpl.qpt); 
  pn=fpl.pn; s_convex=fpl.s_convex;
  return *this;
}
...
int polygon::range(const point& fpt, const vec& dir, 
		   vfloat& rng,  point &fptenr, vfloat prec) const 
{
  pvecerror("int polygon::range(const point& fpt, const vec& dir, vfloat& rng,  point &fptenr)");
  //mcout<<"polygon::range:\n"<<*this;
  //mcout<<"dir="<<'\n';
...
(absref absref::*)&rectangle::dir1, 
(absref absref::*)&rectangle::dir2};
...
rectangle::rectangle(const point& fpiv, vec fdir[2], vfloat fdim[2], 
		     vfloat prec)
{
  pvecerror("rectangle::rectangle(point fpiv, vec fdir[2], vfloat fdim[2], vfloat prec)");
  ...
  piv=fpiv; 
  dir1=unit_vec(fdir[0]); dir2=unit_vec(fdir[1]); 
  dim[0]=fdim[0]; dim[1]=fdim[1];  
  //mcout<<"piv:\n"<<piv;
...
spquadr::spquadr(      spquadr& sq):
  polygon(sq),piv(sq.piv),dir1(sq.dir1),dir2(sq.dir2),awidth(sq.awidth){;}
spquadr::spquadr(const spquadr& sq):
  polygon(sq),piv(sq.piv),dir1(sq.dir1),dir2(sq.dir2),awidth(sq.awidth){;}

spquadr::spquadr(const point& fpiv, 
