...
absref absref::*(straight::aref[2])=
{(absref absref::*)&straight::piv, (absref absref::*)&straight::dir};
...
int apeq(const straight &sl1, const straight &sl2,
	 vfloat prec)
{
  pvecerror(
"int apeq(const straight &sl1, const straight &sl2, vfloat prec=vprecision)");
...
point straight::cross(const straight& sl, vfloat prec) const 
{
  ...
  {
    vecerror = type_of_cross; return ptt;
  }
  if( fabs(f) <= prec )
  {
    vecerror = 0; return pt[0];
  }
  else
  {
    vecerror = 1; return ptt;
  }
}
/*
point straight::cross(const straight& sl, vfloat prec) const 
...
*/
vfloat straight::vecdistance(const straight& sl, 
			     int& type_of_cross,  point pt[2]) const
{
  pvecerror("vfloat straight::distance(const straight& sl, int type_of_cross,  point pt[2])");
  pt[0]=point();
  pt[1]=point();
  type_of_cross=0;
  straight s1,s2;
  ...
  else
  {
    type_of_cross=1;
    vfloat d=pt[1].v.y;
    pt[0]=pt[1]; pt[0].v.y=0;
    pt[0].down(&scl); pt[1].down(&scl);
    return d;
  }
}

vfloat straight::distance(const straight& sl, int& type_of_cross,  point pt[2])
  const
{
...
{
  pvecerror("vfloat straight::vecdistance(const vec normal, const straight& slt)");
...
straight::straight(const point* pt, int qpt, int anum)  // interpolates by xi2
{
  ...
  if(anum == 0)
  {
    ...
    piv1=point(lcy.x_mean+1 , lcy.line(lcy.x_mean+1), lcz.line(lcy.x_mean+1) );
  }
  else if(anum == 1)
  {
    linexi2 lcx(qpt, y, x);  
    ...
    piv1=point(lcx.line(lcx.x_mean+1), lcx.x_mean+1, lcz.line(lcx.x_mean+1) );
  }
  else
  {
    linexi2 lcx(qpt, z, x);  
    ...
    piv1=point(lcx.line(lcx.x_mean+1), lcy.line(lcx.x_mean+1), lcx.x_mean+1);
  }
...
{
  pvecerror("straight::straight(const straight sl[4], point pt[2],  vfloat precision)");
  int i;
  vfloat meandist;
  point ptprev[2];
  point ptcurr[2];
