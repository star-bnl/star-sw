...
void absref::down(const abssyscoor* fasc )
{
  ...
  for(n=0; n<qaref; n++)             // changes only vectors
  ...
  // If one of vectors has zero length, it returns 2.
{
  ...
  vfloat lr1=length2(r1);  vfloat lr2=length2(r2);
  //mcout<<"cos2vec:\n";
  //Iprintn(mcout, lr1);
  //Iprintn(mcout, lr2);
  if(lr1 == 0 || lr2 == 0){ vecerror=1; return 0; }
  vfloat cs=r1*r2;
  int sign=1; if(cs<0) sign=-1;
  cs=cs*cs;
...
vfloat ang2vec(const vec& r1, const vec& r2) //  angle between vectors
{
...
vec project_to_plane(const vec& r, const vec& normal)
{
  ...
  vec per(normal || r);
  if(per==dv0) // either one of vectors is 0 or they are parallel
    return dv0;
...
vfloat ang2projvec(const vec& r1, const vec& r2, const vec& normal)
{
  ...
  else return tang; // it works if angle <= PI
} 
...
{// it is assumed that fabas_new is derivative from old
  ...
  if(fabas_new==NULL)
  {
    funnw.ehdr(mcerr); mcerr<<"fabas_new==NULL\n"; spexit(mcerr);
  }
...
vec vec::turn_new(const vec& dir, vfloat angle)
{
  ...
  if(check_par(*this, dir, 0.0) !=0 )
    return *this;  // parallel vectors are not changed
...
{
  //pvecerror("inline void vec::random_round_vec(void)");
  vfloat phi;
#ifdef USE_SRANLUX
  phi=M_PI*2.0 * SRANLUX();
#else
  phi=M_PI*2.0 * random_engine.flat();
#endif
  //phi=M_PI*2.0 * RandEng.flat();
  x=sin(phi);
  y=cos(phi);
  z=0;
}
...
{
  //pvecerror("inline void vec::random_sfer_vec()");
#ifdef USE_SRANLUX
  vfloat cteta = 2.0 * SRANLUX()-1.0;
#else
  vfloat cteta = 2.0 * random_engine.flat()-1.0;
#endif
  //vfloat cteta=2.0*RandEng.flat()-1.0;
  random_round_vec();
...
basis::basis(const vec &p, const String& pname)
{
  ...
  {
    ez=unit_vec(p); ey=unit_vec(ez||dez); ex=ey||ez; 
  }
  //strcpy(name,"b.from vec");
}
...
basis::basis(const vec &p, const vec &c, const String& pname)
{
  ...
  {
    ez=unit_vec(p); ey=unit_vec(ez||c); ex=ey||ez; 
  }

}
basis::basis(const basis& pb, const String& pname):ex(pb.ex),ey(pb.ey),ez(pb.ez)
...
absref absref::*(fixsyscoor::aref[2])=
{reinterpret_cast<absref absref::*>(static_cast<point absref::*>
				    (&fixsyscoor::piv)), 
 reinterpret_cast<absref absref::*>(static_cast<basis absref::*>
				    (&fixsyscoor::bas))};
