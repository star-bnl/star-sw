...
#define pvecerror {if(vecerror!=0)                                        \
{cerr<<"vecerror is not zero(put any key)\n";char c;cin>>c;exit(1);}}
#define pvecerrorp {if(vecerror!=0)                                        \
{cerr<<"vecerror is not zero(put any key)\n";char c;cin>>c;exit(1);}}
#else
#define pvecerror(string)   mfunname(string); \
  if(vecerror!=0)                    \
  { mcerr<<"vecerror is not zero, program is terminated\n"            \
        <<" function detected error is "<<string<<'\n'; spexit(mcerr);}
#define pvecerrorp(string)   mfunnamep(string); \
  if(vecerror!=0)                    \
  { mcerr<<"vecerror is not zero, program is terminated\n"            \
        <<" function detected error is "<<string<<'\n'; spexit(mcerr);}
#endif
...
/* Introduces type vfloat which is used thorought the geometrical calculations
instead of double. 'double' is meant to be replaceble by 'float' for 
speeding up, but no consistent research was made to check that it really 
works in this way. So now vfloat is sinonim of double.
*/
...
{public:
  virtual void down(const abssyscoor* fasc);
    // converts numbering representation of object to basical system of fasc

  virtual void   up(const abssyscoor* fasc);
    // converts numbering representation of objects to new system
...
/* 
Contains three methods of transmittion, the fastest, slower and the slowest
 */
...
{public:
  ...
  absref_transmit( int fqaref_pointer , absref **faref_pointer):
    qaref(0), qaref_pointer(fqaref_pointer), aref_pointer(faref_pointer),
    qaref_other(0) {;}
  absref_transmit( int fqaref , absref absref::**faref,
		   int fqaref_pointer , absref **faref_pointer):
    qaref(fqaref), aref(faref),
    qaref_pointer(fqaref_pointer), aref_pointer(faref_pointer), qaref_other(0)
    {;}
...
{public:
  ...
  friend vec operator||(const vec& r1, const vec& r2)  // vector product
    {    return vec(r1.y*r2.z-r1.z*r2.y,
  		    r1.z*r2.x-r1.x*r2.z,
  		    r1.x*r2.y-r1.y*r2.x); }
  friend wl_inline int operator==(const vec &r1, const vec &r2);
    // returns 1 if precisely the same vectors and 0 otherwise

  friend wl_inline int operator!=(const vec &r1, const vec &r2);
    // returns 0 if precisely the same vectors and 1 otherwise

  friend wl_inline int apeq(const vec &r1, const vec &r2, 
			    vfloat prec);
  ...
  friend wl_inline int not_apeq(const vec &r1, const vec &r2, 
				vfloat prec);
    // returns 0 if approximately the same vectors and 1 otherwise
  ...
  friend vfloat ang2projvec(const vec& r1, const vec& r2, 
				      const vec& normal); 
  ...
  friend wl_inline int check_par(const vec& r1, const vec& r2, 
				 vfloat prec);
    // checks whether the vectors are parallel, or anti-parallel. 
  ...
  friend wl_inline int check_perp(const vec& r1, const vec& r2, 
				  vfloat prec); 
    // checks whether the vectors are perpendicular.
  ...
  vec(vfloat xx, vfloat yy, vfloat zz )   // for auxiliary
    {   x=xx; y=yy; z=zz; }
  vec(void)
    {   x=0; y=0; z=0; }
  ...
  // the help of SRANLUX.
  void random_round_vec(void); // any direction in plane
                                         // perpendicular to z-axis.
  void random_conic_vec(double theta); // any direction in conus
       // with simmetry axis along z-axis
       // and with angle theta (radian) 
  void random_sfer_vec(void); // any direction in 3-dim.space
};
...
extern ostream& operator<<(ostream& file, const basis& b);
...
{public:
  ...
  basisReg(const vec &p, const vec &c, const String& pname):
    basis(p, c, pname) {}
  basisReg(const vec &pex, const vec &pey, const vec &pez, 
      const String& pname): basis(pex, pey, pez, pname) {}
  AnyType_copy(basisReg, basis);
};
ostream& operator<<(ostream& file, const basisReg& v);
...
public:
  virtual void down(const abssyscoor *fasc);
  virtual void up(const abssyscoor *fasc);
  virtual void shift(const vec& dir){v+=dir;}  // not defined for vectors, but
                                 // defined for points
  point(void):v(){;}        // v is not inited
  ...
  point& operator=(const point& fp) { v=fp.v; return *this; }
...
ostream& operator<<(ostream& file, const pointReg& v);
...
extern ostream& operator<<(ostream& file, const abssyscoor& s);
...
{public:
  ...
  fixsyscoor(const point* const fapiv, const basis* const fabas, 
	     const String& fname): 
    abssyscoor(fname), piv( (fapiv!=NULL) ? (*fapiv) : point() ),
...
extern ostream& operator<<(ostream& file, const fixsyscoor& s);
