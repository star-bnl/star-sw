...
#if defined( USE_PRIVATE_PARAM_IN_PASSIVEPTR ) && !defined( USE_GETSETTERS_IN_PASSIVEPTR )
...
#if defined( USE_CHAR_CONTROL_VARIABLES ) && !defined( USE_CHAR_GETSETTERS_PARAMETERS )
"options incompatible\n"; // any line to trigger compiler diagnostic
#endif
...
#define virt_common_base   public virtual CommonBase
#define virt_common_base_col :public virtual CommonBase  // for single case
#define virt_common_base_comma public virtual CommonBase, // one of many
...
// return type
#define AbsCont_copy(type)  virtual type* copy(void) const \
{return new type(*this);}
#define AnyType_copy(derived_type, base_type)  \
virtual derived_type* copy(void) const \
{return new derived_type(*this);}
#else
// return CommonBase
#define AbsCont_copy(type)  virtual CommonBase* copy(void) const \
{return new type(*this);}
#define AnyType_copy(derived_type, base_type)  \
virtual CommonBase* copy(void) const \
{return new derived_type(*this);}
#endif
...
#define macro_copy_total(type)  virtual type* copy(void) const \
{return new type(*this);}
...
#define macro_copy_body(type)  type* type::copy(void) const \
{return new type(*this);}
#define macro_copy_body_not_defined(type)  type* type::copy(void) const \
{ mcerr<<"macro_copy_body_not_defined("<<#type<<"): forbidden call\n"; \
  spexit(mcerr);  return NULL; }
...
#define macro_copy_total(type)  virtual CommonBase* copy(void) const \
{return new type(*this);}
...
#define macro_copy_body(type)  CommonBase* type::copy(void) const \
{return new type(*this);}
#define macro_copy_body_not_defined(type)  CommonBase* type::copy(void) const \
{ mcerr<<"macro_copy_body_not_defined("<<#type<<"): forbidden call\n"; \
  spexit(mcerr);  return NULL; }
// return NULL to calm compiler of Solaris which wants to see return value
#endif
...
{public:
  static X* copy(const X* f) 
	...
	mcerr<<"Error in X* StandardCopyDefinition::copy(const X* f): "
      ...
      //#ifndef COPY_RETURNS_VOID // this macro should NOT be used, leads to errors
      X* p = f->copy();   
      //#else
	...
	mcerr<<"Possible reason is omiting of copy function in one of the derivative classes\n";
	spexit(mcerr);
      }
      return p;
#endif
...
{public:
  static X* copy(const X* f)
	...
	mcerr<<"Type of *p is (in internal notations) "<<typeid(*p).name()<<'\n';
	mcerr<<"Type of *f is (in internal notations) "<<typeid(*f).name()<<'\n';
	mcerr<<"Possible reasone is omiting of copy function in one of the derivative classes\n";
	spexit(mcerr);
      }
      return p;
#endif
...
template<class X, class C = StandardCopyDefinition<X> >class ActivePtr  virt_common_base_col  
...
{public:
  ...
  inline void put(const X* fptr)  // delete old owned object and 
    ...
    { //
      ...
      Iprintn(mcout, ptr);
      mcout<<"Type of X is (in internal notations) "<<typeid(X).name()<<'\n';
      mcout<<"Type of *fptr is (in internal notations) "
      <<typeid(*fptr).name()<<'\n';
#endif
  ...
  /* 
  inline ActivePtr(const X* fptr):ptr(fptr != NULL ? C::copy(fptr) : ((X*)NULL))    //inline ActivePtr(const X* fptr):ptr(fptr != NULL ? fptr->copy() : ((X*)NULL))
  ...
  */
  ...
  // debug
#endif
  //inline ActivePtr(      ActivePtr<X,C>& f):   // was required at some old systems, don't remember why.
  ...
  // Pilfer constructor
  inline ActivePtr(PILF_CONST ActivePtr<X,C>& f, Pilfer): ptr(f.ptr) 
  {                //!Attention: actually not const
#ifdef DEBUG_ACTIVEPTR
...
#endif
    if(this != &f) put(f.ptr) ; return *this; 
  }
...
{public:
  ...
  inline ActivePtrWI(const ActivePtrWI<X>& f): ptr(NULL)
    { if(f.ptr != NULL) ptr= new X(*(f.ptr)); }
  inline ActivePtrWI& operator=(const ActivePtrWI<X>& f) { if(this != &f) put(f.ptr) ; return *this; }
  inline X* operator->(void) const ;
  inline X& operator*(void) const;
#ifdef IMPLICIT_X_STAR  
...
template<class X>
inline X* ActivePtrWI<X>::operator->(void) const 
{
#ifdef SKIP_CHECKS_NULL
  return ptr;
#else
...
#ifdef IMPLICIT_X_STAR  
template<class X>
ActivePtrWI<X>::operator X*(void) const 
{
#ifdef SKIP_CHECKS_NULL
  return ptr;
#else
...
inline void CountPassivePtr::book(void)
{
  if(number_of_booked > LONG_MAX-1)
  {
    mcerr<<"Error in CountPassivePtr::book(void):\n"
	 <<" too much booked counters, number_of_booked > LONG_MAX-1, number_of_booked="<<number_of_booked<<'\n';
    spexit(mcerr);
  }
  number_of_booked++;
}
...
private:
  ...
  inline static void rise_bit(unsigned char& cw, unsigned char b)
  {
    cw |= b;
  }
public:
// To define auxiliary class, used only in the following case:
  ...
  { public:
...
#endif
    ControlParam(void):s_ban_del(0), s_ban_sub(0), s_ban_cop(0)
#ifdef USE_DELETE_AT_ZERO_COUNT
		       , s_allow_del_at_zero_count(0)
#endif 
    {}
#ifdef USE_CHAR_GETSETTERS_PARAMETERS
    ControlParam(char fs_ban_del, char fs_ban_sub, 
		 char fs_ban_cop = 0
#ifdef USE_DELETE_AT_ZERO_COUNT
...
#else
    ControlParam(int fs_ban_del, int fs_ban_sub, 
		 int fs_ban_cop = 0 
#ifdef USE_DELETE_AT_ZERO_COUNT
...
#endif 
    {
      ...
      {
        mcerr<<"ERROR in ControlParam::ControlParam(...)\n";
        mcerr<<"s_allow_del_at_zero_count is outside limits, s_allow_del_at_zero_count="<<fs_allow_del_at_zero_count<<'\n';
        spexit(mcerr);
      }
#endif
    }
  ...
  inline RegPassivePtr(void): 
    ...
    conparam(),
#else
  s_ban_del(0), s_ban_sub(0), s_ban_cop(0),
#ifdef USE_DELETE_AT_ZERO_COUNT
			      s_allow_del_at_zero_count(0),
#endif
#endif      
...
public:
...
#endif
  {
    ...
    {
      mcerr<<"ERROR in inline void set_s_ban_del_ignore(int fs_ban_del_ignore ):\n";
...
#endif
  {
    ...
    {
      mcerr<<"ERROR in inline void set_s_allow_del_at_zero_count(int fs_allow_del_at_zero_count):\n";
      mcerr<<"s_allow_del_at_zero_count is outside limits, s_allow_del_at_zero_count="<<int(fs_allow_del_at_zero_count)<<'\n';
      spexit(mcerr);
    }
  }
...
{public:
  ...
  //inline PassivePtr(PassivePtr<X>& f, int s); // s==0 do not copy, s==1 copy to new
  ...
  //template< class Y > explicit operator PassivePtr< Y >() const ; does not work,
...
template<class X >
template< class Y > 
PassivePtr<X>::PassivePtr(const PassivePtr < Y >& f): cpp(NULL)
{ 
  put((f.get())); 
}
...
{ 
  ...
  if(cpp->get_rpp() == NULL)
  {
    mcerr<<"Error in X* PassivePtr<X>::operator->(void) const: cpp->get_rpp() == NULL\n"; 
    mcerr<<"This means that the pointer is emtpy, "
	 <<"there is no addressed object.\n";  
    mcerr<<"Type of X is (in internal notations) "<<typeid(X).name()<<'\n';
    spexit(mcerr);
...
inline X* PassivePtr<X>::getver(void) const  
{
  ...
  if(cpp->get_rpp() == NULL)
  {
    mcerr<<"Error in X* PassivePtr<X>::getver(void) const: cpp->get_rpp() == NULL\n"; 
    mcerr<<"This means that the pointer is emtpy, "
	 <<"there is no addressed object.\n";  
    mcerr<<"Type of X is (in internal notations) "<<typeid(X).name()<<'\n';
    spexit(mcerr);
...
inline PassivePtr<X>::operator X*(void) const  
{
  ...
  if(cpp->get_rpp() == NULL)
  {
    mcerr<<"Error in X* PassivePtr<X>::operator X*(void) const: cpp->get_rpp() == NULL\n"; 
    mcerr<<"This means that the pointer is emtpy, "
	 <<"there is no addressed object.\n";  
    mcerr<<"Type of X is (in internal notations) "<<typeid(X).name()<<'\n';
    spexit(mcerr);
  ...
  if(cpp == NULL)
  {
    mcerr<<"Error in X* PassivePtr<X>::operator X*(void) const: cpp == NULL\n"; 
    mcerr<<"This means that the pointer is emtpy, "
	 <<"there is no addressed object.\n";  
    mcerr<<"Type of X is (in internal notations) "<<typeid(X).name()<<'\n';
    spexit(mcerr);
...
template<class X, class C = StandardCopyDefinition<X> >class ActivePtrReg: 
...
template<class X>class ActivePtrWIReg:
  public RegPassivePtr,
  public ActivePtrWI<X>

{public:
  inline ActivePtrWIReg(void): ActivePtrWI<X>(), RegPassivePtr() {}
  inline ActivePtrWIReg(const X* fptr): ActivePtrWI<X>(fptr), RegPassivePtr() {}
  inline ActivePtrWIReg(ActivePtrWIReg<X>& f): ActivePtrWI<X>(fptr), RegPassivePtr() {} 
