...
template<class T>class AbsListNode: public RegPassivePtr
#else
template<class T>class AbsListNode
#endif
{
...
private:
  ...
  // It is assumed to be called only from ListNode functions. 
  AbsListNode(AbsList<T>* fhead_node, 
	      AbsListNode<T>* fprev_node,
...
public:
  ...
  friend AbsListNode<T>*  glob_insert_before<>
  (AbsList<T>&, AbsListNode<T>*, const T& fel);
  friend AbsListNode<T>*  glob_insert_after<>
  (AbsList<T>&, AbsListNode<T>*, const T& fel);
  friend void glob_pilfer<>(AbsList<T>&, PILF_CONST AbsList<T>&);

  // The following is more logical, but not compiled at some newer compilers, alas!
  //friend AbsListNode<T>*  AbsList<T>::insert_before(AbsListNode<T>*, const T& fel);
  //friend AbsListNode<T>*  AbsList<T>::insert_after(AbsListNode<T>*, const T& fel);
...
#ifndef DONT_USE_ABSPTR
template<class T>class AbsList: public RegPassivePtr
#else
template<class T>class AbsList
#endif
{
...
public:
  ...
  // The two following functions do the same as those above and actually called from
  ...
  AbsListNode<T>*  push_front(const T& fel){ return insert_before(first_node, fel); }
  AbsListNode<T>*  prepend(const T& fel){ return push_front(fel);}
  AbsListNode<T>*  push_back(const T& fel){ return insert_after(last_node, fel); }
  AbsListNode<T>*  append(const T& fel){ return push_back(fel);}

  void erase(AbsListNode<T>* an); // function to exclude the node
  inline void remove(AbsListNode<T>* an){ erase(an); } // function to exclude the node
...
(AbsList<T>* fhead_node, 
 ...
 AbsListNode<T>* fnext_node, const T& fel): 
  ...
  prev_node(fprev_node), next_node(fnext_node),
  el(fel)
{ 
  //mcout<<"Constructor\n";
  if(fprev_node != NULL)
  {
    if(fprev_node->next_node !=  fnext_node )
    { // for debug
  ...
  if(fnext_node != NULL)
  {
    if(fnext_node->prev_node !=  fprev_node )
    { // for debug
      mcerr<<"template<class T> AbsListNode<T>::AbsListNode<T>\n";
...
{  
  ...
  if(head_node != NULL) // normally cannot be
  {
    ...
    if(head_node->first_node == this)
      head_node->first_node = next_node;
    if(head_node->last_node == this)
      head_node->last_node = prev_node;
    head_node->qel--;
  }
  head_node = NULL; prev_node = NULL; next_node = NULL; 
}
  
template<class T> AbsListNode<T>* 
glob_insert_before(AbsList<T>& al, AbsListNode<T>* aln, const T& fel)
{
  ...
  if(aln == NULL)  // means to insert at the first place
  {
    if(al.qel == 0)  // no elements, then the addresses should be empty as well
    {
      ...
      {
	mcerr<<"ERROR in template<class T> AbsListNode<T>*  glob_insert_before\n";
...
template<class T> void AbsList<T>::erase
(AbsListNode<T>* aln)
{
  if(aln->get_head_node() != this)
  {
...
template<class T> void AbsList<T>::remove(const T& t)  
...
template<class T> void AbsList<T>::remove_all(const T& t)  
// function to exclude the node
{
  AbsListNode<T>* an=NULL;
  while( (an = get_next_node(an)) != NULL)
...
{
#ifdef DEBUG_ABSLIST
  mcout<<"AbsList<T>& AbsList<T>::operator=(const AbsList<T>& f) is starting\n";
#endif
  clear();
...
{
#ifdef DEBUG_ABSLIST
  mcout<<"void glob_pilfer(AbsList<T>& this_al, PILF_CONST AbsList<T>& al) is starting\n";
#endif
  if(this_al.qel != 0)
  {
    if(al.qel != 0)
...
template<class T> void  AbsList<T>::pilfer
