...
template<class T>class DynLinArr: public RegPassivePtr
#else
...
{public:
  ...
  */
  ...
  DynLinArr& assignAll(const T& f) 
    { check(); long n; for( n=0; n<qel; n++) el[n]=f; return *this; }
  template<class X> DynLinArr<T>& assignAll1(const X& f)  // assumes that 
    // element is object
    // which also accepts assignAll, which is called for it. 
    { check(); long n; for( n=0; n<qel; n++) el[n].assignAll(f); return *this;}
  ...
  */
  ...
  template<class P, class X> friend void apply2(DynLinArr<P>& ar, 
  ...
  void sort_select_increasing(DynLinArr< long >& sort_ind, long q_to_sort = 0) 
    const;  
  void sort_select_decreasing(DynLinArr< long >& sort_ind, long q_to_sort = 0) 
    const;  
...
{ long n; for(n=0; n<ar.qel; n++) (*fun)(ar.el[n]); }
...
{
#ifdef DEBUG_DYNLINARR
  mcout<<"DynLinArr<T>& DynLinArr<T>::operator=(const DynLinArr<T>& f) is working\n";
#endif
  if(this != &f)
  {
    ...
    if(q > 0)
    {
      ...
      long n; for(n=0; n<q; n++) new(&(temp_el[n])) T(f.el[n]);  // here
...
{
#ifdef DEBUG_DYNLINARR
  mcout<<"DynLinArr<T>& DynLinArr<T>::operator=(const DynLinArr<D>& f) is working\n";
#endif
  //if(this != &f)
  //{
    //mcout<<"DynLinArr<T>& operator=(const DynLinArr<T>& f): long(el)="
...
#ifdef USE_REPLACE_ALLOC
      el = (T*)malloc(sizeof(T) * fqel);
      for (long n = 0; n < fqel; ++n) new(&(el[n])) T;
#else
      el = new T[fqel];
#endif
    } 
...
void DynLinArr<T>::put_qel(long fqel, const T* val, ArgInterp_SingleAdr t) {
  ...
  //mcout<<"put_qel: *this="<<(*this);
  if (fqel < 0) {
    mcerr<<"ERROR in template<class T> void DynLinArr<T>::put_qel(long fqel, const T* val, ArgInterp_SingleAdr):\n";
    mcerr<<"fqel < 0, fqel="<<fqel<<'\n';
    mcerr<<"Type of T is (in internal notations) "<<typeid(T).name()<<'\n';
    spexit(mcerr);
  ...
  if (el == NULL) {
    ...
    if (val != NULL) for (long n = 0; n < qel; ++n) el[n] = *val;
  } else { 
    if (qel != fqel) {
      if (fqel <= 0) {
	qel = 0; 
...
#ifdef USE_REPLACE_ALLOC
	elh = (T*)malloc(sizeof(T) * fqel);
	for (long n = 0; n < fqel; ++n) new(&(elh[n])) T;
#else
	elh = new T[fqel]; // long q = find_min(qel,fqel);
#endif
	for (long n = 0; n < fqel; ++n) {
	  if (n < qel) elh[n] = el[n];
	  else if (val != NULL) elh[n] = *val;
	}
#ifdef USE_REPLACE_ALLOC
	for (long n = 0; n < qel; ++n) el[n].~T();
	free(el);
...
{
  mfunnamep("void DynLinArr<T>::sort(DynLinArr< long >& sort_ind, long q_to_sort = 0) const");
...
{
  mfunnamep("void DynLinArr<T>::sort_select_increasing(DynLinArr< long >& sort_ind, long q_to_sort = 0) const");
...
void DynLinArr<T>::sort_select_decreasing(DynLinArr< long >& sort_ind, 
			       long q_to_sort) const
{
  mfunnamep("void DynLinArr<T>::sort_select_decreasing(DynLinArr< long >& sort_ind, long q_to_sort = 0) const");
	    ...
	    )
{
  if(dla.get_qel() < qael)
  {
    mcerr<<"ERROR in long append(class DynLinArr& dla, ...): dla.get_qel() < qael\n"
...
#define Iprintdla_int( file , name ) file<<indn<<#name<<"="<<noindent; \
print_DynLinArr_int(file, name);
#define Iprintdla_long( file , name ) file<<indn<<#name<<"="<<noindent; \
print_DynLinArr_long(file, name);
#define Iprintdla_float( file , name ) file<<indn<<#name<<"="<<noindent; \
print_DynLinArr_float(file, name);
#define Iprintdla_double( file , name ) file<<indn<<#name<<"="<<noindent; \
...
{public:
  ...
  T* more(void) 
    {if(ncur < dar->get_qel()-1) return &((*dar)[++ncur]); 
    else ncur = dar->get_qel(); return NULL;}
  T* current(void)
    {if(ncur >= 0 || ncur < dar->get_qel()) return &((*dar)[ncur]); 
    else return NULL;}
  T* less(void)  // switch current to previous
    {if(ncur >= 1 ) return &((*dar)[--ncur]); 
    else ncur = -1; return NULL;}
...
template<class T>
int ifequal(const DynLinArr<T>& fd1, const T* fd2, 
	  long qfirst=-1)
{
  long n;
  if(qfirst==-1)
...
template<class T>
DynLinArr<T> merge(const DynLinArr<T>& fd1, long qfd1, 
		   const DynLinArr<T>& fd2, long qfd2)
{
  long n;
  if(qfd1 < 0)
  {
...
#ifndef DONT_USE_ABSPTR
template<class T>class DynArr: public RegPassivePtr
#else
template<class T>class DynArr
#endif
{public:
  ...
  DynArr(PILF_CONST DynArr<T>& f, Pilfer):qel(f.qel,steal), 
				cum_qel(f.cum_qel,steal), 
				el(f.el,steal) 
  {
#ifdef DEBUG_DYNARR
  ...
  void pilfer(PILF_CONST DynArr<T>& f)
  {
	...
	else { qel.clear(); cum_qel.clear(); el.clear(); }
      }
      qel.pilfer(f.qel); cum_qel.pilfer(f.cum_qel); el.pilfer(f.el);
      f.qel.clear(); f.cum_qel.clear(); f.el.clear();
    }
  }
  ...
  void pass(long q, DynLinArr<long> fqel, DynLinArr<long> fcum_qel, T* fel) 
    ...
    // from assignment operator above 
  { clear(); qel = fqel; cum_qel = fcum_qel; el.pass(q, fel); }
  ...
  {public:
    ...
    operator D& () const 
    {
      ...
      {
	mcerr<<"ERROR in IndexingProvider::operator D& (): q_deref_ind != qel.get_qel()\n";
    ...
    D& operator=(const D& f) 
    {
      ...
      {
	mcerr<<"ERROR in T& IndexingProvider::operator=(T& f): q_deref_ind != arr.get_qel().get_qel()\n";
    ...
    inline IndexingProvider<D>& operator[](long n)
    {
      if(q_deref_ind >= arr.get_qel().get_qel())
      {
	mcerr<<"ERROR in DynArr::IndexingProvider& DynArr::operator[](long n): q_deref_ind >= arr.get_qel().get_qel()\n";
	Iprint2n(mcerr, q_deref_ind, arr.get_qel().get_qel());
	mcerr<<"Type of T is (in internal notations) "<<typeid(D).name()<<'\n';
	spexit(mcerr); 
      }
	...
	return *this;
      ...
      {
	mcerr<<"Error in IndexingProvider<D>& IndexingProvider::operator[](long n): n < 0 || n >= qel.acu(q_deref_ind)\n";
  ...
  inline IndexingProvider<T> operator[](long n)
  {
    if(qel.get_qel() < 1)
    {
      mcerr<<"ERROR in DynArr::IndexingProvider DynArr::operator[](long n): qel.get_qel()< 1, qel.get_qel()="
	   <<qel.get_qel()<<'\n'; 
      mcerr<<"Type of T is (in internal notations) "<<typeid(T).name()<<'\n';
      spexit(mcerr); 
    }
    ...
    {
      mcerr<<"Error in IndexingProvider<T> DynArr::operator[](long n): n < 0 || n >= qel.acu(0)\n";
  ...
  inline const T& acu(long i1) const // for 1-dimensional array, completely unchecked
  {
    return el.acu(i1);
  }
  ...
  T& ac(long i1, long i2)  // for 2-dimensional array
  {
	...
	{
	  mcerr<<"Error in DynArr::ac(long i1, long i2): i2 < 0 || i2 >= qel.acu(1)\n";
	  Iprint2n(mcout, i2, qel[1]);
	}
      }
      else
      {
	mcerr<<"Error in DynArr::ac(long i1, long i2): i1 < 0 || i1 >= qel.acu(0)\n";
  ...
  */
    ...
    /*
  {
    ...
    */
  inline T& acu(long i1, long i2)  // for 2-dimensional array, completely unchecked
  {
    return el.acu(i1*cum_qel.acu(0) + i2);
  }
  ...
  T& ac(long i1, long i2, long i3)  // for 3-dimensional array
    {
      if(qel.get_qel() != 3)
      { mcerr<<"ERROR in DynArr::ac(long i1, long i2, long i3): qel.get_qel()!= 3,"
	     <<" qel.get_qel()=" <<qel.get_qel()<<'\n';       
      mcerr<<"Type of T is (in internal notations) "<<typeid(T).name()<<'\n';
      spexit(mcerr); }
#ifdef ALR_CHECK_EACH_BOUND
    if(i1 < 0 || i1 >= qel.acu(0))
    {
      mcerr<<"Error in DynArr::ac(long i1, long i2, long i3): i1 < 0 || i1 >= qel.acu(0)\n";
      Iprint2n(mcout, i1, qel[0]);
      spexit(mcerr);
    }
    if(i2 < 0 || i2 >= qel.acu(1))
    {
      mcerr<<"Error in DynArr::ac(long i1, long i2, long i3): i2 < 0 || i2 >= qel.acu(1)\n";
      Iprint2n(mcout, i2, qel[1]);
      spexit(mcerr);
    }
    if(i3 < 0 || i3 >= qel.acu(2))
    {
      mcerr<<"Error in DynArr::ac(long i1, long i2, long i3): i3 < 0 || i3 >= qel.acu(2)\n";
  ...
  void clear(void) {qel.clear(); cum_qel.clear(); el.clear(); }
  ...
  // (first of which is of type T and the second is of type of address 
  // of another function (possibly apply1) to each element.
  template<class P, class X> friend void apply2(DynArr<P>& ar, 
			void (*fun1)(P& f, void (*fun21)(X& f) ), 
			void (*fun2)(X& f) );

  void check(void) const { qel.check(); cum_qel.check(); el.check(); }
  int get_s_non_emplty(void) const
  { 
    long q = qel.get_qel();
    if(q == 0) return 0;
    long n; for(n=0; n<q; n++){ if(qel[n] <=0) return 0;}
    return 1;
  }
#ifndef DONT_USE_ABSPTR
...
private:
  ...
  long calc_lin_ind(const DynLinArr<long>& ind) const
    {
      long i=0; long n; long qdim1=qel.get_qel() - 1;
      /* 
      ...
      {
	mcerr<<"ERROR in long DynArr::calc_lin_ind(const DynLinArr<long>& ind) const\n";
...
DynArr<T>& DynArr<T>::operator=(const DynArr<D>& f)
{
  ...
  //{
    ...
    T* temp_el = (q > 0) ? (new T[q]) : (T*)NULL;
#endif
    long n; for( n=0; n<q; n++) temp_el[n] = f.acu_lin(n);
...
{ 
  ...
  long n; for(n=0; n<q; n++) (*fun)(ar.el[n]); 
}
template<class T, class X>
...
{ 
  long q = ar.el.get_qel(); 
  long n; for(n=0; n<q; n++) (*fun1)(ar.el[n], fun2); 
}
...
{public:
  IterDynArr(const DynArr<T>* fdar): 
    ...
    {
      long n; long qdim1=ncur.get_qel()-1; 
      if(qdim1>=0) {for( n=0; n<qdim1; n++) ncur[n] = 0; ncur[qdim1] = -1; }
    }
  ...
  T* current(void)   // Element currently pointed by ncut.
    {
      long n; long qdim=ncur.get_qel(); if(qdim <=0 ) return NULL;
      for( n=0; n<qdim; n++ )
...
{ 
  ...
  if(qel.get_qel() == 0)
  {
    *this=DynArr<T>(qel_communicat, val); return;
  }
...
{
  mfunnamep("template<class T, class X> void copy_DynArr(const DynArr<T>& s, DynArr<X>& d)");
  s.check();  d.check();
...
{
  mfunnamep("template<class T, class X> void convert_DynArr(const DynArr<T>& s, DynArr<X>& d)");
  s.check();  d.check();
  d = DynArr<X>(s.get_qel(), NULL);
  IterDynArr<T> iter(&s);
  T* at;
...
DynLinArr<T>::DynLinArr(const DynArr<T>& f)
{
  ...
  if(qel > 0)
  {
    ...
    long n; for(n=0; n<qel; n++) new(&(el[n])) T;
#else
    el=new T[qel];
#endif
    long n;
  ...
  // takes only mentioned raw or column.
{
  ...
  if(qdim != 2)
  {
    mcerr<<"ERROR in DynLinArr<T>::DynLinArr(const DynArr<T>& f, int n_of_dim,long roc_number):\n"
...
std::ostream& operator<<(std::ostream& file, const DynArr<T>& f)
{
  ...
  if(f.get_s_non_emplty() == 1)
  {
    if(s_short_output == 0)
    {
      ...
      Ifile<<"(The first number is sequencial number, then there are indexes, the last is the element)\n";
      //DynArr<T>& ff(f);
    }
    long nseq=0;
...
std::istream& operator>>(std::istream& file, DynArr<T>& f)
{
  ...
  if(s_short_output == 0)
  {
    set_position("cum_qel=DynLinArr<T>:", 
...
#define Iprintda_double( file , name ) file<<indn<<#name<<"="<<noindent; \
