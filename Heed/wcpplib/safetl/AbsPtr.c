...
//#endif
#else
  s_ban_del(f.s_ban_del), s_ban_sub(f.s_ban_sub), 
...
#ifdef USE_BIT_OPERA
  else if( f.get_s_ban_cop() == 1 && 
	   f.cpp->get_number_of_booked() > 0 )
#elif defined( USE_BIT_FIELDS )
  else if( f.conparam.s_ban_cop == 1 && 
	   f.cpp->get_number_of_booked() > 0 )
#else
...
#endif
  {
       ...
       cpp != NULL && cpp->get_number_of_booked() > 0 )
    {
      mcerr<<"Error in "
	   ...
	   <<"This can lead to loss of pointers to copied object, not only to this one.\n";
      spexit(mcerr);
    }
    clear_pointers();
  }
#ifdef USE_BIT_OPERA
  else if(get_s_ban_sub() == 2 && 
	  cpp != NULL && cpp->get_number_of_booked() > 0)
#elif defined( USE_BIT_FIELDS )
  else if(conparam.s_ban_sub == 2 && 
	  cpp != NULL && cpp->get_number_of_booked() > 0)
#else
...
#elif defined( USE_BIT_FIELDS )
  Ifile<<"s_allow_del_at_zero_count="
       <<f.conparam.s_allow_del_at_zero_count<<'\n';
#else
#ifdef USE_CHAR_CONTROL_VARIABLES
...
{  
  ...
  if(cpp != NULL) 
  {
	...
	mcerr<<"cpp->number_of_registered="
	     <<cpp->get_number_of_booked()<<'\n';
	s_ban_del_ignore = 1;
	spexit(mcerr);
      }
    }	
