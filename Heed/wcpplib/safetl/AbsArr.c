				 ...
				 const DynLinArr<double>& dar3)
{
  ...
  Ifile<<"qiar="<<qiar
       <<" qdar1="<<qdar1<<" qdar2="<<qdar2<<" qdar3="<<qdar3<<'\n';
  iar.check();
  dar1.check();
  dar2.check();
  dar3.check();
  if( q_max <= 0 ) {indn.n -= 2; return;}
  Ifile<<"index int array    double array1      double array2      double array3\n";
  long i;
  for( i=0; i<q_max; i++)
  { 
    Ifile<<setw(4)<<i<<' ';
...
{
  mfunname("void print_DynArr_int_w(ostream& file, const DynArr<int>& f, int w)");
  f.check();
  Ifile<<"DynArr<int>:";
...
{
  long n; long qdim=qel.get_qel();  if(qdim <=0 ) return 0;
...
int find_prev_comb(const DynLinArr<long>& qel, DynLinArr<long>& f)
{
  ...
  for( n=0; n<qdim-1; n++)
    f[n]=0;  // the first element
  f[qdim-1] = -1; return 0;   // previous before first
}
