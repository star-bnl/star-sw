...
template<class T>class BlkArr: public RegPassivePtr
#else
template<class T>class BlkArr
#endif
{public:
  BlkArr(void): qel(0), la(), size_of_element(sizeof(T)) {;}
  explicit BlkArr(long fqel): qel(fqel), la( DynLinArr< T >(0) ),
    size_of_element(sizeof(T)) 
    { if(qel > 0) la.get_first_node()->el.put_qel(qel); }
  BlkArr(long fqel, const T& val): qel(fqel), la( DynLinArr< T >(0) ),
    size_of_element(sizeof(T)) 
    { if(qel > 0) la.get_first_node()->el.put_qel(qel, val); }
  BlkArr(long fqel, const T* ar, ArgInterp_Arr t): 
    qel(fqel), la( DynLinArr< T >(0) ) ,
    size_of_element(sizeof(T))
    { if(qel > 0) 
  ...
  BlkArr<T>& operator=(const BlkArr<T>& f)
    {
      ...
      if(this != &f)
      {
	qel = f.qel; size_of_element=f.size_of_element;
	la=f.la;
      }
      return *this;
    }
  ...
  BlkArr(PILF_CONST BlkArr<T>& f, Pilfer):qel(f.qel), 
  ...
  void pilfer(PILF_CONST BlkArr<T>& f)
  {
	...
	else { la.clear(); size_of_element = 0; qel = 0; }
      }
      qel = f.qel; 
      size_of_element = f.size_of_element;
      la.pilfer(f.la);
  ...
  BlkArr& assignAll(const T& f) 
    { long n; for( n=0; n<qel; n++) operator[](n)=f; return *this; }
  T& operator[](long n);
  ...
  void increment(void)
    { long q=qel+1; put_qel(q); }
  void decrement(void)
    { long q=qel-1; put_qel(q); }
  void append(const T& val)
    {
      increment(); operator[](qel-1) = val;
    }
...
T& BlkArr<T>::operator[](long n)
{
  ...
  long q;
  AbsListNode< DynLinArr<T> >* 
    aln( la.get_first_node() ); 
  do
  {
  ...
  */
  ...
  else
  {
	...
	if(q >= left_old_qel)  // this node, there is space 
	{
	  ...
	  { // in the current node. But there may be next ones inited already
	    left_new_qel -= q;
	    left_new_qel_res -= q;
	    AbsListNode< DynLinArr<T> >* aln1;  // pointer to block
	    while((aln1 = aln->get_next_node()) != NULL)
	    {  // scanning next blocks, which may be present already
	      aln = aln1;
	      q = aln->el.get_qel();
	...
	if(q >= left_new_qel)  // this node, there is space 
	{
	  ...
	  if(aln1 != NULL)  // so preserving next node as histeresis
	  {
	    ...
	    {  // if fqel=0, all the next nodes will be deleted in total later
	      aln = aln1;
	      q = aln->el.get_qel();
