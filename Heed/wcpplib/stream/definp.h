...
{public:
  ...
  definp_endpar(void): istrm(NULL), s_rewind(0), s_req_sep(0), s_print(0), 
		       s_short(0) {;}
  definp_endpar(std::istream* fistrm, int fs_rewind, int fs_req_sep, 
		int fs_print=1, int fs_short = 0 ):
  istrm(fistrm), s_rewind(fs_rewind), s_req_sep(fs_req_sep), s_print(fs_print),
  s_short(fs_short) {;}
};
...
//	int s_print=1 )
template< class T > void definp_any_par
	(T& inp, const String& word, const definp_endpar& dep, 
	 int fs_short = 0)
{
  ...
  //Iprintn(mcout, inp);
  check_econd11a(dep.istrm->good() , != 1 , "after input of variable named "<<word<<"\nwhose input value is "<<inp<<'\n' , 
				mcerr);
}
...
//	(T& inp1, T& inp2, String& word, std::istream& istrm, int s_rewind, int s_req_sep, 
//	int s_print=1 )
template< class T > void definp_any_2par
	(T& inp1, T& inp2, const String& word, const definp_endpar& dep, 
	 int fs_short = 0 )
{
  mfunnamep("template< class T > definp_any_par(...)");
  if(fs_short == 0)
  ...
  //Iprintn(mcout, inp);
  check_econd11a(dep.istrm->good() , != 1 , "after input of variables after word "<<word<<"\nwhose input values are "<<inp1<<' '<<inp2<<'\n' , 
				mcerr);
}
