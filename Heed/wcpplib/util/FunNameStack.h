...
#ifdef FUNNAMESTACK
#define mfunname(string) static char* FunNameIIII=string;  \
FunNameWatch funnw(FunNameIIII)
#define mfunname1(string) static char* FunNameIIII1=string;  \
FunNameWatch funnw1(FunNameIIII1)
#define mfunname2(string) static char* FunNameIIII2=string;  \
FunNameWatch funnw2(FunNameIIII2)
#define mfunname3(string) static char* FunNameIIII3=string;  \
FunNameWatch funnw3(FunNameIIII3)
#define mfunname4(string) static char* FunNameIIII4=string;  \
FunNameWatch funnw4(FunNameIIII4)
#define mfunname5(string) static char* FunNameIIII5=string;  \
...
#define mfunnamep(string) static const char* FunNameIIII=string;  \
FunNameWatch funnw(FunNameIIII)
#define mfunnamep1(string) static const char* FunNameIIII1=string;  \
FunNameWatch funnw1(FunNameIIII1)
#define mfunnamep2(string) static const char* FunNameIIII2=string;  \
FunNameWatch funnw2(FunNameIIII2)
#define mfunnamep3(string) static const char* FunNameIIII3=string;  \
FunNameWatch funnw3(FunNameIIII3)
#define mfunnamep4(string) static const char* FunNameIIII4=string;  \
FunNameWatch funnw4(FunNameIIII4)
#define mfunnamep5(string) static const char* FunNameIIII5=string;  \
FunNameWatch funnw5(FunNameIIII5)
...
#define check_econd(condition, add, stream) if(condition) \
{ funnw.ehdr(stream); stream<<'\n'<<#condition<<'\n'; \
stream<<add; spexit(stream); } 
#define check_wcond(condition, add, stream) if(condition) \
{ funnw.whdr(stream); stream<<'\n'<<#condition<<'\n'; \
stream<<add; } 

#define check_econd1(condition, a1, stream) if(condition) \
{ funnw.ehdr(stream); stream<<'\n'<<#condition<<'\n'; \
stream<<#a1<<'='<<(a1)<<'\n'; spexit(stream); } 
#define check_wcond1(condition, a1, stream) if(condition) \
{ funnw.whdr(stream); stream<<'\n'<<#condition<<'\n'; \
stream<<#a1<<'='<<(a1)<<'\n'; } 

#define check_econd2(condition, a1, a2, stream) if(condition) \
{ funnw.ehdr(stream); stream<<'\n'<<#condition<<'\n'; \
stream<<#a1<<'='<<(a1)<<' '<<#a2<<'='<<(a2)<<'\n'; spexit(stream); } 
#define check_wcond2(condition, a1, a2, stream) if(condition) \
{ funnw.whdr(stream); stream<<'\n'<<#condition<<'\n'; \
stream<<#a1<<'='<<(a1)<<' '<<#a2<<'='<<(a2)<<'\n'; } 

#define check_econd3(condition, a1, a2, a3, stream) if(condition) \
{ funnw.ehdr(stream); stream<<'\n'<<#condition<<'\n'; \
stream<<#a1<<'='<<(a1)<<' '<<#a2<<'='<<(a2)<<' '<<#a3<<'='<<(a3)<<'\n'; \
spexit(stream); } 
#define check_wcond3(condition, a1, a2, a3, stream) if(condition) \
{ funnw.whdr(stream); stream<<'\n'<<#condition<<'\n'; \
stream<<#a1<<'='<<(a1)<<' '<<#a2<<'='<<(a2)<<' '<<#a3<<'='<<(a3)<<'\n'; } 

#define check_econd4(condition, a1, a2, a3, a4, stream) if(condition) \
{ funnw.ehdr(stream); stream<<'\n'<<#condition<<'\n'; \
stream<<#a1<<'='<<(a1)<<' '<<#a2<<'='<<(a2)<<' '<<#a3<<'='<<(a3)<<' ' \
<<#a4<<'='<<(a4)<<'\n'; \
spexit(stream); } 
#define check_wcond4(condition, a1, a2, a3, a4, stream) if(condition) \
{ funnw.whdr(stream); stream<<'\n'<<#condition<<'\n'; \
stream<<#a1<<'='<<(a1)<<' '<<#a2<<'='<<(a2)<<' '<<#a3<<'='<<(a3)<<' ' \
<<#a4<<'='<<(a4)<<'\n'; } 

#define check_econd11(a, signb, stream) if(a  signb) \
{ funnw.ehdr(stream); stream<<'\n'<<#a<<#signb<<'\n'; \
stream<<#a<<'='<<(a)<<'\n'; spexit(stream); } 
#define check_wcond11(a, signb, stream) if(a  signb) \
{ funnw.whdr(stream); stream<<'\n'<<#a<<#signb<<'\n'; \
stream<<#a<<'='<<(a)<<'\n'; }  

#define check_econd12(a, sign, b, stream) if(a  sign  b) \
{ funnw.ehdr(stream); stream<<'\n'<<#a<<#sign<<#b<<'\n'; \
stream<<#a<<'='<<(a)<<' '<<#b<<'='<<(b)<<'\n'; spexit(stream); } 
#define check_wcond12(a, sign, b, stream) if(a  sign  b) \
{ funnw.whdr(stream); stream<<'\n'<<#a<<#sign<<#b<<'\n'; \
stream<<#a<<'='<<(a)<<' '<<#b<<'='<<(b)<<'\n'; }  

//condition + additional any commands
#define check_econd11a(a, signb, add, stream) if(a  signb) \
{ funnw.ehdr(stream); stream<<'\n'<<#a<<#signb<<'\n'; \
stream<<#a<<'='<<(a)<<'\n'; stream<<add; spexit(stream); } 
#define check_wcond11a(a, signb, add, stream) if(a  signb) \
{ funnw.whdr(stream); stream<<'\n'<<#a<<#signb<<'\n'; \
stream<<#a<<'='<<(a)<<'\n'; stream<<add; }  

#define check_econd12a(a, sign, b, add, stream) if(a  sign  b) \
{ funnw.ehdr(stream); stream<<'\n'<<#a<<#sign<<#b<<'\n'; \
stream<<#a<<'='<<(a)<<' '<<#b<<'='<<(b)<<'\n'; stream<<add;spexit(stream); } 
#define check_wcond12a(a, sign, b, add, stream) if(a  sign  b) \
{ funnw.whdr(stream); stream<<'\n'<<#a<<#sign<<#b<<'\n'; \
stream<<#a<<'='<<(a)<<' '<<#b<<'='<<(b)<<'\n'; stream<<add;}  
...
// Normal exit:
#define spexit(stream) {stackprt(stream); \
...
{public:
  ...
  NameStack(const NameStack& f): qname(0), id(0), nmode(0) 
  { *this = f; }
