...
template <class T> long find_interval(T x, DynLinArr<T2DPoint<T> > ar) {
  long n1, n2, n3;
  long q = ar.get_qel();
  if (q <= 0) return -1;
...
template <class T> double lin_interpolation(T x, DynLinArr<T2DPoint<T> > ar) {
...
extern void hist_lin_integ_ar(float *x, float *y, long q,
                              ...
                              float xhist_start, float xhist_step, float *yhist,
                              long qhist);
// histogram

extern float lin_integ_ar(float *x, float *y, long q, float x1, float x2);
// fit table by a straight line and integrate the area below it.

extern double lin_integ_ar(DynLinArr<double> x, DynLinArr<double> y, long q,
                           double x1, double x2);
// fit table by a straight line and integrate the area below it.

extern float step_integ_ar(float *x, float *y, long q, float x1, float x2);
...
extern double step_integ_ar(const double *x, const double *y, long q, double x1,
                            double x2);
// here x is a left side of interval on which
// fuction f is a constant.
// x and y are arrays with dimen. q and q-1 respectively;
...
extern double find_x_for_known_int_hist(long q, double xmin, double xmax,
                                        const double *y, double integ,
                                        int *s_err);
extern double find_x_for_known_int_hist(long q, double xmin, double xmax,
...
extern double new_step_integ_ar_hist(const long q, double xmin, double xmax,
                                     DynLinArr<double> y, double x1, double x2);

extern double new_step_integ_ar_x_hist(const long q, double xmin, double xmax,
                                       DynLinArr<double> y, double x1,
                                       double x2);
...
s_err = 1 if ..hist ==0
*/
extern double new_step_mean_x_hist(const long q, double xmin, double xmax,
                                   DynLinArr<double> y, double x1, double x2,
                                   int &s_err);
