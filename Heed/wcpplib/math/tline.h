...
{public:
  inline long get_qi(void) const  {return q;} // get number of intervals
  ...
  virtual int get_interval(T x, 
			   long& n1, T& b1, // the bin number and left border
			   long& n2, T& b2) // the next bin number and its left
    const ;    
  
  virtual int get_interval_extrap(T x, 
				  long& n1, T& b1, 
				  long& n2, T& b2) const ;  
...
{
  mfunname("template<class T> EqualStepCoorMesh<T>::EqualStepCoorMesh<T>(long fq, T fxmin, T fxmax)");
...
template<class T>
int EqualStepCoorMesh<T>::get_interval(T x, 
				       long& n1 ) const 
{
  if(x < xmin || x >= xmax)
  {
  ...
  if(n1 < 0 )
  {
    ...
    //{
    //mcerr<<"ERROR in EqualStepCoorMesh<T>::get_interval:\n"
    //	 <<"n1 < 0 || n1 >= q || n2 < 0 || n2 > q || b1 < xmin || b1 > xmax || b2 < xmin || b2 > xmax\n";
    print(mcerr);
    Iprintn(mcerr, x);
    Iprintn(mcerr, n1);
    spexit(mcerr);
...
template<class T>
int EqualStepCoorMesh<T>::get_interval_extrap(T x, 
					      long& n1, T& b1, 
					      long& n2, T& b2) const 
{
  int i_ret = 1;
...
istream& operator>>(istream& file, EqualStepCoorMesh<T>& f)
{
  ...
  set_position("Type of T is (in internal notations)", 
...
template<class T, class D>  // D is anything allowing indexing
...
long t_find_interval_end(double x, 
			 long q, const D& coor, long n_start)
{
  long n1,n2,n3;
...
{public:
  ...
  PointCoorMesh<T,D>(void): q(0), xmin(0), xmax(0), x_old(0), n_old(-1), 
  amesh(NULL) {;}
...
template<class T, class D>
int PointCoorMesh<T,D>::get_interval(T x, 
				     long& n1) const 
{
  if(x < xmin || x >= xmax)
  {
...
template<class T, class D>
int PointCoorMesh<T,D>::get_interval_extrap(T x, 
					    long& n1, T& b1, 
					    long& n2, T& b2) const 
{
  int i_ret = 1;
...
{public:
  ...
  CopiedPointCoorMesh<T>(void): 
    q(0), xmin(0), xmax(0), x_old(0), n_old(-1) {;}
  ...
  CopiedPointCoorMesh<T>(const CopiedPointCoorMesh<T>& f):
    q(f.q), mesh(f.mesh), xmin(f.xmin), xmax(f.xmax),
    x_old(f.x_old), n_old(f.n_old)
#ifdef TLINE_REDUCE_TO_RAW_ARR 
    amesh(&mesh)
#endif
  {}
...
template<class T, class B> int verify_types(const T* ths, const B* b2, 
					    const T** tb2)
{
  mfunnamep("template<class T> verify_types(const T* ths, const B* b2, const T* tb2)");
  ...
  {
    funnw.ehdr(cerr);
    mcerr<<"Operator == for class T is called for first object of different class\n";
    mcerr<<"This may mean that this operator is missed in one of derived classes\n";
...
{public:
  ...
  virtual int apeq_mant(const AbsCoorMesh<T>& m2, T prec)
  {
    ...
    funnw.ehdr(cerr);
    mcerr<<"AbsCoorMesh::apeq_mant != cannot be called since this is abstract class\n";
    spexit(mcerr);
#ifdef INS_CRETURN
    return 0; // to calm compiler
#endif
...
template<class T> class VirtEqualStepCoorMesh: 
  public AbsCoorMesh< T >, public EqualStepCoorMesh< T >
{public:
  ...
  virtual int operator==(const AbsCoorMesh<T>& m2)
  {
    mfunname("virtual int VirtEqualStepCoorMesh<T>::operator==(const AbsCoorMesh<T>& m2)");
...
{  
  ...
  definp_endpar dep(&file, 0, 1, 0);
  set_position("EqualStepCoorMesh<T>:", *dep.istrm, 
	       dep.s_rewind, dep.s_req_sep); 
  //file>>f;
...
template<class T> class VirtCopiedPointCoorMesh: 
  public AbsCoorMesh< T >, public CopiedPointCoorMesh< T >
{public:
  virtual long get_qi(void) const  
  { return CopiedPointCoorMesh< T >::get_qi(); }
  virtual T get_xmin(void) const 
  { return CopiedPointCoorMesh< T >::get_xmin(); }
  virtual T get_xmax(void) const 
  { return CopiedPointCoorMesh< T >::get_xmax(); }
...
istream& operator>>(istream& file, VirtCopiedPointCoorMesh<T>& f)
{
  ...
  definp_endpar dep(&file, 0, 1, 0);
  set_position("CopiedPointCoorMesh<T>:", *dep.istrm, 
	       dep.s_rewind, dep.s_req_sep); 
                  ...
                  // 0 - not include, 1 - include  
{
  ...
  i_ret = mesh1.get_interval(x1, 
			     n11, b1, 
			     n12, b2);
  check_econd11(i_ret , != 1 , mcerr);

  i_ret = mesh2.get_interval(x2, 
			     n21, b1, 
			     n22, b2);
...
template<class T, class D, class M>
T t_find_x_for_already_integ_step_ar
(const M& mesh,               
 const D& y,        // array of function values         
 T integ,  
 int* s_err)   // for power = 0 only
		   ...
		   ) 
{
  ...
  for( n=0; n<qi; n++)
  {
    ...
    mesh.get_scoor(n+1, xp2); 
    T step = xp2 - xp1;
    check_econd11a(y[n] , < 0.0 , "n="<<n<<" xp1="<<xp1<<" xp2="<<xp2<<'\n' ,
		   mcerr);
    s = s + y[n] * step;
    integ_y[n] = s;
    //Iprint3n(mcout, n, s1, integ);
		   ...
		   T rannum)  
{
  ...
  T ret = t_find_x_for_already_integ_step_ar
    (mesh,               // dimension q 
     integ_y,                  // dimension q-1
     rannum,  
     &s_err);
  check_econd11a(s_err , !=  0 , 
			  ...
			  T rannum)  
{
  ...
  long ret =  t_find_entire_x_for_already_integ_step_ar
    (mesh,               // dimension q 
     integ_y,                  // dimension q-1
     rannum,  
     &s_err);   
  ...
  // 0 - not include, 1 - include  
{
  ...
  T b1, b2;
  int i_ret=0;
  i_ret = mesh.get_interval_extrap(x, 
				   n1, b1, 
				   n2, b2);
  T x1;
  ...
  return t_value_straight_2point
    (x1, y[n1], x2, y[n2],   
     x,
     s_ban_neg
     );
...
(const M& mesh,  
 const D& y,             // array of function values
 T (*funval)( T xp1, T yp1, T xp2, T yp2, 
	      T xmin, T xmax,  // may be necessary for shape determination
	      T x), 
 T x,
 int s_extrap_left, T left_bond,
 int s_extrap_right, T right_bond
...
template<class T>
T t_value_power_extended_2point
(T x1, T y1, T x2, T y2,   
 T x)
{
  mfunname("double t_value_power_2point(...)");
...

{
  ...
  if(x1 <= xmin && s_extrap_left == 0)
    x1 = xmin;
  if(x2 > xmax && s_extrap_left == 0)
    x2 = xmax;
  long np1, np2;
  T bp1, bp2;
  int i_ret=0;
  // restore the interval in which x1 reside
  i_ret = mesh.get_interval_extrap(x1, 
				   np1, bp1, 
				   np2, bp2);
			   ...
			   int& s_err)   
{
  ...
  s_err = 0;
  T integ = t_integ_straight_point_ar
    (mesh, y,  x1, x2, 0, 1, 
     s_extrap_left, left_bond, s_extrap_right, right_bond);   
  if(integ == 0)
  {
    s_err = 1;
    return 0.0;
  }
  T xinteg = t_integ_straight_point_ar
    (mesh, y,  x1, x2, 1, 1, 
     s_extrap_left, left_bond, s_extrap_right, right_bond);  
  return xinteg / integ;
}
  ...
  // 0 - not include, 1 - include  
{
  ...
  for(n=nmax; n<qi; n++)
  {
    ...
    if(y[n] > ylev && y[n+1] <= ylev)
    {
      ...
      step1 = step1 / 2.0;
      step2 = step2 / 2.0;
      s2+=t_value_straight_2point(y[n], x1+step1, y[n+1], x2+step2, ylev, 0);
      //Iprint2n(mcout, x1, x2);
      //Iprint2n(mcout, x1+step1, x2+step2);
      //Iprint2n(mcout, y[n], y[n+1]);
      //Iprint2n(mcout, n, t_value_straight_2point(y[n], x1+step1, y[n+1], x2+step2, ylev, 0));
      q++;
    }
  }
  check_econd11(q , <= 0 , mcerr);
  ...
  for(n=nmax; n>= 0; n--)
  {
    ...
    if(y[n] > ylev && y[n-1] <= ylev)
    {
      ...
      step1 = step1 / 2.0;
      step2 = step2 / 2.0;
      s1+=t_value_straight_2point(y[n-1], x1+step1, y[n], x2+step2, ylev, 0);
      //Iprint2n(mcout, x1, x2);
      //Iprint2n(mcout, x1+step1, x2+step2);
      //Iprint2n(mcout, y[n-1], y[n]);
      //Iprint2n(mcout, n, t_value_straight_2point(y[n-1], x1+step1, y[n], x2+step2, ylev, 0));
      q++;
    }
  }
  check_econd11(q , <= 0 , mcerr);
