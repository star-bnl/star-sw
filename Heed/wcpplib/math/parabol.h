...
{public:
  ...
  inline void put_a(double fa) { da=fa; s_det=0; s_dxzero=0;} 
  inline void put_b(double fb) { db=fb; s_det=0; s_dxzero=0;} 
  inline void put_c(double fc) { dc=fc; s_det=0; s_dxzero=0;} 

  Parabol(void): da(0.0), db(0.0), dc(0.0), s_det(0), s_dxzero(0) {}
  Parabol(const Parabol& f);
  Parabol(double fa, double fb, double fc): da(fa), db(fb), dc(fc), 
    s_det(0), s_dxzero(0) {}
  ...
  Parabol(double x1, double x2, double x3, 
	  double y1, double y2, double y3); // creates parabola by 3 points
  ...
  inline double determinant(void) const 
    { 
      convmut(Parabol);
      if(s_det == 0) { t.s_det = 1; t.det=db * db - 4 * da * dc; }
      return det;
    }  
