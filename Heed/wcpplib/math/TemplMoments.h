...
template <class T> class TemplMoments {
 public:
  ...
  T get_xmean(void) const {
    if (s_sync == 0) calc();
    return xmean;
  }
  T get_sigma(void) const {
    if (s_sync == 0) calc();
    return sigma;
  }
...
// ///// No:If xmin ==1 and xmax == 0, the filling is not yet started
 ...
 public:
  ...
  TemplMoments<T>() {
    xnorm = xmean = sigma = xsum = x2sum = 0;
    s_sync = 1;
    qfill = 0;
#ifdef INC_MINMAX
    xmax = 0;
...
#endif
                  int anything)
      :  // to differ it from previous constructor
        qfill(fqfill),
        xnorm(fxnorm),
        xmean(fxmean),
        sigma(fsigma)
...
template <class T> void TemplMoments<T>::calc(void) const {
  if (xnorm <= 0) {
    xmean = 0;
    sigma = 0;
  } else {
...
template <class T> void TemplMoments<T>::plus_pilar(T a, T b, T v) {
#ifdef INC_MINMAX
  if (qfill == 0) {
    xmin = a;
    xmax = a;
...
template <class T> void TemplMoments<T>::plus_event(T a, T v) {
#ifdef INC_MINMAX
  if (qfill == 0) {
    xmin = a;
    xmax = a;
...
template <class T> void TemplMoments<T>::minus_event(T a, T v) {
#ifdef INC_MINMAX
  if (qfill == 0) {
    xmin = a;
    xmax = a;
...
template <class T> void TemplMoments<T>::retrieve_sum() const {
  xsum = xmean * xnorm;
...
template <class T> TemplMoments<T> TemplMoments<T>::operator*(T s) const {
  TemplMoments<T> temp = *this;
  temp.xnorm *= s;
  temp.xsum *= s;
  temp.x2sum *= s;
...
template <class T> TemplMoments<T>& TemplMoments<T>::operator*=(T s) {
  xnorm *= s;
  xsum *= s;
  x2sum *= s;
  xmean = sigma = 0;  //after must be called calc!
...
template <class T> TemplMoments<T>& TemplMoments<T>::scale_x(T s) {
  xsum *= s;
  x2sum *= s * s;
  xmean = sigma = 0;  //after must be called calc!
#ifdef INC_MINMAX
...
template <class T> void clear(TemplMoments<T>& f) { f.clear(); }
...
template <class T> istream& operator>>(istream& file, TemplMoments<T>& mom) {
...
template <class T>
  ...
  if (s_short_output == 0) {
    Ifile << "TemplMoments<T>: qfill=" << setw(5) << mom.get_qfill()
          << " norm=" << setw(5) << mom.get_xnorm() << " mean=" << setw(5)
          << mom.get_xmean() << " RMS=" << setw(5) << mom.get_sigma()
#ifdef INC_MINMAX
          << " xmin/xmax=" << mom.get_xmin() << ' ' << mom.get_xmax()
#endif
          << '\n';
  } else {
    Ifile << mom.get_qfill() << ' ' << mom.get_xnorm() << ' ' << mom.get_xmean()
          << ' ' << mom.get_sigma()
#ifdef INC_MINMAX
...
template <class T>
  ...
  if (f1.get_qfill() == f2.get_qfill() && f1.get_xnorm() == f2.get_xnorm() &&
      f1.get_xmean() == f2.get_xmean() && f1.get_sigma() == f2.get_sigma()
#ifdef INC_MINMAX
      && f1.get_xmin() == f2.get_xmin() && f1.get_xmax() == f2.get_xmax()
#endif
      ) {
    return 1;
  } else {
