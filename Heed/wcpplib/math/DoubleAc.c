...
DoubleAc& DoubleAc::operator*=(DoubleAc f)
{
  ...
  if(d  <  di)
  {
    ...
    f.print(mcerr, 6);
    if(d > di)
      mcerr<<"if(d > di) is also positive\n";
    if(d == di)
      mcerr<<"if(d == di) is also positive\n";
    Iprintn(mcerr, d - di);
#ifdef DEBUG_OPERATOR_MULT
  ...
  if(d  >  da)
  {
    ...
    f.print(mcerr, 6);
    if(d < da)
      mcerr<<"if(d < da) is also positive\n";
    if(d == da)
      mcerr<<"if(d == da) is also positive\n";
    Iprintn(mcerr, d - da);
#ifdef DEBUG_OPERATOR_MULT
...
DoubleAc asin(const DoubleAc& f)
{
  ...
  double di;
  if(f.left_limit() <-1.0) di = std::asin(-1.0);
  else di = std::asin(f.left_limit());
  double da;
  if(f.right_limit() > 1.0) da = std::asin(1.0);
  else da = std::asin(f.right_limit());
  return DoubleAc(d,di,da);
} 
...
DoubleAc acos(const DoubleAc& f)
{
  ...
  double da;
  if(f.left_limit() <-1.0) da = std::acos(-1.0);
  else da = std::acos(f.left_limit());
  double di;
  if(f.right_limit() > 1.0) di = std::acos(1.0);
  else di = std::acos(f.right_limit());
  return DoubleAc(d,di,da);
} 
