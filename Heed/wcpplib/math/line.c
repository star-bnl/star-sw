...
long find_interval(double x, DynLinArr<double> coor) {
...
long find_interval(double x, double coor[], long q) {
...
double lin_interpolation(double x, DynLinArr<double> coor,
                         DynLinArr<double> arr) {
  mfunname("double lin_interpolation(T x, DynLinArr< double > coor, DynLinArr< "
           "double > arr)");
...
double lin_interpolation(double x, double coor[], double arr[], long q) {
  mfunname(
      "double lin_interpolation(T x, double coor[], double arr[], long q)");
...
double double_parab_interpolation(double x, DynLinArr<double> coor,
                                  DynLinArr<double> arr) {
  mfunname("double double_parab_interpolation(T x, DynLinArr< double > coor, "
           "DynLinArr< double > arr)");
  ...
  if (q == 1)  // line
    ...
    return arr[nstart] +
      ...
      {
    Parabol pr(coor[nstart], coor[nstart + 1], coor[nstart + 2], arr[nstart],
               arr[nstart + 1], arr[nstart + 2]);
    return pr.y(x);
    ...
    Parabol pr1(coor[nstart], coor[nstart + 1], coor[nstart + 2], arr[nstart],
                arr[nstart + 1], arr[nstart + 2]);
    double y1 = pr1.y(x);
...
void hist_lin_integ_ar(float *x, float *y, long q, float xhist_start,
                       float xhist_step, float *yhist, long qhist) {
  long nh;
  float x1, x2;
  x2 = xhist_start;
...
double lin_integ_ar(DynLinArr<double> x, DynLinArr<double> y, long q, double x1,
                    double x2)
    // fit table by a straight line and integrate the area below it.
    {
...
double step_integ_ar(const double *x, const double *y, long q, double x1,
                     double x2)
    // here x is a left side of interval on which
    // fuction f is a constant.
...
double find_x_for_known_int_hist(long q, double xmin, double xmax,
                                 const double *y, double integ, int *s_err) {
  ...
  *s_err = 0;
  ...
  for (n = 0; n < q; n++) {
    ...
    if (s1 > integ_step) break;
    if (s1 == integ_step) return xmin + step * (n + 1);
    s = s1;
  }
  if (n == q) {
    *s_err = 1;
...
double find_x_for_known_int_hist(long q, double xmin, double xmax,
                                 DynLinArr<double> y, double integ,
                                 int *s_err) {
  mfunname("double find_x_for_known_int_hist(...)");
  check_econd11(q, < 0, mcerr);
  check_econd12(xmax, <, xmin, mcerr);
...
double new_step_integ_ar_hist(const long q, double xmin, double xmax,
                              DynLinArr<double> y, double x1, double x2) {
...
double new_step_integ_ar_x_hist(const long q, double xmin, double xmax,
                                DynLinArr<double> y, double x1, double x2) {
...
double new_step_mean_x_hist(const long q, double xmin, double xmax,
                            DynLinArr<double> y, double x1, double x2,
                            int &s_err) {
