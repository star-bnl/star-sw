  ...
  qlr(fqlr), ax(fax)
{
  ...
  for( n=0; n<qlr; n++)
  {
    x_mean+=ax[n]; Dx+=ax[n]*ax[n];
  }
  x_mean/=qlr; Dx/=qlr; Dx = Dx-x_mean * x_mean;
}      
ostream& operator<<(ostream& file, const linexi2_coor& l)
{
  Ifile<<"linexi2_coor: qlr="<<l.qlr<<" x_mean="<<l.x_mean<<" Dx="<<l.Dx<<'\n';
  int n;
  for( n=0; n<l.qlr; n++)
    Ifile<<"n="<<n<<" x="<<l.ax[n]<<'\n';
  return file;
  ...
  linexi2_coor(lc), ay(fay) 
{
  ...
  for( n=0; n<qlr; n++)
  {
    y_mean+=ay[n]; xy_mean+=ax[n]*ay[n];
  }
  y_mean/=qlr; xy_mean/=qlr; 
  if(Dx>0)
