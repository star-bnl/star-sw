...
my $debug = 0;
my $day = $ARGV[0];
my $trigger = "AuAu_200_production_2016";
...
if (! -r $TriggerFile) {
  my $cmd = "get_file_list.pl -delim '/' -keys 'path,filename' -cond 'production=P16ij,trgsetupname=" . $trigger . ",filetype=daq_reco_MuDst,filename~st_physics,tpx=1,pxl=1,ist=1,sanity=1,events>10,runnumber>17000000' -limit 0"; 
  print "$cmd\n" if ($debug);
  my  @list = `$cmd`;
