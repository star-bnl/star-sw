...
my @list = `get_file_list.pl -delim '/' -keys 'path,filename' -cond 'filetype=online_daq,filename~st_physics_15,tpx=1,pxl=1,ist=1,tof=1,sanity=1,trgsetupname=AuAu_200_Production_mid_2014' -limit 0`;
...
# if ($run < 15100084 || $run > 15100090) {next;}
  if ($run < 15108000) {next;}
  if ($run > 15111000) {next;}
  my $basename = $file;
