...
my @list = `get_file_list.pl -delim '/' -keys 'path,filename' -cond 'filetype=online_daq,filename~st_physics_15,runnumber>15096017,runnumber<15096060,tpx=1,tof=1,sanity=1,trgsetupname=AuAu_200_Production_mid_2014' -limit 100`;
...
#my @list = `get_file_list.pl -delim '/' -keys 'path,filename' -cond 'filetype=daq_reco_MuDst,filename~st_physics_15,trgsetupname=AuAu_200_Production_mid_2014,tpx=1,storage!=HPSS' -limit 0`;
