...
#my @list = `get_file_list.pl -delim '/' -keys 'path,filename' -cond 'filetype=online_daq,filename~st_physics_15069,tpx=1,gmt=1,sanity=1,events>100,trgsetupname=production_15GeV_2014' -limit 0`; 
my @list = `get_file_list.pl -delim '/' -keys 'path,filename' -cond 'filetype=online_daq,filename~st_physics_15,tpx=1,gmt=1,sanity=1,events>100,trgsetupname=production_15GeV_2014' -limit 0`; 
my $i = 1;   
my $OldRun = 0;
