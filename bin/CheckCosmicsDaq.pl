...
#  print "$line";
  ...
  my ($dev, $ino, $mode, $nlink, $uid, $gid, $rdev, $size, $atime, $mtime, $ctime, $blksize, $blocks ) = stat $mudst_file;
  my $dt = $now - $ctime;
#  print "$mudst_file  size = $size time = $dt $now $ctime\n";
  next if $size < 10000000;
#  next if $dt   < 3600;
#  next if $dt   < 700;
