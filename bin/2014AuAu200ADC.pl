...
# $cmd = "get_file_list.pl -delim '/' -keys 'path,filename' -cond 'magscale=" . $field . ",filetype=online_daq,filename~st_physics_adc,tpx=1,gmt=1,sanity=1,runnumber=17126034' -limit 0"; 
 $cmd = "/afs/rhic.bnl.gov/star/packages/scripts/get_file_list.pl -delim '/' -keys 'path,filename' -cond 'magscale=" . $field . ",filetype=online_daq,filename~st_physics_adc,tpx=1,pxl=1,sanity=1,daynumber=166,trgsetupname=AuAu_200_production_mid_2014 ' -limit 0"; 
print "$cmd\n" if ($debug);
my @list = `$cmd`;
