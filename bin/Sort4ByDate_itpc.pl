#!/usr/bin/env perl
# Sort 4 list for itpcPadGainT0
use Env;
my $debug = 0;
my @List3 = glob "$STAR/StarDb/Calibrations/tpc/Path2itpcGain*";
my @dt3 = ();
...
#die;
my @List2 = glob "$STAR/hold/StarDb/Calibrations/tpc/itpcPadGainT0.*.root*";
my @dt2 = ();
...
my @List1 = glob "db/itpcPadGainT0*.root";
my @dt1 = ();
...
my @List0 = `mysql -h robinson.star.bnl.gov --port=3306 -u "fisyak" Calibrations_tpc -e "select beginTime,entryTime,deactive from itpcPadGainT0  order by beginTime;" | grep -v deactive`;
my @dt0 = ();
