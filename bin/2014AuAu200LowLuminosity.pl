...
my @list = `get_file_list.pl -delim '/' -keys 'path,filename' -cond 'filetype=online_daq,filename~st_physics_15,tpx=1,sanity=1,magscale=FieldOff,trgsetupname=AuAu_200_LowLuminosity_2014' -limit 0`;
#my @list = `get_file_list.pl -delim '/' -keys 'path,filename' -cond 'filetype=online_daq,filename~st_physics_15,tpx=1,sanity=1,trgsetupname=AuAu_200_LowLuminosity_2014' -limit 0`;
my ($run,$dummy);
