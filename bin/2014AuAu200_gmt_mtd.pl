...
#my @list = `get_file_list.pl -delim '/' -keys 'path,filename' -cond 'filetype=online_daq,filename~st_mtd_15088,tpx=1,gmt=1,sanity=1,events>100,trgsetupname=AuAu_200_production_high_2014' -limit 0`; 
my @list = `get_file_list.pl -delim '/' -keys 'path,filename' -cond 'filetype=online_daq,filename~st_mtd_15164,tpx=1,gmt=1,sanity=1,events>100,trgname=e-mu,trgsetupname=AuAu_200_production_high_2014' -limit 0`; 
my $i = 1;   
my $OldRun = 0;
