...
# $cmd = "get_file_list.pl -delim '/' -keys 'path,filename' -cond 'magscale=" . $field . ",filetype=online_daq,filename~st_physics_adc,tpx=1,gmt=1,sanity=1,runnumber=17126034' -limit 0"; 
 $cmd = "get_file_list.pl -delim '/' -keys 'path,filename' -cond 'magscale=" . $field . ",filetype=online_daq,filename~st_physics_adc,tpx=1,pxl=1,sanity=1,runnumber>17127000' -limit 0"; 
#print "$cmd\n";
my @list = `$cmd`;
