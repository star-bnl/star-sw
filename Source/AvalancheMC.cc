...
void AvalancheMC::GetDriftLinePoint(const int i, double& x, double& y,
                                    double& z, double& t) {
...
void AvalancheMC::GetHoleEndpoint(const int i, double& x0, double& y0,
  ...
  x1 = endpointsHoles[i].x1;
  y0 = endpointsHoles[i].y0;
  y1 = endpointsHoles[i].y1;
  z0 = endpointsHoles[i].z0;
  z1 = endpointsHoles[i].z1;
  t0 = endpointsHoles[i].t0;
  t1 = endpointsHoles[i].t1;
  status = endpointsHoles[i].status;
}

void AvalancheMC::GetIonEndpoint(const int i, double& x0, double& y0,
  ...
  x0 = endpointsIons[i].x0;
  x1 = endpointsIons[i].x1;
  y0 = endpointsIons[i].y0;
  y1 = endpointsIons[i].y1;
  z0 = endpointsIons[i].z0;
  z1 = endpointsIons[i].z1;
  t0 = endpointsIons[i].t0;
  t1 = endpointsIons[i].t1;
  status = endpointsIons[i].status;
}

void AvalancheMC::GetElectronEndpoint(const int i, double& x0, double& y0,
                                      double& z0, double& t0, double& x1,
                                      double& y1, double& z1, double& t1,
                                      int& status) const {
  ...
  x1 = endpointsElectrons[i].x1;
  y0 = endpointsElectrons[i].y0;
  y1 = endpointsElectrons[i].y1;
  z0 = endpointsElectrons[i].z0;
  z1 = endpointsElectrons[i].z1;
  t0 = endpointsElectrons[i].t0;
  t1 = endpointsElectrons[i].t1;
...
bool AvalancheMC::DriftLine(const double x0, const double y0, const double z0,
  ...
  while (ok) {
    ...
    if (sensor->IsWireCrossed(point.x, point.y, point.z, x, y, z, xCross,
                              yCross, zCross)) {
      delta *=
        ...
        if (trapped) {
          ...
          if (debug) {
            ...
            std::cout << "    At " << drift[nDrift - 1].x << ", "
                      << drift[nDrift - 1].y << ", " << drift[nDrift - 1].z
                      << "\n";
          }
          break;
        }
  ...
  if (usePlotting && nDrift > 0) {
    int jL;
    if (type < 0) {
      viewer->NewElectronDriftLine(nDrift, jL, drift[0].x, drift[0].y,
                                   drift[0].z);
    } else if (type == 1) {
      viewer->NewHoleDriftLine(nDrift, jL, drift[0].x, drift[0].y, drift[0].z);
    } else {
      viewer->NewIonDriftLine(nDrift, jL, drift[0].x, drift[0].y, drift[0].z);
    }
    for (int iP = 0; iP < nDrift; ++iP) {
      viewer->SetDriftLinePoint(jL, iP, drift[iP].x, drift[iP].y, drift[iP].z);
    }
  }
    ...
    for (int iAval = nAval; iAval--;) {
      if (withElectrons) {
        ...
        for (int iE = aval[iAval].ne; iE--;) {
          ...
          for (int iDrift = 0; iDrift < nDrift - 2; ++iDrift) {
            if (drift[iDrift].ne > 0 || drift[iDrift].nh > 0 ||
                drift[iDrift].ni > 0) {
      ...
      if (withHoles) {
        ...
        for (int iI = 0; iI < aval[iAval].ni; ++iI) {
          ...
          DriftLine(aval[iAval].x, aval[iAval].y, aval[iAval].z, aval[iAval].t,
                    2, false);
          continue;
        }
...
void AvalancheMC::ComputeSignal(const double q) {
  ...
  for (int i = 0; i < nDrift - 1; ++i) {
    ...
    sensor->AddSignal(q, drift[i].t, dt, drift[i].x + 0.5 * dx,
                      drift[i].y + 0.5 * dy, drift[i].z + 0.5 * dz, dx / dt,
                      dy / dt, dz / dt);
  }
}
...
void AvalancheMC::ComputeInducedCharge(const double q) {
  ...
  if (nDrift < 2) return;
  sensor->AddInducedCharge(q, drift[0].x, drift[0].y, drift[0].z,
                           drift[nDrift - 1].x, drift[nDrift - 1].y,
                           drift[nDrift - 1].z);
}
}
