...
ComponentAnsys121::ComponentAnsys121(char* elist, char* nlist, 
                                     char* mplist, char* prnsol, 
                                     char* unit) : 
  ComponentFieldMap() {
  ...
  while (fmplist.getline(line, size, '\n')) {
    ...
    if (strcmp(token,"LIST") == 0) {
      ...
      if (readerror) {
        printf("ComponentAnsys121: Error reading file %s (line %d).\n", mplist, il);
        fmplist.close();
        ok = false;
        return;
      }
      ...
      if (debug) {
        printf("ComponentAnsys121: Number of materials: %d\n", nMaterials);
      }
  ...
  if (iepsmin < 0) {
    printf("FieldMapAnsys121: No material with positive permittivity found in material list %s.\n", mplist);
    ok = false;
  } else {
    for (int imat = 0; imat < nMaterials; imat++) {
      if (imat == iepsmin) {
  ...
  printf("ComponentAnsys121: Read properties of %d materials from file %s.\n", nMaterials, mplist);
  ...
  while (felist.getline(line, size, '\n')) {
    ...
    token = strtok(NULL, " ");
    token = strtok(NULL, " ");
    token = strtok(NULL, " "); if (!token) {printf("No token available\n");} int in0 = ReadInteger(token, -1, readerror);
    ...
    if (readerror) {
      ...
      printf("ComponentAnsys121: Synchronisation lost on file %s (line %d).\n", elist, il);
    ...
    if (imat < 1 || imat > nMaterials) {
      printf("ComponentAnsys121: Out-of-range material number on file %s (line %d).\n", elist, il);
      printf("                  Element: %d, material: %d, nodes: (%d %d %d %d %d %d %d %d)\n",
             ielem, imat, in0, in1, in2, in3, in4, in5, in6, in7);
      ok = false;
    }      
    if (materials[imat - 1].eps < 0) {
      printf("ComponentAnsys121: Element %d in element list %s uses material %d which has not\n", ielem, elist, imat);
      printf("    been assigned a positive permittivity in material list %s.\n", mplist);
      ok = false;
    }
     // Check the node numbers
    if (in0 < 1 || in1 < 1 || in2 < 1 || in3 < 1 || in4 < 1 || in5 < 1 || in6 < 1 || in7 < 1) {
      printf("ComponentAnsys121: Found a node number < 1 on file %s (line %d).\n", elist, il);
      printf("                  Element: %d, material: %d, nodes: (%d %d %d %d %d %d %d %d).\n",
             ielem, imat, in0, in1, in2, in3, in4, in5, in6, in7);
      ok = false;
    }
  ...
  felist.close();
  // Tell how many lines read
  printf("ComponentAnsys121: Read %d elements from file %s,\n", nElements, elist);
  printf("                  highest node number: %d, degenerate elements: %d, background elements skipped: %d.\n",
         highestnode, ndegenerate, nbackground);
  ...
  if (strcmp(unit,"mum") == 0 || strcmp(unit,"micron") == 0 || strcmp(unit,"micrometer") == 0) {
    ...
    printf("FieldMapAnsys121: Unknown length unit %s.\n", unit);
    ok = false;
    funit = 1.0;    
  }
  if (debug) {printf("FieldMapAnsys121: Unit scaling factor = %g.\n", funit);}
                       
  // Open the node list
  std::ifstream fnlist;
  fnlist.open(nlist, std::ios::in);
  if (fnlist.fail()) {
    printf("FieldMapAnsys121: Could not open nodes file %s for reading; the file perhaps does not exist.\n", nlist);
    return;
  }
  // Read the node list
  nodes.clear();
  ...
  fnlist.close();
  // Tell how many lines read
  printf("ComponentAnsys121: Read %d nodes from file %s.\n", nNodes, nlist);
  // Check number of nodes
  if (nNodes != highestnode) {
    printf("ComponentAnsys121: Number of nodes read (%d) on %s does not match element list (%d).\n",
  ...
  if (fprnsol.fail()) {
    printf("ComponentAnsys121: Could not open potential file %s for reading; the file perhaps does not exist.\n", prnsol);
    return;
  }
  ...
  while (fprnsol.getline(line, size, '\n')) {
    ...
    if (inode < 1 || inode > highestnode) {
      printf("ComponentAnsys121: Node number %d out of range on potential file %s (line %d).\n", inode, prnsol, il);
      ok = false;
    } else {
  ...
  fprnsol.close();
  // Tell how many lines read
  printf("ComponentAnsys121: Read %d potentials from file %s.\n", nread, prnsol);
  // Check number of nodes
  if (nread != nNodes) {
    printf("ComponentAnsys121: Number of nodes read (%d) on potential file %s does not match the node list (%d).\n",
           nread, prnsol, nNodes);
    ok = false;
  }
   // Set the ready flag
  if (ok) {
    ...
    printf("ComponentAnsys121: Field map could not be read and can not be interpolated.\n");
  }
...
void 
ComponentAnsys121::ElectricField(const double xin, const double yin, const double zin, 
  ...
  if (!ready) {
    status = -10;
    printf("ComponentAnsys121::ElectricField: Field map not available for interpolation.\n");
    return;
  }
  
  if (warning) {
    printf("FieldMapAnsys121::ElectricField: Warnings have been issued for this field map.\n");
  }
  ...
  if (imap<0) {
    if (debug) {
      printf("ComponentAnsys121::ElectricField: Point (%g,%g) not in the mesh.\n", x, y);
    }
    status = -6;
    return;
  }
  ...
  if (debug) {
    printf("ComponentAnsys121::ElectricField: Global: (%g,%g), Local: (%g,%g,%g,%g) in element %d (degenerate: %d)\n",
  ...
  if (elements[imap].degenerate) {
    ...
    ex = -(nodes[elements[imap].emap[0]].vmap * (4 * t1 - 1) * jac[0][1] +
       ...
       nodes[elements[imap].emap[3]].vmap * (4 * t2 * jac[0][1]+4*t1 * jac[1][1]) +
       nodes[elements[imap].emap[4]].vmap * (4 * t3 * jac[0][1]+4*t1 * jac[2][1]) +
       nodes[elements[imap].emap[5]].vmap * (4 * t3 * jac[1][1]+4*t2 * jac[2][1])) / det;
    ey = -(nodes[elements[imap].emap[0]].vmap * (4 * t1 - 1) * jac[0][2] +
	   ...
	   nodes[elements[imap].emap[3]].vmap * (4 * t2 * jac[0][2] + 4 * t1 * jac[1][2]) +
	   nodes[elements[imap].emap[4]].vmap * (4 * t3 * jac[0][2] + 4 * t1 * jac[2][2]) +
	   nodes[elements[imap].emap[5]].vmap * (4 * t3 * jac[1][2] + 4 * t2 * jac[2][2])) / det;
  } else {
    volt = -nodes[elements[imap].emap[0]].vmap * (1 - t1) * (1 - t2) * (1 + t1 + t2) / 4 -
            nodes[elements[imap].emap[1]].vmap * (1 + t1) * (1 - t2) * (1 - t1 + t2) / 4 -
            nodes[elements[imap].emap[2]].vmap * (1 + t1) * (1 + t2) * (1 - t1 - t2) / 4 -
            nodes[elements[imap].emap[3]].vmap * (1 - t1) * (1 + t2) * (1 + t1 - t2) / 4 +
            nodes[elements[imap].emap[4]].vmap * (1 - t1) * (1 + t1) * (1 - t2) / 2 +
            nodes[elements[imap].emap[5]].vmap * (1 + t1) * (1 + t2) * (1 - t2) / 2 +
            nodes[elements[imap].emap[6]].vmap * (1 - t1) * (1 + t1) * (1 + t2) / 2 +
            nodes[elements[imap].emap[7]].vmap * (1 - t1) * (1 + t2) * (1 - t2) / 2;
    ex = -(nodes[elements[imap].emap[0]].vmap * ((1 - t2) * (2 * t1 + t2) * jac[0][0] + (1 - t1) * (t1 + 2 * t2) * jac[1][0]) / 4 +
           nodes[elements[imap].emap[1]].vmap * ((1 - t2) * (2 * t1 - t2) * jac[0][0] - (1 + t1) * (t1 - 2 * t2) * jac[1][0]) / 4 +
           nodes[elements[imap].emap[2]].vmap * ((1 + t2) * (2 * t1 + t2) * jac[0][0] + (1 + t1) * (t1 + 2 * t2) * jac[1][0]) / 4 +
           nodes[elements[imap].emap[3]].vmap * ((1 + t2) * (2 * t1 - t2) * jac[0][0] - (1 - t1) * (t1 - 2 * t2) * jac[1][0]) / 4 +
           nodes[elements[imap].emap[4]].vmap * (t1 * (t2 - 1)        * jac[0][0] + (t1 - 1) * (t1 + 1)    * jac[1][0] / 2) +
           nodes[elements[imap].emap[5]].vmap * ((1 - t2) * (1 + t2)  * jac[0][0] / 2 - (1 + t1) * t2      * jac[1][0]) +
           nodes[elements[imap].emap[6]].vmap * (-t1 * (1 + t2)       * jac[0][0] + (1 - t1) * (1 + t1)    * jac[1][0] / 2) +
           nodes[elements[imap].emap[7]].vmap * ((t2 - 1) * (t2+1)    * jac[0][0] / 2 + (t1 - 1) * t2      * jac[1][0])) / det;
    ey = -(nodes[elements[imap].emap[0]].vmap * ((1 - t2) * (2 * t1 + t2) * jac[0][1] + (1 - t1) * (t1 + 2 * t2) * jac[1][1]) / 4 +
           nodes[elements[imap].emap[1]].vmap * ((1 - t2) * (2 * t1 - t2) * jac[0][1] - (1 + t1) * (t1 - 2 * t2) * jac[1][1]) / 4 +
           nodes[elements[imap].emap[2]].vmap * ((1 + t2) * (2 * t1 + t2) * jac[0][1] + (1 + t1) * (t1 + 2 * t2) * jac[1][1]) / 4 +
           nodes[elements[imap].emap[3]].vmap * ((1 + t2) * (2 * t1 - t2) * jac[0][1] - (1 - t1) * (t1 - 2 * t2) * jac[1][1]) / 4 +
           nodes[elements[imap].emap[4]].vmap * (t1 * (t2 - 1)        * jac[0][1] +  (t1 - 1) * (t1 + 1)  * jac[1][1] / 2) +
           nodes[elements[imap].emap[5]].vmap * ((1 - t2) * (1 + t2)  * jac[0][1] / 2 - (1 + t1) * t2     * jac[1][1]) +
           nodes[elements[imap].emap[6]].vmap * (-t1 * (1 + t2)       * jac[0][1] +  (1 - t1) * (1 + t1)  * jac[1][1] / 2) +
           nodes[elements[imap].emap[7]].vmap * ((t2 - 1) * (t2 + 1)  * jac[0][1] / 2 + (t1 - 1) * t2     * jac[1][1])) / det;
  }
  ...
  // Drift medium ?
  if (debug) {
    printf("ComponentAnsys121::ElectricField: Material %d, drift flag %d.\n",
...
bool 
ComponentAnsys121::GetMedium(const double xin, const double yin, const double zin, Medium*& m) {
