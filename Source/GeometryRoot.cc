...
GeometryRoot::SetGeometry(TGeoManager* geoman) {
  ...
  if (geoman == 0) {
    std::cerr << "GeometryRoot::SetGeometry:" << std::endl;
    std::cerr << "    Pointer to TGeoManager is null." << std::endl;
    return;
  }
...
GeometryRoot::GetNumberOfMaterials() {
  ...
  if (theGeoManager == 0) {
    std::cerr << "GeometryRoot::GetNumberOfMaterials:" << std::endl;
    std::cerr << "    ROOT geometry manager is not defined." << std::endl;
    return 0;
  }
...
GeometryRoot::GetMaterial(const int i) {
  ...
  if (theGeoManager == 0) {
    std::cerr << "GeometryRoot::GetMaterial:" << std::endl;
    std::cerr << "    ROOT geometry manager is not defined." << std::endl;
    return 0;
  }
...
GeometryRoot::SetMedium(const int imat, Medium* med) {
  ...
  for (int i = nMaterials; i--;) {
    if (name == materials[i].name) {
      std::cout << "GeometryRoot::GetMaterial:" << std::endl;
  ...
  std::cout << "GeometryROOT::SetMedium:" << std::endl;
  std::cout << "    ROOT material: " << name << std::endl;
  std::cout << "      Density: " << rho1 << " g / cm3" << std::endl;
  std::cout << "    Medium: " << med->GetName() << std::endl;
  std::cout << "      Density: " << rho2 << " g / cm3" << std::endl;
  if (rho1 > 0 && fabs(rho1 - rho2) / rho1 > 0.01) {
    std::cout << "    WARNING: Densities differ by > 1%." << std::endl;
  }
