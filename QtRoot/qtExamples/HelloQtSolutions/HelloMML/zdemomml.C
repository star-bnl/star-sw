...
void zdemomml()
{
   ...
   // Create a new canvas.
   TCanvas *c1mml = new TCanvas("zdemomml",
      "Monte Carlo Study of Z scaling",10,40,800,600);
   c1mml->Range(0,0,25,18);
   c1mml->SetFillColor(40);
   
   TPaveLabel *pl = new TPaveLabel(1,16.3,24,17.5,"<math><mtext>Z-scaling of Direct Photon Productions in pp Collisions at RHIC Energies</mtext></math>","br");
   pl->SetFillColor(18);
   pl->SetTextFont(32);
   pl->SetTextColor(49);
   pl->Draw();
...
//
   ...
   t = new TLatex();
   t->SetNDC();
   t->SetTextFont(61); // use 61 instead of 62 to disable the LaTex erendering engine
   t->SetTextColor(36);
   t->SetTextSize(0.08);
   t->SetTextAlign(12);
   ...
   t->SetTextSize(0.05);
   const char *directGamma = "<math><mtext>Direct</mtext><mspace width=\"5\"></mspace><mi>&gamma;</mi></math>";
   const char *theta90 = "<math><mi>&theta;</mi><mo>=</mo><msup><mn>90</mn><mi>o</mi></msup></math>";
   t->DrawLatex(0.6,0.79,directGamma);
   t->DrawLatex(0.6,0.75,theta90);

   t->DrawLatex(0.20,0.45,"<math><mi>E</mi><mrow><msup><mo>d</mo><mn>3</mn></msup><mi>&sigma;</mi></mrow><mo>/</mo><mrow><mo>d</mo><msup><mi>q</mi><mn>3</mn></msup></mrow></math>");
   t->DrawLatex(0.18,0.40,"<math><mo>(</mo><mi>barn</mi><mo>/</mo><msup><mi>Gev</mi> <mn>2</mn></msup><mo>)</mo></math>");

   t->SetTextSize(0.045);
   t->SetTextColor(kBlue);
   t->DrawLatex(0.22,0.260,"<math><msqrt>s</msqrt><mo>=</mo><mn>63</mn><mtext>(GeV)</mtext></math>");
   t->SetTextColor(kRed);
   t->DrawLatex(0.22,0.205,"<math><msqrt>s</msqrt><mo>=</mo><mn>200</mn><mtext>(GeV)</mtext></math>");
   t->SetTextColor(6);
   t->DrawLatex(0.22,0.15,"<math><msqrt>s</msqrt><mo>=</mo><mn>500</mn><mtext>(GeV)</mtext></math>");

   t->SetTextSize(0.05);
   t->SetTextColor(1);
   t->DrawLatex(0.6,0.06,"<math><msub><mi>q</mi><mi>T</mi></msub><mtext>(Gev/c)</mtext></math>");
...
//
   ...
   t->SetTextSize(0.045);
   t->SetTextColor(46);
   t->DrawLatex(0.20,0.30,"<math><msqrt>s</msqrt><mtext>, GeV</mtext></math>");
   ...
   t->SetTextSize(0.05);
   t->SetTextColor(1);
   t->DrawLatex(0.88,0.06,"<math><mi>z</mi></math>");
