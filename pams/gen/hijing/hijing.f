...
C
	BB=SQRT(BMIN**2+RNDM(-1)*(BMAX**2-BMIN**2))
	PHI=2.0*HIPR1(40)*RNDM(-1)
	BBX=BB*COS(PHI)
	BBY=BB*SIN(PHI)
	HINT1(19)=BB
	HINT1(20)=PHI
...
C
	IF(IHPR2(3).NE.0) THEN
	   NHARD=1+INT(RNDM(-1)*(NCOLT-1)+0.5)
...
C
	IF(IHPR2(9).EQ.1) THEN
		NMINI=1+INT(RNDM(-1)*(NCOLT-1)+0.5)
...
C				already been ptoduced here
	   XR1=-ALOG(EXP(-TTRIG)+RNDM(-1)*(1.0-EXP(-TTRIG)))
 106	   NJET=NJET+1
	   XR1=XR1-ALOG(RNDM(-1))
...
110	XR=-ALOG(EXP(-TT)+RNDM(-1)*(1.0-EXP(-TT)))
111	NJET=NJET+1
	XR=XR-ALOG(RNDM(-1))
...
C           WRITE (6, *) '  NATT','  IDST ', '  IMOH', '  JDA1', '  JDA2'
		N_STR=0
		DO 344 I=1,N
		   NATT=NATT+1
		   KATT(NATT,1)=K(I,2)
		   ...
		   KATT(NATT,4)=K(I,1)
                    IF(K(I,3) .NE. 0) THEN
		        KATT(NATT,3)= NATSUM + K(I,3)
                    ELSE
                        KATT(NATT,3) = 0
                    ENDIF
                    IF(K(I,1) .NE. 1) THEN
                   KATT(NATT,5)= NATSUM + K(I,4)
                   KATT(NATT,6)= NATSUM + K(I,5)
                    ELSE
                   KATT(NATT,5)= 0
                   KATT(NATT,6)= 0
                   ENDIF		     
C                  ****** identify the mother particle
 ...
 341            CONTINUE
                   ...
                   VATT(NATT,5)= V(I,5)                      
		   EATT=EATT+P(I,4)
C            WRITE (6, 91) NATT, KATT(NATT,4), KATT(NATT,3), KATT(NATT,5),
C     +   KATT(NATT,6)
C            WRITE (6, 91) I, K(I,1), K(I,3), K(I,4), K(I,5) 
 344            CONTINUE
                   NATSUM = NATSUM + N
C                 WRITE (6, *) 'FIRST',' NATSUM =',NATSUM, ' N =', N,
C     + ' NATT =', NATT
 340             CONTINUE
...
C            WRITE (6, 91) I, K(I,1), K(I,3), K(I,4), K(I,5)
 362            CONTINUE
                   NATSUM = NATSUM + N
C           WRITE(6, *) 'SECOND', NATT,'  NATSUM=', NATSUM,'   N=', N,
C     +    ' NATT =', NATT
 360            CONTINUE
...
390		CONTINUE 
                   NATSUM = NATSUM + N
C            WRITE (6, *) 'THIRD', NATT,' NATSUM =', NATSUM,' N =',N,
C     +    ' NATT =', NATT
400	   CONTINUE
...
C           WRITE (6, *) '  NATT','  IDST ', '  IMOH', '  JDA1', '  JDA2'
 ...
 451            CONTINUE
		EATT=EATT+PDR(I,4)
          PATT(NATT,5) = SQRT(ABS(PDR(I,4)**2-PDR(I,1)**2-PDR(I,2)**2
     +    -PDR(I,3)**2))
                 VATT(NATT,5) = 0.
450	CONTINUE
                NATSUM = NATSUM + NDR
C        WRITE (6, *) 'FORTH', NATT,' NATSUM=', NATSUM,' NDR=',NDR,
C     +    ' NATT =', NATT
...
580		CONTINUE
		N=N+NTJ(JTP)
	ENDIF
	IF(IHPR2(1).GT.0.AND.RNDM(-1).LE.HIDAT(3)) THEN
	     HIDAT20=HIDAT(2)
	     HIPR150=HIPR1(5)
...
C
	...
	I_INIRAD=0
	IF(RNDM(-1).LE.PINIRAD) I_INIRAD=1
...
C			the pt kick is the pt from hard-collision.
	I_SNG=0
	IF(IHPR2(13).NE.0 .AND. RNDM(-1).LE.HIDAT(4)) I_SNG=1
...
C               ********decite whether to have single-diffractive
	IF(IHPR2(5).EQ.0) THEN
		PKC=HIPR1(2)*SQRT(-ALOG(1.0-RNDM(-1)
	...
	IF(PKC.GT.HIPR1(20)) 
     ...
     &	       -RNDM(-1)*(EXP(-HIPR1(20)**2/HIPR1(2)**2)-
     &	       EXP(-PKCMX**2/HIPR1(2)**2))))
C
	IF(I_SNG.EQ.1) PKC=0.65*SQRT(
     &		-ALOG(1.0-RNDM(-1)*(1.0-EXP(-PKCMX**2/0.65**2))))
C			********select PT kick
30	PHI0=2.0*HIPR1(40)*RNDM(-1)
	PKC11=PKC*SIN(PHI0)
	PKC12=PKC*COS(PHI0)
	PKC21=-PKC11
	PKC22=-PKC12
...
C			single-diffr 
	      IF(RNDM(-1).GT.0.5.OR.(NFT(JT,5).GT.2.OR.
...
C		********have single diffractive collision
 ...
 50	   CONTINUE
     ...
     &       .AND.RNDM(-1).GT.0.5).OR.(NPJ(JP).EQ.0
     &        .AND.NTJ(JT).NE.0)).AND.NFP(JP,5).LE.2) THEN
	      D1=DPN
	      D2=DTX
	      NFP3=NFP(JP,3)
...
C       so that pt=p1+p2
	...
	IF(ABS(NFT(JT,1)*NFT(JT,2)).GT.1000000.OR.
     &			ABS(NFT(JT,1)*NFT(JT,2)).LT.100) THEN
		KICKDIT=0
	ENDIF
	IF((KICKDIP.EQ.0.AND.RNDM(-1).LT.0.5)
     &     .OR.(KICKDIP.NE.0.AND.RNDM(-1)
...
C				********pt kick for proj
	PT(JT,1)=PT11-PKC21
	PT(JT,2)=PT12-PKC22
	IF((KICKDIT.EQ.0.AND.RNDM(-1).LT.0.5)
     &     .OR.(KICKDIT.NE.0.AND.RNDM(-1)
...
C
	...
	IF(IOPT.EQ.2) THEN
     ...
     &			-RNDM(-1)*(EXP(-HIPR1(8)**2/HIPR1(2)**2)-
     &			EXP(-PTMAX**2/HIPR1(2)**2))))

	ELSE
		PT=HIPR1(2)*SQRT(-ALOG(1.0-RNDM(-1)*
...
C	This generate random number between XMIN and XMAX
	...
	RX=RR(I,JMIN)+(RR(I,JMAX)-RR(I,JMIN))*RNDM(-1)
	JL=0
	JU=202
10	IF(JU-JL.GT.1) THEN
	   JM=(JU+JL)/2
...
C*******************************************************
	...
	F(1,3)=CND*X1**AT3*(1.D0-X1)**AT4*(1.D0+GMD*X1)
	F(1,1)=FUD1-F(1,3)
...
C
	...
	F(2,3)=CND*X2**AT3*(1.D0-X2)**AT4*(1.D0+GMD*X2)
	F(2,1)=FUD2-F(2,3)
...
C...initialize all the data common blocks
	...
	DATA KATT/600000*0/PATT/500000*0.0/VATT/500000*0.0/
