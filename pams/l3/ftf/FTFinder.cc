...
//      Event reset
//
   if ( para->event_reset >= -1 ) { 
      if ( Event_Reset ( ) ) return 1 ;  
//
//       Set pointers
//
      if ( Set_Pointers ( ) ) return 1 ;   
   }
   load_time = Time ( );
//
...
//         Print out time info   
//
    total_time = init_time + load_time + trak_time + dedx_time ;
    printf ( "\n Timing     Setup   Tracking     dEdx     Total  " );
    printf ( "\n           %7.3f  %7.3f   %7.3f   %7.3f  \n ", 
                       load_time, trak_time, dedx_time, total_time ) ;
...
//     Reset event
//**********************************************************************
int FTFinder::Event_Reset (  ) {
/*----------------------------------------------------------------------
       Set some counters to zero
------------------------------------------------------------------------*/
   if ( para->event_reset >= 0 ){
      n_tracks = 0 ;

   if ( para->merge_primaries ) 
	 memset ( track_area, 0, 
	          para->n_phit*para->n_etat*sizeof(AREA) ) ;         
   }
   return 0 ;
}
//********************************************************************** 
...
//********************************************************************
//
void FTFinder::Merge_Primary_Tracks ( ) {
}
//********************************************************************
//      Resets program
//*********************************************************************
...
//	Set hit pointers
//*********************************************************************
int FTFinder::Set_Conformal_Coordinates ( )
{
/*-------------------------------------------------------------------------
        Loop over hits 
-------------------------------------------------------------------------*/
...
-------------------------------------------------------------------------*/
      ...
      r             = (float)sqrt ( r2 ) ;
      phi           = (float)atan2(this_hit->y,this_hit->x) ;
