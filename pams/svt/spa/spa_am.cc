  ...
  TABLE_HEAD_ST     *condition_h, SDM_CONDITION_DB_ST     *condition )
{
  ...
  test = mySsd->readStripFromTable(sls_strip_h, sls_strip);
  cout<<"####        NUMBER OF SLS STRIPS "<<sls_strip_h->nok<<"       ####"<<endl;

  test=mySsd->readNoiseFromTable(noise_h, noise);
  cout<<"####       NUMBER OF DB ENTRIES "<<noise_h->nok<<"       ####"<<endl;
  test = mySsd->readConditionDbFromTable(condition_h, condition);
  ...
  mySsd->doDaqSimulation(ctrl);
   test = mySsd->writeStripToTable(out_strip_h, out_strip, sls_strip_h, sls_strip);
   cout<<"####       NUMBER OF SPA STRIP "<<test<<"          ####"<<endl;
  delete mySsd;
  cout<<"#################################################"<<endl;
