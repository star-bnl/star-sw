       ...
       TABLE_HEAD_ST   *sca_out_h,          SCA_OUT_ST     *sca_out  )   
{
  ...
  // makeEnsembleAve & doAnalysis.  -- DSW  Aug 10,1998
  ...
  if ( sca_switch->makeEnsembleAve ) {
    iret = ensemble_sum( sca_out_h, sca_out, sca_ensemble_ave_h, sca_ensemble_ave );
    if (iret) {
  ...
  if ( sca_switch->doAnalysis ) {
      iret = sca_deltad (sca_out_h, sca_out, sca_ensemble_ave_h, sca_ensemble_ave);
    if (iret) {
       ...
       TABLE_HEAD_ST   *sca_out_h,          SCA_OUT_ST     *sca_out  )  
{
  ...
  // rank & scales. Loop over the first rank to determine the number 
  // of scales. Note that each rank has the same number of scales. 
  
  while ( sca_out[irow].rank == sca_out[irow++].rank )  {
    numscales++;
  }
  ...
  while (irank < nranks) {
    ...
    while(iscale < numscales && 
	  sca_out[iscale].scale_x < sca_ensemble_ave[iref_scale].scale_x)
      iscale++;
   // walk through the rest of the scales 
    
    
    for(;iscale < numscales; iscale++){
      //skip to the appropiate reference scale
     while(iref_scale < numscales-1 && 
	   sca_ensemble_ave[iref_scale+1].scale_x < sca_out[iscale].scale_x)
       iref_scale++;
     
     if(iref_scale == numscales-1)
