...
*  Original  version:  W.B. Christie 15-NOV-1993
   ...
   Content   MAGP,COIL,MTCL,MPTV,MPCV,MRET,MRGV,MSEC,MBAR,MCSE,PANC,PAWT
   ...
   structure MBAR { CoilRmn,CoilRmx,CoilLen,RetYRmn,RetYLen,
                    ...
                    Ncoil,zcoil(6),dzcoil(6),dzPanCake,dyPanCake,RWaterRoud}

   structure MEND { PoleRmn,poleZ,PoleRmx,tcoilRmn,tcoilRmx,PoleCavR,
                    PoleCavD,tcoilDZ,etacut  }

   real      TanTheta,Rcorner,Zcut,d,dr,r1,r2,dz
   Integer   I_coil,nZPanCakes,nYPanCakes,i,j
********************************************************************************
*
...
"=>"  Zcoil    = {30.95, 89.05, 147.17, 205.25, 249.0,  288.35} ! coil position
      ...
      dzPanCake= 5.39       ! Allimunium
      dyPanCake= 4.75       ! -"-
      RWaterRoud=0.75  !sqrt(3.14159265358979312*1.68**2/4.)/2.  ! central round hole radius for cooling water approximated by square
...
Block MCSE is a single barrel coil
	...
	Create PANC
        do j = 1, nZPanCakes
          Position PANC z= -mbar_DzCoil( I_coil )/2 + dZ*(j-0.5)
        enddo
      enddo
      
EndBlock
* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
Block PANC is the coil conducter
      Material  Aluminium  
      Attribute PANK    seen=0  colo=3
      SHAPE     TUBE    Rmin=r1  Rmax=r2  dz=mbar_dzPanCake/2
      Create and Position PAWT in PANC
EndBlock
Block PAWT is the coil conducter
      Component H2     A=1   Z=1   W=2
      Component O      A=16  Z=8   W=1
      Mixture   Water  Dens=1.0
      Material  Water  
      Attribute PAWT    seen=0  colo=2
      d = 0.75
*      SHAPE     TUBE    Rmin=(r1+r2)/2-mbar_RWaterRoud  Rmax=(r1+r2)/2+mbar_RWaterRoud  dz=mbar_RWaterRoud
      SHAPE     TUBE    Rmin=(r1+r2)/2-d  Rmax=(r1+r2)/2+d  dz=d
EndBlock
* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
