...
int rdEztStale(
 ...
 char* inDir   = "./"
  ...
  assert( !gSystem->Load("StEEmcPoolmuEztStaleData"));  // just reads EEMC+TRIG data from muEzt
  ...
  myMk3=new StEEstaleMaker("muTurtle","MuDst");
  ...
  //muMk->chain()->SetBranchStatus("MuEvent",1);
/*
  muMk->chain()->SetBranchStatus("EztHead*",1);
  muMk->chain()->SetBranchStatus("EztTrig*",1);
  muMk->chain()->SetBranchStatus("EztETow*",1);
  muMk->chain()->SetBranchStatus("EztESmd*",1);
*/
  chain->Init();
  chain->ls(3);
  // muMk->printArrays();
