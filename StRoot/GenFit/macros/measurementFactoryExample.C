...
int measurementFactoryExample() {
  ...
  if (debug) measurementCreator.setDebug(debug);
  ...
  genfit::MaterialEffects::getInstance()->setDebugLvl(debug);
#ifdef __DISPLAY__
  // init event display
  ...
  if (debug) fitter->setDebugLvl(10);
  ...
  for (unsigned int iEvent=0; iEvent<100; ++iEvent){
    if (debug) cout << "======================================== Event " << iEvent << " ==========" << endl;
