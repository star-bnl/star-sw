...
*/
  ...
  if(! mEvent)  {
     LOG_ERROR << "::Make() There is no StEvent, skip " <<endm;
     return  kStErr;
  }
  StEmcCollection* ecolA =(StEmcCollection*)mEvent->emcCollection();
  if(ecolA==0)    {
    LOG_WARN<<"::Make(), raw2pixels() no emc collection"<<endm;
    ecolA = new StEmcCollection();
    mEvent->setEmcCollection(ecolA);
  }
...
*/                   
  ...
  for(int det = kEndcapEmcTowerId; det<= kEndcapSmdVStripId; det++){
	...
	for( hitB=myHitB.begin() ; hitB< myHitB.end(); hitB++) {
	  ...
	  */
	  //yf 09/09/14	  hitB=(vector<StEmcRawHit*>::iterator)NULL; // siher ist siher
