...
void StPicoDstMaker::setBranchAddresses(TChain* chain) {
  ...
  for (Int_t i = 0; i < StPicoArrays::NAllPicoArrays; ++i) {
    ...
    Char_t const* bname = StPicoArrays::picoArrayNames[i];
...
#ifdef __TFG_MUDST__FORMAT_EXT__
    if (! mPicoDst->IsGoodTrigger()) return kStSkip;
#endif /* __TFG_MUDST__FORMAT_EXT__ */
  }
...
void StPicoDstMaker::fillTracks() {
  ...
  for (int i = 0; i < nGlobals; ++i) {
    ...
    /// Global trach should correspondence to some covariance information
    ...
    if( dcaG && (mCovMtxMode == PicoCovMtxMode::Write) ) {
	...
	ii += iVal+2;
      ...
      /// Set track index to covariance matrix
      picoTrk->setTrackCovMatrixIndex( cov_index );
    } //if(dcaG)
...
Bool_t StPicoDstMaker::getBEMC(const StMuTrack* t, int* id, int* adc, float* ene, float* d, int* nep, int* towid) {
  ...
  *adc = 0;
  ...
  Double_t magneticField = mBField * kilogauss / tesla; // in Tesla
    ...
    for (StSPtrVecEmcPointIterator it = bEmcPoints.begin(); it != bEmcPoints.end(); ++it, ++index) {
      ...
      if (associated) {
        *id = index;
        ...
        Float_t deltaphi = (*it)->position().phi() - positionBSMDP.phi();
        if (deltaphi >= TMath::Pi()) deltaphi = deltaphi - TMath::TwoPi();
        if (deltaphi < -TMath::Pi()) deltaphi = deltaphi + TMath::TwoPi();

        Float_t rsmdp = mEmcGeom[3]->Radius();
        Float_t pointz = (*it)->position().z();
        Float_t deltaz = pointz - positionBSMDE.z();
  ...
  Int_t towerId = 0;
  Int_t localTowerId = -1;
  Int_t localId1 = -1;
  Int_t localId2 = -1;
  Double_t energy1 = 0, energy2 = 0;
  Double_t energyTemp = 0;
  Double_t dist1 = 1000, dist2 = 1000;
  Double_t distTemp = 0;
  Float_t etaTemp = 0, phiTemp = 0;

  if (mEmcGeom[0]->getId(position.phi(), position.pseudoRapidity(), towerId) == 1) return kTRUE;

  for (Int_t ieta = -1; ieta < 2; ++ieta) {
    for (int iphi = -1; iphi < 2; ++iphi) {
      localTowerId++;//loops from 0 to 8
      Int_t nextTowerId = mEmcPosition->getNextTowerId(towerId, ieta, iphi);
...
void StPicoDstMaker::fillEvent() {
  ...
  Int_t counter = mPicoArrays[StPicoArrays::Event]->GetEntries();
  ...
  for(Int_t iStrip=1; iStrip<9; ++iStrip) {
...
void StPicoDstMaker::fillEmcTrigger() {
  ...
  Int_t bht0 = trigSimu->bemc->barrelHighTowerTh(0);
  Int_t bht1 = trigSimu->bemc->barrelHighTowerTh(1);
  Int_t bht2 = trigSimu->bemc->barrelHighTowerTh(2);
  Int_t bht3 = trigSimu->bemc->barrelHighTowerTh(3);
  ...
  Int_t const bjpth0 = trigSimu->bemc->barrelJetPatchTh(0);
  Int_t const bjpth1 = trigSimu->bemc->barrelJetPatchTh(1);
  Int_t const bjpth2 = trigSimu->bemc->barrelJetPatchTh(2);

  for (Int_t i = 0; i < 3; ++i) {
    mPicoDst->event()->setJetPatchThreshold(i, trigSimu->bemc->barrelJetPatchTh(i));
  }

  for (Int_t jp = 0; jp < 18; ++jp) {
    ...
    Int_t const jpAdc = trigSimu->bemc->barrelJetPatchAdc(jp);
...
void StPicoDstMaker::fillMtdTrigger() {
  ...
  Int_t counter = mPicoArrays[StPicoArrays::MtdTrigger]->GetEntries();
  ...
  /// run year
  const Int_t runnumber = mMuDst->event()->runNumber();
  Int_t year = runnumber / 1e6 + 1999;
  // Oct. 1st (approx. 273rd day) is the start of a new running year
  if ((runnumber % 1000000) / 1000 >= 273) year += 1;

  /// Get QT information
  const Int_t kNQTboard = 8;
...
void StPicoDstMaker::fillBTofHits() {
  ...
  for (UInt_t i = 0; i < mMuDst->numberOfBTofHit(); ++i) {
    ...
    Int_t counter = mPicoArrays[StPicoArrays::BTofHit]->GetEntries();
...
void StPicoDstMaker::fillBbcHits() {
  ...
  if (trg) {
  for (Int_t ew=-1; ew<2; ew+=2){            // note loop -1,+1
    StBeamDirection dir = (ew<0)?east:west;
    for (Int_t pmt=1; pmt<17; pmt++){
      Int_t adc = trg->bbcADC(dir,pmt);
      if (adc>0){
	Int_t tdc = trg->bbcTDC5bit(dir,pmt);
	Int_t tac = trg->bbcTDC(dir,pmt);
	Bool_t trueval = kTRUE;
	Int_t counter = mPicoArrays[StPicoArrays::BbcHit]->GetEntries();
...
void StPicoDstMaker::fillEpdHits() {
  for (UInt_t i=0; i < mMuDst->numberOfEpdHit(); i++){
    ...
    Int_t counter = mPicoArrays[StPicoArrays::EpdHit]->GetEntries();
...
Bool_t StPicoDstMaker::selectVertex() {
  ...
  if (mVtxMode == PicoVtxMode::Default) {
    ...
    if (mBTofHeader && fabs(mBTofHeader->vpdVz()) < 200) {
      ...
      Float_t vzVpd = mBTofHeader->vpdVz();
