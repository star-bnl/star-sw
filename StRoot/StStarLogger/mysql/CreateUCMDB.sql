...
CREATE TABLE IF NOT EXISTS "Jobs" (
  "jobID"           int(11)      NOT NULL COMMENT 'ID of job when entry is created, unique within table',
  ...
  "gridJobID"       varchar(64)  NOT NULL COMMENT 'ID for job as assigned by Grid Resource Allocation Manager (GRAM)',
  "localJobID"      int(11)      NOT NULL COMMENT 'ID for job as assigned by local resource manager or scheduler',
  "gridSubmitTime"  datetime     NOT NULL COMMENT 'Time that job was submitted to the GRAM',
  "localSubmitTime" datetime     NOT NULL COMMENT 'Time that job was submitted to the local resource manager or scheduler',
  ...
  "startTime"       datetime     NOT NULL COMMENT 'Time that job started execution',
  "updateTime"      timestamp    NOT NULL default CURRENT_TIMESTAMP COMMENT 'Time that job execution state was last updated',
  "endTime"         datetime     NOT NULL COMMENT 'Time that job completed execution',
  "executionUserID" int(11)      NOT NULL COMMENT 'A login ID on the local resource site & worker node that actually executes',
  "stateID"         int(11)      NOT NULL COMMENT 'Foreign key reference to StateDictionary table',
  PRIMARY KEY  ("jobID")
);
...
CREATE TABLE IF NOT EXISTS "Messages" (
  ...
  "sverityID"        int(11)      NOT NULL COMMENT 'The ID of the severity (warning, debug, error, etc.) of the message',
...
CREATE TABLE IF NOT EXISTS "Tasks" (
  "taskID"           int(11)     NOT NULL COMMENT 'ID of task when entry is created, unique in table',
  "brokerID"         int(11)     NOT NULL COMMENT 'The ID of the broker that created task',
  "requesterID"      int(11)     NOT NULL COMMENT 'An ID from the TaskRequesters table',
  ...
  "taskSize"         int(11)     NOT NULL COMMENT 'Number of jobs in the task',
  "taskRemainSize"   int(11)     NOT NULL COMMENT 'Number of jobs no completed',
  "submitTime"       datetime    NOT NULL COMMENT 'Wall time that task was submitted to broker',
  "updateTime"       timestamp   NOT NULL default CURRENT_TIMESTAMP COMMENT 'Wall time that task row was last updated',
  "endTime"          datetime    NOT NULL COMMENT 'Wall time that task completed execution',
