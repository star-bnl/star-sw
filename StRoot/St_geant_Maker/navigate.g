...
*
*
 function agvolume(node,par,pos,mot)
+CDE,TYPING,GCBANK,GCVOLU,GCUNIT.
 integer  agvolume,node,par,pos,mot,LOCB
*
 Integer k,n,mother,daughter,where,who,copy,found,ier,ia;
 Integer item(20),count(20),list(20),nodes(0:20),Lnam(20),Lnum(20),Iax(20);
 save    k,mother,daughter,where,who,copy,found;
...
* integer i;
 ...
 { nodes(k)=node;  
   ...
   { k-=1; if (k>1) go to :next:; node=nodes(1);  return }
   ...
   Lnam(k)=IQ(JVOLUM+who); Lnum(k)=copy;
...
*   print *,count(k),copy,Nlevel,cn,(GRMAT(i,Nlevel),i=1,9);
  ...
  subroutine navigate
  integer agvolume,found,node,i1,i2,moth
  node=0; found=0;
  while agvolume(node,i1,i2,moth)>0  {  found+=1; node=found; }
