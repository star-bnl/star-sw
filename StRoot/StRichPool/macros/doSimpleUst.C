///////////////////////////////////////////////////////////////////////////////
//
// $Id: doSimpleUst.C,v 1.3 2006/08/15 21:42:10 jeromel Exp $
//
// Description: 
...
//
// example invocation:
// .x doSimpleUst.C(10,"some_directory/some_dst_file.xdf")
//
// example ROOT file invocation:
// .x doSimpleUst.C(10,"some_directory/some_dst_file.root")
//
// example multi-ROOT file invocation:
// .x doSimpleUst.C(9999,"some_directory/*.dst.root")
//
//////////////////////////////////////////////////////////////////////////////
//
...
void doSimpleUst()
{
    cout << "Usage: doSimpleUst.C(2)  // work with default event.root file" << endl;
    cout << "       doSimpleUst.C(startEvent,nEvents,\"path/somefile.event.root\")" << endl;
    cout << "       doSimpleUst.C(nEvents,\"path/*.event.root\")" << endl;
    cout << "       doSimpleUst.C(nEvents,\"path/file.dst.root\",\"evout\") //Write out StEvent" << endl;	
    cout << "       doSimpleUst.C(nEvents,\"path/file.dst.root\",\"display\") //EventDispay" << endl;	
    cout << "       doSimpleUst.C(nEvents,\"path/file.dst.root\",\"dbon\") //DB on" << endl;	
}
//		ProtoTypes

void doSimpleUst(Int_t nEvents, const Char_t ** fileList, const Char_t *qaflag =0);
void doSimpleUst(Int_t startEvent, Int_t nEvents, const Char_t ** fileList, const Char_t *qaflag =0);

void doSimpleUst(Int_t nEvents, 
              const Char_t *file="/afs/rhic.bnl.gov/star/data/samples/example.event.root",
              const Char_t *qaflag = 0); 

void doSimpleUst(Int_t startEvent,Int_t nEvents, 
              const Char_t *file="/afs/rhic.bnl.gov/star/data/samples/example.event.root",
              const Char_t *qaflag = 0);

void doSimpleUst(const Int_t nEvents, 
              const Char_t *path,
              const Char_t *file,
...
void doSimpleUst(Int_t startEvent, Int_t nEventsQQ, const Char_t **fileList, const Char_t *qaflag)
{
  ...
  cout <<  endl << endl <<" doSimpleUst -  input # events = " << nEvents << endl;
  Int_t ilist=0;
  while(fileList[ilist]){ 
      cout << " doSimpleUst -  input fileList = " << fileList[ilist] << endl;
      ilist++; 
    }
  cout << " doSimpleUst -  input qaflag   = " << qaflag << endl;
...
//		DB ON
    ...
    //
//      if (mainBranch.Contains("evtselBranch")) {
...
//      }
    ...
    StRichSimpleUst *lambMaker = new StRichSimpleUst("lambda");
...
void doSimpleUst(const Int_t startEvent, const Int_t nEvents, const Char_t *file, const Char_t *qaflag)
{
    printf("*file = %s\n",file);
    ...
    doSimpleUst(startEvent,nEvents,fileListQQ,qaflag);
}
//--------------------------------------------------------------------------
void doSimpleUst(const Int_t nEvents, const Char_t *file, const Char_t *qaflag)
{
    doSimpleUst(1,nEvents,file,qaflag);
}

//--------------------------------------------------------------------------
void doSimpleUst(const Int_t nEvents, const Char_t *path,const Char_t *file, const Char_t *qaflag, int flag)
{
    TString F;
    ...
    doSimpleUst(1,nEvents,F.Data(),opt.Data());
}

//--------------------------------------------------------------------------
void doSimpleUst(Int_t nEvents, const Char_t **fileList, const Char_t *qaflag)
{ doSimpleUst(1,nEvents,fileList,qaflag); }

///////////////////////////////////////////////////////////////////////////////
//
// $Log: doSimpleUst.C,v $
// Revision 1.3  2006/08/15 21:42:10  jeromel
...
// now unpack BfcStatus table in doSimpleUst so people can see if maker errors exist
//
...
// put Victor's code to write out .event.root file from doSimpleUst.C in here with a flag to turn off and on - now we can get rid of doEventsOut.C ... don't have to keep up 2 sets of macros
//
...
// make TurnDisplay macro available for doSimpleUst
//
