...
//
// example invocation:
// .x doShift.C(10,"some_directory/some_dst_file.xdf")
//
// example ROOT file invocation:
// .x doShift.C(10,"some_directory/some_dst_file.root")
//
// example multi-ROOT file invocation:
// .x doShift.C(9999,"some_directory/*.dst.root")
//
//////////////////////////////////////////////////////////////////////////////
//
...
void doShift()
{
    cout << "Usage: doShift.C(2)  // work with default event.root file" << endl;
    cout << "       doShift.C(startEvent,nEvents,\"path/somefile.event.root\")" << endl;
    cout << "       doShift.C(nEvents,\"path/*.event.root\")" << endl;
    cout << "       doShift.C(nEvents,\"path/file.dst.root\",\"evout\") //Write out StEvent" << endl;	
    cout << "       doShift.C(nEvents,\"path/file.dst.root\",\"display\") //EventDispay" << endl;	
    cout << "       doShift.C(nEvents,\"path/file.dst.root\",\"dbon\") //DB on" << endl;	
}
//		ProtoTypes

void doShift(Int_t nEvents, const Char_t ** fileList, const Char_t *qaflag =0);
...
void doShift(Int_t startEvent, Int_t nEventsQQ, const Char_t **fileList, const Char_t *qaflag)
{
  ...
  cout <<  endl << endl <<" doShift -  input # events = " << nEvents << endl;
  Int_t ilist=0;
  while(fileList[ilist]){ 
      cout << " doShift -  input fileList = " << fileList[ilist] << endl;
      ilist++; 
    }
  cout << " doShift -  input qaflag   = " << qaflag << endl;
...
//		DB ON
    ...
    //
    StMagFMaker *magF = new StMagFMaker();
    StTpcDbMaker *tpcDb = new StTpcDbMaker("tpcDb");
    StDetectorDbMaker *detDb = new StDetectorDbMaker("detDb");
    
    StUndoDistortionMaker *richCheck = new StUndoDistortionMaker("richCheck");
    StRichPIDMaker *pidMaker = new StRichPIDMaker("pidMaker");
...
void doShift(const Int_t startEvent, const Int_t nEvents, const Char_t *file, const Char_t *qaflag)
{
    printf("*file = %s\n",file);
    ...
    doShift(startEvent,nEvents,fileListQQ,qaflag);
}
//--------------------------------------------------------------------------
void doShift(const Int_t nEvents, const Char_t *file, const Char_t *qaflag)
{
    doShift(1,nEvents,file,qaflag);
}

//--------------------------------------------------------------------------
void doShift(const Int_t nEvents, const Char_t *path,const Char_t *file, const Char_t *qaflag, int flag)
{
    TString F;
    ...
    doShift(1,nEvents,F.Data(),opt.Data());
}

//--------------------------------------------------------------------------
void doShift(Int_t nEvents, const Char_t **fileList, const Char_t *qaflag)
{ doShift(1,nEvents,fileList,qaflag); }
...
// now unpack BfcStatus table in doShift so people can see if maker errors exist
//
...
// put Victor's code to write out .event.root file from doShift.C in here with a flag to turn off and on - now we can get rid of doShiftOut.C ... don't have to keep up 2 sets of macros
//
...
// make TurnDisplay macro available for doShift
//
