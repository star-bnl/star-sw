...
VecBosEvent::~VecBosEvent()
{
   ...
   //mTracksCandidateOldIsolation.clear();
...
TVector3 VecBosEvent::CalcRecoilCorrected()
{
   ...
   } else if (sRhicRunId == 13) { // TEST!! TO BE UPDATED as soon as run13 embedding available!!! (now use run12 file)
     hCorrFacVsRecoilPt = (TH2*) fileMCWplusR12->Get("event_mc_pass_wbos/hTrackRecoilTpcNeutralsPt_GenOverReco");
   } else {
...
void VecBosEvent::Process()
{
   ...
   for ( ; iTrack != mTracks.end(); ++iTrack)
   {
      ...
      //if ( track.IsCandidateOldIsolation() ) {
      //   mTracksCandidateOldIsolation.insert(*iTrack);
      //}
   }
...
TVector3 VecBosEvent::CalcP3InConeTpc_noEleP(VecBosTrack *vbTrack, UShort_t cone1d2d, Float_t scale)
{
   TVector3 totalP3InCone;
   ...
   for ( ; iTrack != mTracks.end(); ++iTrack)
   {
      ...
      // Don't count the same track
      //if (&track == vbTrack) continue;
      //if (track.mStMuTrack->id() == vbTrack->mStMuTrack->id()) continue;
      // XXX:ds: move this requirement to the track class where appropriate
      //if (track.GetFitHitFrac() < sMinTrackHitFrac) continue;
