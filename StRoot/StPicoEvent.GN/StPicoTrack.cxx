...
StPicoTrack::StPicoTrack() : TObject(),
  ...
  mMtdPidTraitsIndex(-1), mTrackCovMatrixIndex(-1) {
  /* empty */
}
...
StPicoTrack::StPicoTrack(const StPicoTrack &track) {
  ...
  mTrackCovMatrixIndex = track.mTrackCovMatrixIndex;
}
...
void StPicoTrack::setChi2(Float_t chi2) {
  mChi2 = (chi2 * 1000. > std::numeric_limits<unsigned short>::max()) ? std::numeric_limits<unsigned short>::max() : (UShort_t)(TMath::Nint(chi2 * 1000.));
}

//_________________
void StPicoTrack::setDedx(Float_t dEdx) {
  mDedx = (Float_t)(dEdx * 1.e6);
}

//_________________
void StPicoTrack::setNHitsFit(Int_t nhits) {
  /// It MUST be (charge * nHitsFit)
  mNHitsFit = (Char_t)nhits;
}
...
void StPicoTrack::setNSigmaPion(Float_t ns) {
  mNSigmaPion = (fabs(ns * 100.) > std::numeric_limits<short>::max()) ? std::numeric_limits<short>::max() : (Short_t)(TMath::Nint(ns * 100.));
}

//_________________
void StPicoTrack::setNSigmaKaon(Float_t ns) {
  mNSigmaKaon = (fabs(ns * 100.) > std::numeric_limits<short>::max()) ? std::numeric_limits<short>::max() : (Short_t)(TMath::Nint(ns * 100.));
}

//_________________
void StPicoTrack::setNSigmaProton(Float_t ns) {
  mNSigmaProton = (fabs(ns * 100.) > std::numeric_limits<short>::max()) ? std::numeric_limits<short>::max() : (Short_t)(TMath::Nint(ns * 100.));
}

//_________________
void StPicoTrack::setNSigmaElectron(Float_t ns) {
  mNSigmaElectron = (fabs(ns * 100.) > std::numeric_limits<short>::max()) ? std::numeric_limits<short>::max() : (Short_t)(TMath::Nint(ns * 100.));
}
