...
//_________________
void PicoDstAnalyzer(const Char_t *inFile = "../st_physics_12126101_raw_3040006.picoDst.root") {
  ...
  for(Long64_t iEvent=0; iEvent<events2read; iEvent++) {
    ...
    //std::cout << "Number of BTOW hits: " << nBTowHits << std::endl;
    for(Int_t iHit=0; iHit<nBTowHits; iHit++) {
      StPicoBTowHit *btowHit = dst->btowHit(iHit);
      if(!btowHit) continue;
      //std::cout << "Tower id = " << btowHit->id() << std::endl;
    }
  } //for(Long64_t iEvent=0; iEvent<events2read; iEvent++)
