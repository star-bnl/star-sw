...
#define LOG_DEBUG std::cout << "DEBUG INFO: "
#define LOG_ERROR std::cout << "ERROR INFO: "
#else
...
void StPicoMtdTrigger::setQTtacSum(Int_t runnumber, UShort_t mtdQTadc[8][16],
				   ...
				   const Int_t QTSlewBinEdge[8][16][8], const Int_t QTSlewCorr[8][16][8]) {
...
StPicoMtdTrigger::StPicoMtdTrigger(const StMuDst& muDst, const int QTtoModule[8][8],
  ...
  if (trigger) {
    ...
    UInt_t decision2 = 0;
    if (year == 2016) {
      decision2 = trigger->dsmTF201Ch(6);
    }
    mTF201TriggerBit = 0;
