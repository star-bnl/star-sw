...
void getCellPosition2017(int nstb, int row, int col, float &x1, float &y1, float &x2, float &y2, float &z){      
  const float large_width = 58.0; // large cell width
  const float small_width = 38.0; // small cell width
  float x=0.0, y=0.0;

  //FMS Position February 23, 2017
  //North position unchanged from 2015
  //South position Dx=-5mm (5 mm north)
  //from position in 2015.  Offset in
  //program changed from -8 to -13 mm
  switch(nstb){
  case 1:
      x = -25.0 - 58.6 * col;
      y = 965.5 - 58.5 * row;
