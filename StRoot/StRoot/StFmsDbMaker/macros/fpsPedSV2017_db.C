...
void fpsPedSV2017_db(char* opt="", char* year="", char* filename="fps_good_physics_ped/18080012.txt"){
  ...
  if(yr.Contains("17ofl")){
	...
	StDbTable* dbtable = node->addDbTable("fpsPed");
	if(flag==0){
    ...
    std::cout << "Dataset validity range: [ " << val[0].GetDate() << "." << val[0].GetTime() << " - " 
	      ...
	      << std::endl;    
    fpsPed_st *tbl = dataset->GetTable();
