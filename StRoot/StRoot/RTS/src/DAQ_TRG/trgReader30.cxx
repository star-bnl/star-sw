...
int trgReader30(char *arg, struct trg_t *trg)
{
	int i ;
	int sdes, ssum, sraw ;
	int len ;
	...
	int rdb = swap ? swap16(trgd->raw[0].RawDetBytes) : trgd->raw[0].RawDetBytes;
      
	sraw = ssum + rdb;

	int minsize = sizeof(trgd->bh) + sizeof(trgd->desc) + sizeof(trgd->sum) + 1772;
	...
	if(len < sraw) {
		LOG(NOTE,"Trigger Bank too small %d vs %d",len,sraw);
		return 0 ;
	}
...
//	int trg_length = len - sizeof(struct bankHeader) - 40 ;	// 40 is supurious
	...
	if(trg_length < sizeof(EvtDescData) + sizeof(trgd->sum)) {
	  LOG(ERR, "Trigger data is too small:  %d bytes less than min of %d",trg_length,(sizeof(EvtDescData) +sizeof(trgd->sum)));
	}
...
// 	}
	...
	for(i=0;i<32;i++) trg->MTD[i] = trgd->raw[0].MTD[i];
	for(i=0;i<64;i++) trg->VPD[i] = trgd->raw[0].VPD[i];
	for(i=0;i<16;i++) trg->TOF[i] = trgd->raw[0].TOF[i];
	...
	*/
	...
	for(i=0;i<256;i++) trg->FPDW[i] = trgd->raw[0].FPDW[i];
