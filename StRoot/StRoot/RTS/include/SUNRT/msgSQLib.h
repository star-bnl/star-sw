...
STATUS msgSQCreate(int maxMsgs, int maxMsgLength, int options, int id=-1);
...
//
STATUS msgSQSend(MSG_Q_ID msgQId, char *buffer, int nBytes, int timeout=NO_WAIT, int priority=0);

STATUS msgSQReceive(MSG_Q_ID msgQId, char *buffer, int maxNBytes, int timeout=WAIT_FOREVER);
