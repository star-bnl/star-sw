...
struct L2pedResults2009 { // all output bits lump together
  enum{mySizeChar=4};
...
//---------------
inline void L2pedResults2009_print(struct L2pedResults2009  *p) {
  if(p==0) {printf("print L2pedResults2009() - NULL pointer ????\n"); return;}
  unsigned int x=p->int0.decision;
  printf("L2pedResults2009():  bemcIn=%d  eemcIn=%d ,  pedSubtr=%d  \n", GET_OB(x,3),  GET_OB(x,4),  GET_OB(x,6));
