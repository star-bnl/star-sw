...
//
  ...
  if ( h11 == 0.0 || h22 == 0.0 ){
	  ftfLog ( " Problems fitting a circle " ) ;
	  return 1 ;
  }
...
//    ftfLog ( "FtfBaseTrack: fitLine: problem calculating s \n" ) ;
...
//
   ...
   for ( startLoop() ; done() ; nextHit() ) {
	 ...
	 dpsi = 0.5F * (double)sqrt ( dx*dx + dy*dy ) / radius ;
         if ( dpsi > 1.) {
	   //ftfLog ( "FtfBaseHit::fitLine(): dpsi=%f\n", dpsi);
            dpsi = 1.;
         }
...
//    ftfLog ( "particle does not intersect \n" ) ;
      return  1 ;
    }
//  
//     Find intersection
...
//    Intersection in z
   ...
   if ( r < r0 ) dangle *= -1 ;
// ftfLog ( "dangle %f q %d \n", dangle, q ) ;
   if ( (float(q) * dangle) < 0 ) dangle = dangle + float(q) * 2. * M_PI ;
 
   double stot = fabs(dangle) * rc ;
// ftfLog ( "dangle %f z0 %f stot %f \n", dangle, z0, stot ) ;
...
int FtfBaseTrack::intersectorZLine ( double a, double b, 
    ...
    double rc   = pt  / ( bFactor * getPara()->bField )  ;
...
//
   double x0   = r0*cos(phi0);
   double y0   = r0*sin(phi0);
   double trackPhi0= psi + q*0.5*M_PI/fabs(q);
   double rcoc = pt  / ( bFactor * getPara()->bField )  ;
...
int FtfBaseTrack::refitHelix ( int mod, int modEqual, int rowMin, int rowMax ) {
   ...
   if ( nHits < 1 || nHits > 500 ) {
      ftfLog ( "FtfBaseTrack:refitHelix: nHits %d out of range \n", nHits ) ;
      return 1 ;
   }
//
...
void FtfBaseTrack::updateToRadius  ( double radius ) {
   ...
   if ( ok ) {
//    ftfLog ( "FtfBaseTrack::updateToRadius: track %d does not intersect radius %f\n", 
//              id, radius ) ;
      return ;
   }
//
...
//     ftfLog ( "problem extrapolating track \n" ) ;
//     return ;  
// }
...
Ftf3DHit FtfBaseTrack::extrapolate2PathLength(double pathlength)
{
 ...
 double Bfield=getPara()->bField * 0.01;
 double c=0.3;
...
{
 double radius=pt / ( bFactor * getPara()->bField );
