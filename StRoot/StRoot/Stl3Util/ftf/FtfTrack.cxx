...
//
  //   cout << "." << flush;
   double xc, yc ;
   //cout << ".( a1Xy = " << a1Xy << " )" << flush;
   double rc   = sqrt ( a2Xy * a2Xy + 1 ) / ( 2 * fabs(a1Xy) ) ;
   //cout << "." << flush;
   double field = getPara()->bField;
   //cout << "." << endl;
   //pt          = (double)(bFactor * getPara()->bField * rc );
   pt          = (double)(bFactor * field * rc );
...
int FtfTrack::follow ( FtfContainer *volume, int way, int ir_stop ) {
   ...
   if ( way < 0 )
      ...
      nextHit = (FtfHit *)firstHit ; 
        ...
        ftfLog ( "FtfTrack::follow: ===> Going into Track extension <===\n" );
#endif
//
...
//
      ...
      if ( getPara()->trackDebug && getPara()->debugLevel >= 1 ){
         if ( nextHit != 0 ){
            ftfLog ( "FtfTrack::follow: Search succesful, hit selected %d\n", 
                      nextHit->id );
//		    nextHit->Show ( getPara()->color_track ) ;
         }
         else{
            ftfLog ( "FtfTrack::follow: Search unsuccesful\n" );
            if ( chi2[0]+chi2[1] > getPara()->hitChi2Cut )
               ftfLog ( " hit chi2 %f larger than cut %f ", chi2[0]+chi2[1], 
                                                            getPara()->hitChi2Cut ) ;
         }
         debugAsk () ;
      }
...
//
         ...
         if ( getPara()->trackDebug && getPara()->debugLevel >= 4 ) 
            ftfLog ( "FtfTrack::seekNextHit: search in row %d \n",irp ) ;
#endif
//
...
#ifdef TRDEBUG
   if ( getPara()->trackDebug && getPara()->debugLevel >= 4 )
        ftfLog ( "FtfTrack:segment: **** Trying to form segment ****\n" );
#endif
//
//    Loop as long a a hit is found and the segment
//    is shorter than n_hit_segm
//
   while ( nextHit != 0 && nHits < getPara()->nHitsForSegment ) {
      ...
      if ( getPara()->trackDebug && getPara()->debugLevel > 0 ) {
         if ( nextHit != 0 ) {
            ftfLog ( "FtfTrack::segment: Search succesful, hit %d selected\n",
                      nextHit->id );
//       nextHit->Show ( getPara()->color_track ) ;
         }
         else
            ftfLog ( "FtfTrack::segment: Search unsuccesful\n" );
         debugAsk () ;
      }
#endif
//
...
//
   ...
   if ( getPara()->trackDebug && getPara()->debugLevel >= 3 ) {
      ftfLog ( "FtfTrack::segmentHitSelection:\n");
      ftfLog ( "dr,dphi,deta,distance, Min distance  %7.2f %7.2f %7.2f %7.2f %7.2f\n",
                dr,dphi,deta,d3,chi2[0] ) ;
      if ( d3 < chi2[0] )
         ftfLog ( "Best point, keep it !!!\n" );  
      else{
         ftfLog ( "Worse than previous, reject !!\n" );
...
void FtfTrack::debugAsk (void) 
{
    ...
    ftfLog ( "stop(s), continue (any other key)\n" );
    cc = getchar();
    ...
    ftfLog ( "FtfTrack::debugAsk: Debug Level %d\n", getPara()->debugLevel ) ;
...
//  }
  ftfLog ( "FtfTrack::debugDeleteCandidate: Track %d has %d hits <==\n"
                           ,id, nHits );
  ftfLog ( "FtfTrack::debugDeleteCandidate: Minimum is %d, delete it \n",
...
//
  ftfLog ( "FtfTrack::debugFollowCandidate ===> Extension in Follow <===\n" ) ;
 // print ( 31 ) ;

  ftfLog ( "FtfTrack::debugFollowCandidate: Try hit %d\n", candidateHit->id ) ;
...
//
  ...
  {
      ftfLog ( "FtfTrack::debugFollowCandidate: hit %d used in track %d\n",
                    candidateHit->id, id );
      debugAsk () ;
//    candidateHit->Show ( getPara()->color_candidate ) ;
...
//
   ...
   ftfLog ( " \n ------------------------------------- " ) ;
   if ( lchi2 < chi2_min ){
       ftfLog ( "FtfTrack::debugFollowSuccess: %f Best Chi2, keep point !!!\n", 
                    lchi2 );
       if ( lchi2 < getPara()->goodHitChi2 ){
           ftfLog ( "This Chi2 is better than the good cut %f\n",
                           lchi2, getPara()->goodHitChi2 );
           ftfLog ( "Stop search !!! " );
       }
   }
   else{
      ftfLog ( "FtfTrack::debugFollowSuccess: Hit %d worse than previous, forget it !! ",
                candidateHit->id );
...
//
   if ( getPara()->debugLevel > 2 ) {
      ftfLog ( "FtfTrack::debugFollowSuccess:\n");
      ftfLog ( "dis_xy dis_sz   %7.2e %7.2e\n ", dxy, dsz );
      ftfLog ( "Error xy   sz   %7.2e %7.2e\n ",  
                       candidateHit->wxy, candidateHit->wz );
      ftfLog ( "xy:a1,a2;sz:a1,a2  %7.2f %7.2f %7.2f %7.2f\n",
                                       a1Xy, a2Xy, a1Sz, a2Sz );
      ftfLog ( "ch2:xy sz tot min  %7.2f %7.2f %7.2f %7.2f\n", 
...
----------------------------------------------------------------------------*/
      ...
      ftfLog ( "FtfTrack:debugInVolume: Try hit %d\n", candidateHit->id ) ; 
      candidateHit->print ( 11 ) ;
/*----------------------------------------------------------------------------
        If the hit already used say it and forget about it
----------------------------------------------------------------------------*/
      if ( candidateHit->track != 0 ) {
         ftfLog ( "FtfTrack:debugInVolume: hit %d used in track %d\n", 
...
//       candidateHit->Show ( 0 );
         ...
         if ( dphi > getPara()->dphi )
            ftfLog ( "FtfTrack:debugInVolume: Hits too far apart in phi: %f \n", 
                      dphi ) ;
         if ( deta > getPara()->deta )
            ftfLog ( "FtfTrack:debugInVolume: Hits too far apart in eta: %f \n", 
                      deta ) ;
      }
      debugAsk () ;
   }
...
void FtfTrack::debugNew (  )
{
  ...
  {
     ftfLog ( "================================================ \n" );
     ftfLog ( "FtfTrack::debugNew:Starting track %d from point %d\n", 
              id, firstHit->id );
     ftfLog ( "================================================ \n" );
