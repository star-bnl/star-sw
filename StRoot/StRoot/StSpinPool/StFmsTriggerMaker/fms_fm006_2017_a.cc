...
void fms_fm006_2017_a(Board& fm006, int t, int simdat){
  // thresholds
  ...
  int BS1EF =(bs[0]>BSThr1) | (bs[1]>BSThr1) | (bs[2]>BSThr1);
  ...
  if(PRINT){
    ...
    printf("%s out=%08x BS3=%1d BS2=%1d BS1EF=%1d sum=%4d %4d %4d JpEF=%3d\n",
	   fm006.name,fm006.output[t],BS3,BS2,BS1EF,bs[0],bs[1],bs[2],JpEF);
  }
}

int getFM006_2017a_BS3(int out)   {return getbits(out, 0, 1);}
int getFM006_2017a_BS2(int out)   {return getbits(out, 1, 1);}
int getFM006_2017a_BS1EF(int out) {return getbits(out, 2, 1);}
int getFM006_2017a_JpEF(int out)  {return getbits(out,16, 8);}
