...
class StiHitErrorCalculator : public TChair {
 public:
  ...
  Double_t* 	coeff(Int_t i = 0) 	{return Struct(i)->coeff;} /// coeff[6] = 0:intrinsicY  1: driftY   2: crossY 3:intrinsicZ  4: driftZ   5: crossZ
    ...
    virtual void  calculateError(const StiNodePars *pars,Double_t &ecross,Double_t &edip, Double_t fudgeFactor = 1) const {
      calculateError(pars->z(),  pars->eta(), pars->tanl(), ecross, edip, fudgeFactor);
  }
 protected:
