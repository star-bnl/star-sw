void doBsmdSpread(char *f_20,char *f_10,char *f_30){
  ...
  h_effMB_plus->Multiply(mult);
  h_effHT1_plus->Multiply(mult);
  h_effHT2_plus->Multiply(mult);
  h_effMB_min->Multiply(mult);
  h_effHT1_min->Multiply(mult);
  h_effHT2_min->Multiply(mult);
  ...
  leg->AddEntry(gMB_min,"minimum bias, BSMD #sigma(C)/C=30%","p");
  leg->AddEntry(gHT1_min,"hightower 1","p");
  leg->AddEntry(gHT2_min,"hightower 2","p");
  leg->AddEntry(gMB_plus,"BSMD #sigma(C)/C=10%","p");
  leg->Draw();

  c->SaveAs("bsmd_spread_pp.eps");
  c->SaveAs("bsmd_spread_pp.root");
