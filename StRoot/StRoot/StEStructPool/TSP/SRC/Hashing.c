...
void HashInsert(HashTable * T, unsigned tspHash, GainType Cost)
{
    int i, p;

    i = tspHash % HashTableSize;
...
int HashSearch(HashTable * T, unsigned tspHash, GainType Cost)
{
    int i, p;

    i = tspHash % HashTableSize;
    p = tspHash % 97 + 1;
    while ((T->Entry[i].Hash != tspHash || T->Entry[i].Cost != Cost)
           && T->Entry[i].Cost != MINUS_INFINITY)
        if ((i -= p) < 0)
            i += HashTableSize;
    return T->Entry[i].Hash == tspHash;
}
