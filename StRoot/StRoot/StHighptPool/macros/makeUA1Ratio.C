		  ...
		  float extraValue = 10)
{
  ...
  for(int ibin=0;ibin<2; ibin++){
    ...
    gRatio->Draw("ap"); sprintf(title,"%sOverUA1",gSpec[ibin]->GetName());
    gRatio->SetMaximum(1.2); gRatio->SetMinimum(.2);
    ...
    for(int ipm=0;ipm<2;ipm++){
      ...
      Divide(&c1,1,1,title,inName);
      //gUA1=makeUA1(gSpec[ibin][ipm]);
      gRatio=makeRatio(gSpecPM[ibin][ipm],gUA1);
      gRatio->Draw("ap");
      gRatio->SetMaximum(1.2); gRatio->SetMinimum(.2);
...
makeUA1(const TGraphAsymmErrors* graph)
{
  ...
  */
  ...
  sprintf(func,"2*pi*%.0f*%.0f*(1+x/%.2f)^(-%.2f)",A,TAA,p0,n);

  sprintf(mean,"2*pi*x*%.0f*%.0f*(1+x/%.2f)^(-%.2f)",A,TAA,p0,n);
