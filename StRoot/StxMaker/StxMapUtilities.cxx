...
bool StxDetectorNodePositionLessThan::operator() (const StxDetectorNode * lhs,
						  const StxDetectorNode * rhs) const {
  if (lhs->Data()==0 || rhs->Data()==0) 
    cout <<"StxDetectorNodeLessThan::operator(). ERROR:\t" <<"null data.  Return false"<<endl;
...
void SetHitUsed::operator()(StxKalmanTrackNode& node)
{
    StxHit* hit = node.Hit();
    if(!hit) 			return;
