...
class FitParameters { // base class for fit paramters
 public:
  //                                SC       SD       Stx       Dca
  enum eTrParType {kUndefined=-1,  kSC = 0, kSD = 1, kStx = 2, kDca = 3, kAll};
  struct ParamSC_t {// 
  ...
  struct ParamStx_t {// Stx parameters
    //                    -q/pT
    Double_t x, y, z, eta, ptin, tanl; // at fixed x (eta == Phi)
  };
  
  struct CovMatrixStx_t {// covariance matrix of  Stx parameters
    Double_t 
      cXX,
      cYX, cYY,
      cZX, cZY, cZZ,
 ...
 public:
  ...
  FitParameters     &SC2Stx (const FitParameters &source);	      
  FitParameters     &SC2Dca (const FitParameters &source);	      

  FitParameters     &SD2SC  (const FitParameters &source);	      
  FitParameters     &SD2Stx (const FitParameters &source);	      
  FitParameters     &SD2Dca (const FitParameters &source);        

  FitParameters     &Stx2SC (const FitParameters &source);	      
  FitParameters     &Stx2SD (const FitParameters &source);	      
  FitParameters     &Stx2Dca (const FitParameters &source);	      

  FitParameters     &Dca2SC (const FitParameters &source);	      
  FitParameters     &Dca2Stx(const FitParameters &source);	      
  ...
  TRVector           PxyzGStx() const;
  TRSymMatrix        PxyzGCovMatrixStx() const;
  TRVector           XyzGStx() const;
  TRSymMatrix        XyzGCovMatrixStx()  const;
