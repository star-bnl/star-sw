/***************************************************************************
 *
 * $Id: StxStEventFiller.cxx,v 2.95 2009/08/20 22:25:30 fisyak Exp $
 *
 * Author: Manuel Calderon de la Barca Sanchez, Mar 2002
 ***************************************************************************
 *
 * $Log: StxStEventFiller.cxx,v $
 ...
 * Add +x11 short tracks pointing to EEMC, clean up StxDedxCalculator
 *
 ...
 * StTrack::SetKey(...StxKalmanTrack::Id()) now
 *
 ...
 * the maxPointCount(unsigned int detId) method of StxKalmanTrack.
 *
 ...
 *  StxKalmanTrackNode has to satisfy the chi2 criterion.
 *
 ...
 * Added an "accept" function to filter unwanted tracks from Stx into StEvent.
 ...
 * nan's have been trapped (one hopes!) in StxKalmanTrack, and for these
 ...
 * order in Stx than in StEvent.  In Stx the array is counted starting from
 ...
 * have to be modified when the helicity convention in StxStKalmanTrack
 * is updated.
 *
 ...
 * Adding Editable Parameters and dynamic StxOptionFrame
 *
 * Revision 1.18  2002/08/29 21:09:22  andrewar
 * Fixed seg violation bug.
 *
 * Revision 1.17  2002/08/22 21:46:00  pruneau
 * Made a fix to StxStEventFiller to remove calls to StHelix and StPhysicalHelix.
 ...
 * StxStEventFiller::fillEventPrimaries()
 *
 ...
 * Check if primary vertex is available in StxStEventFiller::impactParameter(),
...
//StxVMCMaker
#include "StxStEventFiller.h"

#include "TMath.h"

//_____________________________________________________________________________
StxStEventFiller::StxStEventFiller() : mEvent(0), mTrackStore(0), mTrkNodeMap()
{
   mGloPri = 0;
  ...
  mStxEncoded = kITKalmanFitId + bit; // adding that to the proper fitting Id

}

//_____________________________________________________________________________
StxStEventFiller::~StxStEventFiller()
{
 delete physicalHelix; physicalHelix=0;
 delete originD;       originD      =0;
   cout <<"StxStEventFiller::~StxStEventFiller()"<<endl;
}

//Helper functor, gotta live some place else, just a temp. test of StxKalmanTrack::StHits() method
//_____________________________________________________________________________
struct StreamStHit
{
...
/*! 
  Algorithm:
  Loop over all tracks in the StxKalmanTrackContainer, doing for each track:
  - Create a new global track and associated information (see below)
    and set its data members according to the StxKalmanTrack,
  ...
  for (trackIterator trackIt = mTrackStore->begin(); trackIt != mTrackStore->end(); ++trackIt) { \n
     StxKalmanTrack* kTrack = (*trackIt).second; // the container is a <map>, need second entry of <pair> \n
\n
...
*/
//_____________________________________________________________________________
void StxStEventFiller::fillEvent(StEvent* e, StxKalmanTrackContainer* t)
{
  //cout << "StxStEventFiller::fillEvent() -I- Started"<<endl;
  mGloPri=0;
  if (e==0 || t==0) 
    {
      cout <<"StxStEventFiller::fillEvent(). ERROR:\t"
	   <<"Null StEvent ("<<e<<") || StxKalmanTrackContainer ("<<t<<").  Exit"<<endl;
      return;
    }
  mEvent = e;
  mTrackStore = t;
  ...
  StErrorHelper errh;
  mTrackNumber=0;
  for (vector<StxKalmanTrack*>::iterator trackIt = mTrackStore->begin(); trackIt!=mTrackStore->end();++trackIt) 
    {
      StxKalmanTrack* kTrack = static_cast<StxKalmanTrack*>(*trackIt);
      ...
      try 
	{
	  ...
	  if (trackNode->entries(global)<1)
	    cout << "StxStEventFiller::fillEvent() -E- Track Node has no entries!! -------------------------" << endl;  
          Int_t ibad = gTrack->bad();
	  errh.Add(ibad);
          if (ibad) {
//VP	    printf("GTrack error: %s\n",errh.Say(ibad).Data());
//VP	    throw runtime_error("StxStEventFiller::fillEvent() StTrack::bad() non zero");
          }
	  fillTrackCount2++;
	...
	{
	  cout << "StxStEventFiller::fillEvent() -W- runtime-e filling track"<<rte.what() << endl;
	  delete trackNode;
	  delete detInfo;
	  delete gTrack;
	}
	...
	{
	  cout << "StxStEventFiller::fillEvent() -W- Unknown exception filling track."<<endl;
	  delete trackNode;
	  delete detInfo;
	  delete gTrack;
	}
  ...
  cout <<"StxStEventFiller::fillEvent() -I- Number of filled as global(1):"<< fillTrackCount1<<endl;
  cout <<"StxStEventFiller::fillEvent() -I- Number of filled as global(2):"<< fillTrackCount2<<endl;
  cout <<"StxStEventFiller::fillEvent() -I- Number of filled GOOD globals:"<< fillTrackCountG<<endl;
  errh.Print();

  return;
}
//_____________________________________________________________________________
void StxStEventFiller::fillEventPrimaries() 
{
  //cout <<"StxStEventFiller::fillEventPrimaries() -I- Started"<<endl;
  mGloPri=1;
  if (!mTrkNodeMap.size()) 
    {
      cout <<"StxStEventFiller::fillEventPrimaries(). ERROR:\t"
	   << "Mapping between the StTrackNodes and the StxKalmanTracks is empty.  Exit." << endl;
      return;
    }
  //Added residual maker...aar
  StPrimaryVertex* vertex = 0;
  StSPtrVecTrackDetectorInfo& detInfoVec = mEvent->trackDetectorInfo();
  cout << "StxStEventFiller::fillEventPrimaries() -I- Tracks in container:" << mTrackStore->size() << endl;
...
//VP	        throw runtime_error("StxStEventFiller::fillEventPrimaries() StTrack::bad() non zero");
      }
  ...
  cout <<"StxStEventFiller::fillEventPrimaries() -I- Primaries (1):"<< fillTrackCount1<< " (2):"<< fillTrackCount2<< " no pipe node:"<<noPipe<<" with IFC:"<< ifcOK<<endl;
  cout <<"StxStEventFiller::fillEventPrimaries() -I- GOOD:"<< fillTrackCountG <<endl;
  errh.Print();
  return;
}
//_____________________________________________________________________________
/// use the vector of StHits to fill the detector info
/// change: currently point and fit points are the same for StxKalmanTracks,
/// if this gets modified later in ITTF, this must be changed here
/// but maybe use track->PointCount() later?
//_____________________________________________________________________________
void StxStEventFiller::fillDetectorInfo(StTrackDetectorInfo* detInfo, StxKalmanTrack* track, bool refCountIncr) 
{
  //cout << "StxStEventFiller::fillDetectorInfo() -I- Started"<<endl;
  ...
  StxKTNIterator tNode = track->rbegin();
  StxKTNIterator eNode = track->rend();
  StxKalmanTrackNode *lastNode=0,*fistNode=0;
  for (;tNode!=eNode;++tNode) 
  {
      StxKalmanTrackNode *node = &(*tNode);
      if(!node->IsValid()) 	continue;

      StxHit *stiHit = node->Hit();
      if (!stiHit)		continue;
...
// 	Fill StHit errors for Gene
  ...
  //cout << "StxStEventFiller::fillDetectorInfo() -I- Done"<<endl;
}
//_____________________________________________________________________________
void StxStEventFiller::fillGeometry(StTrack* gTrack, StxKalmanTrack* track, bool outer) {
  assert(gTrack);
  assert(track) ;

  StxKalmanTrackNode* node = track->InnOutMostNode(outer,3);
...
//_____________________________________________________________________________
void StxStEventFiller::fillFitTraits(StTrack* gTrack, StxKalmanTrack* track){
  // mass
  ...
  StxKalmanTrackNode* node = track->InnerMostHitNode(3);
...
void StxStEventFiller::fillFlags(StTrack* gTrack) {
...
//_____________________________________________________________________________
void StxStEventFiller::fillTrack(StTrack* gTrack, StxKalmanTrack* track,StTrackDetectorInfo* detInfo )
{

  //cout << "StxStEventFiller::fillTrack()" << endl;
...
//_____________________________________________________________________________
bool StxStEventFiller::accept(StxKalmanTrack* track)
{
...
//_____________________________________________________________________________
Double_t StxStEventFiller::impactParameter(StxKalmanTrack* track, StThreeVectorD &vertexPosition) {
  return   track->InnerMostNode(2)->Helix()->helix().distance(vertexPosition);
}
//_____________________________________________________________________________
Double_t StxStEventFiller::impactParameter(StTrack* track, StThreeVectorD &vertex) {
  return track->geometry()->helix().distance(vertex);
}
//_____________________________________________________________________________
void StxStEventFiller::fillDca(StTrack* stTrack, StxKalmanTrack* track)
{
  StGlobalTrack *gTrack = dynamic_cast<StGlobalTrack*>(stTrack);
  assert(gTrack);

  StxKalmanTrackNode *tNode = track->InnerMostNode();
...
//_____________________________________________________________________________
void StxStEventFiller::FillStHitErr(StHit *hh,const StxKalmanTrackNode *node) {
#if 0
  Double_t stiErr[6];
