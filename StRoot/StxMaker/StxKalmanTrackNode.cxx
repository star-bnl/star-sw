/*
 * $Id: StxKalmanTrackNode.cxx,v 2.149 2014/05/29 12:51:21 fisyak Exp $
 *
 * /author Claude Pruneau
 *
 * $Log: StxKalmanTrackNode.cxx,v $
 ...
 * Dependency from StxKalmanTrackNode removed
 *
 * Revision 2.90  2005/12/08 22:05:45  perev
 * nudge assert replaced by print. But very strangeStxKalmanTrackNode.cxx
 *
 ...
 * Add test suit for matrix manipulation debugging under overall CPPFLAGS=-DStx_DEBUG
 *
 ...
 * Fixed StxHit to use global coordinates as well as locals.
 * Fixed Logic Bug in StxKalmanTrackFinder
 *
...
#define PrPP(A,B) {if (Debug() & 32) {LOG_INFO  << "=== StxKalmanTrackNode::" << (#A) << "\t" << (#B) << " = \t" << ( B ) << endm;}}
// derivatives debug print out
#define PrPD(A,B) {if (Debug() & 16) {LOG_INFO  << "=== StxKalmanTrackNode::" << (#A) << "\t" << (#B) << " = \t" << ( B ) << endm;}}
// Local Track Model
//
// x[0] = y  coordinate
...
TString StxKalmanTrackNode::comment("Legend: \tE - extapolation\tM Multiple scattering\tV at Vertex\tB at beam\tR at Radius\tU Updated\n");
TString StxKalmanTrackNode::commentdEdx(""); 

void StxKalmanTrackNode::Break(Int_t kase)
{
...
/* bit mask for debug printout  
   ...
   3   => 8 - test locate
 */
Int_t StxKalmanTrackNode::_debug = 0;
Int_t StxKalmanTrackNode::_laser = 0;

//______________________________________________________________________________
void StxKalmanTrackNode::Reset() { 
...
//______________________________________________________________________________
void StxKalmanTrackNode::SetState(const StxKalmanTrackNode * n)
{
  _state   = n->_state;
...
*/
//______________________________________________________________________________
void StxKalmanTrackNode::Get(
			     Double_t  *x, 
			     Double_t  *e, 
			     double& chi2) {
  FitParameters FP(FitParameters::kStx);
  FP = Fitted();
...
*/
//______________________________________________________________________________
Double_t StxKalmanTrackNode::Hz() const
{
...
*/
//______________________________________________________________________________
void StxKalmanTrackNode::GlobalRadial(Double_t  x[6],Double_t  e[15])
{
...
*/
//______________________________________________________________________________
void StxKalmanTrackNode::GlobalTpt(Float_t  x[6],Float_t  e[15])
{
...
*/ 
//______________________________________________________________________________
void StxKalmanTrackNode::UpdatePrediction(const StxKalmanTrackNode *pNode, TRVector *refFitPar) {
...
//______________________________________________________________________________
Int_t StxKalmanTrackNode::Propagate(StxKalmanTrackNode *pNode, StxDetector *tDet, Int_t dir, TRVector *refFitPar) {
  Int_t position = 0;
  ...
  if (pNode->Detector() == tDet) {PrPP(Propagate,Fitted());
    ...
    position = StGeanePropagator::instance()->Propagate(*(pNode->Detector()->GetPhysicalNode()->GetMatrix()), 
							*(tDet->GetPhysicalNode()->GetMatrix()),
							Dir);
    if (position) {
      if (Debug() > 1) cout << "StxKalmanTrackNode::Propagate to detector " << tDet->GetName() 
			    << "\tfailed\t" << position << endl;
      return position;
    }
    StxDetector *tDetC = (StxDetector *) StxToolkit::instance()->DetectorContainer()->FindDetector(gGeoManager->GetPath());
...
//______________________________________________________________________________
Int_t StxKalmanTrackNode::Propagate(StxKalmanTrackNode *pNode, Int_t dir, TRVector *refFitPar) {
 static Int_t nCall=0; nCall++;
  ...
  if (position) {
    if (Debug() > 1) cout << "StxKalmanTrackNode::Propagate failed\t" << position << endl;
    return position;
  }
  StxDetector *tDet = (StxDetector *) StxToolkit::instance()->DetectorContainer()->FindDetector(gGeoManager->GetPath());
  SetDetector(tDet);
  if ( tDet) {
    if (Debug()) ResetComment(Form("%30s ",tDet->GetName()));
    LoadPrediction(); PrPP(Propagate,Predicted());
...
<p>
*/
Bool_t StxKalmanTrackNode::Propagate(const StxKalmanTrackNode *parentNode, StxHit * vertex,Int_t dir, TRVector *refFitPar) {
  static Int_t nCall=0; nCall++;
  Break(nCall);
...
Bool_t StxKalmanTrackNode::PropagateToBeam(const StxKalmanTrackNode *parentNode,Int_t dir, TRVector *refFitPar)
{
  SetState(parentNode);
  if (Debug()) {
...
Bool_t StxKalmanTrackNode::PropagateToRadius(StxKalmanTrackNode *pNode, Double_t radius,Int_t dir, TRVector *refFitPar) {
...
//______________________________________________________________________________
inline Double_t StxKalmanTrackNode::Length(const StThreeVector<Double_t>& delta, Double_t curv)
{
...
//______________________________________________________________________________
Double_t StxKalmanTrackNode::EvaluateChi2(const StxHit * hit) {
...
//______________________________________________________________________________
Int_t StxKalmanTrackNode::UpdateNode() {/*! Update the track parameters using this node.*/
  static Int_t nCall=0; nCall++;
...
ostream& operator<<(ostream& os, const StxKalmanTrackNode& n) {
  const StxDetector *det = n.Detector();
  if (det) os  <<"Det:"<< det->GetName();
  else     os << "Det:UNknown";
  os         << StxKalmanTrackNode::PrintpTasString(n.Predicted(), -1, "Predicted") << endl;
  os << "\t" << StxKalmanTrackNode::PrintpTasString(n.Fitted(), n.chi2(), "Fitted") << endl;
  os << "\t" << StxKalmanTrackNode::PrintpTasString(n.Smoothed(), n.chi2(), "Smoothed") << endl;
  if (n.Debug() & 2) {
    os << n.Fitted() << endl;
    StxHit * hit = n.Hit();
    if (hit) os << endl 
...
//______________________________________________________________________________
Double_t StxKalmanTrackNode::WindowY()
{	  
  const StxDetector * detector = Detector();
...
//_____________________________________________________________________________
Double_t StxKalmanTrackNode::WindowZ() {	 
  const StxDetector * detector = Detector();
  const StiTrackingParameters * parameters = detector->TrackingParameters();
  Double_t searchWindowScale = parameters->getSearchWindowScale();
  Double_t minSearchWindow   = parameters->getMinSearchWindow();
...
//______________________________________________________________________________
void StxKalmanTrackNode::Initialize(StxHit *h) {
  Reset();
  SetHit(h);
...
//______________________________________________________________________________
const StxKalmanTrackNode& StxKalmanTrackNode::operator=(const StxKalmanTrackNode &n) {
...
#if 0
//______________________________________________________________________________
void StxKalmanTrackNode::SetHitErrors(const StxHit *hit, Int_t kind) {
  if (!hit) hit = _hit;
...
const TRSymMatrix &StxKalmanTrackNode::V(const FitParameters& Prediction)  const {
  assert(_detector);
...
#if 0
//______________________________________________________________________________
TRSymMatrix StxKalmanTrackNode::GlobalHitErrs(const StxHit *hit) const {
...
#endif
//________________________________________________________________________________
Char_t *StxKalmanTrackNode::PrintpTasString(const FitParameters &Fitted, Double_t chi2,const Char_t *opt) {
  return Form(" %s %s chi2 %6.2f",opt,Fitted.PrintpTasString(),chi2);
}
//________________________________________________________________________________
void StxKalmanTrackNode::PrintpT(const FitParameters &Fitted, Double_t chi2, const Char_t *opt) {
  comment += PrintpTasString(Fitted,chi2,opt);
}
//________________________________________________________________________________
void StxKalmanTrackNode::PrintpT(const Char_t *opt) {
  // opt = "E" extapolation
...
//________________________________________________________________________________
void StxKalmanTrackNode::PrintStep() {
  LOG_INFO << comment << "\t" << commentdEdx << endm;
  ResetComment();
}
#if 0
//________________________________________________________________________________
const Double_t *StxKalmanTrackNode::Gxyz() const {
...
#endif
//________________________________________________________________________________
Double_t StxKalmanTrackNode::TimeOfFlight(const FitParameters &Prediction) {
...
//________________________________________________________________________________
void StxKalmanTrackNode::LoadS2D(const StxKalmanTrackNode *node, TRVector *refFitPar) {
  if (! node) node = this;
  LoadS2D(node->Detector(),node->Fitted(), refFitPar);
}
//________________________________________________________________________________
void StxKalmanTrackNode::LoadS2D(const StxDetector *detector,const FitParameters &Fitted, TRVector *refFitPar) {
...
//________________________________________________________________________________
void StxKalmanTrackNode::LoadPrediction(StxKalmanTrackNode *node) {
  if (! node) node = this;
  LoadD2S(node->Detector(),node->Predicted());
}
//________________________________________________________________________________
void StxKalmanTrackNode::LoadD2S(const StxDetector *detector,FitParameters &Prediction) {
  StGeanePropagator *propagator = StGeanePropagator::instance();
  ...
  if (Prediction.Type() == FitParameters::kStx) {
    fdSdD = FP.F();                                 PrPD(LoadD2S,fdSdD);
    //  dStx_new/dPD_old = dStx_new/dPD_new x dPD_new/dPD_old
    fdSdDI = TRMatrix(fdSdD,TRArray::kAxBT,fdDdD);  PrPD(LoadD2S,fdSdDI);
    //   dStx_new/dStx_old = dStx_new/dPD_old * dPD_old /d Stx_old        
...
//________________________________________________________________________________
StTrackGeometry* StxKalmanTrackNode::Helix() {
