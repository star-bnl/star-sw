...
static Int_t __addelement__(Int_t NElem,const TGeoMaterial *mat, Elem_t *ElementList) {
   return StxVMCToolKit::Add2ElementList(NElem, mat, ElementList);
}
...
#else
  {"TSWH","the TPC supporting endcap Wheel","HALL_1/CAVE_1/TPCE_1/TSWH_1-2/*","",""},
  {"TIFC","defines the Inner Field Cage placed in TPC","HALL_1/CAVE_1/TPCE_1/TIFC_1/*","",""},
  {"TOFC","defines outer field cage - fill it with insulating gas already","HALL_1/CAVE_1/TPCE_1/TOFC_1/*","",""},
  {"TPGV","the Gas Volume placed in TPC","HALL_1/CAVE_1/TPCE_1/TPGV_1-2/*","",""},
#endif
...
void StxVMCToolKit::SetDebug(Int_t m) {m_Debug = m;}
//________________________________________________________________________________
Int_t StxVMCToolKit::Debug() {return m_Debug;}
//________________________________________________________________________________
void StxVMCToolKit::PrintShape(TGeoShape *shape) {
...
//________________________________________________________________________________
Double_t StxVMCToolKit::GetShapeVolume(TGeoShape *shape) {
  TGeoBBox *box = 0, *Box = 0; 
  TGeoTrd1 *trd1 = 0;
  TGeoTrd2 *trd2 = 0;
  TGeoTube *tube = 0;
...
//________________________________________________________________________________ 
Int_t StxVMCToolKit::Add2ElementList(Int_t NElem,const TGeoMaterial *mat, Elem_t *ElementList) {
...
//________________________________________________________________________________ 
Int_t StxVMCToolKit::Merge2ElementList(Int_t NElem, Elem_t *ElementList, 
...
//________________________________________________________________________________
Int_t StxVMCToolKit::NormolizeElementList(Int_t NElem, Elem_t *ElementList){
  assert(NElem>=0 && NElem<NoElemMax);
  Double_t W = 0;
...
//________________________________________________________________________________ 
Double_t StxVMCToolKit::GetWeight(TGeoNode *nodeT, const TString &pathT, 
...
//________________________________________________________________________________ 
Double_t StxVMCToolKit::GetVolumeWeight(TGeoVolume *volT, Int_t *NElem, Elem_t *ElementList) 
{
...
#else
  assert(ElementList && "StxVMCToolKit::GetWeight: Create the the array first!");
#endif
  Int_t im1 = *NElem;
...
//________________________________________________________________________________
void StxVMCToolKit::MakeAverageVolume(TGeoVolume *volT, TGeoShape *&newshape, TGeoMedium *&newmed, Double_t *xyzM) {
  if (! volT) return;
...
//________________________________________________________________________________
TGeoManager  * StxVMCToolKit::GetVMC() {
  TGeoManager *gGeo = 0;
  /*! Load Geometry
   */
  gGeo = gGeoManager;
  if (gGeo) return gGeo;
  LOG_INFO << "StxVMCToolKit::GetVMC() Get VMC geometry" <<endm;
  if (StMaker::GetChain()) {
    StMaker::GetChain()->GetDataBase("VmcGeometry");
    if (! gGeoManager) {
      LOG_WARN << "StxVMCToolKit::GetVMC() Can't get VMC geometry, try AgiGeometry" <<endm;
      StMaker::GetChain()->GetDataBase("AgiGeometry");
    }
  }
  if (! gGeoManager) 
    LOG_ERROR << "StxVMCToolKit::GetVMC() Can't get VMC geometry" <<endm;
  gGeo = gGeoManager;
  return gGeo;
}
//________________________________________________________________________________
void StxVMCToolKit::TestVMC4Reconstruction(){
...
//________________________________________________________________________________
TGeoPhysicalNode *StxVMCToolKit::Alignment(const TGeoNode *nodeT, const Char_t *pathT, 
  ...
  TGeoShape *shapeT = volT->GetShape();
  if (Debug()) {
    LOG_INFO << "StxVMCToolKit::Alignment node\t" << nodeT->GetName() 
...
//________________________________________________________________________________
TGeoPhysicalNode *StxVMCToolKit::LoopOverNodes(const TGeoNode *nodeT, const Char_t *pathT, 
...
//________________________________________________________________________________
void StxVMCToolKit::MakeVolume(TGeoPhysicalNode *nodeP) {
  if (Debug()) {
    LOG_INFO << "StxVMCToolKit::MakeVolume -I TGeoPhysicalNode\t" << nodeP->GetName() << endm;
...
//________________________________________________________________________________
Double_t StxVMCToolKit::GetPotI(const TGeoMaterial *mat) {
  Double_t PotI = 0;
  if (mat) {
    Double_t s1 = 0, s2 = 0;
    if (mat->InheritsFrom("TGeoMixture")) {
...
//________________________________________________________________________________
void StxVMCToolKit::GetVMC4Reconstruction(const Char_t *pathT, const Char_t *nameT){
  /*! Loop over nodes. 
...
void TestVMCTK() {
  ...
  Double_t weight = 1.e-3*StxVMCToolKit::GetWeight(0,path, 0, 0);  // It leads to the memory leak
  cout << "StxVMCToolKit::TestVMCTK() -I- total weight for " 
       << path.Data() << "\t" << weight << "[kg]" << endl;
  StxVMCToolKit::TestVMC4Reconstruction();
}
#endif
//________________________________________________________________________________
Double_t StxVMCToolKit::Nice(Double_t phi) {
