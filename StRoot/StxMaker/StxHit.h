/*! \class StxHit
  StxHit is a simple class that encapsulates a three dimensional position
  ...
  a plane (once the location of the plane is known) StxHit stores one more
  ...
  <p>
  StxHit stores information that represents a full error matrix.  For efficiency
  ...
  <p>
  StxHit also stores a pointer to an StHit that it corresponds to.
  Additionally, StxHit stores a pointer to the StDetector object from which its
  measurement arose.
...
class StxHit {
 public: 
  ...
  //!Return a const pointer to the StxDetector object from which the hit
  //!arose.
  ...
  void makeDca();
  //!Return a const pointer to the StHit object corresponding to this StxHit
  //!instance
  ...
  //! StMcHit associated with this StxHit.
  ...
  void set(const StxDetector * detector, Float_t y, Float_t z);
  //!Set the local position and error in one function call
  void set(const StxDetector * detector,
  ...
  void setGlobal(const StxDetector * detector,
		 const StMeasuredPoint * stHit,
		 Float_t x, Float_t y, Float_t z);
  ...
  //!Set the pointer to the StxDetector from which the hit arose.
  void SetDetector(const StxDetector*det) {_detector=det;};
  ...
  friend ostream& operator<<(ostream& os, const StxHit& h);
...
struct StxHitRadiusLessThan {
  bool operator() (const StxHit*h1, const StxHit*h2) const  {
...
//________________________________________________________________________________
struct StxdHitLessThan {
  bool operator() (const StxHit*lhs, const StxHit*rhs) const    {
    return (lhs->y() < rhs->y()) ? true : false;
  }
};
//________________________________________________________________________________
struct StxzHitLessThan {
  bool operator() (const StxHit*lhs, const StxHit*rhs) const  {
...
//________________________________________________________________________________
struct StxHitIsUsed {
  bool operator() (const StxHit*hit) const  {return (hit->timesUsed()==0); }
};
#endif
