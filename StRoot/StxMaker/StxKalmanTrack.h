/** 
 * \file  StxKalmanTrack.h
 * \brief Definition of Kalman Track
 * 
 * Subclass of StxKalmanTrack defining a Kalman track to be used by the Kalman Track Finder.
 *
 ...
 */
#ifndef StxKalmanTrack_H
#define StxKalmanTrack_H 1
...
/*! 
  ...
  A concrete subclass of StxKalmanTrack defining a Kalman track to be 
  used by the Kalman Track Finder.
  
  The track reconstruction is driven by an instance of class 
  StxKalmanTrackFinder while the Kalman state of the track at any 
  given location is held by instances of the StxKalmanTrackNode 
  ...
  instead call the "getObject" method of the StxKalmantrackFactory 
  class to get instances of this class. The StxKalmanTrackFactory holds 
  ...
  is invoked whenever instances of StxKalmanTrackNode are needed. The class 
  ...
  \see StxKalmanTrackNode
  \see StxKalmanTrackFinder
  \author Claude A Pruneau (Wayne State University)
*/
/** 
    \enum Direction
...
class StxKalmanTrack {
 public:
  ...
  enum StxKalmanTrackProperty {kCharge=0,
			       kMass,
			       kChi2,
			       kDca2,
			       kDca3,
  ...
  /*! 
    ...
    ctor directly but should instead invoke to the "getInstance" method of the Factory<StxKalmanTrack> class 
    to get instances of this class. The StxKalmanTrackFactory holds (and owns, i.e. has responsibility for
  ...
  */
  StxKalmanTrack() : mId (0), firstNode(0), lastNode(0), mSeedHitCount(0), mFlag(0), m(-1.) {  /* nops */ }
  virtual ~StxKalmanTrack()    {  }
  ...
  void                ExtendTrackToVertices(const std::vector<StxHit*> &vertices);
  StxKalmanTrackNode *ExtendToVertex(StxHit* vertex);
  void                ExtendTrackToVertex(StxHit* vertex);
 private:
  class QAFind {
  public: 
  ...
  virtual void    Find(Int_t direction,StxKalmanTrackNode *leadNode,QAFind &qa);
  void NodeQA(StxKalmanTrackNode *node, Int_t position,Int_t active,QAFind &qa);
 ...
 public:
  ...
  Double_t  Rapidity() const {
    Double_t p[3]; StxKalmanTrackNode *  inner = InnerMostHitNode(); inner->Momentum(p,0);
  ...
  */
  ...
  Double_t calculateTrackSegmentLength(const StxKalmanTrackNode &p1, const StxKalmanTrackNode &p2) const;
  Int_t calculatePointCount() const;
  Int_t calculateMaxPointCount() const;
  
  StxKTNBidirectionalIterator  begin()  const {assert(firstNode); return StxKTNBidirectionalIterator::begin(firstNode);}
  const StxKTNBidirectionalIterator& end()    const {return StxKTNBidirectionalIterator::end();}
  StxKTNBidirectionalIterator  rbegin() const {assert(lastNode); return StxKTNBidirectionalIterator::rbegin(lastNode);}
  const StxKTNBidirectionalIterator& rend()   const {return StxKTNBidirectionalIterator::rend();}
  void  RemoveLastNode();  
  ...
  /*! Accessor method returns the inner/outer most node associated with the track. 
    ...
    Same for NNodes(qua) */
  StxKalmanTrackNode * InnOutMostNode(Int_t inot,Int_t qua)  const;
  /*! Accessor method returns the outer most node associated with the track. */
  StxKalmanTrackNode * OuterMostNode(Int_t qua=0)  const {return InnOutMostNode(1,qua);}
  /*! Accessor method returns the inner most node associated with the track.*/
  StxKalmanTrackNode * InnerMostNode(Int_t qua=0)   const {return InnOutMostNode(0,qua);}
  
  /*! Accessor method returns the outer most hit node associated with the track.*/
  StxKalmanTrackNode * OuterMostHitNode(Int_t qua=0)  const;
  /*! Accessor method returns the inner most hit node associated with the track.*/
  StxKalmanTrackNode * InnerMostHitNode(Int_t qua=0)   const;
  Int_t                  NNodes(Int_t qua=0) const;
  /*! Accessor method returns the first node associated with the track.*/
  StxKalmanTrackNode * FirstNode()  const { return firstNode; };
  /*! Accessor method returns the last node associated with the track.*/
  // Assumes the track has been pruned.
  StxKalmanTrackNode * LastNode()   const { return  lastNode; };
  
  void  SetLastNode (StxKalmanTrackNode *n) { lastNode  = n; };
  void  SetFirstNode(StxKalmanTrackNode *n) { firstNode = n; };   
  void  SetFirstLastNode(StxKalmanTrackNode *n);   
  
  
  /*! Add a kalman track node to this track as a child to the last node of the track,  Return the added node */
  virtual void Add(StxKalmanTrackNode * node,Int_t direction);
  
  /*! Convenience method to Initialize a track based on seed information */
  Int_t Initialize(const vector<StxHit*> &);
  ...
  virtual vector<StxHit*> Hits();
  virtual vector<const StMeasuredPoint*> StHits() const;
  virtual vector<StxKalmanTrackNode*> Nodes(Int_t detectorGroupId) const;
  ...
  Double_t  Dca2(StxKalmanTrack *t) const {return 0;}   // distance of closest approach to given track - 2D calc
  Double_t  Dca3(StxKalmanTrack *t) const {return 0;}   // distance of closest approach to given track - 3D calc
  void ReserveHits();
  void SetFlag(long v) { mFlag = v; }
  long Flag() const { return mFlag; }
  
  StxKalmanTrackNode * ExtrapolateToBeam();
  StxKalmanTrackNode * ExtrapolateToRadius(Double_t radius);
  ...
  StxKalmanTrack &operator=(const StxKalmanTrack &tk);
  void SetComb(int comb=7)		{mComb = comb;}
  int  Comb() const			{return mComb;}
  
  
 protected:
  friend ostream& operator<<(ostream& os, const StxKalmanTrack& track);
  ...
  static Factory<StxKalmanTrackNode> * trackNodeFactory;
  
  Int_t             mId;
  Int_t             mIdDb;
  StxKalmanTrackNode * firstNode;
  StxKalmanTrackNode * lastNode;
