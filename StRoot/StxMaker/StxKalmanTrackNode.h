#ifndef StxKalmanTrackNode_H
#define StxKalmanTrackNode_H 1
...
enum StxDirection {kOutsideIn=0, kInsideOut};

class StxKalmanTrackNode : public StxTreeNode {
 public:
  StxKalmanTrackNode(){Reset();}
  virtual ~StxKalmanTrackNode(){mId=-1;};
  const StxKalmanTrackNode& operator=(const StxKalmanTrackNode &node);  
  
  //! Resets the node to a "null" un-used state
  void Reset();
  //! Initialize this node with the given hit information
  void Initialize(StxHit*h);
  //! Sets the Kalman state of this node equal to that of the given node. 
  void SetState(const StxKalmanTrackNode * node);
...
#endif
  StxHit * Hit() const 		        {return _hit;}
  void  SetHit(StxHit* hit)		{_hit   =hit;}
  const StxDetector *Detector() const   {return _detector;}
  void  SetDetector(const StxDetector *detector) {_detector = detector;}
  ...
  void LoadS2D(const StxKalmanTrackNode *node = 0, TRVector *refFitPar = 0);
  void LoadS2D(const StxDetector *detector,const FitParameters &FP, TRVector *refFitPar = 0);
  void LoadPrediction(StxKalmanTrackNode *node = 0);
  void LoadD2S(const StxDetector *detector, FitParameters &Prediction);
  void UpdatePrediction(const StxKalmanTrackNode *node, TRVector *refFitPar);
  ...
  Int_t  Propagate(StxKalmanTrackNode *p, StxDetector *tDet, Int_t dir, TRVector *refFitPar = 0);
  Int_t  Propagate(StxKalmanTrackNode *p=0, Int_t dir=kOutsideIn, TRVector *refFitPar = 0);
  
  //! Propagates a track encapsulated by the given node "p" to the given vertex
  Bool_t Propagate(const StxKalmanTrackNode *p, StxHit * vertex, Int_t dir, TRVector *refFitPar = 0);
  
  Bool_t PropagateToBeam(const StxKalmanTrackNode *p, Int_t dir, TRVector *refFitPar = 0);
  Bool_t PropagateToRadius(StxKalmanTrackNode *pNode, Double_t radius,Int_t dir, TRVector *refFitPar = 0);
  ...
  Double_t EvaluateChi2(const StxHit *hit); 
  Int_t UpdateNode(); 
  ...
  void SetHitErrors(const StxHit *hit=0, Int_t kind = 0);
  TRSymMatrix GlobalHitErrs(const StxHit *hit) const;
#endif
  friend ostream& operator<<(ostream& os, const StxKalmanTrackNode& n);
 ...
 protected:   
  ...
  TRMatrix    fdDdS; // Stx_old => PD_old  =  dPD_old /d Stx_old
  TRMatrix    fdSdD; // PD_new  => Stx_new =  dStx_new/dPD_new
  TRMatrix    fdSdDI;// PD_old  => Stx_new =  dStx_new/dPD_old
  TRMatrix    fdDdD; // PD_old  => PD_new  =  dPD_new/dPD_old (transported due to fortran convention)
  TRMatrix    fdSdS; // Stx_old => Stx_new =  dStx_new/dStx_old
