...
void StxHitContino::Reset()
{
   memset(this,0,sizeof(StxHitContino));
   mChi2[0]=1e61;
}
//______________________________________________________________________________
void StxHitContino::Add(StxHit *hit,Double_t chi2,Double_t detr)
{
   Int_t i=0;
   for (;i<kMaxSize;i++) {
     if (!mHits[i]) 		break;
...
//______________________________________________________________________________
void StxHitContino::Print(const char* tit) const
{
  if (!tit || !*tit) tit ="print()";
  Int_t n=NHits();	
  LOG_DEBUG << Form(" ****	StxHitContino::%s nHits=%d",tit,n)<< endm;
  for (Int_t i=0;i<n;i++) {
    LOG_DEBUG << Form("%3d - hit=%p chi2 = %g",i,(void*)mHits[i],mChi2[i]);}
    LOG_DEBUG << endm;
}
//______________________________________________________________________________
Int_t StxHitContino::NHits() const { 
  Int_t n=0; 
