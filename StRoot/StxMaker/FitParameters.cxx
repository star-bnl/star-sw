...
ostream& operator<<(ostream& s,const FitParameters &target) {
  ...
  case FitParameters::kStx:  s << "\tStx";  break;
...
void FitParameters::Setup(const TRVector *P, const TRSymMatrix *C) { 
  ...
  if (! _H[_type]) {
    ...
    case kStx:  _H[_type] = new TRMatrix(2,6, 
					 //x y   z Phi 1/pT  tanl
					 0., 1., 0., 0., 0., 0., // y
					 0., 0., 1., 0., 0., 0.);// z
      break;
...
TRSymMatrix &FitParameters::PxyzGCovMatrix() const {		
  ...
  case kStx:  hold = PxyzGCovMatrixStx(); break;	
  default:;				
  }					
  return *&hold;	
}
...
TRSymMatrix &FitParameters::XyzGCovMatrix() const {		
  ...
  case kStx:  hold = XyzGCovMatrixStx(); break;	
  default:;
  }						
  return *&hold;	
}
...
Int_t FitParameters::Charge() const {
  ...
  case kStx:  return (Int_t)  -TMath::Sign(1., P()[4]); // ptin = -q/pT
  default:                   assert(0);
  }
  return 0;
}
...
FitParameters   &FitParameters::operator=(const    FitParameters &source) {
  if (this != &source) { 
	...
	case kStx:  *this = SD2Stx(source); break;
	default:       assert(0);
	}
	break;
      case kSC:
	...
	case kStx: *this = SC2Stx(source); break;
	default:       assert(0);
	}
	break;
      case kDca:
	...
	case kStx: *this = Dca2Stx(source); break;
	default:       assert(0);
	}
	break;
      case kStx:
	switch (_type) {
	case kSD:  *this =Stx2SD (source); break;   
	case kSC:  *this =Stx2SC (source); break;    
	case kDca: *this =Stx2Dca(source); break;    
	default:       assert(0);
	}
	break;
      default:         assert(0);
...
//________________________________________________________________________________
FitParameters  &FitParameters::Stx2SC(const FitParameters &source) {
...
//________________________________________________________________________________
FitParameters &FitParameters::Stx2SD(const FitParameters &source) {// LoadS2D
  if (this != &source) {
    // source = {_y, _z, _eta, _ptin, _tanl}  eta == Psi;
    _rot = (TGeoHMatrix *) source.Rot();
    assert(_rot);
    ParamStx_t *pars = (ParamStx_t *) source.P().GetArray();
    ...
    TRVector nG     (3, cosPhi*cosL, sinPhi*cosL, sinL); PrPC(Stx2SD,nG);
    ...
    TRVector nL(3);
    _rot->MasterToLocalVect(nG.GetArray(),nL.GetArray()); PrPC(Stx2SD,nL);
    //                              v' = tY      w' = tZ            Y        Z
    TRVector PD = TRVector(5, q/p, nL[1]/nL[0], nL[2]/nL[0], xyzL[1], xyzL[2]); PrPC(Stx2SD,PD);
    TRMatrix   dtYtZdnL(2,3,//nx ny  nz 
			-PD[1], 1., 0.,  // tY 
			-PD[2], 0., 1.); // tZ
    dtYtZdnL /= nL[0];
    PrPC(Stx2SD,dtYtZdnL);
    TRMatrix   dnGdPhitanl(3,2,//    Phi              tanL
			   -sinPhi*cosL, -sinL*cosL2*cosPhi,     // nx
			   cosPhi*cosL , -sinL*cosL2*sinPhi,     // ny
			   0.          ,             cosL3);     // nz
    PrPC(Stx2SD,dnGdPhitanl);
    ...
    PrPC(Stx2SD,F());
    TRSymMatrix RS6(6, source.C().GetArray()); PrPC(Stx2SD,RS6);
    TRSymMatrix RS(5, 
		   RS6(1,1),
		   RS6(1,2), RS6(2,2),
		   RS6(1,3), RS6(2,3), RS6(3,3),
		   RS6(1,4), RS6(2,4), RS6(3,4), RS6(4,4),
		   RS6(1,5), RS6(2,5), RS6(3,5), RS6(4,5), RS6(5,5)); PrPC(Stx2SD,RS);
    TRSymMatrix RD(F(), TRArray::kAxSxAT,RS); PrPC(Stx2SD,RD);
...
//______________________  Stx ___________________________________________________
//________________________________________________________________________________
TRVector FitParameters::PxyzGStx() const {
  //           pars =>  (x_fixed,y,z,eta==Phi,ptin=-q/pT,tanl)
  ParamStx_t *pars = (ParamStx_t *) P().GetArray();
  Double_t pT = (TMath::Abs(pars->ptin)<1e-6) ? 1e6: 1./TMath::Abs(pars->ptin);
  Double_t pxyz[3] = {pT*TMath::Cos(pars->eta), pT*TMath::Sin(pars->eta),  pT*pars->tanl};
  return TRVector(3,pxyz);
}
//________________________________________________________________________________
TRSymMatrix FitParameters::PxyzGCovMatrixStx() const {
  //           pars =>  (x_fixed,y,z,eta==Phi,ptin=-q/pT,tanl)
  ParamStx_t *pars = (ParamStx_t *) P().GetArray();
  Double_t pT = (TMath::Abs(pars->ptin)<1e-6) ? 1e6: 1./TMath::Abs(pars->ptin);
  Double_t dPTdPi = -pT*pT; if (pars->ptin < 0 ) dPTdPi=-dPTdPi;
  Double_t dPxyzdStx[18] = {// I have not check 
    //x,y,z  eta                         ptin                    tanl
  ...
  TRMatrix F(3,6,dPxyzdStx);
  return TRSymMatrix(F,TRArray::kAxSxAT,C());
}
//________________________________________________________________________________
TRVector FitParameters::XyzGStx() const {
  //           pars =>  (x_fixed,y,z,eta==Phi,ptin=-q/pT,tanl)
  assert(_rot);
  ParamStx_t *pars = (ParamStx_t *) P().GetArray();
...
//________________________________________________________________________________
TRSymMatrix FitParameters::XyzGCovMatrixStx() const {
  //           pars =>  (x_fixed,y,z,eta==Phi,ptin=-q/pT,tanl)
  assert(_rot);
...
//________________________________________________________________________________
FitParameters &FitParameters::SC2Stx(const FitParameters &source) {
  if (this != &source) {
...
//________________________________________________________________________________
FitParameters &FitParameters::SD2Stx(const FitParameters &source) { // LoadD2S
  if (this != &source) {
    _rot = (TGeoHMatrix *) source.Rot();
    assert(_rot);
    ...
    TVector3 pxyz(source.PxyzG().GetArray()); PrPC(SD2Stx,source.P());
    Double_t p = pxyz.Mag();
    ...
    //                   x,         y,         z, Psi, ptin, TanL
    Double_t pStx[6] = {0., parsSD->y, parsSD->z, Phi, -q/pT, TanL};
    P() = TRVector(6,pStx);                   PrPC(SD2Stx,P());
    const ParamStx_t *pars = (const ParamStx_t *) P().GetArray();
    Double_t cosPhi = TMath::Cos(Phi);
    Double_t sinPhi = TMath::Sin(Phi);
    Double_t cosL = 1./TMath::Sqrt(1. + pars->tanl*pars->tanl);
    Double_t sinL = cosL*pars->tanl;
    ...
    TRMatrix dPhiTanldnG(2,3,//nx     ny  nz
			 ...
			 0.     ,     0., 1./cosL2); // tanl
    dPhiTanldnG /= cosL;
    PrPC(SD2Stx,dPhiTanldnG);
    TRVector nG(3, cosPhi*cosL, sinPhi*cosL, sinL); PrPC(SD2Stx,nG);
    TRVector nL(3);
    ...
    Double_t tY = nL[1]/nx;
    Double_t tZ = nL[2]/nx;
    TRVector tN(3, 1., tY, tZ); PrPC(SD2Stx,tN);
    TRMatrix dnLdtYtZ(3,2,//  tY       tZ
		      -tY      ,      -tZ,  // nL_x
		      (1+tZ*tZ),   -tY*tZ,  // nL_y
		      -tY*tZ   ,(1+tY*tY)); // nL_z
    dnLdtYtZ *= TMath::Power(nx,3);                             PrPC(SD2Stx,dnLdtYtZ);
    TRMatrix dnGdtYtZ(dnGdnL,TRArray::kAxB,dnLdtYtZ);           PrPC(SD2Stx,dnGdtYtZ);
    TRMatrix dPhitanldtYtZ(dPhiTanldnG,TRArray::kAxB,dnGdtYtZ); PrPC(SD2Stx,dPhitanldtYtZ);
    Double_t dPSdPD[25] = {
      //1/p,                           tY,                          tZ, y, z
      0,                                0,                           0, 1, 0, // y
      0,                                0,                           0, 0, 1, // z
    ...
    PrPC(SD2Stx,F());
    PrPC(SD2Stx,source.C());
    TRSymMatrix RS(F(), TRArray::kAxSxAT,source.C()); PrPC(SD2Stx,RS);
    TRSymMatrix RS6(6,
		    0.,
		    0., RS(0,0),
		    0., RS(1,0), RS(1,1),
...
//________________________________________________________________________________
FitParameters  &FitParameters::Dca2Stx(const FitParameters &source) {
  if (this != &source) {
    assert(0);
  }
  return *this;
