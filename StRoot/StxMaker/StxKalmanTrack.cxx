//StxKalmanTrack.cxx
/*
 * $Id: StxKalmanTrack.cxx,v 2.140 2014/05/29 12:51:21 fisyak Exp $
 * $Id: StxKalmanTrack.cxx,v 2.140 2014/05/29 12:51:21 fisyak Exp $
 *
 * /author Claude Pruneau
 *
 * $Log: StxKalmanTrack.cxx,v $
 ...
 * Eliminate StxKalmanTrackFitter
 *
 ...
 * Move out all Stx Chairs into StDetectorDb
 *
 ...
 * 2 cm => StiKalmanTrackFinderParameters::instance()->maxDca3dVertex() in StxKalmanTrack::IsPrimary()
 *
 ...
 * take out Central represantation, remove StxDedxCalculator
 *
 ...
 * (1) Changed the StxPlacement class to hold keys to both the radial and angle placement. Propagated the use
 * of those keys in StxSvt StxTpc StxSsd and all relevant Stx classes.
 * (2) Changed the StxKalmanTrackFinder::find(StxKalmanTrack*) function's algorithm for the navigation of the
 ...
 * (3) Changed the chi2 calculation in StxKalmanTrack::Chi2 and Propagated the effects of this change
 * in both StxKalmanTrackingPlots and StxStEventFiller classes.
 *
 ...
 * -Fixed memory leak in StxDetectorTreeBuilder::HangWhere (100 chars were lost
 *  every time this function was called)
 * -Changed algorithm to count fit points in StxKalmanTrack.  Now it is based
 ...
 *  to be used in StxStEventFiller.
 * -Which was then modified to calculate the encoded fit points based on
 *  a similar algorithm (chi2<chi2Max test).
 * -Cleaned up the includes in StxKalmanTrack.h, left only the ones
 *  needed to keep the code compiling.
 * -Which required a slight modification in the include of StxKalmanTrackFinder
 * -StxKalmanTrackKalmanTrackFitter now also sets a pointer to itself in
 *  static StxKalmanTrack::setFitParameters()
 ...
 * in the gui, or filtered in the StxStEventFiller.
 *
 ...
 * iCommitting changes to turn on multiple regions (StxPlacement::StxRegion -> kMidRapidity, kForwardRapidity, etc).
 * Also added a point to StxToolkit for StxMaker.  This allows for the req. GetDataSet calls in the FTPC code.
 * Not so elegant...
 *
 ...
 * Added groupId member and accessor functions to StxDetector, StxDetectorGroup, StxDetectorBuilder,
 * and modified Nodes of the StxKalmanTrack class to use it. This eliminates explicit
 * references to Tpc and Svt within StxKalmanTrack...
 *
...
#include "StMessMgr.h"
#define PrPP(A,B) {if (Debug() & 32) {LOG_INFO  << "=== StxKalmanTrack::" << (#A) << "\t" << (#B) << " = \t" << ( B ) << endm;}}
ostream& operator<<(ostream&, const StxHit&);

Int_t StxKalmanTrack::mgMaxRefiter = 100;
Int_t StxKalmanTrack::_debug = 0;
...
Double_t StxKalmanTrack::Value(Int_t key) const
{
  Double_t value;
  switch (key)    {
...
<LI>Throws a logic_error exception if the factory
  is not a castable to a factory of StxKalmanTrackNode.</li>
<li>Throws a logic error exception if hits do not have a valid pointer to a detector object.</li>
</OL>
*/

//_____________________________________________________________________________
Int_t StxKalmanTrack::Initialize(const std::vector<StxHit*> &hits)
{
  //cout << "StxKalmanTrack::Initialize() -I- Started"<<endl;
  Reset();
  //StxKalmanTrackNode * node  = 0;
  const StxDetector* detector=0;
  Int_t nhits = hits.size();
  for (Int_t ihit=0;ihit<nhits;ihit++)  {
    StxHit *hit = hits[ihit];
    detector = hit->Detector();
    assert(detector);
    StxKalmanTrackNode * n = StxToolkit::instance()->TrackNodeFactory()->getInstance();
    n->Initialize(hit);
    Add(n,kOutsideIn);
  }
...
//_____________________________________________________________________________
Int_t StxKalmanTrack::Approx(Int_t mode) {
  Int_t nNode,nNodeIn,iNode=0;
  Double_t Xi2=0;
  //		Loop over nodes and collect global xyz
  
  StxKTNIterator source;
  StxKalmanTrackNode *targetNode;
  nNode=0;
  THelixFitter circ;
  THelixTrack  cirl;
  TRVector xyzG(3);
  for (source=rbegin();(targetNode=source());++source) {
    ...
    const StxHit * hit = targetNode->Hit();
    if (!hit) 				continue;
  ...
  Xi2 =circ.Fit();
  if (Debug() & 32) {cout << "StxKalmanTrack::Approx circ\t"; circ.Print();}
  ...
  for (source=rbegin();(targetNode=source());++source) {
    ...
    cirl = circ;
    Double_t xyzL[3];
    const StxDetector *det = targetNode->Detector();
    TGeoHMatrix *rot = 0;
    ...
    targetNode->Fitted() = FitParameters(FitParameters::kStx, rot, P, C);
#else
    // SD
...
//_____________________________________________________________________________
StThreeVector<double> StxKalmanTrack::MomentumAtOrigin() const
{
  Double_t px,py,pz;
  px=py=pz=0;

  StxKalmanTrackNode * inner = InnerMostNode();

  if (inner==0) {
    cout << "StxKalmanTrack::MomentumAtOrigin() - ERROR - No node" << endl;
    assert(0);
  }
  StxKalmanTrackNode targetNode;
...
*/
//_____________________________________________________________________________
Int_t StxKalmanTrack::Charge() const
{
  StxKalmanTrackNode *node = InnerMostNode();
  if (!node) return 0;
  return  node->Charge();
}
...
Double_t  StxKalmanTrack::Chi2() const
{
  Double_t fitHits   = 0;
...
*/
//_____________________________________________________________________________
Int_t StxKalmanTrack::PointCount(Int_t detectorId) const
{
  const StxDetector *detector=0;   
  Int_t nPts = 0;
  StxKTNIterator it;
...
*/
//_____________________________________________________________________________
Int_t StxKalmanTrack::MaxPointCount(Int_t detectorId) const
{
  Int_t nPts = 0;
  StxKTNIterator it;

  for (it=begin();it!=end();it++){
    const StxKalmanTrackNode *node = &(*it);
    if (!node->IsValid()) 					continue;
    const StxDetector *detector = node->Detector();
    if (!detector)						continue;
    StxHit* h = node->Hit();
...
*/
Int_t    StxKalmanTrack::GapCount() const {
  Int_t gaps = 0;
  if (firstNode) {
    StxKTNIterator it;
    Bool_t inGap = kFALSE;
    for (it=begin();it!=end();it++) {
      const StxDetector * detector = (*it).Detector();
...
//_____________________________________________________________________________
Int_t StxKalmanTrack::FitPointCount(Int_t detectorId)    const  
{
...
//_____________________________________________________________________________
void StxKalmanTrack::AllPointCount(Int_t count[1][3],Int_t maxDetId) const
{
...
/*! Calculate and return the track length.
  <h3>Notes</h3> 
   ...
   \throws runtime_error
*/
Double_t StxKalmanTrack::TrackLength() const
{
  Double_t x[2][4],len=0;
  Int_t iready=0;
  StxKalmanTrackNode *node;
  StxKTNIterator it = begin();
  for (;(node=it());it++){
    if (!node->IsValid()) 	continue;
    if (!node->Hit()) 	continue;
    ...
    if (iready) {
      ...
      if (dsin>0.9) {
         LOG_DEBUG <<
           Form("StxKalmanTrack::TrackLength ***ERROR*** dsin %g >.9",dsin)
         << endm;
        dsin = 0.9;
      }
...
//_____________________________________________________________________________
Double_t StxKalmanTrack::NearBeam(StThreeVectorD *pnt,StThreeVectorD *dir) const {
  StxKalmanTrackNode * inNode = lastNode;
...
*/
//_____________________________________________________________________________
StxKalmanTrackNode * StxKalmanTrack::InnOutMostNode(Int_t inot,Int_t qua)  const {
  if (firstNode==0 || lastNode==0) {
    cout << "StxKalmanTrack::InnOutMostNode() -E- firstNode||lastNode==0" << endl;
    assert(0);
  }
  
  StxKalmanTrackNode *node;
  StxKTNIterator it =(inot) ? begin():rbegin();
  for (;(node=it());it++){
    if (!node->IsValid()) 				continue;
    StxHit *hit = node->Hit();
  ...
  cout << "StxKalmanTrack::InnOutMostNode() -E- No requested nodes " << endl;
  return 0;
}
//_____________________________________________________________________________
StxKalmanTrackNode * StxKalmanTrack::OuterMostHitNode(Int_t qua)  const
{
...
StxKalmanTrackNode * StxKalmanTrack::InnerMostHitNode(Int_t qua)   const
{
  return InnOutMostNode(0,qua|1);
}
//_____________________________________________________________________________
Int_t StxKalmanTrack::NNodes(Int_t qua)  const
{
  StxKalmanTrackNode *node;
  StxKTNIterator it = begin();
  Int_t nn=0;
...
*/
//_____________________________________________________________________________
Bool_t  StxKalmanTrack::IsPrimary() const
{
  StxKalmanTrackNode * node = InnerMostHitNode(3);
  if (node->Detector()) 	return 0;
  const StxHit *hit = node->Hit();
...
vector<StxKalmanTrackNode*> StxKalmanTrack::Nodes(Int_t detectorId) const
{
  StxKTNIterator it;
  vector<StxKalmanTrackNode*> nodeVec;
  for (it=begin();it!=end();++it) {
          StxKalmanTrackNode* node = &(*it);
    const StxHit* hit = node->Hit();
    if(!hit) 				continue;
...
//_____________________________________________________________________________
///return hits;
vector<const StMeasuredPoint*> StxKalmanTrack::StHits() const
{
  StxKTNIterator it;
...
*/	
//_____________________________________________________________________________
void StxKalmanTrack::ReserveHits()
{
  StxKTNForwardIterator it(lastNode);
...
//_____________________________________________________________________________
StxKalmanTrackNode *StxKalmanTrack::ExtendToVertex(StxHit* vertex) {
  static Int_t nCall=0; nCall++;
  Double_t chi2;
  Int_t dcaHit = vertex->IsDca();
  StxKalmanTrackNode * sNode=0;
  StxKalmanTrackNode * tNode=0;
  Bool_t trackExtended = kFALSE;

  StxKalmanTrackNode * innerMostHitNode = InnerMostHitNode();
  if (!innerMostHitNode) 		return 0;
		
  StxHit localVertex = *vertex;
  sNode = InnerMostNode();
  ...
  tNode = StxToolkit::instance()->TrackNodeFactory()->getInstance();
  StxHit *myHit;
  ...
  if (tNode->Propagate(sNode, &localVertex,kOutsideIn))    { 
    ...
    if (chi2<StiKalmanTrackFinderParameters::instance()->maxChi2Vertex())   {
      myHit = StxToolkit::instance()->HitFactory()->getInstance();
      *myHit = localVertex;
      tNode->SetHit(myHit);
      tNode->setChi2(chi2);
      tNode->SetDetector(0);
      trackExtended = (tNode->UpdateNode()==0);
      if (Debug()) cout << "ExtendToVertex:: " << StxKalmanTrackNode::Comment() << endl;
      if (trackExtended) return tNode;
      StxToolkit::instance()->TrackNodeFactory()->free(tNode);             
    } else if (d < 4) {
...
//_____________________________________________________________________________
vector<StxHit*> StxKalmanTrack::Hits()
{
  vector<StxHit*> hits;
  StxKalmanTrackNode* leaf = LastNode();
  StxKTNForwardIterator it(leaf);
  StxKTNForwardIterator end = it.end();
  for (;it!=end;++it) 
    {
      const StxKalmanTrackNode& node = *it;
      if (!node.IsValid())		continue;
...
Double_t  StxKalmanTrack::Dca(const StxHit * vertex) const {
  StThreeVectorD vxDD(vertex->Gxyz().GetArray());
  Double_t dca = InnerMostHitNode(2)->Helix()->helix().distance(vxDD);
  return dca;
}
//_____________________________________________________________________________
ostream& operator<<(ostream& os, const StxKalmanTrack& track) {
  os << "Id: " << track.Id()  
     <<" Chi2: "<<track.Chi2()
     <<" q: "<<track.Charge()
     <<" pt: "<<track.Pt()
  ...
  if (track.FirstNode()) {
    os <<"List of nodes";
    StxKTNIterator tNode = track.begin();
    StxKTNIterator eNode = track.end();
    //set initial conditions for tNode, the 'current' node;
    //will also need 'nextNode', ie node which is next 
    while (tNode != eNode) {
      StxKalmanTrackNode *thisNode = &(*tNode);
...
///the beam axis plane.
StxKalmanTrackNode * StxKalmanTrack::ExtrapolateToBeam()
{
  StxKalmanTrackNode * innerMostNode = InnerMostNode();
  ...
  StxToolkit::instance()->TrackNodeFactory()->free(n);
  return 0;
}

//_____________________________________________________________________________
StxKalmanTrackNode * StxKalmanTrack::ExtrapolateToRadius(Double_t radius)
{
  StxKalmanTrackNode * outerMostNode = OuterMostNode();
  //return null if there is no node to Extrapolate from.
...
//_____________________________________________________________________________
void StxKalmanTrack::Add(StxKalmanTrackNode *Node,Int_t direction) {
...
//_____________________________________________________________________________
void StxKalmanTrack::SetFirstLastNode(StxKalmanTrackNode * node)
{
 firstNode = (StxKalmanTrackNode*)node->FirstNode();
  lastNode = (StxKalmanTrackNode*)node->LastNode ();
}
//_____________________________________________________________________________
void StxKalmanTrack::RemoveLastNode()
{
  StxKalmanTrackNode *node = lastNode;
  lastNode = (StxKalmanTrackNode*)node->Disconnect();
  BFactory::Free(node);
}
//_____________________________________________________________________________
void StxKalmanTrack::Unset() 
{
  if (!lastNode) return;
  StxKTNIterator source;
...
//_____________________________________________________________________________
StxKalmanTrack &StxKalmanTrack::operator=(const StxKalmanTrack &tk) {
  mId = tk.mId;
  firstNode=0;
  lastNode=0;
  ...
  for (it=tk.begin();it!=tk.end();it++){
    const StxKalmanTrackNode *node = &(*it);
    if (!node->IsValid()) continue;
    StxKalmanTrackNode *myNode=StxToolkit::instance()->TrackNodeFactory()->getInstance();
    *myNode=*node;
...
//_____________________________________________________________________________
StThreeVector<double> StxKalmanTrack::Point(Int_t firstLast) const
{
  const StxKalmanTrackNode* node = InnOutMostNode(firstLast,3);
...
*/
Int_t StxKalmanTrack::Fit(Int_t fitDirection, TRVector *refFitPar) {
  static Int_t nCall=0; nCall++;
  StxKalmanTrackNode::Break(nCall);
  
  if (Debug() > 2) cout << "StxKalmanTrack::fit() -I- Started:"<<endl;
  StxHit * tarHit;
  StxKalmanTrackNode *targetNode; // parent node
  StxDetector  *targetDet;  // parent detector
  
  StxKTNBidirectionalIterator first;
  StxKTNBidirectionalIterator last;
  StxKTNBidirectionalIterator source;
  Double_t chi2;
  ...
  for (source=first;source!=last;source++) {
    ...
    StxKalmanTrackNode tryNode = *targetNode;
    ...
    if (Debug()) {cout << Form("%5d ",status); StxKalmanTrackNode::PrintStep();}
    *targetNode=tryNode;
    pNode = targetNode;
  } //end for of nodes
  nGoodNodes = NNodes(3);
...
//________________________________________________________________________________
Bool_t StxKalmanTrack::Find(Int_t direction,Double_t rmin) {
  static Int_t nCall=0; nCall++;
  gLevelOfFind = 0;
  StxKalmanTrackNode::Break(nCall);
  Int_t nnBef,nnAft;
  ...
  StxKalmanTrackNode *leadNode = InnOutMostNode(direction,2);
...
//______________________________________________________________________________
void StxKalmanTrack::Find(Int_t direction
			  ,StxKalmanTrackNode *leadNode,QAFind &qa) {
  static Int_t nCall=0; nCall++;
  StxKalmanTrackNode::Break(nCall);
  ...
  while (1) {
    ...
    StxHitContino hitCont;
    vector<StxHit*> & candidateHits = StxToolkit::instance()->HitContainer()->Hits(testNode);//,true);
    if (! candidateHits.size()) continue;
    vector<StxHit*>::iterator hitIter;
    ...
    for (Int_t jHit=0;jHit<nHits; jHit++) {//Loop over Hits
      ...
      StxKalmanTrackNode * node = StxToolkit::instance()->TrackNodeFactory()->getInstance();
      *node = testNode;
      status = 0;
      do {//fake do
	if (!stiHit) break;
	...
	if (Debug() > 0) {cout << Form("%5d ",status); StxKalmanTrackNode::PrintStep();}
      } while(0);
      if (status)  {StxToolkit::instance()->TrackNodeFactory()->free(node); continue;}
      if (! leadNode->Parent()) {
	qaBest = qa;
	continue;
      } else {
...
//________________________________________________________________________________
Int_t StxKalmanTrack::Smooth(Int_t fitDirection) {
  static Int_t nCall=0; nCall++;
  StxKalmanTrackNode::Break(nCall);
  if (Debug() > 2) cout << "StxKalmanTrack::Smooth() -I- Started:"<<endl;
  ...
  Int_t nGoodNodes = NNodes(3);
  if (nGoodNodes<3) 			return 1;
  StxKalmanTrackNode *k1 = 0; // parent node k+1
  Int_t iNode=0; status = 0;
  Double_t maxChi2   = StiKalmanTrackFitterParameters::instance()->getMaxChi2();
  for (source=first;source!=last;source++) {
    StxKalmanTrackNode *k = &(*source);
    k->Smoothed() = k->Fitted();
    if (iNode == 0 && ! k->Hit()) {k->SetInvalid();}
    if (!k->IsValid()) continue;
    iNode++;
    const StxDetector *det = k->Detector();
    if (Debug()) {
   ...
   TRSymMatrix dC(k1->Smoothed().C());
	...
	if (GG.IsValid()) {
	  ...
	  if (Debug()) {
	    ...
	    StxKalmanTrackNode::PrintpT(k->Smoothed(),chi2,"R"); StxKalmanTrackNode::PrintStep();
	  }
	}
	k->SetHit(0);
      } else {
...
//______________________________________________________________________________
void StxKalmanTrack::NodeQA(StxKalmanTrackNode *node, Int_t position
...
//  const StxDetector *detector = hit->Detector();
    qa.sum += node->Chi2();
    qa.hits++; qa.qa=1;
...
//______________________________________________________________________________
Int_t StxKalmanTrack::CompQA(QAFind &qaBest,QAFind &qaTry,Double_t maxChi2)
{
   Int_t ians;
...
Int_t StxKalmanTrack::Fit() {
  ...
  const StxKalmanTrackNode * outerMostNode = OuterMostNode(2);
  if (!outerMostNode) {
    SetFlag(-1);
    return -1;
  }
  ...
  if (outerMostNode->NormalX()<185. ) {
    ...
    if (Debug()) cout << "StxKalmanTrack::Build (track,kInsideOut)" << *((StxKalmanTrack *) this);
    if (Debug() > 1) cout << "chi2/ndf after find   " << Chi2() << endl;
    Smooth(kInsideOut);
    if (Debug() > 1) cout << "chi2/ndf after Smooth " << Chi2() << endl;
  }
  ...
  //================= not ready yet
  //		Add DCA node
  StxHit dcaHit; dcaHit.makeDca();
  StxKalmanTrackNode *extenDca = ExtendToVertex(&dcaHit);
  if (extenDca) {
    Add(extenDca,kOutsideIn);
    if (Debug() >= 1) StxKalmanTrackNode::PrintStep();
  }
  //		End DCA node
#endif
  return 0;
...
//______________________________________________________________________________
void StxKalmanTrack::ExtendTrackToVertices(const std::vector<StxHit*> &vertices) {
...
//______________________________________________________________________________
void StxKalmanTrack::ExtendTrackToVertex(StxHit* vertex) {
  StxKalmanTrackNode *Extended = ExtendToVertex(vertex);
  if (Extended) {
