...
/***************************************************************************
 *
 * $Id: StxStEventFiller.h,v 2.29 2009/08/19 19:56:40 fisyak Exp $
 *
 * Author: Manuel Calderon de la Barca Sanchez, Mar 2002
 ***************************************************************************
 *
 * $Log: StxStEventFiller.h,v $
 ...
 *
 **************************************************************************/
#ifndef StxStEventFiller_HH
#define StxStEventFiller_HH
#include <map>
using std::map;
...
/*! \class StxStEventFiller
    StxStEventFiller is a utilitity class meant to properly convert StxKalmanTrack
...
public:
    StxStEventFiller();
    virtual ~StxStEventFiller();
    void fillEvent(StEvent* e, StxKalmanTrackContainer* t);
    void fillEventPrimaries();
    void fillDetectorInfo(StTrackDetectorInfo* detInfo, StxKalmanTrack* kTrack,bool refCountIncr);
    void fillGeometry(StTrack* track, StxKalmanTrack* kTrack, bool outer);
    //void fillTopologyMap(StTrack* track, StxKalmanTrack* kTrack);
    void fillFitTraits(StTrack* track, StxKalmanTrack* kTrack);
    void fillTrack(StTrack* track, StxKalmanTrack* kTrack,StTrackDetectorInfo* detInfo );
    void fillDca(StTrack* track, StxKalmanTrack* kTrack);
    void fillFlags(StTrack* track);
    Double_t impactParameter(StxKalmanTrack* kTrack, StThreeVectorD &vertexPosition);
    Double_t impactParameter(StTrack* strack, StThreeVectorD &vertexPosition);
private:
 bool accept(StxKalmanTrack* kTrack);
 void FillStHitErr(StHit *hh,const StxKalmanTrackNode *node);
private:
    StEvent* mEvent;
    StxKalmanTrackContainer* mTrackStore;
    Int_t mGloPri;		//0=filing global,1=filing primary
    Int_t mTrackNumber;
    map<StxKalmanTrack*, StTrackNode*> mTrkNodeMap;

    unsigned short mStxEncoded;
    //helix parameters
