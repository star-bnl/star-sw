...
"StDbT,ctf_T,ebyeT,emc_T,ftpcT,gen_T,geomT,globT,l3_T,mwc_T,sim_T,svt_T,tpc_T,trg_T,vpd_T","","","",kFALSE},
  ...
  {"mwc"         ,"mwc","l0Chain","mwc_T,db"               ,"St_mwc_Maker","St_mwc,St_mwc_Maker","",kFALSE}, 
  ...
  {"tpcDB"       ,"tpcDB","tpcChain","tpc_T,dbutil,db"                 ,"StTpcDbMaker","StTpcDb","",kFALSE},
...
Int_t StBFChain::Instantiate() 
{
  ...
  for (i = 1; i< NoChainOptions; i++) {// Instantiate Makers if any
	...
	if (maker == "St_db_Maker"){
	  Char_t *Dbs[] = {"calib","$STAR_ROOT/calib","calib", //calib
			   "db","MySQL:StarDb",0, // StarDb
			   //			   "db","$STAR/params","./params", //params
			   //			   "db","$STAR/params",0, //params
			   0};
	  Char_t *Maker,*mainDB,*userDB;
	  for (Int_t db=0;(Maker=Dbs[db]);db+=3) {
	    TString DbKey(Maker);
	    if (DbKey.CompareTo(Key,TString::kIgnoreCase) != 0) continue;
	    St_db_Maker *mk = (St_db_Maker *) GetMaker(DbKey.Data());
	    if (mk) continue;
	    mainDB = Dbs[db+1];
	    userDB = Dbs[db+2];
	    TString USERDB(userDB);
	    gSystem->ExpandPathName(USERDB);
	    gMessMgr->QAInfo() << "   DataBase Maker == \t" << Maker 
			       << "\t Main DataBase  == \t" << mainDB 
			       << "\t User DataBase  == \t" << USERDB.Data() << endm;
	    mk = new St_db_Maker(Maker,mainDB,userDB);
	    if (!mk) status = kStErr;
	    else {
...
//_____________________________________________________________________
// $Id: StBFChain.cxx,v 1.107 2000/06/26 22:08:04 fisyak Exp $
