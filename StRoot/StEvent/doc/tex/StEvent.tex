...
%
% $Id: StEvent.tex,v 2.0 1999/11/10 21:21:08 ullrich Exp $
%
...
% and added description of StPrimaryVertex::flag().
  {\LARGE $ $Revision: 2.0 $ $}  \\[5mm] % replaced by cvs with current revision
  {\LARGE $ $Date: 1999/11/10 21:21:08 $ $}  % replaced by cvs with current revision
...
\begin{figure}[htb]
    \begin{center}
        \includegraphics[width=0.9\textwidth]{cartoon4.eps}
...
Needless to say that every container comes with two iterators a constant
and a non-constant version.
The name of each iterators is composed of the name of the objects it iterates
over and the suffix \texttt{Iterator} or \texttt{ConstIterator}.\\
Example: For the structural container \texttt{StSPtrVecTrackNode} the iterators
StTrackNodeIterator and StTrackNodeConstIterator are defined.
Iterators do no care about structural or non-structural so there is only one
set of iterators for  \texttt{StSPtrVecTrackNode} and \texttt{StPtrVecTrackNode}
containers.
...
StSPtrVecFtpcHit
StSPtrVecKinkVertex
for (StTrackIterator i = container.begin(); i != container.end(); i++)
...
case. Don't say you haven't been warned.  \vfill
    
\subsection{Getting StEvent: StEventMaker}
\subsection{A Standard Example}
...
listed.  \texttt{Inline} declarations are omitted throughout the
documentation.
\begin{figure}[hb]
    \begin{center}
        \includegraphics[width=0.47\textheight]{cartoon3.eps}
