...
%
% $Id: StEvent.tex,v 1.21 1999/06/07 14:57:21 ullrich Exp $
%
...
%
% $Log: StEvent.tex,v $
% Revision 1.21  1999/06/07 14:57:21  ullrich
% Added changes in StGlobalTrack
%
% Revision 1.21  1999/06/07 14:57:21  ullrich
% Added changes in StGlobalTrack
%
...
% Add info on order in which primary vertices are stored
  ...
  {\LARGE $ $Date: 1999/06/07 14:57:21 $ $}  % replaced by cvs with current revision
...
\item[Public Member\\ Functions]
    ...
    \verb+virtual int  numberOfTpcHits() const;+\\
    Returns the number of hits associated with the track.
    If (!) the hits are stored on the DST, i.e. the
    hit collection is filled, this is the same as
    returned by tpcHits().size(). Otherwise this number is
    taken from a data member which gets defined during the 
    setup of StEvent. 
    
    \verb+virtual int  numberOfSvtHits() const;+\\
    Same as above but for the SVT hits.
...
\item[Description]
    \name{StHit} provides the basic functionality for all derived classes
...
\item[Public Member\\ Functions]
    ...
    \verb+virtual const StThreeVector<float>& positionError() const;+\\
    Hit position error.

    \verb+virtual float charge() const;+\\
    Total hit charge.

    \verb+virtual unsigned char trackReferenceCount() const;+\\
