...
%
% $Id: StEvent.tex,v 2.3 1999/11/15 20:29:44 ullrich Exp $
%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% Log doesn't make much sense yet.
...
% and added description of StPrimaryVertex::flag().
  {\LARGE $ $Revision: 2.3 $ $}  \\[5mm] % replaced by cvs with current revision
  {\LARGE $ $Date: 1999/11/15 20:29:44 $ $}  % replaced by cvs with current revision
...
\item Virtual destructor
    \verb+StDedxPidTraits(StDetectorId, Short_t,+\\
    \verb+                UShort_t, Float_t , Float_t);+\\
    \verb+StDedxPidTraits(const dst_dedx_st&);+\\
keyword.  The state-of-the-art reference is always the class
definition in the header file.
\item[Summary] Central Trigger Barrel (CTB) data.
    \verb+Float_t variance() const;+\\
...
\item[Public Member\\ Functions]
    ...
    \verb+StFtpcPlaneHitCollection* plane(UInt_t);+\\
    \verb+const StFtpcPlaneHitCollection* plane(UInt_t) const;+\\
...
\index{StEventTypes|textbf}
    \verb+StFtpcSectorHitCollection* sector(UInt_t);+\\
    \verb+const StFtpcSectorHitCollection* sector(UInt_t) const;+\\
...
\item[Public Member\\ Functions]
\end{Entry}
    \verb+StTpcSectorHitCollection* sector(UInt_t);+\\
    \verb+const StTpcSectorHitCollection* sector(UInt_t) const;+\\
