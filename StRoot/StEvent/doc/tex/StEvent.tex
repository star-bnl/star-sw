...
%
% $Id: StEvent.tex,v 2.10 1999/12/06 23:01:54 ullrich Exp $
%
...
% Define header and footer style
  {\LARGE $ $Revision: 2.10 $ $}  \\[5mm] % replaced by cvs with current revision
  {\LARGE $ $Date: 1999/12/06 23:01:54 $ $}  % replaced by cvs with current revision
...
\index{TPC hits}
\index{TPC sectors and rows}
\index{StTpcHit}
\index{sectors}
\index{rows}
Each hit returns its position in global
coordinates. In addition hit classes also provide information on
their \emph{local} coordinates through methods which
decode a detector specific data word. For the TPC hits there
are methods to return the sector number (0-23) and row number (0-44).
Please read section \ref{sec:conventionsNumbering} if you are puzzled
about the numbering scheme.  There are two additional member functions
...
The FTPC hit class \texttt{StFtpcHit} is very similar to the TPC version.
However, because of the different detector geometry the hits are stored
according to planes (0--19) and then sectors (0--5). This is depicted in
Fig.~\ref{fig:ftpc}. Other than the TPC hit the FTPC hit has a method to return
the size of the hit in time direction (\texttt{timebinsInHit()}) but no method
to return the number of pixels per hit. See \ref{sec:StFtpcHit} for details. 
...
has up to 7 wafers. Consequently the \texttt{StSvtHit} class provides method to
The collection you obtain (by pointer) via
returns 0--2, \texttt{layer()} returns 0--5, \texttt{ladder()} returns 0--7,
and \texttt{wafer()} returns 0--6. If you are puzzled why we start to count at
0 you better have a look at section \ref{sec:conventionsNumbering}.
...
\index{layer}
\index{wafer}
The SVT consist of 6 layers (or 3 barrels) with up to 8 ladders each. Each ladder
has up to 7 wafers. Consequently the \texttt{StSvtHit} class provides methods to
return the local coordinates exactly in these ``units'': \texttt{barrel()}
returns 1--3, \texttt{layer()} returns 1--6, \texttt{ladder()} returns 1--8,
and \texttt{wafer()} returns 1--7. If you are puzzled why these numbers start at
1 have a look at section \ref{sec:conventionsNumbering}.
Because of the more detailed local coordinates
there is not enough space to store any further details on the hits as it is the case
for the TPC and FTPC.
The SVT hits are stored in a tree organized as shown in Fig.~\ref{fig:svt}. 
The top collection (\texttt{StSvtHitCollection}), the layer collection
(\texttt{StSvtLayerHitCollection}), and the 
(\texttt{StSvtLadderHitCollection}) provide a method to return the number
of hits stored in the referring subcomponent (\texttt{numberOfHits()}).
The hits are stored per wafer.
\texttt{numberOfHits()} which returns just that, the number of all TPC
hits and the number of hits in the corresponding sector.

        \caption{Class diagrams of the SVT hit storage scheme: layer/ladder/wafer.}
...
\item[Related Classes] \texttt{StBrowsableEvent} is derived from
    \texttt{StEvent}.
\item[Public\\ Constructors]
