...
%
% $Id: StEvent.tex,v 1.2 1999/03/09 21:49:25 ullrich Exp $
%
...
% Revision 2.27  2000/04/10 19:59:34  genevb
  ...
  {\LARGE $ $Date: 1999/03/09 21:49:25 $ $}  % replaced by cvs with current revision  
...
\comp{size()}, \comp{begin()}, \comp{end()}.  Every collection in
...
containers are actually implemented as STL containers although this
    ...
    RogueWave's \name{Tools.h++} class library. Without exception the
...
\begin{table}[htb]
    \begin{center}
Making local copies is often time- and memory-intensive.
...
//  Function calculates and returns the number of
//  mips from all CTB counter.
...
\subsection{StDedx \index{StDedx|textbf}} \index{dE/dx}
    \verb+float time() const;+ \\
}%footnotesize    

    energy loss quantities (dE/dx) for a specific detector.   
    Sets the counter id.

    \verb+#include "StDedx.hh"+

    \verb+void setTime(float);+ \\
    StDedx represents the dE/dx information as
    stored in the \comp{dst\_dedx.idl} table. Contains dE/dx and
    number of used hits. The class is used for charged tracks in the
    TPC, FTPC and SVT. The status word hold information on the quality
    and status of the extracted dE/dx.
...
//
    ...
    about the energy loss of self within the various detectors.
...
\subsection{StDstEventSummary \index{StDstEventSummary|textbf}} \index{event summary}
...
\begin{Entry}
\subsection{StEmcHit \index{StEmcHit|textbf}} \index{EMC hit} \index{SMD hit}
...
\subsection{StEmcPreShowerHit \index{StEmcPreShowerHit|textbf}} \index{EMC pre-shower hit}
    Set $\phi$ coordinate.
...
\item[Related Classes]
\subsection{StEmcTowerHit \index{StEmcTowerHit|textbf}} \index{EMC tower hit}
}%footnotesize    
...
\item[Examples] \index{accumulate} 
    double operator()(double s, MyHit h) {return s += h.e();}
...
\subsection{StSmdEtaHit \index{StSmdEtaHit|textbf}} \index{SMD hit}
{\footnotesize
\begin{verbatim}
//
...
\subsection{StSmdPhiHit \index{StSmdPhiHit|textbf}} \index{SMD hit}
}%footnotesize    
