...
%
% $Id: StEvent.tex,v 2.14 2000/01/11 16:31:56 ullrich Exp $
%
...
% Define header and footer style
  {\LARGE $ $Revision: 2.14 $ $}  \\[5mm] % replaced by cvs with current revision
  {\LARGE $ $Date: 2000/01/11 16:31:56 $ $}  % replaced by cvs with current revision
...
enum StDetectorId           {kUnknownId,
                             kTpcId,
at 1.  Examples are TPC sectors and padrows, SVT layers, ladders and
                             kRichId,
                             kFtpcWestId,
                             kFtpcEastId,
                             kTofPatchId,
...
const int iplane = 11;
const int isec   = 3;
\index{SSD hits} \index{StSsdHit} \index{ladder} \index{wafer} The SSD
consist of one layer of 20 ladders with 16 wafers each.  The
     << " hits" << endl;

StSPtrVecFtpcHit& theHits =
event->ftpcHitCollection()->plane(iplane)->sector(isec)->hits();
member functions to return the strip number and the cluster size
of the p and n side, respectively.  The SSD hits are stored in a tree
     << theHits.size() << " hits" << endl;
layer collection (there's only one).  The hits are stored per wafer.
...
\begin{Entry}
\item[Description] 
\item[Related Classes] \texttt{StTpcDedxPidAlgorithm} inherits directly from
...
\item[Public Member\\ Functions]
    ...
    Returns foreign key as defined in the dst_track table.
...
\begin{Entry}
    ...
    \texttt{flag()} $< 0$ bad. Contains which detectors were used in the refit,
    the status of the refit and much more. For a detailed (and actual) description
    see the referring reconstruction web pages.    
