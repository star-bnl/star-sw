...
%
% $Id: StEvent.tex,v 2.9 1999/12/03 18:42:17 ullrich Exp $
%
...
% Define header and footer style
  {\LARGE $ $Revision: 2.9 $ $}  \\[5mm] % replaced by cvs with current revision
  {\LARGE $ $Date: 1999/12/03 18:42:17 $ $}  % replaced by cvs with current revision
...
typing.
of tracks which use the hits, and a method which returns a list of
...
coordinates. Besides that many hit classes also provide information on
their \emph{local} coordinates. For the TPC hits that means that there
...
The primary vertices are ordered according to the number of daughters
        \caption{Class diagrams of the TPC hit storage scheme.}
the vertex with the most daughter tracks. \index{primary vertices, order of}
\begin{figure}[htb]
    \begin{center}
        \includegraphics{vertex.eps}
The organization of the hit storage is such that the hits are ordered
        \label{fig:vertex}
That means the collection you get (by pointer) from
StEvent::tpcHitCollection() holds a collection of sectors
(StTpcSectorHitCollection) which itself holds a collection of padrows
(StTpcPadrowHitCollection). Each padrow finally contains the list of
...
\begin{figure}[htb]
    ...
    \end{center}
    cout << theHits[i]->charge() << endl << endl;
\texttt{StMeasuredPoint}. So all the 'position' related stuff comes
with the measured point: position, position errors, and a covariant
The whole collection (\texttt{StTpcHitCollection}) and each sector
