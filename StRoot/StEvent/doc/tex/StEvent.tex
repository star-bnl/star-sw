...
%
% $Id: StEvent.tex,v 1.7 1999/03/19 17:22:44 ullrich Exp $
%
...
% Revision 2.22  2000/03/02 12:40:40  ullrich
  ...
  {\LARGE $ $Date: 1999/03/19 17:22:44 $ $}  % replaced by cvs with current revision
...
\parindent 0pt
\parskip 6pt
model to access, manipulate and analyze STAR DSTs. It consist of a set
of C++ classes which are connected in a tree like structure. The top
class is \name{StEvent} which also is the single entry point to
the whole tree. All what is needed to access every instance in the
tree and navigate freely through the structure is a pointer to
\clearpage
\name{StEventReaderMaker} which is explained below in detail.
...
basic class diagrams using the Unified Modelling Language (UML).
...
\subsection{Enumeration Types} \index{enumerations}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%    Reference Manual
%
...
\name{Inline} declarations are omitted throughout the documentation.
...
%    Reference: StCtbCounter
    is identified by an ID. The class holds the number of mips and the counter
...
\index{StEmcHit|textbf} \index{EMC hit} \index{SMD hit}
\item[Public\\ Constructors]
...
\item[Persistence]
    \verb+unsigned long triggerMask() const;+\\

\item[Related Classes]
    integers.
    \verb+unsigned long bunchCrossingNumber() const;+\\
\item[Public\\ Constructors]
    ...
    \verb+StDstEventSummary* summary();+\\
    Experiment run number.
    \index{run number}
    ...
    examples below.\index{type cast} \index{StVertexCollection}
    ...
    \verb+void setId(const pair<long, long>&);+\\
    \verb+void setTime(time_t);+\\
...
\item[Description]
    ...
    list of pointers to tracks associated with an individual hit.
...
\item[Summary]
    ...
    \verb+short beamMassNumber(StBeamDirection dir) const;+\\
\item[Persistence] None

\item[Related Classes]
    None
    \index{trigger mask} Trigger mask.
...
\item[Public Member\\ Functions]
    ...
    \index{beam mass number} A of east (\name{dir=east}) or west
...
\index{track}
\index{track model}
    \name{StTrack} provides the basic features and
    functionality to describe a general track. It serves as base class
    for \name{StGlobalTrack} which is on a DST.  \name{StTrack}
    provides information on the start and end vertices which mark the
    beginning and end of a track, the results of the fitting procedure
    used in the track reconstruction and on the track parameters
    themself. The current track model is described by a helix. The
    helix is modelled by the \name{StPhysicalHelix} class which itself
    is derived from \name{StHelix}. Both classes are part of the
    StarClassLibrary (SCL). More details on the model can be found in
    the SCL manual (see sec.~\ref{sec:furtherdoc} on how to get the
    manual).  The helix classes offer a lot of functionality to
    extrapolate tracks, find the intersection of a track with planes
    and cylinders, obtain the distance of closest approach to a
...
\item[Related Classes]
    \verb+StTrackPidTraits& pidTraits();+\\
...
\item[Public\\ Constructors]
    ...
    Constructs an instance of \name{StHit} with all values initialized
    to 0 (zero).
    \verb+void setStartVertex(StVertex*);+\\
\item[Public Member\\ Functions]
...
%
    The class \name{StTrackFitTraits} describes the quality and of a
...
\item[Public\\ Constructors]
    ...
    \verb+void setDcaDaughterToPrimaryVertex(StTrackSign sign, float);+\\
...
\item[Description]
    ...
    \verb+StVecPtrGlobalTrack& daughters();+\\
...
\item[Persistence]
    ...
    \verb+unsigned long qualityBitmask();+\\
    ...
    \verb+void setQualityBitmask(unsigned long); +\\
