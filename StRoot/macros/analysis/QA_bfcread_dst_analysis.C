// $Id: QA_bfcread_dst_analysis.C,v 1.1 2000/01/31 21:28:41 kathy Exp $
// $Log: QA_bfcread_dst_analysis.C,v $
...
// owner:  Kathy Turner
// what it does: 
//    - reads  *.dst.root & *.runco.root files using StIOMaker
//    - runs 
...
//
// Can be used for real or simulated data!
//=======================================================================
//
...
{
//
  cout << "QA_bfcread_dst_analysis.C, num events to process " << 
        nevents  << endl;
  cout << "QA_bfcread_dst_analysis.C, input file name       " << 
        MainFile << endl;
  cout << "QA_bfcread_dst_analysis.C, output psfile name    " << 
        psFile   << endl;
  cout <<"QA_bfcread_dst_analysis.C, Maker directory containing histograms =  " <<
        MakerHistDir   << endl;
  cout << "QA_bfcread_dst_analysis.C, hist page title " << 
        PageTitle  << endl;
  cout << "QA_bfcread_dst_analysis.C, subset list name of which histograms to draw,print = " <<
        PrintList  << endl;
...
// loop over events:
 ...
 EventLoop: if (iev<nevents && !iret) {  // goto loop code
   evnum=iev+1;
   cout <<  " !!! QA_bfcread_dst_analysis.C, processing event !!! " << evnum << endl ;
   chain->Clear();
   iret = chain->Make();
   iev++;                                // goto loop code
  ...
  cout <<  " !!! QA_bfcread_dst_analysis.C, passed chain->Make !!!" << endl ;
...
//can do this anytime after they're booked
  ...
  cout << " !!! QA_bfcread_dst_analysis.C, No. of Hist we have == " << NoHist << endl;
...
//    gStyle->SetOptStat(111111);
  ...
  cout << " QA_bfcread_dst_analysis.C, Number hist to plot with log scale = " << numLog << endl;

  Int_t numPrint = 0;
  numPrint = HU->ExaminePrintList();
  cout << " QA_bfcread_dst_analysis.C, Number hist to print = " << numPrint << endl;

//  Now draw the actual histograms to canvas and to ps file
  HU->DrawHists(MakerHistDir);

  chain->Finish();
  cout <<  "QA_bfcread_dst_analysis.C, passed chain->Finish" << endl ; 
