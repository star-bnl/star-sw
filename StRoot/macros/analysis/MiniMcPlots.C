...
#if 1
  if (GeantId == 8) return  1; // pions
  if (GeantId == 9) return -1;
#else
  if (GeantId == 2) return  1;
  if (GeantId == 3) return -1;
#endif
  return 0;
}
//________________________________________________________________________________
...
void DrawEff() {
  ...
  TH1D * pTGhost    = (TH1D*) gDirectory->Get("pTGhost");
  
  TH1D *effN   = (TH1D*) gDirectory->Get("effN");
  TH1D *effP   = (TH1D*) gDirectory->Get("effP");
  TH1D *effGlN = (TH1D*) gDirectory->Get("effGlN");
  TH1D *effGlP = (TH1D*) gDirectory->Get("effGlP");
  TH1D *effGhost = (TH1D*) gDirectory->Get("effGhost");
  ...
  for (Int_t l = 1; l <= nx; l++) {
    ...
    val = pTGhost->GetBinContent(l);
    //sum = pTGlA->GetBinContent(l);
    sum = pTMcP->GetBinContent(l);
    if (sum < 1.e-7) {val = 0; err = 0;}
    else { val /= sum;     // err = TMath::Sqrt(val*(1.-val)/sum);
    }
    effGhost->SetBinContent(l,100.*val);
    err = 0.01;   
    effGhost->SetBinError(l,100.*err);
  }
  ...
  effGhost->SetMarkerStyle(21);
  effGhost->SetMarkerColor(2);
...
//   const Short_t             *&mMcTracks_mNFtpcHitMc = iter("mMcTracks.mNFtpcHitMc");
  const UShort_t                *&mMcTracks_mGeantId = iter("mMcTracks.mGeantId");
...
//   const Short_t         *&mMatchedPairs_mNFtpcHitMc = iter("mMatchedPairs.mNFtpcHitMc");
  const UShort_t            *&mMatchedPairs_mGeantId = iter("mMatchedPairs.mGeantId");
...
//yf  const Char_t          *&mMatchedPairs_mIsValidPr = iter("mMatchedPairs.mIsValidPr");
  const UShort_t*&         mMatchedPairs_mSeedQA    = iter("mMatchedPairs.mSeedQA");
  ...
  TH1D * pTGhost    = new TH1D("pTGhost","pT for ghost",100,0.,5.); // ghost
  ...
  TH1D *effGhost   = (TH1D *) pTGhost->Clone();   effGhost->SetName("effGhost");    effGhost->Reset();

  TH1I *SeedQA    = new TH1I("SeedQA","N Hits in Seed",201,-0.5,200.5);
  TH1I *SeedQA_Sti    = new TH1I("SeedQA_Sti","N Hits in Sti Seed",201,-0.5,200.5);
  ...
  while (iter.Next()) {
    ...
    std::map<int, int> NMcMap; // number MCtracks with same key, indexed by key.
    for (k = 0; k < mNMatchedPair; k++) {
      if( NMcMap.find(mMatchedPairs_mKey[k]) == NMcMap.end() ) NMcMap[mMatchedPairs_mKey[k]] = 0;// new entry
      NMcMap[mMatchedPairs_mKey[k]]++;
    }
    
    for (k = 0; k < mNMatchedPair; k++) {    
      // cout << "mMatchedPairs " << k << "\tpT = " << mMatchedPairs_mPtMc[k] << "\tpZ = " << mMatchedPairs_mPzMc[k] << endl;

      if ( (mMatchedPairs_mFlag[k]%100 == 11) || (mMatchedPairs_mFlag[k] <= 0) ) continue; // bad tracks
      
      bool isGhost = 0;
      isGhost |= (mMatchedPairs_mKey[k] == 0); // really don't need it. all reco tracks should have some dominant MCtrack
      isGhost |= (mMatchedPairs_mAvgQuality[k] < 90.0);
  
      if (isGhost){
        pTGhost->Fill(mMatchedPairs_mPtGl[k]);
        continue;
      }
      
      if (NMcMap[mMatchedPairs_mKey[k]] == -1) continue; // not first clone
      if (NMcMap[mMatchedPairs_mKey[k]] > 1) NMcMap[mMatchedPairs_mKey[k]] = -1; // accept first clone

      if (mMatchedPairs_mSeedQA[k]>0) {
        SeedQA->Fill(mMatchedPairs_mSeedQA[k]);
        if (mMatchedPairs_mSeedQA[k]<100) SeedQA_Sti->Fill(mMatchedPairs_mSeedQA[k]);
      }
  ...
  can->cd();
  SeedQA->Draw(); // TODO
    TCanvas *can2 = new TCanvas("c1","c1");
  can2->cd();
  SeedQA_Sti->Draw(); // TODO   
