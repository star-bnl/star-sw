...
TF1 *FitRL5(TH1 *hist)
{
 ...
 l5xg_func = 0; func_pi=0; func_pr=0; func_k=0; func_el=0;func_de=0; frac_pr=0.;frac_k=0.;frac_el=-0.;frac_de=0.; frac_pi=0.;
  ...
  static RooRealVar *mg, *sg;
  static RooLandauZ5 *landauZ5 ;
  static RooGaussian *gauss;
  static RooFFTConvPdf *l5xg;
  static TF1 *l5xg_mult;
  ...
  fElektron = new RooRealVar("fElektron","fElektron",0.,0.,1.57) ;
  fDeuteron = new RooRealVar("fDeuteron","fDeuteron",0.,0.,1.57) ;
  total     = new RooRealVar("total"    ,"total"    ,0.,0.,10.);
  width     = new RooRealVar("width"    ,"width"    ,0.07);//0.1,0.01,2.5) ;

  mg = new RooRealVar("mg","mg",0.) ;
  sg = new RooRealVar("sg","sg",0.25,0.01,10.) ;

  landauZ5= new RooLandauZ5("landauZ5","landauZ5",*t,*norm,*mu,*sigma,*fProton,*fKaon,*fElektron,*fDeuteron,*total,*width) ;
...
//------- Convolution of 5 Landau Z functions and Gaussian function ----//
  l5xg = new RooFFTConvPdf("l5xg","landauZ5 (X) gauss",*t,*landauZ5,*gauss) ; 

  //------- Fit to data -----------------------//
  l5xg->fitTo(*data,Save());
  //----- Create TF1 -------------//
  l5xg_func = (TF1*)l5xg->asTF(RooArgList(*t), RooArgList(*norm,*mu,*sg,*fProton,*fKaon,*fElektron,*fDeuteron,*total,*width),*t );
  ...
  l5xg_mult = new TF1("mult",func_lz5xg_mult,-2.,5.,2);
  l5xg_mult->SetParameter(0,mult_ev);
  l5xg_mult->SetParameter(1,5.);
...
//-- Fits for particles: 0.Pion, 1.Proton, 2.Kaon, 3.Elektron, 4.Deuteron --//
  for (Int_t i = 1; i <= 4; i++) {
    Sep_func(l5xg,t,norm,mu,sg,fProton,fKaon,fElektron,fDeuteron,total,width,i);
    TF1 *g0 = new TF1(Peaks[i].Name,func_lz5xg_mult,-2.,5.,2);
    g0->SetParameter(0,mult_ev);
    g0->SetParameter(1,i);
  ...
  frac_pi=1.-frac_pi;
  Sep_func(l5xg,t,norm,mu,sg,fProton,fKaon,fElektron,fDeuteron,total,width,0);
  TF1 *g0 = new TF1(Peaks[0].Name,func_lz5xg_mult,-2.,5.,2);
  g0->SetParameter(0,mult_ev);
  g0->SetParameter(1,0);
  g0->SetLineColor(2);
  g0->Draw("same");
