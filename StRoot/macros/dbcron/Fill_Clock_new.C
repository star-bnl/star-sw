...
int Fill_Clock_new(unsigned int runNumber,unsigned int startRunTime,unsigned int endRunTime){
...
//	    "select unix_timestamp(beginTime),frequency from trgClock where beginTime < from_unixtime(%u) order by 1 desc limit 1",startRunTime+61);
    ...
    db = TSQLServer::Connect("mysql://heston.star.bnl.gov:3606/mq_collector_Conditions_daq","","");
    sprintf(query,
	"SELECT unix_timestamp(beginTime), DAQ_clock as frequency FROM det_clocks WHERE beginTime < FROM_UNIXTIME(%u) ORDER BY beginTime DESC LIMIT 1",startRunTime+61);
...
//	    "select unix_timestamp(beginTime),frequency from trgClock where beginTime between from_unixtime(%u) and from_unixtime(%u)order by 1 limit 1",startRunTime+61,endRunTime);
    ...
    sprintf(query,
	    "SELECT UNIX_TIMESTAMP(beginTime), DAQ_clock AS frequency FROM det_clocks WHERE beginTime BETWEEN FROM_UNIXTIME(%u) AND FROM_UNIXTIME(%u) ORDER BY beginTime ASC LIMIT 1",startRunTime+61,endRunTime);
...
//	  "select unix_timestamp(beginTime),frequency from trgClock where beginTime >= from_unixtime(%u) order by 1 limit 1",endRunTime);
  ...
  sprintf(query,
	  "SELECT UNIX_TIMESTAMP(beginTime), DAQ_clock AS frequency FROM det_clocks WHERE beginTime >= FROM_UNIXTIME(%u) ORDER BY beginTime ASC LIMIT 1",endRunTime);
...
//    if(!dbI->QueryDbFunction(table," where runNumber > 0 AND runNumber < 14999999","max")){
    if(!dbI->QueryDbFunction(table," where runNumber > 0 AND runNumber < 27999999","max")){
      cout<<"Error getting max"<<endl;
      return;
    }

    maxRunNumber=*(unsigned int*)table->getDataValue("runNumber");
	if ( maxRunNumber < 22026001 ) { maxRunNumber = 22026001; } // Run 21, first test run on Tue
    ...
    if (!endRunTime)
      {
        ...
        Fill_Clock_new(runNumber,startRunTime,endRunTime);
      }
    delete row;
    }
...
/* 
    ...
    ifs.open ("runs_ts.txt", std::ifstream::in);
	...
	std::cout << "run: " << runNumber << ", start: " << startRunTime << ", end: " << endRunTime << std::endl;
	Fill_Clock_new(runNumber,startRunTime,endRunTime);
