...
void Fill_tpcPadGainT0B(){
  ...
  query = "SELECT MAX(UNIX_TIMESTAMP(beginTime)), MAX(beginTime) FROM tpcPadGainT0B WHERE flavor = 'ofl'";
  ...
  os.str("");
  os << "SELECT beginTime, run, Gain, T0 FROM tpcPadGainT0 WHERE beginTime > FROM_UNIXTIME(" << unixtime_latest << ")";
