...
#  include "TStyle.h"
  ...
  class StTrack;
#endif  
static int ___draw_3d_init = 0;
static const Color_t colorDefault = Color_t(-1);
static const Style_t styDefault   = Style_t(-1);
static const Size_t  sizDefault   = Size_t (-1);
...
TObject *Draw3DLine(int n,  const float *xyz, Color_t col=colorDefault,Style_t sty=styDefault,Size_t siz=sizDefault)
{
   ...
   if (siz != sizDefault)   plLine->SetLineWidth(siz);
   Draw3DDraw(plLine);
   return plLine;
}
