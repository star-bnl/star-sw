...
void StMinuitVertexFinder::setExternalSeed(const StThreeVectorD& s)
{
  mExternalSeedPresent = kTRUE;
  mExternalSeed = s;
}

StMinuitVertexFinder::StMinuitVertexFinder(VertexFit_t fitMode) : StGenericVertexFinder(SeedFinder_t::MinuitVF, fitMode)
{
...
void StMinuitVertexFinder::InitRun(int run_number, const St_db_Maker* db_maker)
{
  ...
  LOG_INFO << "Set cuts: MinNumberOfFitPointsOnTrack = " << mMinNumberOfFitPointsOnTrack << " DcaZMax = " << mDcaZMax
           << " MinTrack = " << mMinTrack << " RImpactMax = " << mRImpactMax << " ZMin = " << mZMin
           << " ZMax = " << mZMax << endm;
}
//________________________________________________________________________________

void StMinuitVertexFinder::setFlagBase()
{
  if (mUseITTF) {
    mFlagBase = 8000;
  } else {
...
Int_t StMinuitVertexFinder::findSeeds()
{
  ...
  for (Int_t i = 0; i < zIdxMax; i++) zImpactArr[i] = 0;

  Int_t nTrk = mZImpact.size();
  for (Int_t iTrk = 0; iTrk < nTrk; iTrk++) {
    if ((mZImpact[iTrk] > mZMin) && (mZImpact[iTrk] < mZMax)) zImpactArr[int(mZImpact[iTrk] - mZMin)]++;
  }
  ...
  for (Int_t iBin = 0; iBin < zIdxMax - nBinZ; iBin++) {
    ...
    if (nTrkBin > nOldBin)
      ...
      if (slope == 1) {
        if (mNSeed < maxSeed) {
          ...
          if (nTrkZ >= mMinTrack) {
            if (mDebugLevel) {
              LOG_INFO << "Seed " << mNSeed << ", z " << seed_z << " nTrk " << nTrkZ << " meanZ/nTrkZ " << meanZ / nTrkZ
                       << endm;
            }
            seed_z = meanZ / nTrkZ;
    ...
    if (mDebugLevel > 1) {
      LOG_INFO << "iBin " << iBin << " nTrkBin " << nTrkBin << " nOldBin " << nOldBin << ", slope " << slope
               << " mNSeed " << mNSeed << endm;
    }
    nOldBin = nTrkBin;
  }
...
void StMinuitVertexFinder::fillBemcHits(StEvent* event)
{
  ...
  for (Int_t m = 0; m < nMod; m++)
    for (Int_t e = 0; e < nEta; e++)
      for (Int_t s = 0; s < nSub; s++) mBemcHit[m][e][s] = 0;
  ...
  if (event->emcCollection() && event->emcCollection()->detector(kBarrelEmcTowerId)) {
    ...
    for (Int_t iMod = 0; iMod < nMod; iMod++) {
      if (!bemcDet->module(iMod)) continue;
...
Int_t StMinuitVertexFinder::matchTrack2BEMC(const StTrack* track)
{
  ...
  if (d2.first > 99999999 && d2.second > 99999999) return 0;
  ...
  if (d2.first >= 0 && d2.first < d2.second)
    ...
    LOG_WARN << "Unexpected solution for track crossing Cyl:"
             << " track mom = " << track->geometry()->momentum().mag() << ", d2.first=" << d2.first
...
Int_t StMinuitVertexFinder::checkCrossMembrane(const StTrack* track)
{
  ...
  for (StHitIterator hitIter = hits.begin(); hitIter != hits.end(); hitIter++) {
    if ((*hitIter)->position().z() < 0) n_pnt_neg++;
    if ((*hitIter)->position().z() > 0) n_pnt_pos++;
  }
...
void StMinuitVertexFinder::calculateRanks()
{
  ...
  for (Int_t iVertex = 0; iVertex < size(); iVertex++) {
    ...
    Float_t n_cross_expected = fabs(primV->position().z()) * 0.0020 *
                               primV->numTracksUsedInFinder();  // old coeff 0.0016 with dca 3 and 10 points on track

    if (mDebugLevel) {
      LOG_INFO << "vertex z " << primV->position().z() << " dip expected " << avg_dip_expected << " bemc "
               << n_bemc_expected << " cross " << n_cross_expected << endm;
    }

    Float_t rank_avg_dip = 1 - fabs(primV->meanDip() - avg_dip_expected) * sqrt((float)primV->numTracksUsedInFinder()) /
                                   0.67;  // Sigma was 0.8 for old cuts
    ...
    if (n_bemc_expected >= 1) {
      ...
      if (mUseOldBEMCRank) rank_bemc += 0.5;  // distribution is asymmetric; add 0.5
    }
...
int StMinuitVertexFinder::fit(StEvent* event)
{
  ...
  if (mDebugLevel) {
    LOG_INFO << "Found " << n_ctb_match_tot << " ctb matches, " << n_bemc_match_tot << " bemc matches, " << n_cross_tot
  ...
  for (Int_t iSeed = 0; iSeed < mNSeed; iSeed++) {
    ...
    if (mExternalSeedPresent) seed_z = mExternalSeed.z();
    if (mDebugLevel) {
    ...
    do {
      ...
      // Check fit result
      if (mStatusMin) {
        LOG_WARN << "StMinuitVertexFinder::fit: error in Minuit::mnexcm(), check status flag. ( iter=" << iter << ")"
                 << endm;
        done = 0;  // refit
      }
    ...
    if (n_trk_vtx < mMinTrack) continue;
    ...
    if (mVertexFitMode == VertexFit_t::Beamline1D) {
      ...
      XVertex.setX(beamX(val));
      cov[0] = 0.1;  // non-zero error values needed for Sti
      XVertex.setY(beamY(val));
      cov[2] = 0.1;
    } else {
    ...
    for (UInt_t i = 0; i < mHelixFlags.size(); i++) {
      if (!(mHelixFlags[i] & kFlagDcaz)) continue;
      n_trk_vtx++;
      if (mHelixFlags[i] & kFlagCTBMatch) n_ctb_match++;
      if (mHelixFlags[i] & kFlagBEMCMatch) n_bemc_match++;
      if (mHelixFlags[i] & kFlagCrossMembrane) n_cross++;
    ...
    if (mDebugLevel) {
      LOG_INFO << "check n_trk_vtx " << n_trk_vtx << ", found " << n_ctb_match << " ctb matches, " << n_bemc_match
               << " bemc matches, " << n_cross << " tracks crossing central membrane\n"
...
double StMinuitVertexFinder::CalcChi2DCAs(const StThreeVectorD& vtx)
{
  ...
  for (UInt_t i = 0; i < mDCAs.size(); i++) {
    if (!(mHelixFlags[i] & kFlagDcaz) || (requireCTB && !(mHelixFlags[i] & kFlagCTBMatch))) continue;
...
{
  return (track && track->flag() >= 0 && track->fitTraits().numberOfFitPoints() >= mMinNumberOfFitPointsOnTrack &&
          !track->topologyMap().trackFtpc() && finite(track->length()) &&  // LSB another temporary check
          track->geometry()->helix().valid());
}

/// Use mMinuit print level
void StMinuitVertexFinder::setPrintLevel(Int_t level) { mMinuit->SetPrintLevel(level); }
...
Int_t StMinuitVertexFinder::NCtbMatches() { return nCTBHits; }
