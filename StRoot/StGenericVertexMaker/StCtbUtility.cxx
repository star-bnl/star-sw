...
{
  mCtbEtaSeg = 0.5;
  mCtbPhiSeg = M_PI / 30;  // NEVER chang this two , JB
...
//==========================================================
//==========================================================
void StCtbUtility::collectCTBhitsMC(St_DataSet *gds)
{  // M-C CTB
...
void StCtbUtility::printCtb()
{
  ...
  for (ih = 0; ih < mCtbHits.size(); ih++) {  // loop over CTB hits
    LOG_INFO << Form("ih=%d eta=%.3f phi/deg=%.1f adc=%.1f\n", ih, mCtbHits[ih].eta, mCtbHits[ih].phi / 3.1416 * 180,
                     mCtbHits[ih].adc)
             << endm;
  }
}
