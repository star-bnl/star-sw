...
{  
  ...
  {  
			...
			for(UInt_t j=1;j<121;j++)
      {
         ...
         if(module)
         {
           ...
           for(Int_t k=0;k<(Int_t)rawHit.size();k++)
           {
             ...
             if(energy>mTh[i]) mHist2D[1][i]->Fill(eta,phi,energy);
           }
         }
      }
    ...
    if(mDraw)
    {
      TString CN = "Display_"; CN+=detname[i];
      if(!gROOT->FindObject(CN.Data())) mCanvas[i] = 0;
      if(!mCanvas[i]) mCanvas[i] = new TCanvas(CN.Data(),CN.Data(),400,600);
      mCanvas[i]->cd();
