void doBemcSpread(char *f_7,char *f_5,char *f_9){
  ...
  TF1 *fit=new TF1("fit","[0]",2.,15.);
  TF1 *fit2=new TF1("fit2","[0]",2.,15.);
  ...
  leg->AddEntry(gMB_plus,"minimum bias, #sigma(C)/C=7%","p");
  leg->AddEntry(gHT1_plus,"hightower 1","p");
  leg->AddEntry(gHT2_plus,"hightower 2","p");
  leg->AddEntry(gMB_min,"#sigma(C)/C=11%","p");
  leg->Draw();

  c->SaveAs("bemc_spread_dau.eps");
  c->SaveAs("bemc_spread_dau.root");
