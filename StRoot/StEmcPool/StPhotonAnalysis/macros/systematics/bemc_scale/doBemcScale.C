void doBemcScale(char *f_100,char *f_104,char *f_96){
  ...
  TF1 *fit=new TF1("fit","[0]*x+[1]",1.,15.);
  fit->SetParameters(1.,1.);
  TF1 *fit2=new TF1("fit2","[0]*x+[1]",1.,15.);
  fit2->SetParameters(1.,1.);
  ...
  leg->AddEntry(gMB_plus,"minimum bias, E scale +4%","p");
  leg->AddEntry(gHT1_plus,"hightower 1","p");
  leg->AddEntry(gHT2_plus,"hightower 2%","p");
  leg->AddEntry(gMB_min,"E scale -4%","p");
  leg->Draw();

  c->SaveAs("bemc_escale_pp.eps");
  c->SaveAs("bemc_escale_pp.root");
