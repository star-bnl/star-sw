void doBsmdSpreadRatio(char *f_100,char *f_110,char *f_90){
  ...
  leg->AddEntry(gHT2_plus,"BSMD gain spread 30%","p");
  leg->AddEntry(gHT2_min,"BSMD gain spread 10%","p");
  leg->Draw();

  c->SaveAs("bsmd_spread_ratio_pp.eps");
  c->SaveAs("bsmd_spread_ratio_pp.root");
