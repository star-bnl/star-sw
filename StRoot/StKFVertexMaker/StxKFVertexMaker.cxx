// $Id: StxKFVertexMaker.cxx,v 2.7 2015/12/20 01:06:39 fisyak Exp $
#include "RVersion.h"
...
ClassImp(StxKFVertexMaker);
#ifdef StTrackMassFit_hh
//#define __V0__
//#define __UseMakeV0__
#endif
StKFVerticesCollection *StxKFVertexMaker::fgcVertices = 0;
#define PrP(A,B)                    {LOG_INFO << "StxKFVertexMaker::" << (#A) << "\t" << (#B) << " = \t" << (B) << endm;}
#define PrPP(A,B)  if (Debug() > 1) {LOG_INFO << "StxKFVertexMaker::" << (#A) << "\t" << (#B) << " = \t" << (B) << endm;}
#define PrPP2(A,B) if (Debug() > 2) {LOG_INFO << "StxKFVertexMaker::" << (#A) << "\t" << (#B) << " = \t" << (B) << endm;}
#define PrParticle2(A) if (Debug() > 2) {cout << "StxKFVertexMaker::" << (#A)  << endl; PrintParticles();}
static map<Int_t,StTrackNode*> TrackNodeMap;
Double_t StxKFVertexMaker::fgProbCut = 1e-5;
...
StxKFVertexMaker::StxKFVertexMaker(const Char_t *name) : StMaker(name),
...
//________________________________________________________________________________
KFParticle *StxKFVertexMaker::AddBeamTrack() {
...
//________________________________________________________________________________
KFParticle *StxKFVertexMaker::AddTrackAt(const StDcaGeometry *dca, Int_t kg) {
  if (! dca) return 0;
...
//________________________________________________________________________________
KFParticle *StxKFVertexMaker::AddTrackAt(const StGlobalTrack *gTrack) {
  if (! gTrack) return 0;
...
//________________________________________________________________________________
Double_t StxKFVertexMaker::AnnelingFcn(Double_t TInv) {
  if (! fgcVertices) return 0;
  Double_t Temperature = 1./TInv;
  StAnneling::SetTemperature(Temperature);
  Double_t Chi2 =  fgcVertices->Fit();
  LOG_INFO << "StxKFVertexMaker::AnnelingFcn\tTemperature = " << Temperature << " Chi2 = " << Chi2 
	   << " with " << fgcVertices->NoVertices() << " vertices" << endm;
  return Chi2;
}
//________________________________________________________________________________
void StxKFVertexMaker::CalculateRank(StPrimaryVertex *primV) {    
...
//________________________________________________________________________________
void StxKFVertexMaker::ClearParentIDs() {
...
//________________________________________________________________________________
void StxKFVertexMaker::Clear(Option_t *option) {
  for (Int_t pass = 0; pass < fNPasses; pass++) {
...
Int_t StxKFVertexMaker::Init(){
  mBeamLine = IAttr("beamLine");
  return StMaker::Init();
}
//________________________________________________________________________________
void StxKFVertexMaker::Fit() {
...
//________________________________________________________________________________
StPrimaryTrack *StxKFVertexMaker::FitTrack2Vertex(StKFVertex *V, StKFTrack*   track) {
...
//________________________________________________________________________________
Int_t StxKFVertexMaker::Make() {
  pEvent = dynamic_cast<StEvent*> (GetInputDS("StEvent"));
  if (! pEvent) {
    LOG_WARN << "StxKFVertexMaker::fit: no StEvent " << endm;
...
//_____________________________________________________________________________
Int_t StxKFVertexMaker::MakeParticles() {
...
//________________________________________________________________________________
Bool_t StxKFVertexMaker::FillVertex(const KFParticle *KVx, StVertex *primV) {
...
//________________________________________________________________________________
Bool_t StxKFVertexMaker::MakeV0(StPrimaryVertex *Vtx) {
  Bool_t ok = kTRUE;
  return ok;
}  
//________________________________________________________________________________
void StxKFVertexMaker::PrintParticles() {
...
//________________________________________________________________________________
Bool_t StxKFVertexMaker::ParticleFinder() {
...
//________________________________________________________________________________
void StxKFVertexMaker::PrimaryVertices() {
...
//________________________________________________________________________________
StKFVerticesCollection *StxKFVertexMaker::PrimaryVertexSeeds(Int_t *Parents) {
  Int_t LastGlobal = fParticles->GetLast();
  Int_t nAccepted = 0;
...
//________________________________________________________________________________
void StxKFVertexMaker::ReFitToVertex() {
...
//________________________________________________________________________________
void StxKFVertexMaker::ResetDaughterIds(KFParticle *particle, vector<KFParticle> &particles) {
...
//________________________________________________________________________________
void StxKFVertexMaker::SecondaryVertices() {
...
// Rename StKFVertexFitter => StxKFVertexMaker
//
