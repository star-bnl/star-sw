...
void plotMCDataComp_4GeV() {
  ...
  TH1F *pos_data_repack = new TH1F("pos_data_repack","pos_data_repack",24,3.,99.);
  TH1F *neg_data_repack = new TH1F("neg_data_repack","neg_data_repack",24,3.,99.);

  // repack the data and MC into 4 GeV wide bins
  for (int i=1; i<=24; i++) {
    pos_data_repack->SetBinContent(i,pos_data->GetBinContent(2*i)+
                                     pos_data->GetBinContent(2*i+1));
    neg_data_repack->SetBinContent(i,neg_data->GetBinContent(2*i)+
                                     neg_data->GetBinContent(2*i+1));
  }
  
  TH1F *pos_mc_repack = new TH1F("pos_mc_repack","pos_mc_repack",24,3.,99.);
  TH1F *neg_mc_repack = new TH1F("neg_mc_repack","neg_mc_repack",24,3.,99.);
  for (int i=1; i<=24; i++) {
    cout << "Summing bins " << pos_mc->GetBinCenter(4*i) << ","
         << pos_mc->GetBinCenter(4*i+1) << ","   
         << pos_mc->GetBinCenter(4*i+2) << ","
         << pos_mc->GetBinCenter(4*i+3) << " into bin " << pos_mc_repack->GetBinCenter(i) << endl;
    pos_mc_repack->SetBinContent(i,pos_mc->GetBinContent(4*i)+
                                   pos_mc->GetBinContent(4*i+1)+
                                   pos_mc->GetBinContent(4*i+2)+
                                   pos_mc->GetBinContent(4*i+3));
    neg_mc_repack->SetBinContent(i,neg_mc->GetBinContent(4*i)+
                                   neg_mc->GetBinContent(4*i+1)+
                                   neg_mc->GetBinContent(4*i+2)+
                                   neg_mc->GetBinContent(4*i+3));
  } 
  ...
  float pos_high[24], pos_low[24];
  float neg_high[24], neg_low[24];
  float bin_cent[24], pos_bin_val[24], neg_bin_val[24];
  float ex[24];
  float err;
  ...
  for (int i=1; i<=24; i++) {
    ex[i-1] = 2.0; 
    err = 0.;
    err += pos_raw_stat_err2->GetBinContent(2*i)+
           pos_raw_stat_err2->GetBinContent(2*i+1);
    err += pos_QCD_stat_err2->GetBinContent(2*i)+
           pos_QCD_stat_err2->GetBinContent(2*i+1);
    err += pos_eemc_stat_err2->GetBinContent(2*i)+
           pos_eemc_stat_err2->GetBinContent(2*i+1);
    err += pos_tau_stat_err2->GetBinContent(2*i)+
           pos_tau_stat_err2->GetBinContent(2*i+1);
    pos_data_repack->SetBinError(i,sqrt(err));
    err = 0.;
    err += neg_raw_stat_err2->GetBinContent(2*i)+
           neg_raw_stat_err2->GetBinContent(2*i+1);
    err += neg_QCD_stat_err2->GetBinContent(2*i)+
           neg_QCD_stat_err2->GetBinContent(2*i+1);
    err += neg_eemc_stat_err2->GetBinContent(2*i)+
           neg_eemc_stat_err2->GetBinContent(2*i+1);
    err += neg_tau_stat_err2->GetBinContent(2*i)+
           neg_tau_stat_err2->GetBinContent(2*i+1);
    ...
    // put in their arrays
    pos_high[i-1] = pos_QCD_syst_high_err->GetBinContent(2*i)+
                    pos_QCD_syst_high_err->GetBinContent(2*i+1);
    neg_high[i-1] = neg_QCD_syst_high_err->GetBinContent(2*i)+
                    neg_QCD_syst_high_err->GetBinContent(2*i+1);
    pos_low[i-1]  = pos_QCD_syst_low_err->GetBinContent(2*i)+
                    pos_QCD_syst_low_err->GetBinContent(2*i+1);
    neg_low[i-1]  = neg_QCD_syst_low_err->GetBinContent(2*i)+
                    neg_QCD_syst_low_err->GetBinContent(2*i+1);
  }
  ...
  TH1F *pos_data_en_up_repack = new TH1F("pos_data_en_up_repack","pos_data_en_up_repack",24,3.,99.);
  TH1F *pos_data_en_down_repack = new TH1F("pos_data_en_down_repack","pos_data_en_down_repack",24,3.,99.);
  TH1F *neg_data_en_up_repack = new TH1F("neg_data_en_up_repack","neg_data_en_up_repack",24,3.,99.);
  TH1F *neg_data_en_down_repack = new TH1F("neg_data_en_down_repack","neg_data_en_down_repack",24,3.,99.);

  for (int i=1; i<=24; i++) {
    pos_data_en_up_repack->SetBinContent(i,pos_data_en_up->GetBinContent(2*i)+
                                           pos_data_en_up->GetBinContent(2*i+1));
    pos_data_en_down_repack->SetBinContent(i,pos_data_en_down->GetBinContent(2*i)+
                                             pos_data_en_down->GetBinContent(2*i+1));
    neg_data_en_up_repack->SetBinContent(i,neg_data_en_up->GetBinContent(2*i)+
                                           neg_data_en_up->GetBinContent(2*i+1));
    neg_data_en_down_repack->SetBinContent(i,neg_data_en_down->GetBinContent(2*i)+
                                             neg_data_en_down->GetBinContent(2*i+1));
  ...
  float pos_data_ensys_low[24];
  float pos_data_ensys_high[24];
  float neg_data_ensys_low[24];
  float neg_data_ensys_high[24];
  for (int i=1; i<24; i++) {
...
" " << neg_data_ensys_low[i-1] << " " << neg_data_ensys_high[i-1] << endl;
  ...
  TGraphAsymmErrors *pos_syst_err_box = new TGraphAsymmErrors(24,bin_cent,pos_bin_val,ex,ex,pos_high,pos_low);
  TGraphAsymmErrors *neg_syst_err_box = new TGraphAsymmErrors(24,bin_cent,neg_bin_val,ex,ex,neg_high,neg_low);
  ...
  float pos_ratio_syst_high[24], pos_ratio_syst_low[24]; 
  float neg_ratio_syst_high[24], neg_ratio_syst_low[24]; 
  float pos_ratio_bin_val[24], neg_ratio_bin_val[24];
  ...
  TGraphAsymmErrors *pos_ratio_syst_err_box = new TGraphAsymmErrors(24,bin_cent,pos_ratio_bin_val,ex,ex,pos_ratio_syst_high,pos_ratio_syst_low);
  TGraphAsymmErrors *neg_ratio_syst_err_box = new TGraphAsymmErrors(24,bin_cent,neg_ratio_bin_val,ex,ex,neg_ratio_syst_high,neg_ratio_syst_low);
  ...
  gPad->SetGridx(0);
  gPad->SetGridy(0);
  pos_mc_repack->GetYaxis()->SetRangeUser(-10.,180.);
  ...
  can1->Print("data_mc_ET_comp_output4_W+_4GeV.eps");
  ...
  neg_mc_repack->GetYaxis()->SetRangeUser(-10.,100.);
  ...
  lat1->DrawLatex(0.14,0.69,"Preliminary");
  lat1->DrawLatex(0.14,0.64,"#sqrt{s}=500 GeV");
  can2->Print("data_mc_ET_comp_output4_W-_4GeV.eps");
