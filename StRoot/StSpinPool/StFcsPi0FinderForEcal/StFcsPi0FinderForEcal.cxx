...
//------------------------
StFcsPi0FinderForEcal::StFcsPi0FinderForEcal(const Char_t* name):
	StMaker(name){}
...
Int_t StFcsPi0FinderForEcal::Make()
	{
		...
		if(!event) {LOG_ERROR << "StFcsPi0FinderForEcal::Make did not find StEvent"<<endm; return kStErr;}
		mFcsColl = event->fcsCollection();
		if(!mFcsColl) {LOG_ERROR << "StFcsPi0FinderForEcal::Make did not find StEvent->StFcsCollection"<<endm; return kStErr;}
			...
			for (int i=0;i<nh;i++)
                        {
                                ...
                                if (det==0) {h1list_NEtower[hit_id]->Fill(hit_energy);} else if (det==1) {h1list_SEtower[hit_id]->Fill(hit_energy);} //fill in energy spectrum for tower
