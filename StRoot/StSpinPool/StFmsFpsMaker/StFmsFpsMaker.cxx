...
// \author Akio Ogawa
//
//  $Id: StFmsFpsMaker.cxx,v 1.3 2015/09/18 18:50:06 akio Exp $
...
StFmsFpsMaker::StFmsFpsMaker(const Char_t* name):StMaker(name),mQA(false),mFilename((char *)"fmsfps.root"),mPrint(0),mReadMuDST(0){}
...
void StFmsFpsMaker::fmsFpsAlignment(){
	...
	if(e>=EThr){
	    cut1[1]=1;
	    if(pid2==1)      {cut1[2]=1;}
	    else if(pid2==2) {cut1[3]=1;}
	    else if(pid2==3) {cut1[4]=1;}
	    else             {cut1[5]=1;}
	}
	...
	float e0=pair->point(0)->energy();
	float e1=pair->point(1)->energy();
	int pid=pair->fpsPid();
	cut2[0]=1; //all
	if(e0>=EThr && e1>=EThr && pair->energy()>ETotThr){
	    cut2[1]=1;    
	    if(pid==11)        {cut2[2]=1;}
	    else if(pid==22)   {cut2[3]=1;}
	    else if(pid==33)   {cut2[4]=1;}
	    else               {cut2[5]=1;}
	}
