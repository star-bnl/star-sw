...
#ifndef STFWDANA_STFWDANAEPDMATCHQA_HH
#define STFWDANA_STFWDANAEPDMATCHQA_HH
...
class StFwdAnaEpdMatchQa : public StFwdAnaVirtual {
public:
  ...
  Int_t DrawEpdTileHitDistWithFcs(StFwdAnaData* anadata, TCanvas* canvas, const char* savename);
  ...
  void PaintFoundChargeId(TCanvas* canv, const char* savename = "test_foundchargeid.png") const;
  Int_t DrawEpdProjection(StFwdAnaData* anadata, TCanvas* canvas, const char* savename);       ///< For drawing event level QA of FCS clusters and points to EPD hits and tiles
...
protected:
  ...
  //The matching algorithm should find matches for all clusters and points, if it ever doesn't, turn on these histograms to debug
  TH1* mH2F_ClusBadProjVcut_yVx = 0;    ///< y vs. x position of clusters with an invalid projection to EPD and passed vertex cut
  TH1* mH2F_PointBadProjVcut_yVx = 0;   ///< y vs. x position of points with an invalid projection to EPD and passed vertex cut
  ...
  TH1* mH1F_ClusEpdFoundRegion = 0;               ///< Histogram of found regions for clusters
  TH1* mH1F_PointEpdFoundRegion = 0;              ///< Histogram of found regions for points
  TH1* mH1F_ClusChargeId = 0;                     ///< Histogram of number of clusters identified as charged or neutral
  TH1* mH1F_PointChargeId = 0;                    ///< Histogram of number of points identified as charged or neutral
