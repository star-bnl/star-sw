...
Int_t StFwdAnaSpin::DoMake(StFwdAnaData* anadata)
{
  ...
  const StTriggerData* TrigData = anadata->trigData();  //If no trigger data then mBx7Id will not be set correctly
  //Spin information
  if( SpinDbMkr==0 || TrigData==0 ){
    ...
    if( rndm<=0.25 ){ EvtData->mSpin = 5; }                    //Bits 0101 is B+ and Y+
    else if( 0.25<rndm && rndm<=0.5 ){ EvtData->mSpin = 6; }   //Bits 0110 is B+ and Y-
    else if( 0.5<rndm && rndm<=0.75 ){ EvtData->mSpin = 9; }   //Bits 1001 is B- and Y+
    else{ EvtData->mSpin = 10; }                               //Bits 1010 is B- and Y-
  }
  else{
    EvtData->mSpin = SpinDbMkr->spin4usingBX7( EvtData->mBx7Id ); //This is also source polarization
  }
  return kStOk;
}
