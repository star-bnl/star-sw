...
#define PrPP(A,B)
#endif
static const char rcsid[] = "$Id: StTpcRSMaker.cxx,v 1.52 2011/10/14 23:23:56 fisyak Exp $";
...
Int_t StTpcRSMaker::InitRun(Int_t runnumberOf) {
  ...
  for (Int_t sec = 1; sec <= 24; sec++) {
    for (Int_t row = 1; row <= 45; row++) {
      if (St_tpcAnodeHVC::instance()->livePadrow(sec,row)) {
	if (row <= NoOfInnerRows) {
	  nAliveInner++;
	  innerSectorAnodeVoltage += St_tpcAnodeHVC::instance()->voltagePadrow(sec,row);
	} else {
	  nAliveOuter++;
	  outerSectorAnodeVoltage += St_tpcAnodeHVC::instance()->voltagePadrow(sec,row);
	}
      }
    }
  }
...
#undef PrPP
//________________________________________________________________________________
// $Id: StTpcRSMaker.cxx,v 1.52 2011/10/14 23:23:56 fisyak Exp $
// $Log: StTpcRSMaker.cxx,v $
// Revision 1.52  2011/10/14 23:23:56  fisyak
// Step back for SL09g_2Embedding_v2
//
