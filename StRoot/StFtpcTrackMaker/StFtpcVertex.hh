...
public:
  ...
  void SetId(Int_t f)      { mId = f;        }
    ...
    void CheckXerr()  { if (TMath::IsNaN(mError.x())) mError.SetX(0.); }
    void CheckYerr()  { if (TMath::IsNaN(mError.y())) mError.SetY(0.); }
    void CheckZerr()  { if (TMath::IsNaN(mError.z())) mError.SetZ(0.); }
    void CheckErr()   { CheckXerr(); CheckYerr(); CheckZerr();  }
  Bool_t CoordIsNan() { return (Bool_t)(TMath::IsNaN(GetX()) || TMath::IsNaN(GetY()) || TMath::IsNaN(GetZ())); }
   Int_t CheckVertex();
