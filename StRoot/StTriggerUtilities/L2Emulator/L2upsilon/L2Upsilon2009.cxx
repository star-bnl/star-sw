...
//=================================================
//=================================================
L2Upsilon2009::L2Upsilon2009(const char* name, L2EmcDb* db, L2EmcGeom *geoX, char* outDir, int resOff)  :  L2VirtualAlgo2009( name,  db, outDir, true, false, resOff) { 
  /* called one per days
  ...
  */
  ...
  mGeom=geoX;
  if (!mGeom)
    criticalError("L2Upsilon2009 is broken -- can't find geom.");

  setMaxHist(32); // set upper range, I uses only 2^N -it is easier to remember
  createHisto();

  //------- self-consistency checks, should never fail
  if (!(sizeof(L2UpsilonResult2009)== L2UpsilonResult2009::mySizeChar))
    {
      criticalError("L2Upsilon2009 has failed consistency check 'sizeof(L2UpsilonResult2009)== L2UpsilonResult2009::mySizeChar'");
    }
...
/* ========================================
  ======================================== */
int 
L2Upsilon2009::initRunUser( int runNo, int *rc_ints, float *rc_floats) {
...
/* ========================================
  ======================================== */
void 
L2Upsilon2009::computeUser(int token){
  clearEvent(token);

  EventSeen++;
  if(EventSeen%fHowManyEventPerUpdateDynamicMask==0) update_DynamicMask();

  L2UpsilonEvent2009 *btowEve=mBtow+token;
...
/* ========================================
  ======================================== */
bool 
L2Upsilon2009::decisionUser(int token, int *myL2Result){
...
//  (*myL2Result)=&(btowEve->resultBlob);
  memcpy(myL2Result,&(btowEve->resultBlob),sizeof(L2UpsilonResult2009));
  if (par_prescale > 1) {
    prescale++;
...
/* ========================================
  ======================================== */
void
L2Upsilon2009::finishRunUser() {  /* called once at the end of the run */
...
//=======================================
void 
L2Upsilon2009::clearEvent(int token){
  ...
  memset(&(mBtow[token].resultBlob),0, sizeof(L2UpsilonResult2009));
  wrkNumberOfL2=wrkNumberOfL0=0;
}

void
L2Upsilon2009::update_DynamicMask(){
	int count=0;
