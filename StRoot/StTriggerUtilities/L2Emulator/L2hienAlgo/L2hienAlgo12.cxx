...
#include "L2hienAlgo12.h"
#include "L2hienResult2012.h"
...
L2hienAlgo12::L2hienAlgo12(const char* name, const char *uid, L2EmcDb2012* db, L2EmcGeom2012 *geoX, char* outDir, int resOff, L2VirtualAlgo2012::EmcSwitch  beSwitch)  :  L2VirtualAlgo2012( name, uid,  db, outDir, (beSwitch==kIsBtow),(beSwitch==kIsEtow), resOff ) { 
  ...
  */
  ...
  if (beSwitch==kIsBtow)
    {
      if (sizeof(L2hienResult2012)!= L2hienResult2012::mySizeChar) 
	criticalError("L2hienAlgo12 (Barrel) has failed consistency check. sizeof(L2hienResult2012)!= L2hienResult2012::mySizeChar");
    }
  mSwitch=beSwitch;
...
/* ========================================
  ======================================== */
int 
L2hienAlgo12::initRunUser( int runNo, int *rc_ints, float *rc_floats) {
  ...
  if(par_maxList>L2hienList12::mxListSize ) par_maxList=L2hienList12::mxListSize; 
...
/* ========================================
  ======================================== */
void 
L2hienAlgo12::computeUser(int token){
  ...
  if (mSwitch==kIsBtow) 
    {
      resultMaxTowers=L2hienResult2012::maxTowers; //only grab the barrel towers.
  ...
  for(i=0;i< hitSize;i++,hit++) {
    ...
    if ((mSwitch==kIsBtow) && (i<L2hienResult2012::maxTowers))
...
/* ========================================
  ======================================== */
bool 
L2hienAlgo12::decisionUser(int token, int *myL2Result){
  ...
  if(hiTwEve->isFresh>L2hienList12::kDataFresh) mhN->fill(6); // stale data
  ...
  if(mSwitch==kIsBtow) //only if we're the barrel instance of this, otherwise we'd clobber.
    {
      memcpy(myL2Result,hienResult,sizeof(L2hienResult2012));
...
/* ========================================
  ======================================== */
void
L2hienAlgo12::finishRunUser() {  /* called once at the end of the run */
...
/* ========================================
  ======================================== */
void 
L2hienAlgo12::print2(int token){ // full , local ADC array
  if (!(token>0 && token<L2eventStream2012::mxToken))
    {
      printf("L2hienAlgo12::print2 called with token out of bounds!\n");
      return;
    }
