/*********************************************************************
 * $Id: StRichSelectWire.cxx,v 2.0 2000/08/09 16:17:04 gans Exp $
 *
 * Description:
 *
...
double StRichSelectWire::whichWire(const StRichMiniHit* hit) const 
{
	...
	/* Old Non Cast Method
	wireNumber =
	    nearestInteger(( mFirstWirePositionInYBottom - hit->position().y())/mWirePitch)
	    + mNumberOfWires/2.;
	*/
	wireNumber = static_cast<int>(
	    static_cast<double>(nearestInteger(( mFirstWirePositionInYBottom - hit->position().y())/mWirePitch))
	    + mNumberOfWires/2.);
