...
Int_t StRHICfFunction::checkRunTypeForRHICf2017(Int_t runNum)
{
    static const Int_t starNoTL[17] = {18175022, 18175023, 18175024, 18175025, 18175026, 
                                       18175027, 18175029, 18175030, 18176011, 18176012, 
                                       18176014, 18176016, 18176017, 18176018, 18176019, 
                                       18176020, 18176021};

    static const Int_t starNoTS[29] = {18176033, 18176034, 18176035, 18176040, 18176042, 
    ...
    static const Int_t starNoTOP[19] = {18177011, 18177012, 18177014, 18177015, 18177016, 
...
Int_t StRHICfFunction::checkGSOBarSize(Int_t tower)
{
...
Float_t StRHICfFunction::rescaleEnergyFactor(Int_t tower, Int_t layer)
{
    if(mRunType == 999){
        std::cout << "StRHICfFunction::rescaleFactorByTower() - Not set the RunType !!!" << std::endl;
        return kRHICfFatal;
    }

    Float_t rescaleFactor = 0.;

    // energy rescale factor by tower each runtpye.
    static const Float_t factorByTowerTL[2] = {1.043, 1.042};
    static const Float_t factorByTowerTS[2] = {1.042, 1.061};
    static const Float_t factorByTowerTOP[2] = {1.055, 1.067};

    // energy rescale factor by layer
	static const Float_t factorByLayer[2][16] = {{0.9687, 1.006, 0.9749, 1.06, 1.01,
...
Bool_t StRHICfFunction::checkGSOBarEnergy(Float_t val)
{
...
Bool_t StRHICfFunction::checkGSOBarTable(Float_t val)
{
...
Bool_t StRHICfFunction::checkPlateEnergy(Float_t val)
{
...
Bool_t StRHICfFunction::checkRecoValue(TString opt, Float_t val)
{
    opt.ToLower();
    if(opt=="resulthitpos"){
        if(val > -900.){mValueForRecoNum[0] += 1;}
        if(mValueForRecoNum[0] == kRHICfNtower*kRHICfNxy){
            return kRHICfOk;
        }
        else{return kRHICfFatal;}
    }
        ...
        if(mValueForRecoNum[1] == kRHICfNtower){
            return kRHICfOk;
        }
        else{return kRHICfFatal;}
    }
        ...
        if(mValueForRecoNum[2] == kRHICfNtower*2*kRHICfNxy*2){
            return kRHICfOk;
        }
        else{return kRHICfFatal;}
    }
        ...
        if(mValueForRecoNum[3] == kRHICfNtower*2*kRHICfNxy*2){
            return kRHICfOk;
        }
        else{return kRHICfFatal;}
    }
        ...
        if(mValueForRecoNum[4] == kRHICfNtower*kRHICfNxy){
            return kRHICfOk;
        }
        else{return kRHICfFatal;}
    }
...
Bool_t StRHICfFunction::checkLeakageTable(TString opt, Bool_t val)
{
    opt.ToLower();
    if(opt=="photonin"){
        if(val == true){mLeakageNum[0] += 1;}
        if(mLeakageNum[0] == kRHICfNtower*kRHICfNplate){
            return kRHICfOk;
        }
        else{return kRHICfFatal;}
    }
        ...
        if(mLeakageNum[1] == kRHICfNtower*kRHICfNplate){
            return kRHICfOk;
        }
        else{return kRHICfFatal;}
    }
