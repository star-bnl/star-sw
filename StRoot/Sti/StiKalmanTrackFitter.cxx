...
// 1st count number of accepted already good nodes
  ...
  for (source=first;source!=last;source++) {
    ...
    if (targetNode->getChi2()>1e10)	continue;
    ++nGoodNodes;
  }
  ...
  for (source=first;source!=last;source++) {
    do { //do block
      ...
      targetHit = targetNode->getHit();
      if (!pNode && ((!targetHit)||targetNode->getChi2()>1e10)) continue;
