...
StGmtSimpleClusterAlgo::StGmtSimpleClusterAlgo()
{
  //nothing else to do....
};
...
Int_t StGmtSimpleClusterAlgo::Init()
{
  ...
  /*// Allocation
    ...
    pp[i] = new double [m];
  */
  LOG_INFO << "StGmtSimpleClusterAlgo::Init()" << endm;
...
//   Char_t layer,prvLayer,noLayer='z';
  ...
  //return mEv>5;//mCalcOk[module] ; //for status
  return kStOk;
}
...
void StGmtSimpleClusterAlgo::applyPedestals( StGmtStripCollection& strips, UInt_t module )
{
  ...
  for( Int_t mapIdx=0; mapIdx < loopMax; mapIdx++)
    {    
      ...
      //LOG_INFO << "mPec : " <<mPedestals[mapIdx][module] << "  dev: " << mPedestalStdDev[mapIdx][module]<< endm; 
...
//   LOG_INFO << "StGmtSimpleClusterAlgo::getMaximumElement module: " << module << 
//   " isY " << isY << 
//   " *maxADC " << *maxADC <<
//   " *element " << *element << 
//   " *tb " << *tb << endm;
...
void StGmtSimpleClusterAlgo::findClusters( StGmtStripCollection& strips, UInt_t module, StGmtHitCollection& clusters )
{
  ...
  for( Int_t idx=0; idx < kGmtNumStripsPerModule; idx++)
  {
    ...
    {
      //      LOG_INFO << "\t idx " << idx << " stripPtr->getMaxPedSubtractedAdc() " << stripPtr->getMaxPedSubtractedAdc() << 
      //	" limit " << kGmtHitCut << " * " <<  stripPtr->getPedStdDev() << endm;
