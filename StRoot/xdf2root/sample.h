...
typedef struct type_a {DS_OCTET b; DS_SHORT s; DS_LONG l;}TYPE_A_T;

#define TYPE_B_S "struct type_b {octet b; unsigned short us; unsigned long ul;}"
typedef struct type_b {DS_OCTET b; DS_U_SHORT us; DS_U_LONG ul;}TYPE_B_T;

#define TYPE_C_S "struct type_c {char c, str[21]; float f[3]; double d;}"
typedef struct type_c {DS_CHAR c, str[21]; DS_FLOAT f[3]; DS_DOUBLE d;}TYPE_C_T;

#define TYPE_D_S "struct type_d {struct T {long a[3][5]; float p[3];}v;}"
typedef struct type_d {struct T {DS_LONG a[3][5]; DS_FLOAT p[3];}v;}TYPE_D_T;
