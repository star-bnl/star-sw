// $Id: St_TableNtuple.h,v 1.1 1999/02/03 22:37:49 fine Exp $
// $Log: St_TableNtuple.h,v $
// Revision 1.1  1999/02/03 22:37:49  fine
...
class St_TableNtuple : public TTree {
 private:
  TString   fTableClass; //  Name of table class
    Int_t   fNvar;       //  Number of columns used
     void **fArgs;       //! Array of pointers to variables
   TClass  *fClassPtr;   //! StAF table class used in the table
     enum   NumType {
    kNAN, kFloat, kInt, kLong, kShort, kDouble, kUInt, kULong, kUShort
  }        *fType;       //! Number type of table columns
    Int_t  *fOffset;     //! Offset values for pointers to table columns

 protected:
   virtual    void LearnTable(const St_Table &table, Bool_t buildTree=kFALSE, Int_t bufsize=8000);

 public:
                   St_TableNtuple();
                   St_TableNtuple(const St_Table &table, Int_t bufsize=8000);
                   ~St_TableNtuple();
   ...
   virtual const Char_t* GetStTableClassName() const { return fTableClass.Data(); }
   virtual TClass* GetTableClass() const { return fClassPtr; }
