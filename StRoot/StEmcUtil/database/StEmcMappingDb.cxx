...
void StEmcMappingDb::SetDateTime(int date, int time) {
    ...
    else {mBeginTime.Set(date, time);
        unsigned int u = TUnixTime::Convert(mBeginTime, true);
        if(mBemcTable && !(mBemcTable->getBeginTime() < u && u < mBemcTable->getEndTime()) ) {
            mBemcDirty = true;
            reset_bemc_cache();
        }
        if(mBprsTable && !(mBprsTable->getBeginTime() < u && u < mBprsTable->getEndTime()) ) {
            mBprsDirty = true;
            reset_bprs_cache();
        }
        if( mSmdeTable && !(mSmdeTable->getBeginTime() < u && u < mSmdeTable->getEndTime()) ) {
            mSmdeDirty = true;
            reset_smde_cache();
        }
        if(mSmdpTable && !(mSmdpTable->getBeginTime() < u && u < mSmdpTable->getEndTime()) ) {
            mSmdpDirty = true;
            reset_smdp_cache();
        }
    }
