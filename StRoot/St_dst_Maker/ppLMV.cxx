// $Id: ppLMV.cxx,v 1.1 2001/04/12 15:46:27 balewski Exp $
// $Log: ppLMV.cxx,v $
// Revision 1.1  2001/04/12 15:46:27  balewski
// *** empty log message ***
//
// 20001/4/9/ this code originates from lmv as of  2000/07/22 and is tuned to
// topology of the pp events with moderate pilup 
//
...
//static const char rcsid[] = "$Id: ppLMV.cxx,v 1.1 2001/04/12 15:46:27 balewski Exp $";

static double  MinTrackLen= 1.;

//_____________________________________________________________________________
long StPrimaryMaker::ppLMV(St_dst_track *track, St_dst_vertex *vertex, Int_t mdate)
{
  printf(" THIS IS PPLMV -START\n");
  static int eveId=0;
  eveId++;
  ...
  if( Ntrk <= 1 ){
    cout<<"ppLMV: Event contains "<<Ntrk<<" global tracks. ";
    cout<<"No vertex can be reconstructed."<<endl;
    return kStWarn; 
  }
  cout<<"ppLMV: Low Multiplicity Vertex Finder. Global tracks: in Table="<<Ntrk;
  long i_non_tpc=0;
  ...
  for (long l=0; l<Ntrk; l++,glb_track_pointer++)
    {
      ...
      // some histogramming
      {
	printf("ppLMV, hist id=%d, Zdca=%.1f\n",eveId,Zdca);
	hppLMV2[0]->Fill(Zdca);
	if(eveId>0 && eveId<15) hppLMV1[eveId]->Fill(Zdca);
      }  
    }
  ...
  for(unsigned int jj=0; jj < helices.size(); jj++){
    ...
    if( R1St < RBPipe ){cout<<"ppLMV: ERROR: Radius of First point < RBeamPipe!! R1St= "<<R1St<<endl; return kStWarn;}
    ...
    if( R1St > RIfc ){
      ...
      if( lpath_ifc <= 0.0 )cout<<"ppLMV: ERROR lpath_ifc= "<<lpath_ifc<<endl;
    ...
    if( (Is_SVT == 1) && (R1St > RIfc) ){
      ...
      if( lpath_svt3 <= 0.0 )cout<<"ppLMV: ERROR lpath_svt3= "<<lpath_svt3<<endl;
    ...
    if( (Is_SVT == 1) && (R1St > R_SVT_Barrel3) ){
      ...
      if( lpath_svt2 <= 0.0 )cout<<"ppLMV: ERROR lpath_svt2= "<<lpath_svt2<<endl;
    ...
    if( (Is_SVT == 1) && (R1St > R_SVT_Barrel2) ){
      ...
      if( lpath_svt1 <= 0.0 )cout<<"ppLMV: ERROR lpath_svt1="<<lpath_svt1<<endl;
    ...
    if( lpath_bp <= 0.0 )cout<<"ppLMV: ERROR lpath_bp= "<<lpath_bp<<endl;
    lpath_tot = lpath_tot + lpath_bp;

    // Do the rescattering in the gas
    double lpath_gas = fabs(spath)/X0_N2;
    if( lpath_gas <= 0.0 )cout<<"ppLMV: ERROR lpath_gas= "<<lpath_gas<<endl;
    lpath_tot = lpath_tot + lpath_gas;

    if( lpath_tot <= 0.0 )cout<<"ppLMV: ERROR lpath_tot= "<<lpath_tot<<endl;
  ...
  if( helices.size() != TrkLength.size() )cout<<"ppLMV: This is a problem!"<<endl;
  ...
  cout<<"ppLMV: Primary Vertex found! Position: "<<XVertex<<endl;

  {// fill some histos
    float rZver=XVertex.z();
    float rXver=XVertex.x();
    float rYver=XVertex.y();
    hppLMV2[1]->Fill(rZver);
    hppLMV2[3]->Fill(rXver);
    hppLMV2[5]->Fill(rYver);
	...
	{printf(" empty St_g2t_vertext table\n"); assert(0);}
      ...
      hppLMV2[2]->Fill(z_true-rZver);
      hppLMV2[4]->Fill(GVER->ge_x[0]-rXver);
      hppLMV2[6]->Fill(GVER->ge_x[1]-rYver);
      printf("z_true-rZver=%.1f, dx=%.2f, dy=%.2f\n",z_true-rZver,GVER->ge_x[0]-rXver,GVER->ge_x[1]-rYver);
    }
  }// end histo
