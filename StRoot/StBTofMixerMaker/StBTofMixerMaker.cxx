...
int StBTofMixerMaker::Make()
{
  ...
  TObjectSet *btofSim = (TObjectSet *) GetInputDS(" BTofSimCollection");
  if (btofSim) {
    mBTofSimCollection = (StBTofCollection *) btofSim->GetObject();  //! Load StBTofSimMaker collection
    if (mBTofSimCollection) mIsEmbedding = kTRUE;
  }
  if (!mIsEmbedding) {
  ...
  mNewCollection->setHeader(mEventCollection->tofHeader()); mEventCollection->setHeader(0);
