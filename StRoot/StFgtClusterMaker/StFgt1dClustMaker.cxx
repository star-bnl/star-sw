// *-- Author : J.Balewski
// 
// $Id: StFgt1dClustMaker.cxx,v 1.1 2011/10/26 19:59:07 balewski Exp $
...
#include "StFgtDbMaker/StFgtNaiveDbMaker.h"
#include "StFgtSimulator/StFgtSlowSimuMaker.h"
#include "StFgt1dClustMaker.h"

ClassImp(StFgt1dClustMaker)
  

//--------------------------------------------
StFgt1dClustMaker::StFgt1dClustMaker(const char *name):StMaker(name){
  /// Class Constructor.  
  setHList(0);
...
//--------------------------------------------
void 
StFgt1dClustMaker::Clear(Option_t *) {
#if 0
  int i;

  for(i=0;i<StFgtGeom::kFgtMxDisk;i++) {
...
//--------------------------------------------
StFgt1dClustMaker::~StFgt1dClustMaker(){
...
void
StFgt1dClustMaker::saveHisto(TString fname){
...
StFgt1dClustMaker::Make(){
  ...
  for(int iDisk=0;iDisk<StFgtGeom::kFgtMxDisk;iDisk++) {
...
#endif
    //::::::::::::::::process Rad-strips::::::::::::::::
    int nRQuad[StFgtGeom::kFgtMxQuad]; memset(nRQuad,0,sizeof(nRQuad));

#if 0 // is it stil needed ? Jan
...
//--------------------------------------------
Int_t 
StFgt1dClustMaker::InitRun(Int_t runNumber){
  LOG_INFO<<"::InitRun()"<<  endm; 
  assert(fgtDb);
  geom=fgtDb->getFgtGeom();
  assert(geom);  
  // geom->printParam();
  return kStOK;
}
...
//--------------------------------------------
Int_t 
StFgt1dClustMaker::Init(){
  LOG_INFO<<"::Init() "<<  endm; 
  assert(HList);
  assert(fgtDb);
  mInpEve=0;
  int i;
  LOG_INFO<<Form("::Init params minPeakWidth=%d, minPeakSepar=%d (# of strips), seedStripThres=%.1f a.u., clusterMinAmpl=%.1f a.u.",par_cl_width,par_bx_valley,par_seedStripThres,par_clusterMinAmpl)<<  endm;
...
//--------------------------------------------
int
StFgt1dClustMaker::findClust1D(vector<fgt_strip> &sL, vector<fgt_cluster1D> &clL) {
  //  printf("AA %d\n",sL.size());
