...
int StSsdWafer::doClusterSplitting(scf_ctrl_st *scf_ctrl, int iSide)
{
  StSsdStripList   *CurrentStripList   =  0;
  StSsdClusterList *CurrentClusterList =  0;
  ...
  for(iCluster = 0 ; iCluster < ClusterListSize ; iCluster++)
    {   
      ...
      int toBeDeleted = CurrentClusterList->splitCluster(scf_ctrl,CurrentCluster,ListAdc,CurrentStripList);
      if(toBeDeleted)
	{
...
int StSsdWafer::doFindPackage(sdm_geom_par_st *geom_par, scm_ctrl_st *scm_ctrl)
{
  ...
  StSsdPackage *currentPackage = new StSsdPackage(0, scm_ctrl);
