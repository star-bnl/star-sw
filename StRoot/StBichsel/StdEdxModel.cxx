...
void StdEdxModel::MakedEdxModel() {
  ...
  TH3F *dEdxFun = new TH3F("dEdxFun","w = log(n_T/n_P) versus log_10(n_P) and sigma",
			   Nxbins, XBins.GetArray(),
			   Nsigma, yBins.GetArray(),
			   Nzbins, zBins.GetArray());
  TH2D *dEdxMPV = new TH2D("dEdxMPV","most probable value of w = log(n_T/n_P) versus log_10(n_P) and sigma",
			   Nxbins, XBins.GetArray(),
			   Nsigma, yBins.GetArray());
  TH2D *dEdxMean = new TH2D("dEdxMean","mean value of w = log(n_T/n_P) versus log_10(n_P) and sigma",
			   Nxbins, XBins.GetArray(),
			   Nsigma, yBins.GetArray());
  TH2D *dEdxRMS = new TH2D("dEdxRMS","RMS value of w = log(n_T/n_P) versus log_10(n_P) and sigma",
			   Nxbins, XBins.GetArray(),
			   Nsigma, yBins.GetArray());
  for (Int_t iY = 1; iY <= Nsigma; iY++) {
	...
	for (Int_t iZ = 1; iZ <= nZ; iZ++) {
	  dEdxFun->SetBinContent(iX,iY,iZ,hist->GetBinContent(iZ)/norm);
	}
