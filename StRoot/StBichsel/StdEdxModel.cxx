...
TH1F         *StdEdxModel::mdEdxMPV[3] = {0};
TH2F         *StdEdxModel::mLogdEdxModel[3][3] = {0};
TH1F         *StdEdxModel::mLogdEdxMPV[3] = {0};
  ...
  if (! fgStdEdxModel) {
	...
	for (Int_t l = 0; l <=  kTpcAll; l++) {
	  for (Int_t j = kProb; j <= kdProbdY; j++) {
	    TString name(Form("dEdx%sNorm%s",TpcName[l],DerName[j]));
	    mdEdxModel[l][j] = (TH2F *) pFile->Get(name);
	    assert(mdEdxModel[l][j]);    mdEdxModel[l][j]->SetDirectory(0);
	    name = Form("LogdEdx%sNorm%s",TpcName[l],DerName[j]);
	    mLogdEdxModel[l][j] = (TH2F *) pFile->Get(name);
	    assert(mLogdEdxModel[l][j]);    mLogdEdxModel[l][j]->SetDirectory(0);
	  }
	  TString name(Form("dEdx%sMPV",TpcName[l]));
	  mdEdxMPV[l] = (TH1F *) pFile->Get(name);
	  assert(mdEdxMPV[l]);    mdEdxMPV[l]->SetDirectory(0);
	  name = Form("LogdEdx%sMPV",TpcName[l]);
	  mLogdEdxMPV[l] = (TH1F *) pFile->Get(name);
	  assert(mLogdEdxMPV[l]);    mLogdEdxMPV[l]->SetDirectory(0);
	}
      } else if (i == 1) {
...
Double_t StdEdxModel::zMPVFunc(Double_t *x, Double_t *p) {
  ...
  return TMath::Log(instance()->GetdEdxMPV((ETpcType) kTpc)->Interpolate(x[0])) + n_PL;
}
//________________________________________________________________________________
...
Double_t StdEdxModel::zdEFunc(Double_t *x, Double_t *p) {
  ...
  TH2 *hist = instance()->GetdEdx(kProb,(ETpcType) kTpc);
  if (hist != histOld) {
    histOld = hist;
  ...
  if (xmin < n_PL      && n_PL      < xmax &&
    ...
    val = instance()->GetdEdx(kProb,(ETpcType) kTpc)->Interpolate(n_PL, dEovern_P);
  }
  return val;
}
//________________________________________________________________________________
