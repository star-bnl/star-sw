...
//____________________________________________________________________
St_MDFCorrection4C::St_MDFCorrection4C(St_MDFCorrection4 *table) : TChair(table), fFunc(0) {
  UInt_t N = table->GetNRows();
  fFunc = new TF1*[N];
  memset(fFunc, 0, N*sizeof(TF1*));
}
//____________________________________________________________________
St_MDFCorrection4C::~St_MDFCorrection4C() {
  UInt_t N = Table()->GetNRows();
  for (UInt_t i = 0; i < N; i++) {SafeDelete(fFunc[i]);}
  delete [] fFunc;
}
//____________________________________________________________________
Double_t St_MDFCorrection4C::MDFunc(Double_t *x, Double_t *p) {
  // Evaluate parameterization at point x. Optional argument coeff is
  // a vector of coefficients for the parameterisation, NCoefficients
  // elements long.
  assert(x);
  UInt_t k = p[0];
  assert(k >= 0 && k < fgMDFCorrection4C->getNumRows());
  Double_t returnValue = fgMDFCorrection4C->DMean(k);
  Double_t term        = 0;
  UChar_t    i, j;
  for (i = 0; i < fgMDFCorrection4C->NCoefficients(k); i++) {
    // Evaluate the ith term in the expansion
    term = fgMDFCorrection4C->Coefficients(k)[i];
    for (j = 0; j < fgMDFCorrection4C->NVariables(k); j++) {
      // Evaluate the factor (polynomial) in the j-th variable.
      Int_t    p  =  fgMDFCorrection4C->Powers(k)[i * fgMDFCorrection4C->NVariables(k) + j];
      Double_t y  =  1 + 2. / (fgMDFCorrection4C->XMax(k)[j] - fgMDFCorrection4C->XMin(k)[j])
	* (x[j] - fgMDFCorrection4C->XMax(k)[j]);
      term        *= fgMDFCorrection4C->EvalFactor(k,p,y);
    }
    // Add this term to the final result
    returnValue += term;
  }
...
//____________________________________________________________________
Double_t St_MDFCorrection4C::Eval(Int_t k, Double_t x0, Double_t x1, Double_t x2, Double_t x3) const {
  Double_t x[4] = {x0, x1, x2, x3};
  return Eval(k,x);
}
//____________________________________________________________________
Double_t St_MDFCorrection4C::Eval(Int_t k, Double_t *x) const {
  // Evaluate parameterization at point x. Optional argument coeff is
  // a vector of coefficients for the parameterisation, NCoefficients
  // elements long.
  assert(x);
  ...
  Double_t returnValue = 0;
  fgMDFCorrection4C = (St_MDFCorrection4C *) this;
  if (NVariables(k) <= 3) { 
    if (! fFunc[k]) {
      if (NVariables(k) <= 0) {
	return returnValue;
      } else if (NVariables(k) == 1) {
	fFunc[k] = new TF1(Form("%s_%i",Table()->GetName(),k),St_MDFCorrection4C::MDFunc,
			   XMin(k)[0],XMax(k)[0],1);
	fFunc[k]->SetParameter(0,k);
	fFunc[k]->Save(XMin(k)[0],XMax(k)[0],0,0,0,0);
      } else if (NVariables(k) == 2) {
	fFunc[k] = new TF2(Form("%s_%i",Table()->GetName(),k),St_MDFCorrection4C::MDFunc,
			   XMin(k)[0],XMax(k)[0],XMin(k)[1],XMax(k)[1],1);
	fFunc[k]->SetParameter(0,k);
	((TF2 *) fFunc[k])->Save(XMin(k)[0],XMax(k)[0],XMin(k)[1],XMax(k)[1],0,0);
      } else if (NVariables(k) == 3) {
	fFunc[k] = new TF3(Form("%s_%i",Table()->GetName(),k),St_MDFCorrection4C::MDFunc,
			   XMin(k)[0],XMax(k)[0],XMin(k)[1],XMax(k)[1],XMin(k)[2],XMax(k)[2],1);
	fFunc[k]->SetParameter(0,k);
	((TF3 *) fFunc[k])->Save(XMin(k)[0],XMax(k)[0],XMin(k)[1],XMax(k)[1],XMin(k)[2],XMax(k)[2]);
      }
...
//____________________________________________________________________
Double_t St_MDFCorrection4C::EvalError(Int_t k, Double_t *x) const {
  // Evaluate parameterization error at point x. Optional argument coeff is
  // a vector of coefficients for the parameterisation, NCoefficients(k)
  // elements long.
  assert(x);
...
//____________________________________________________________________
Double_t St_MDFCorrection4C::EvalFactor(Int_t k, Int_t p, Double_t x) const {
  // Evaluate function with power p at variable value x
  Int_t    i   = 0;
  Double_t p1  = 1;
  Double_t p2  = 0;
...
St_tpcRDOMasksC *St_tpcRDOMasksC::instance() {
  ...
  if (N != 12) {
    needReorder = kTRUE;
    LOG_WARN << "St_tpcRDOMasksC::instance RunLog/onl/tpcRDOMasks has NRows = " << N << " =! 12" << endm;
  } else {
  ...
  if (needReorder) {
	...
	rows[i] = *(row+i);
	if ((row+i)->runNumber > 0 && OldRun < 0) OldRun = (row+i)->runNumber;
	Nactive++;
      } else {
	if ((row+i)->sector > 0 && (row+i)->sector <= 24) {
	  if ((row+i)->runNumber > 0 && OldRun < 0) OldRun = (row+i)->runNumber;
    ...
    if (Nactive != 12) { // Add missing rows
      delete table;
      table = new St_tpcRDOMasks("tpcRDOMasks", 12);
      // fill missing rows
      for (Int_t i = 0; i < 12; i++) {
	if (rows[i].sector == 0) {
	  rows[i].sector = 2*i + 1;
	  rows[i].runNumber = OldRun;
	  if (St_tpcPadConfigC::instance()->iTpc(rows[i].sector)) {
	    rows[i].mask = 65535;
	  } else {
	    rows[i].mask = 4095;
	  }
...
Bool_t St_tpcRDOMasksC::isOn(Int_t sector,Int_t rdo)  {    
  ...
  Sector = sector;
  Rdo = rdo;
  if(sector < 1 || sector > 24 || rdo < 1 || rdo > 8) {
    getSectorMask(0);
    return 0;
  }
...
void St_tpcRDOMasksC::setSectorRDOMaskOff(UInt_t sector,Int_t rdo)  {    
  ...
  if (isOn(sector,rdo)) {
    ...
    if (! St_tpcPadConfigC::instance()->iTpc(sector)) {// no iTPC
      row->mask &= ~(1 << (6*((sector-1)%2) + rdo - 1)); 
    } else {                                           // with iTPC
      row->mask &= ~(1 << (8*((sector-1)%2) + rdo - 1)); 
    }
    isOn(0,rdo); // Reset saved mask 
    LOG_WARN << "St_tpcRDOMasksC::setSectorRDOMaskOff(" << sector << "," << rdo << ") = " << isOn(sector,rdo) << endm;
  }
}
//________________________________________________________________________________
...
St_tpcPadGainT0C *St_tpcPadGainT0C::fgInstance = 0;
//________________________________________________________________________________
St_tpcPadGainT0C *St_tpcPadGainT0C::instance() {
  if (fgInstance) return fgInstance;
  St_tpcPadGainT0 *table = (St_tpcPadGainT0 *) StMaker::GetChain()->GetDataBase("Calibrations/tpc/tpcPadGainT0");
  if (! table) {
    LOG_WARN << "St_tpcPadGainT0C::instance Calibrations/tpc/tpcPadGainT0\twas not found" << endm;
    assert(table);
  }
  ...
  for (Int_t i = 0; i < nrows; i++) {
    ...
    Int_t runMin = extra->runMin(i);
    Int_t runMax = extra->runMax(i);
    ...
    if (status) continue;
    if (padMin < padMax && row > 0) {
      Int_t r = row;
      if (St_tpcPadConfigC::instance()->iTPC(sector)) {
	if (row <= 40) continue;
	r = row - 40 + 13;
      }
      LOG_WARN << "St_tpcPadGainT0C::instance found extra correction for run = " << run << " in [" << runMin << "," << runMax << "]"  
	       << Form(" with sec = %2i, row = %2i, padMin/Max = %3i/%3i, RDO = %2i, FEE = %2i, status = %i", 
		       sector, r, padMin, padMax, RDO, FEE, status) << endm;
      Float_t *gains = St_tpcPadGainT0C::instance()->Gains(sector, r);
      for (Int_t p = padMin; p <= padMax; p++) {
	if (gains[p-1] > 0) {
	  LOG_WARN << "St_tpcPadGainT0C::instance reset gain[" << sector-1 << "][" << r - 1 << "][" << p-1 << "] = " << gains[p-1] << " to zero" << endm;
	}
	gains[p-1] = 0;
      }
    } else if (FEE > 0) { // iTPC stuff, ignorefor tpx
    } else if (RDO > 0) {
      LOG_WARN << "St_tpcPadGainT0C::instance found extra correction for run = " << run << " in [" << runMin << "," << runMax << "]"   
	       << Form(" reset tpcRDOMasks for sector = %i and RDO = %i to zero with sec = %2i, row = %2i, padMin/Max = %3i/%3i, RDO = %2i, FEE = %2i, status = %i", 
		       sector, RDO, sector, row, padMin, padMax, RDO, FEE, status) << endm;
      St_tpcRDOMasksC::instance()->setSectorRDOMaskOff(sector,RDO);
    } else {
      LOG_WARN << "St_tpcPadGainT0C::instance found extra correction for run = " << run << " in [" << runMin << "," << runMax << "]"   
		<< Form(" with sec = %2i, row = %2i, padMin/Max = %3i/%3i, RDO = %2i, FEE = %2i, status = %i => whole sector is marker as DEAD", 
			sector, row, padMin, padMax, RDO, FEE, status) << endm;
      Int_t rdoMin = 1;
      Int_t rdoMax = 6;
      if (!  St_tpcPadConfigC::instance()->iTPC(sector)) {rdoMin = 5; rdoMax = 8;}
      for (Int_t rdoD = rdoMin; rdoD <= rdoMax; rdoD++) {
	St_tpcRDOMasksC::instance()->setSectorRDOMaskOff(sector,rdoD);
      }
    }
  }
  return fgInstance;
...
//________________________________________________________________________________
#include "St_itpcDeadFEEC.h"
MakeChairInstance(itpcDeadFEE,Calibrations/tpc/itpcDeadFEE);
...
St_itpcPadGainT0C *St_itpcPadGainT0C::instance() {
  ...
  for (Int_t i = 0; i < nrows; i++) {
    ...
    if (padMin < padMax && row > 0) {
      Int_t r = row;
      LOG_WARN << "St_itpcPadGainT0C::instance found extra correction for run = " << run << " in [" << runMin << "," << runMax << "]"  
	       << Form(" with sec = %2i, row = %2i, padMin/Max = %3i/%3i, RDO = %2i, FEE = %2i, status = %i", 
		       sector, r, padMin, padMax, RDO, FEE, status) << endm;
      Float_t *gains = St_itpcPadGainT0C::instance()->Gains(sector, r);
      for (Int_t p = padMin; p <= padMax; p++) {
	if (gains[p-1] > 0) {
	  LOG_WARN << "St_itpcPadGainT0C::instance reset gain[" << sector-1 << "][" << r - 1 << "][" << p-1 << "] = " << gains[p-1] << " to zero" << endm;
	}
	gains[p-1] = 0;
      }
    } else if (FEE > 0) {
      for (Int_t j = 0; j < St_itpcPadGainT0C::NCrowpadFEE; j++) {
	if (St_itpcPadGainT0C::rowpadFEE[j].fee != FEE) continue;
	Int_t r = St_itpcPadGainT0C::rowpadFEE[j].row;
	padMin = St_itpcPadGainT0C::rowpadFEE[j].padMin;
	padMax = St_itpcPadGainT0C::rowpadFEE[j].padMax;
	LOG_WARN << "St_itpcPadGainT0C::instance found extra correction for run = " << run << " in [" << runMin << "," << runMax << "]"   
		 << Form(" with sec = %2i, row = %2i, padMin/Max = %3i/%3i, RDO = %2i, FEE = %2i, status = %i", 
			 sector, r, padMin, padMax, RDO, FEE, status) << endm;
	Float_t *gains = St_itpcPadGainT0C::instance()->Gains(sector, r);
...
St_tpcPadGainT0BC *St_tpcPadGainT0BC::instance() {
  if (! fgInstance) fgInstance = new St_tpcPadGainT0BC();
  return fgInstance;
}
//________________________________________________________________________________
