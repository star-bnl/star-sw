 ...
 public:
  ...
  Float_t 	Gain(Int_t sector, Int_t row, Int_t pad) const {
    if (St_tpcPadConfigC::instance()->iTPC(sector)) {
      if (row <= 40) 
	return St_itpcPadGainT0C::instance()->Gain(sector,row,pad);
      else
	return St_tpcPadGainT0C::instance()->Gain(sector,row-40,pad);
    }
  ...
  Float_t 	  T0(Int_t sector, Int_t row, Int_t pad) const {
    if (St_tpcPadConfigC::instance()->iTPC(sector)) {
      if (row <= 40) 
	return St_itpcPadGainT0C::instance()->T0(sector,row,pad);
      else 
	return St_tpcPadGainT0C::instance()->T0(sector,row-40,pad);
    }
