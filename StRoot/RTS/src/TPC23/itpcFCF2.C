...
{	
	...
	LOG(INFO,"itpcFCF2: starting version 0x%X",version) ;
...
int itpc_fcf2_c::x_max(int slice, int y)
{
	if(det_type == 0) {	// TPX
...
int itpc_fcf2_c::x_min(int slice, int y)
{
	switch(det_type) {
	case 0 :		
	case 1 :
...
int itpc_fcf2_c::y_min()
{
	return 0 ;	// always!
}

int itpc_fcf2_c::y_max()
{
	switch(det_type) {
	case 0 :
...
int itpc_fcf2_c::pad_to_x(int pad, int row)
{
	switch(det_type) {
	case 0 :
...
//static member
//int itpc_fcf2_c::rowlen[45+1] ;

itpc_fcf2_c::gain_rp_t *itpc_fcf2_c::sec_gains[MAX_SEC+1] ;

int itpc_fcf2_c::get_bad(int sec1, int row1, int pad1)
{
...
float itpc_fcf2_c::get_gain(int sec1, int row1, int pad1)
{
...
struct itpc_fcf2_c::rp_t *itpc_fcf2_c::get_row_pad(int row, int pad)
{
...
int itpc_fcf2_c::init(daq_dta *gain)
{
	if(gain==0) return -1 ;
...
int itpc_fcf2_c::init(int sec, const char *fname)
{
...
int itpc_fcf2_c::fcf_decode(u_int *p_buff, daq_sim_cld_x *dc, u_int version)
{
	daq_cld cld ;
	int words_used ;
...
int itpc_fcf2_c::fcf_decode(u_int *p_buff, daq_cld *dc, u_int version)
{
	double p, t ;
...
int itpc_fcf2_c::do_ch(int fee_id, int fee_ch, u_int *data, int words) 
{
	int row, pad ;
	int seq_cou ;
	int s_count ;
...
int itpc_fcf2_c::do_ch_sim(int row, int pad, u_short *tb_buff, u_short *track_id) 
{
	int seq_cou ;
	int s_count ;
...
int itpc_fcf2_c::do_fcf(void *v_store, int bytes)
{
...
int itpc_fcf2_c::do_blobs_stage1(int row)
{
	int pads = 0 ;
	int late_merges = 0 ;
...
int itpc_fcf2_c::do_blobs_stage2(int row)
{
//	rp_t *rp = &(row_pad[row][0]) ;
//	rp_t *rp = get_row_pad(row,0) ;
...
int itpc_fcf2_c::do_blobs_stage3(int row)
{
	short *dta_s ;
//	rp_t *rp = &(row_pad[row][0]) ;
//	rp_t *rp = get_row_pad(row,0) ;
...
int itpc_fcf2_c::do_row_check(int row)
{
//	rp_t *rp = &(row_pad[row][0]) ;
//	rp_t *rp = get_row_pad(row,0) ;
