...
#ifdef DBG_PRINT
		printf("blob %2d: pad %d:%d, tb %d:%d, flags 0x%X, area %d\n",i,
		       blob[i].p1,blob[i].p2,
		       blob[i].t1,blob[i].t2,
		       blob[i].flags,
		       blob[i].area) ;
	...
	for(int i=1;i<blob_cou;i++) {
		...
		for(int t=0;t<td;t++) {
			...
			for(int p=0;p<pd;p++) {
				...
				printf("%3d[%3d]%c ",store[p][t],smooth[p][t],pk?'*':' ') ;
			}
			printf("\n") ;
		}
#endif
...
//			printf(" pad %d: seq %d: t_lo %d, t_hi %d\n",pad,got_one,tl_lo,tl_hi) ;
				...
				if(merge) {
					...
					if(bl==0 && br==0) {
						...
						if(bl==br) {
							...
							//printf(" already %d: p2 %d, r_pad %d\n",bl,blob[br].p2,pad+1) ;	
							//printf("    t1 %d vs %d, t2 %d vs %d\n",blob[bl].t1,tr_lo,blob[bl].t2,tr_hi) ;
						}
						else {
							//printf("  WARN: blob left %d, blob right %d???\n",bl,br) ;
...
int tpc23_base::evt_stop()
{
	...
	// here I run stage2!
	if(log_level>=1) LOG(TERR,"evt_stop: S%02d: had %d last_ix, %d sequences",sector1,last_ix,sequence_cou) ;
	...
	if(run_type==1 || run_type==5) {	// ped, pulser
...
{
	LOG(INFO,"%d: run_start: detector %d",id,rts_id) ;
