...
//	}
	...
	if((data[0]&0xFFFF000F) != 0x98000004) {
		LOG(ERR,"start 0x98 = 0x%08X",data[0]) ;
	}

//	LOG(TERR,"wds 0x%08X 0x%08X",data[0],data[1]) ;
	...
	if((data_end[0]&0xFF000000) != 0x58000000) {
		LOG(ERR,"data_end 0x58 = 0x%08X",data_end[0]) ;
	}
	...
	if(!found) {
		LOG(ERR,"data_end 0x98 = 0x%08X",data_end[0]) ;
	}
...
u_int *itpc23::ch_scan(u_int *start)
{
	...
	if(log_level>=5) LOG(TERR,"%d: words10 %d:%d (SAMPA %d,%d): words10 %d",rdo1,fee_ix,ch_ix,sampa_id,sampa_ch,words10) ;
...
//	LOG(WARN,"pkt %d: sampa_id %d, sampa_ch %d, words10 %d",pkt,sampa_id,sampa_ch,words10) ;
	...
	int flags = flags_row_pad(id, sampa_ch, row, pad) ;
	...
	for(int i=0;likely(i<words);i++) {
		...
		if(log_level>=10) LOG(TERR,"FEE %d:%d -- %d = 0x%08X",fee_ix,ch_ix,i,d[i]) ;
...
u_int *itpc23::lane_scan(u_int *start)
{
	...
	if(log_level>=10) LOG(TERR,"%d: lane scan %d: 0x%08X",rdo1,lane_ix,d[0]) ;
...
u_int *itpc23::fee_scan(u_int *start)
{
	...
	fee_pp = (d[0]>>16)&0xFF ;	// padplane id

	LOG(DBG,"FEE %d = 0x%08X",fee_ix,d[0]) ;

	if(log_level>=5) LOG(TERR,"%d: FEE %d (0x%08X) fee_padplane %d, fee_evt_type 0x%02X",rdo1,fee_ix,d[0],fee_pp,fee_evt_type) ;
...
//	}
	...
	int t = 0xFFF ;
	{
		u_short t_hi = (d[6]>>8)&0xF ;
		u_short t_mid = (d[6]>>12)&0xF ;
		u_short t_lo = (d[6]>>16)&0xF ;
