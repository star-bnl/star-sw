...
const char *daq_l4::help_string = "L4\n\
tpx	returns TPX SL3;\n\
trg	returns TRG SL3;\n" ;

class daq_det_l4_factory : public daq_det_factory
{
public:
	daq_det_l4_factory() {
...
static daq_det_l4_factory l4_factory ;
...
daq_dta *daq_l4::get(const char *bank, int sec, int rdo, int pad, void *p1, void *p2) 
{	
	Make() ;
...
daq_dta *daq_l4::handle_tpx(int sec)
{
	char *st ;
	int s_start, s_stop ;
	int bytes ;
...
daq_dta *daq_l4::handle_gl3(int sec, const char *bank)
{
	char *st ;
	int bytes ;
	int cou = 0 ;
	...
	while((entry = caller->sfs->readdir(dir))) {
		...
		st = (char *) gl3->request(bytes+sizeof(l4_gl3_t)) ;
	
		((l4_gl3_t *)st)->buff = st + sizeof(l4_gl3_t) ;
		((l4_gl3_t *)st)->bytes = bytes ;
		char *yada = ((l4_gl3_t *)st)->name ;
		strncpy(yada,entry->d_name,32) ;


		int ret = caller->sfs->read(entry->full_name, st+sizeof(l4_gl3_t), bytes) ;
		if(ret != bytes) {
			LOG(ERR,"ret is %d") ;
		}

		gl3->finalize(bytes+sizeof(l4_gl3_t),cou,1,0) ;	// sector 1; rdo 1; pad irrelevant...
