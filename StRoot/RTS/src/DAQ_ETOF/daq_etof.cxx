...
const char *daq_etof::help_string = "\
\n\
ETOF Help: \n\
Supported Banks: \n\
...
class daq_det_etof_factory : public daq_det_factory
{
public:
        daq_det_etof_factory() {
                daq_det_factory::det_factories[ETOF_ID] = this ;
        }
...
static daq_det_etof_factory etof_factory ;



daq_etof::daq_etof(daqReader *rts_caller) 
{
	rts_id = ETOF_ID ;
...
daq_dta *daq_etof::get(const char *bank, int sec, int row, int pad, void *p1, void *p2) 
{
	Make() ;
...
daq_dta *daq_etof::handle_raw(int sec, int rdo)
{
	char str[128] ;
...
int daq_etof::get_token(char *addr, int words)
{
	LOG(ERR,"get_token") ;
...
int daq_etof::get_l2(char *addr, int words, struct daq_trg_word *trg, int rdo)
{
	int t_cou = 0 ;
	int in_words = words ;
	int err = 0 ;
