...
const char *daq_mtd::help_string = "\
\n\
MTD Help: \n\
Supported Banks: \n\
	raw	returns=ptr of start of DDL data; c1=sector[1..1]; c2=rdo[1..4]; \n\
	legacy	returns=ptr to struct mtd_t; \n\
\n\
\n\
" ;

class daq_det_mtd_factory : public daq_det_factory
{
public:
        daq_det_mtd_factory() {
                daq_det_factory::det_factories[MTD_ID] = this ;
        }
...
static daq_det_mtd_factory mtd_factory ;



daq_mtd::daq_mtd(daqReader *rts_caller) 
{
	rts_id = MTD_ID ;
...
daq_dta *daq_mtd::get(const char *bank, int sec, int row, int pad, void *p1, void *p2) 
{
	Make() ;
	...
	if(strcasecmp(bank,"raw")==0) {
		...
		if((present & DET_PRESENT_SFS)==0) return 0 ;	// no legacy
...
daq_dta *daq_mtd::handle_legacy()
{
	assert(caller) ;

	legacy->create(1,"mtd_t",rts_id,DAQ_DTA_STRUCT(mtd_t)) ;
	

	mtd_t *mtd_p = (mtd_t *) legacy->request(1) ;	// need ONE mtd_t object

	
	memset(mtd_p->ddl_words,0,sizeof(mtd_p->ddl_words)) ;	// zap it!
	
	mtd_p->mode = 1 ;	// backward TOF compatibility
	mtd_p->channels = 0 ;		// signal that we have NO old data...
	mtd_p->max_channels = 0 ;	// stale but compatible with old mtdReadaer...
	...
	for(int r=1;r<=MAX_RDO;r++) {
		...
		if(dd && dd->iterate()) {
			...
			if(words > (sizeof(mtd_p->ddl[0])/4)) {
				LOG(ERR,"MTD: Huge event in RDO %d: %d words -- clipping to %d words",
				    r,words,sizeof(mtd_p->ddl[0])/4) ;

				words = sizeof(mtd_p->ddl[0])/4 ;
			}
...
daq_dta *daq_mtd::handle_raw(int sec, int rdo)
{
	char str[128] ;
	int tot_bytes ;
	int min_rdo, max_rdo ;
...
int daq_mtd::get_token(char *addr, int words)
{
	int cou ;
