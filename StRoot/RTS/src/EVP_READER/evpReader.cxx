...
// CONSTRUCTOR!
evpReader::evpReader(char *mem, int size)
{
  init();
  input_type = pointer;
...
int evpReader::getNextEventFilename(int num, int type)
{
...
int evpReader::fillSummaryInfo(gbPayload *pay)
{
...
int evpReader::fillSummaryInfo(DATAP *datap)
{
  int swap = (datap->bh.byte_order == 0x04030201) ? 0 : 1;
...
int evpReader::getNextEventFilenameFromDir(int eventNum)
{
...
int evpReader::getNextEventFilenameFromLive(int type)
{
  int ret;
  static ic_msg m ;
...
static int ask(int desc, ic_msg *m)
{
  ...
  int evpReader::getStatusBasedEventDelay()
  {
