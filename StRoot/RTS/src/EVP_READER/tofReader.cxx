...
int tofReader(char *m)
{
  ...
  oldbytes = DAQtofReader(rrr->mem);

  if(oldbytes == 0) {    // check for sfs format!
    if(rrr->rts_rr) {
      daq_dta *dta = rrr->rts_rr->det("tof")->get("raw",1,-1);    // get all rb's

      memset(tof.ddl, 0, sizeof(tof.ddl));
      memset(tof.ddl_words, 0, sizeof(tof.ddl_words));

      if(dta) {
	LOG(NOTE, "2008-TOF data found");
	
	while(dta->iterate()) {
	  int sec = dta->sec;
	  int rb = dta->rdo;
	  int bytes = dta->ncontent;
	  
	  if(sec != 1) {
	    LOG(ERR, "TOF Data not sector 1");
	    continue;
	  }
	  
	  if((rb<1)||(rb>=4)) {
	    LOG(ERR, "TOF Data invalid (rb=%d)",rb);
	    continue;
	  }
	  
	  if(bytes > 10000) {
	    LOG(ERR, "Tof Data: rb %d has %d bytes (> than 10000)",
		rb,bytes);
	    continue;
	  }

	  memcpy(tof.ddl[rb-1],dta->Byte,bytes);
	  
	  if((bytes % 4) != 0) {
	    LOG(ERR, "Tof Data not multiple of 4");
	  }
	  
	  tof.ddl_words[rb-1] = bytes/4;
	  
	  oldbytes += bytes;
	  LOG(NOTE, "TOF RB[%d] has %d bytes",rb,bytes);
	}
      }
    }
  }
