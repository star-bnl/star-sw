...
u_int *tpxFCF_2D_scan_to_next(tpxFCF_2D *fcf, u_int *end, u_int *start, tpx_altro_struct *a)
{
	...
	tpx_from_altro(fcf->rdo-1,id,ch,rrow,ppad) ;

//	LOG(TERR,"A %3d:%02d: wc %d: RP %2d:%03d",id,ch,wc,rrow,ppad) ;
	...
	// ONLY if I really want this one...
	*fcf->data_raw_p++ = rrow ;
	*fcf->data_raw_p++ = ppad ;
	fcf->p_row_pad[rrow][ppad] = fcf->data_raw_p ;
					...
					if(*ix1 >= 0) {
						if(*ix2 >= 0) {
							...
							if(late_merge_cou >= MAX_LATE_MERGE) {
								LOG(ERR,"Too many late merges %d/%d: rp %d:%d",late_merge_cou,MAX_LATE_MERGE,row,p) ;
								do_debug = 1 ;
								printf("WARNING: Too many late merges: rp %d:%d, ix1 %d, ix2 %d\n",row,p,*ix1,*ix2) ;
								#ifdef DO_DBG
...
#define MAX_CHAIN_IX	32
	...
	for(int i=0;i<late_merge_cou;i++) {
		...
		if(got_hi) {
			...
			if(chain_cou >= MAX_CHAIN_COU) {
				LOG(ERR,"Too many chains") ;
				printf("***** WARN Too many chains\n") ;
				goto start_chain ;
			}
			else {
	...
	if(blob_ix >= MAX_BLOB_COUNT) {
		LOG(ERR,"Too many blobs %d/%d",blob_ix/MAX_BLOB_COUNT) ;
		do_debug = 1 ;
		printf("**** WARN too many blobs\n") ;
	...
	for(int pad=1;pad<=tpx_rowlen[row];pad++) {
		...
		if(get_static(row,pad)==0) {
			LOG(ERR,"What %d %d",row,pad) ;
			continue ;
		}

		short flags = get_static(row,pad)->f & 0x00FF ;
...
//			int prof4 = PROFILER(0) ;
			...
			if((ix < 0) || (ix > blob_ix)){
				LOG(ERR,"Too many ix: r:p %d:%d: ix %d/%d",
				    row,pad,ix,blob_ix) ;
				#ifdef DO_DBG
...
//				LOG(WARN,"Too many seqs: r:p %d:%d: ix %d: %d",row,pad,ix,seq_cou,MAX_SEQ_PER_BLOB) ;
				#ifdef DO_DBG
...
//			PROFILER(prof4) ;
	...
	for(int ix=0;ix<blob_ix;ix++) {
		...
		// ONEPAD cut but only if not before the trigger (aka wire hits))
		if(dp <= 1) {
//			if((t1>15) && do_cuts && !(flags & FCF_BROKEN_EDGE)) continue ;
		...
		// check size and cut on the timebin?
		...
		// clear
		memset(dta, 0, (dp+2)*dt_2*sizeof(short)*2) ;	// clear both storages
		...
		for(int j=1;j<=(dt);j++) {
			...
			if(unlikely(adc < 5)) continue ;	// this can cause the number of peaks to be 0...

			int iy = j - 1 ;

			for(int ii=-1;ii<=1;ii++) {				
				//register short *dd = &(dta_s[i+ii][iy]) ;
...
//			LOG(WARN,"Peaks_cou = %d: dp %d, dt %d, flags 0x%X",peaks_cou,dp,dt,flags) ;
		...
		printf("      ") ;
		for(int i=0;i<=(dp+3);i++) {
			printf("pd%03d ",p1+i-2) ;
		}
		printf("\n") ;

		for(int j=(dt+3);j>=0;j--) {
			printf("tb%03d ",t1+j-2) ;
		...
		if(peaks_cou <= 1 ) {	// single peak, full mean!
			...
			for(int i=1;i<=(dp);i++) {	// data starts on 2
				...
				//if(gain < 0.001) {
				//	printf("BAD Gain 0: sector %d, rdo %d: rp %d %d\n",sector,rdo,row,pad) ;
				//}
...
int tpxFCF_2D::do_peaks(int row, int peaks_cou)
{
	...
	for(int i=1;i<=(dp);i++) {
		...
		short flags = get_static(row,pad)->f & 0xFF ;
