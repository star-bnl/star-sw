...
int trgReader20(char *arg, struct trg_t *trg)
{
	int i ;
	int sdes, ssum, sraw ;
	int len ;
	...
	len = swap ? swap32(trgd->bh.length)*4 : trgd->bh.length;
	...
	pre = swap ? swap16(trgd->desc.npre) : trgd->desc.npre ;
	post = swap ? swap16(trgd->desc.npost) : trgd->desc.npost ;
	...
	int exp_length = sizeof(EvtDescData) + sizeof(trgd->sum) + sizeof(trgd->raw[0]) + 
			sizeof(trgd->raw[0])*pre + sizeof(trgd->raw[0])*post ;

	if(trg_length != exp_length) {
		LOG(NOTE,"Trigger data: is %d, expect %d bytes, pre %d, post %d",
		    trg_length, exp_length,pre,post,0) ;
	}
	else {
		LOG(DBG,"Trigger data: is %d, expect %d bytes, pre %d, post %d",
		    trg_length, exp_length,pre,post,0) ;
	}
