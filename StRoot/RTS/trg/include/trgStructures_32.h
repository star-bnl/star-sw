...
#define MAX_OFFSET            25              /* Maximum number of offsets in TrgTowerTrnfer OffsetBlock */
...
#define FORMAT_VERSION      0x32              /* Format Version number for trigger data */
#define TRANSFER_VERSION    0x10              /* Format Version number for definition of TrgTowerTrnfer structure */
...
#define BTOW_LEN   ((64 + 2 + 30*164)*2)      /* BTOW header + 4 byte hole + data */
#define ETOW_LEN   ((64 + 2 +  6*164)*2)      /* ETOW header + 4 byte hole + data */
...
#define TRG_TOWER_LEN (4 + (MAX_OFFSET*8) + L1_DATA_LEN + RAW_DET_DATA_LEN + BTOW_LEN + ETOW_LEN)  /* Max size of Trg/DAQ transfer data */
...
RawTrgDet      rawTriggerDet[MAX_RAW_DATA_BLOCKS];    /* Raw Detector Data with pre and post History: 11*8200 bytes */
} TrgDataType;         /* 90848  bytes */
