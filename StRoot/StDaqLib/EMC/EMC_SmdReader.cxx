...
void EMC_SmdReader::Initialize()
{
  ...
  {
    //SMD_eta
    for(int j = 0 ; j < 150; j++) mTheSmdAdcR.SmdE_ADCMatrix[i][j] = 0;
    //SMD_Phi
    for(int j = 0 ; j < 10; j++)
      for(int k = 0;k < 15; k++) mTheSmdAdcR.SmdP_ADCMatrix[i][j][k] = 0;
  }
}
...
{
 //fiber header
 //for(int i=0;i<64;i++) printf("Fiber header**: %x\n",pADCR->fiberHeader[i]);
 ...
 // Fiber data and fill the struct
  ...
  for(int i=0;i<4800;i++)
  {
    ...
    {
      //SMDE
      if(det==3) mTheSmdAdcR.SmdE_ADCMatrix[mod-1][strip-1]=pADCR->fiberData[index];
