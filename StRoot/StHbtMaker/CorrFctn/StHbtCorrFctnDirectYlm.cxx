...
using namespace std;
inline Int_t Isinf(Double_t x)
#if (defined(R__ANSISTREAM) || (defined(R__MACOSX) && defined(__arm__))) && !defined(_AIX) && !defined(__CUDACC__) 
#if defined(isinf) || defined(R__SOLARIS_CC50) || defined(__INTEL_COMPILER)
   // from math.h
  { return ::isinf(x); }
#else
   // from cmath
...
void StHbtCorrFctnDirectYlm::CalcCorrFctn()
{
	...
	complex<double> *tMq0 = new complex<double>[maxjm];
	complex<double> *tTq0 = new complex<double>[maxjm];
	double		*tMTilde = new double[maxjm * maxjm * 4];
	complex<double> *tCq0 = new complex<double>[maxjm];
	...
	for (int ibin = 1; ibin <= numsreal[0]->GetNbinsX(); ibin++) {
		...
		if (recalccov) {
			for (int ilmzero = 0; ilmzero < GetMaxJM(); ilmzero++) {
				for (int ilmprim = 0; ilmprim < GetMaxJM(); ilmprim++) {
					if ( TMath::IsNaN(covmnum[GetBin(ibin - 1, ilmzero, 0, ilmprim, 0)]) ) {
						cout << "NaN !!!! RR " << ilmzero << " " << ilmprim << endl;
					}
					if ( TMath::IsNaN(covmnum[GetBin(ibin - 1, ilmzero, 0, ilmprim, 1)]) ) {
						cout << "NaN !!!! RI " << ilmzero << " " << ilmprim << endl;
					}
					if ( TMath::IsNaN(covmnum[GetBin(ibin - 1, ilmzero, 1, ilmprim, 0)]) ) {
						cout << "NaN !!!! IR " << ilmzero << " " << ilmprim << endl;
					}
					if ( TMath::IsNaN(covmnum[GetBin(ibin - 1, ilmzero, 1, ilmprim, 1)]) ) {
