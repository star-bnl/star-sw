...
int main(int argc, char* argv[]) {
  ...
  aParticleCut->SetNSigmaPion(-2.0,+2.0);   // number of Sigma in TPC dEdx away from nominal pion dEdx
  aParticleCut->SetNSigmaAntiElectron(-2.0,+2.0);   // number of Sigma in TPC dEdx away from nominal pion dEdx
  ...
  StHbtVertexAnalysis* phiAnal = new StHbtVertexAnalysis(80,-40.,40.);
  phiAnal = new StHbtVertexAnalysis;
  ...
  phiEvcut->SetVertZPos(-40.0,40.0);    // selected range of vertex z-position
  ...
  phiAnal->SetSecondParticleCut(kaonTrkcut);  // this is the track cut for the "first" particle
  ...
  // ********************************************************************
  // define example Qinv correlation function
  QinvCorrFctn* QinvCF = new QinvCorrFctn("Qinv",25,0,0.25); 
  phiAnal->AddCorrFctn(QinvCF);   // adds the just-defined correlation function to the analysis
 ...
 ((QinvCorrFctn*)((StHbtVertexAnalysis*)hbtMaker->HbtManager()->Analysis(0))->CorrFctn(0))->Ratio()->Draw();
