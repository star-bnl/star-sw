...
//___________________
dProjector::~dProjector() { delete[] mFitParameters; };
//___________________
TH1D *dProjector::get1dProjection(TString axis, double xmin, double xmax,
  ...
  if (axis == "x") {
    proNbins = __ABS__(xmaxBin - xminBin) + 1;
    proMin = xmin;
    proMax = xmax;
    strcpy(axistitle, mRatio->GetXaxis()->GetTitle());
  } else if (axis == "y") {
    proNbins = __ABS__(ymaxBin - yminBin) + 1;
    proMin = ymin;
    proMax = ymax;
    strcpy(axistitle, mRatio->GetYaxis()->GetTitle());
  } else if (axis == "z") {
    proNbins = __ABS__(zmaxBin - zminBin) + 1;
    proMin = zmin;
    proMax = zmax;
    strcpy(axistitle, mRatio->GetZaxis()->GetTitle());
  } else {
    cout << "Wrong axis in projector, this should not happen ! " << endl;
    return 0;
  };

  // create 1d histo
  int ran =
      (int)(xmin * ymin * zmin * 10000); // to distinguish between objects give
                                         // them a special random id ...
  ...
  for (int indexX = xminBin; indexX <= xmaxBin; indexX++) {
    for (int indexY = yminBin; indexY <= ymaxBin; indexY++) {
      for (int indexZ = zminBin; indexZ <= zmaxBin; indexZ++) {
        ...
        double cellContentFromFit =
            mFitter->ykpCorrelationFunction(position, mFitParameters);

        // fill the 1d histos
        int proBin = -100;
        if (axis == "x") {
          proBin = m1dpro->GetXaxis()->FindFixBin(
              mRatio->GetXaxis()->GetBinCenter(indexX));
        } else if (axis == "y") {
          proBin = m1dpro->GetXaxis()->FindFixBin(
              mRatio->GetYaxis()->GetBinCenter(indexY));
        } else if (axis == "z") {
...
//___________________
TH2D *dProjector::get2dProjection(TString axis, double xmin, double xmax,
                                  double ymin, double ymax, double zmin,
                                  double zmax) {
  // get ranges of the ratio 3d histogram
  int xminBin = mRatio->GetXaxis()->FindFixBin(xmin);
  ...
  if (axis == "x") {
    proNbinsX = __ABS__(ymaxBin - yminBin) + 1;
    proMinX = ymin;
    proMaxX = ymax;
    strcpy(axistitleX, mRatio->GetYaxis()->GetTitle());
    proNbinsY = __ABS__(zmaxBin - zminBin) + 1;
    proMinY = zmin;
    proMaxY = zmax;
    strcpy(axistitleY, mRatio->GetZaxis()->GetTitle());
  } else if (axis == "y") {
    proNbinsX = __ABS__(zmaxBin - zminBin) + 1;
    proMinX = zmin;
    proMaxX = zmax;
    strcpy(axistitleX, mRatio->GetZaxis()->GetTitle());
    proNbinsY = __ABS__(xmaxBin - xminBin) + 1;
    proMinY = xmin;
    proMaxY = xmax;
    strcpy(axistitleY, mRatio->GetXaxis()->GetTitle());
  } else if (axis == "z") {
    proNbinsX = __ABS__(xmaxBin - xminBin) + 1;
    proMinX = xmin;
    proMaxX = xmax;
    strcpy(axistitleX, mRatio->GetXaxis()->GetTitle());
    proNbinsY = __ABS__(ymaxBin - yminBin) + 1;
    proMinY = ymin;
    proMaxY = ymax;
  ...
  int ran =
      (int)(xmin * ymin * zmin * 10000); // to distinguish between objects give
                                         // them a special random id ...
  // projection
  m2dpro = new TH2D("my2dpro", "my2dpro", proNbinsX, proMinX, proMaxX,
                    proNbinsY, proMinY, proMaxY);
  char resName[20];
  sprintf(resName, "res%s%d\n", axis.Data(), ran);
  m2dpro->SetName(resName);
  m2dpro->SetXTitle(axistitleX);
  m2dpro->SetYTitle(axistitleY);
  char restitle[20];
  sprintf(restitle, "%s\n", axis.Data());
  m2dpro->SetTitle(restitle);
  // fit
  m2dfit = new TH2D("my2dfit", "my2dfit", proNbinsX, proMinX, proMaxX,
                    proNbinsY, proMinY, proMaxY);
  char fittitle[20];
  sprintf(fittitle, "fit%s%d\n", axis.Data(), ran);
  m2dfit->SetName(fittitle);
  // norm
  m2dnor = new TH2D("my2dnor", "my2dnor", proNbinsX, proMinX, proMaxX,
                    proNbinsY, proMinY, proMaxY);
  char normtitle[20];
  sprintf(normtitle, "norm%s%d\n", axis.Data(), ran);
  m2dnor->SetName(normtitle);
