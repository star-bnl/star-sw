...
//____________________________
  dFitter3d::dFitter3d(TMinuit* gMinuit,StHbt3DHisto* numerator, StHbt3DHisto* denominator, TString opt , TString opt2)
{
  // get TMinuit in
  mMinuit = gMinuit ;
...
void dFitter3d::FillInternalArrays()
{
  ...
  mVariablePositionArray    = new StThreeVectorD[maximumInternalArraySize] ;
  ...
  for (int index = 0 ; index < maximumInternalArraySize ; index++)
      ...
      if ( mNumerator->GetBinContent(index) > mThresholdNumerator && mDenominator->GetBinContent(index) > mThresholdDenominator )
	{
	  ...
	  mVariablePositionArray[mInternalArraySize].setX(mNumerator->GetXaxis()->GetBinCenter(binX)) ;
	  mVariablePositionArray[mInternalArraySize].setY(mNumerator->GetYaxis()->GetBinCenter(binY)) ;
	  mVariablePositionArray[mInternalArraySize].setZ(mNumerator->GetZaxis()->GetBinCenter(binZ)) ;
  ...
  for (Int_t index = 0 ;  index < mInternalArraySize ; index++)
    {
      ...
      // some silly output
      if(countMinuitCalls == 100 && index%1000==0)
	{
...
//____________________________
double  dFitter3d::mCorrelationFunction(StThreeVectorD& position, double* parameterSet ) 
{
...
//____________________________
double dFitter3d::ykpCorrelationFunction(StThreeVectorD& position, double* parameterSet ) 
{
...
//____________________________
double dFitter3d::bpCorrelationFunction(StThreeVectorD& position, double* parameterSet ) 
{
...
//________________________________
void dFitter3d::SetHistos(StHbt3DHisto* numerator, StHbt3DHisto* denominator)
{
  ...
  // calculate the ratio 
  mRatio = (StHbt3DHisto*) numerator->Clone() ;
  mRatio->Reset() ;
  mRatio->Divide(numerator,denominator,1.0,mNorm) ;
}
//____________________________
void dFitter3d::Bin1ToBin3(StHbt3DHisto* histo, int bin, int& binx, int& biny, int&  binz)
{
...
//____________________________
void dFitter3d::setCorrFctn(string opt)
{
 
};
//____________________________
void dFitter3d::setFitMethod(string opt2)
{
