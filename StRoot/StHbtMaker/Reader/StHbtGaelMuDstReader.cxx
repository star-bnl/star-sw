/***************************************************************************
 *
 * $Id: StHbtGaelMuDstReader.cxx,v 1.1 2002/11/19 23:46:56 renault Exp $
...
#include "StHbtGaelMuDstReader.h"
...
//#include <fstream>//Gael 9 april 2002
ClassImp(StHbtGaelMuDstReader)
...
//-----------------------------------------------------------------------
//-----------------------------------------------------------------------
StHbtGaelMuDstReader::StHbtGaelMuDstReader(int mode, int nameMode, const char* dirName, const char* fileName, const char* filter, int maxFiles) : 
  mStEvent(0), mStStrangeMuDstMaker(0), mIOMaker(0),
  //mIoMode(mode), mIoNameMode(nameMode),//Gael 9 april 2002
  mIoMode((ioMode)mode), mIoNameMode((ioNameMode)nameMode),//Old Gael 9 april 2002
...
//-----------------------------------------------------------------------
//-----------------------------------------------------------------------
StHbtGaelMuDstReader::~StHbtGaelMuDstReader(){
}

//-----------------------------------------------------------------------
//-----------------------------------------------------------------------
//-----------------------------------------------------------------------
void StHbtGaelMuDstReader::clear(){
  DEBUGMESSAGE1("");
...
//-----------------------------------------------------------------------
//-----------------------------------------------------------------------
void StHbtGaelMuDstReader::clear(TClonesArray* t, int& counter){
  if (t) t->Clear(""); counter=0;
}
//-----------------------------------------------------------------------
//-----------------------------------------------------------------------
//-----------------------------------------------------------------------
TClonesArray* StHbtGaelMuDstReader::clonesArray(TClonesArray* p, const char* type, int size, int& counter) {
  DEBUGMESSAGE1("");
  if (!p) {
    DEBUGVALUE2(type);
...
//-----------------------------------------------------------------------
//-----------------------------------------------------------------------
StHbtEvent* StHbtGaelMuDstReader::ReturnHbtEvent(){
  DEBUGMESSAGE1("");
  StTimer timer;
  timer.start();
  clear();
...
//-----------------------------------------------------------------------
//-----------------------------------------------------------------------
void StHbtGaelMuDstReader::fill(){
  DEBUGMESSAGE1("");
...
//-----------------------------------------------------------------------
//-----------------------------------------------------------------------
void StHbtGaelMuDstReader::write(){
  DEBUGMESSAGE1("");
...
//-----------------------------------------------------------------------
//-----------------------------------------------------------------------
void StHbtGaelMuDstReader::Finish() {
  if (mFinish) {
    for ( int i=0; i<10; i++) {
...
//-----------------------------------------------------------------------
//-----------------------------------------------------------------------
void StHbtGaelMuDstReader::openRead() {
...
//-----------------------------------------------------------------------
//-----------------------------------------------------------------------
void StHbtGaelMuDstReader::read(){
  static TFile* tFile;                                         //Gael 9 april 2002
  if(tFile != mChain->GetCurrentFile()){                       //Gael 9 april 2002
    cerr << "Warning, Open : " << mChain->GetCurrentFile()->GetName() << endl;//Gael 9 april 2002
    tFile = mChain->GetCurrentFile();                         //Gael 9 april 2002
  }                                                           //Gael 9 april 2002
  DEBUGMESSAGE1("");
...
//-----------------------------------------------------------------------
//-----------------------------------------------------------------------
void StHbtGaelMuDstReader::closeRead(){
}
//-----------------------------------------------------------------------
//-----------------------------------------------------------------------
//-----------------------------------------------------------------------
void StHbtGaelMuDstReader::openWrite(string fileName) {
...
//-----------------------------------------------------------------------
//-----------------------------------------------------------------------
void StHbtGaelMuDstReader::closeWrite(){
...
//-----------------------------------------------------------------------
//-----------------------------------------------------------------------
void StHbtGaelMuDstReader::fillTrees(StEvent* ev, StMuCut* cut){
  DEBUGMESSAGE1("");
...
//-----------------------------------------------------------------------
//-----------------------------------------------------------------------
void StHbtGaelMuDstReader::fillEvent(StEvent* ev, StMuCut* cut) {
  DEBUGMESSAGE1("");
...
//-----------------------------------------------------------------------
//-----------------------------------------------------------------------
void StHbtGaelMuDstReader::fillL3AlgorithmInfo(StEvent* ev) {
  DEBUGMESSAGE1("");
...
//-----------------------------------------------------------------------
//-----------------------------------------------------------------------
void StHbtGaelMuDstReader::fillTracks(StEvent* ev, StMuCut* cut) {
  DEBUGMESSAGE1("");
  StTimer timer;
  timer.start();
...
//-----------------------------------------------------------------------
//-----------------------------------------------------------------------
void StHbtGaelMuDstReader::fillL3Tracks(StEvent* ev, StMuCut* cut) {
  DEBUGMESSAGE1("");
  if (!ev->l3Trigger()) return;
...
//-----------------------------------------------------------------------
//-----------------------------------------------------------------------
void StHbtGaelMuDstReader::fillDetectorStates(StEvent* ev) {
  DEBUGMESSAGE1("");
  StTimer timer;
  timer.start();
...
//-----------------------------------------------------------------------
//-----------------------------------------------------------------------
void StHbtGaelMuDstReader::addTrackNode(const StEvent* ev, const StTrackNode* node, StMuCut* cut, 
...
//---------------------------------------------------------------------
//---------------------------------------------------------------------
int StHbtGaelMuDstReader::addTrack(TClonesArray* tca, const StEvent*event, const StTrack* track, StMuCut* cut, int index2Global, bool l3) {
  DEBUGMESSAGE3("");
...
//-----------------------------------------------------------------------
//-----------------------------------------------------------------------
StRichSpectra* StHbtGaelMuDstReader::richSpectra(const StTrack* track) {
  DEBUGMESSAGE3("");
...
//-----------------------------------------------------------------------
//-----------------------------------------------------------------------
void StHbtGaelMuDstReader::fillStrange(StStrangeMuDstMaker* maker) {
  DEBUGMESSAGE2("");
...
//-----------------------------------------------------------------------
template <class T>
void StHbtGaelMuDstReader::addType(TClonesArray* tcaFrom, TClonesArray* tcaTo , T t) {
  if (tcaFrom && tcaTo) {
...
//-----------------------------------------------------------------------
template <class T>
int StHbtGaelMuDstReader::addType(TClonesArray* tcaTo , T t) {
  int counter =-1;
  if (tcaTo) {
...
int StHbtGaelMuDstReader::addType(TClonesArray* tcaTo , U u, T t) {
  int counter =-1;
  if (tcaTo) {
    counter = tcaTo->GetEntries();
    DEBUGMESSAGE("");
...
//-----------------------------------------------------------------------
//-----------------------------------------------------------------------
string StHbtGaelMuDstReader::buildFileName(string dir, string fileName, string extention){
  DEBUGMESSAGE1("");
...
//-----------------------------------------------------------------------
//-----------------------------------------------------------------------
string StHbtGaelMuDstReader::basename(string s){
  string name(s);
...
//-----------------------------------------------------------------------
//-----------------------------------------------------------------------
void StHbtGaelMuDstReader::makeChain(const char* dir, const char* filter, int maxFiles) {
  DEBUGMESSAGE1("");
  mChain = new TChain("MuDst");
  //  TChain* mChain2 = new TChain("StrangeMuDst");
  // read directory

  int fileCount(0);                          //Gael 9 april 2002
  if(strncmp(dir+strlen(dir)-4,".lis",4)==0){//Gael 9 april 2002
    ifstream tF(dir);                        //Gael 9 april 2002
    char tFileName[500];                     //Gael 9 april 2002
    int tNFile =0;                           //Gael 9 april 2002
    tF >> tFileName;                         //Gael 9 april 2002
    while(!tF.eof()){ //&& tNFile<=maxFiles){//Gael 9 april 2002
      cout << "Add file " << tFileName << endl;//Gael 9 april 2002
      mChain->Add(tFileName);                //Gael 9 april 2002
      tNFile++;                              //Gael 9 april 2002
      tF >> tFileName;                       //Gael 9 april 2002
    }                                        //Gael 9 april 2002
    fileCount = tNFile;                      //Gael 9 april 2002
  }                                          //Gael 9 april 2002
  else{                                      //Gael 9 april 2002    
...
void StHbtGaelMuDstReader::setProbabilityPidFile(const char* file) {
  if (mProbabilityPidAlgorithm)
    mProbabilityPidAlgorithm->readParametersFromFile(file);
}
/***************************************************************************
 *
 * $Log: StHbtGaelMuDstReader.cxx,v $
 * Revision 1.1  2002/11/19 23:46:56  renault
