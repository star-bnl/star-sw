...
C
     ...
     $        (fomega*gropp/amro**2)**2/120D0
      ...
      ELSEIF (MNUM.EQ.26) THEN  ! +--00
       ...
       ENDDO
C --
       CALL CURR5X(24,PIM2,PIM3,PIM1,PIM4,PIM5,HADCU)
       DO K=1,4
        HADCUR(K)=HADCU(K)  +HADCUR(K)
       ENDDO
       CALL CURR5X(24,PIM2,PIM3,PIM1,PIM5,PIM4,HADCU)
       DO K=1,4
        HADCUR(K)=HADCUR(K)+HADCU(K)
       ENDDO
       CALL CURR5X(24,PIM3,PIM2,PIM1,PIM4,PIM5,HADCU)
       DO K=1,4
        HADCUR(K)=HADCUR(K)+HADCU(K)
       ENDDO
       CALL CURR5X(24,PIM3,PIM2,PIM1,PIM5,PIM4,HADCU)
       DO K=1,4
        HADCUR(K)=HADCUR(K)+HADCU(K)
       ENDDO
C --
...
C                              
      IF (MNUM.EQ.24) THEN ! simple, semi realistic current for 
...
!       write(*,*) sqrt(somega)-amom,amom
      ENDDO
      ELSEIF (MNUM.EQ.26.OR.MNUM.EQ.27.OR.MNUM.EQ.28) THEN ! simple, semi realistic current for 
