...
c     $   "#############################################################"
      ...
      call onefluid(testflag,ithydro)
      write (*,*) '1f hydro finished after',t-dt_hydro,' fm'
...
c.. counting variables to reduce computing power
      integer ir,jr,kr,d,irr,jrr,krr,ien,tail
...
c.. gamma in z-direction      
      real*8 gamma_gauss
      parameter (sigma=1.0d0)
c.. grid parameter
      real*8 pi2,n0,e0,Bag,hc3

c.. rapidty cut for spectators
      real*8 rapypar,rapyzpar
c.. spectator array
...
c.. sort out the spectators
     ...
     $            px(i)*px(i)+py(i)*py(i)))))
          ...
          if ((ncoll(i).lt.1).or.(rapyzpar.gt.CTParam(66)))then
             j=j+1
             if(ncoll(i).ge.1)write(*,*)'hey'
             specr0(j)=r0(i)
             specrx(j)=rx(i)
             specry(j)=ry(i)
             specrz(j)=rz(i)
...
c.. total energy and momenta      
     ...
     $         px(ien)*px(ien)+py(ien)*py(ien)))))
          ...
          if((ncoll(ien).gt.0).and.(rapyzpar.le.CTParam(66)))then
...
c             write(25,*)rx(ien),ry(ien),rz(ien),rapyzpar,rapypar
c     $            ,ityp(ien),1
          end if
       end do
     ...
     $         px(index)*px(index)+py(index)*py(index)))))          
          if((ncoll(index).gt.0).and.(rapyzpar.le.CTParam(66)))then
             xpos=rx(index)
             ypos=ry(index)
             zpos=rz(index)
...
c.. calculation of conserved quantities and check output versus input
      ...
      write(*,*) 'total energy from urqmd:',h_etot
      write(*,*) 'total energy after urqmdtohydro:',
     &            etothy/1.d3, (etothy/1.d3-h_etot)/h_etot*100,'%'
...
c         stop
      ...
      h_btot=bartotu
      write(*,*) 'total baryon number from urqmd:',h_btot
      write(*,*) 'total baryon number after urqmdtohydro:',
     &            bartot, (bartot-h_btot)/h_btot*100,'%'
      write(*,*) 'total strangeness in hydro:', strangebe
      write(*,*) 'charge in hydro:',-zspec+zt+zp
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      real*8 tf(ngr*ngr*ngr) ,tfr(ngr*ngr*ngr), muqfr(ngr*ngr*ngr)
      real*8 musfr(ngr*ngr*ngr), vxfr(ngr*ngr*ngr), vyfr(ngr*ngr*ngr)
      real*8 vzfr(ngr*ngr*ngr), gfr(ngr*ngr*ngr)
      real*8 ifr(ngr*ngr*ngr),jfr(ngr*ngr*ngr),kfr(ngr*ngr*ngr)
c.. hydro parameters
      real*8 dx,dt_hydro,t,vol,cut
      real*8 e0,n0,Bag,hc3,pi2
...
c.. thermodynamic quantities
      common /thermo/ ifr,jfr,kfr,tf,tfr,muqfr,
     $     musfr,vxfr,vyfr,vzfr,gfr,dxfr,mmax
      common /start/ starttime 
      common /line/ n  
c.. transfer array
...
c..   charge conservation
      ...
      real*8 econ,econa,econ2,econa2,econs,econ3,econa3
...
c..   mode=5 Energy conservation by non-strange mesons
...
c..   subroutine mkityp decides if and which particle is produced
            call mkityp(Temp,mubar,mustr,gam,it,iitmax,dx)
...
c..   produce only the wanted particles in this mode             
     ...
     $              strit(it(q)).ne.0))goto 12
               if (mode.eq.5.and.(abs(it(q)).lt.100.or.strit(it(q))
     $              .ne.0))goto 12
...
c..   desired charge corresponding to baryon number 
               des_ch=netbaryons*(Zt+Zp-Zspec+0d0)/(At+Ap-Aspec+0d0)
c..   charge difference before new particle
               diffold=tot_charge-des_ch
               
 87            continue
               
               h_iso3(n)=iso3it(iso)
               h_charge(n)=fchg(h_iso3(n),it(q))
               
c..   charge difference after new particle
               diffnew=tot_charge+h_charge(n)-des_ch
c..   if diffnew gt diffold exponential suppression->new charge
               if (ranf(0).ge.exp(-(abs(diffnew)-abs(diffold)))) goto 87
...
c..   initial baryon number on the hydro grid 
      ...
      if(mode.eq.5.and.econ.lt.h_etot*0.999) goto 10 
      if(mode.eq.5.and.(dabs(econa-h_etot).gt.dabs(econ-h_etot)))goto 10
      ...
      write(*,*) 'mode',mode
      write(*,*) 'B,Z,S,E: ',
     &     netbaryons,tot_charge,tot_strange,econs,des_ch
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      subroutine mkityp(Temp,mubar,mustr,gam,it,iitmax,dx)
...
c..   Bose distribution for pions         
        if(id.eq.101)then
           ...
           pnd(ii)=gam*volume*degfac
     &	     *mass**2*Temp/(2.0d0*pi**2)*bosesum 	 
           
c..   Jüttner distribution for all other particle species         
        else 
           pnd(ii)=gam*volume*degfac
...
c.. for the zeroth of the derivative
	den=vx**2+vy**2+vz**2-1.0d0
        ...
        px=abs(vx)/vx*sqrt(-vx**2*m**2/den)
        py=abs(vy)/vy*sqrt(-vy**2*m**2/den)
        pz=abs(vz)/vz*sqrt(-vz**2*m**2/den)
        p0=sqrt(m**2+px**2+py**2+pz**2)
        fmax=fp(p0,px,py,pz,vx,vy,vz,g,a,T,mu)

c       write(*,*) 'momenta at max:',px,py,pz,p0
        i=0

        if(CTOption(53).eq.0) then

c       define "radius" for cut at exp(-30)*fmax
c        rsph=sqrt((30.d0*T+m)**2-m**2)
        lhs=exp(30.d0)*(exp(m/T)+a*exp(mu/T))-a*exp(mu/T)
        if(lhs.lt.0.d0) then
          write(*,*)'ERROR:negative argument for log!'
          write(*,*) 'm, mu,T,a,lhs', m, mu,T,a,lhs
          stop
        endif
        if((T*log(lhs))**2-m**2.lt.0) then
          write(*,*)'ERROR:sqrt of negative nuber!'
          stop
        endif

c..     radius
        rsph=sqrt((T*log(lhs))**2-m**2)

c        write(*,*)'T,mu,radius',T,mu,rsph

        call lorsphere(rsph,g,vx,vy,vz,m,pxmax,pxmin,
     & pymax,pymin,pzmax,pzmin)

ccccccccccccccccc       check     ccccccccccccccccccccccccccc
        if(px.gt.pxmax.or.px.lt.pxmin) then
            write(*,*)'smt wrong with max/min values of px'
            write(*,*)'px at max:',px,'pxmax,pxmin:',pxmax,pxmin
            stop
        endif
        if(py.gt.pymax.or.py.lt.pymin) then
            write(*,*)'smt wrong with max/min values of py'
            write(*,*)'py at max:',py,'pymax,pymin:',pymax,pymin
            stop
        endif
        if(pz.gt.pzmax.or.pz.lt.pzmin) then
            write(*,*)'smt wrong with max/min values of pz'
            write(*,*)'pz at max:',pz,'pzmax,pzmin:',pzmax,pzmin
            stop
        endif
...
c       write(*,*) 'generated momenta:',px,py,pz,p0,i 
       ...
       if(fp(p0,px,py,pz,vx,vy,vz,g,a,T,mu).gt.fmax)then
        write(*,*)'fmax too small'
        write(*,*) 'fmax',fmax
        write(*,*) 'fp',fp(p0,px,py,pz,vx,vy,vz,g,a,T,mu)
       endif 
       
       if(f.gt.fp(p0,px,py,pz,vx,vy,vz,g,a,T,mu).and.i.le.10000000) then
...
c.. if spectators are on the hydro grid    
      if(CTOption(49).eq.1)then 
       do 39 i=1,npart
        ...
        ncoll(i)=h_ncoll(i)
        origin(i)=h_origin(i)
        dectime(i)=h_r0(i)+h_tform(i)+dectim(i,0)
        tform(i)=h_tform(i)
...
c..   then all particles from cooperfrye
         do 40 i=numspec+1,numspec+npart
            ...
            dectime(i)=h_r0(i-numspec)+h_tform(i-numspec)
     &           +dectim(i,0)
...
c     read options-file
c     call getparams
      write(*,*) 'total energy after freezeout:'
     &           ,etotu,(etotu-h_etot)/h_etot*100,'%'

      if (abs((etotu-h_etot)/h_etot).gt.0.1d0)then
           write(*,*) 'energy difference is greater than 10 %'
           write(*,*) 'evolution is stopped'
      end if   
      write(*,*) 'total baryon number after freezeout:',
     &  bartotu, (bartotu-h_btot)/h_btot*100,'%'
