...
//    int stat      = event.statusHepMC(idx);
      int stat      = event[idx].statusHepMC();
...
{
  Set( Form("Beams:idA=%i",blue) ); // +z --> towards EEMC
  Set( Form("Beams:idB=%i",yell) ); // -z --> towards ETOF
  Set(      "Beams:frameType=1"); // 1=CMS
  Set( Form("Beams:eCM=%f",mRootS) );        
...
int StarPythia8::InitFIXT( int blue, int yell )
{
  Set( Form("Beams:idA=%i",blue) ); // +z --> towards EEMC
  Set( Form("Beams:idB=%i",yell) ); // -z --> towards ETOF
  Set(      "Beams:frameType=2"); // 2=fixed target mode
  double eA = (mRootS>0)? mRootS : 0.0;
  double eB = (mRootS<0)? mRootS : 0.0;
  Set( Form("Beams:eA=%f", eA) );
  Set( Form("Beams:eB=%f", eB) );
  mPythia->init();      
  return 0;
}
// ----------------------------------------------------------------------------
...
{
  LOG_INFO << "Init3/4/5MOM not implemented yet" << endm;
  assert(0);
