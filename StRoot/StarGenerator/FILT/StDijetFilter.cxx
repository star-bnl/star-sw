...
StDijetFilter::StDijetFilter():StarFilterMaker("dijet")
{
...
//_______________________________________________________________
Int_t StDijetFilter::Filter( StarGenEvent *mEvent )
{
  ...
  while( ( p = (StarGenParticle*)Iterator.Next() ) ){
    ...
    if(fabs(atan(p->GetPz()/sqrt((p->GetPx()*p->GetPx())+(p->GetPy()*p->GetPy())))) > mParticleEtaRange)continue;
    JetFourVec* vec = new JetFourVec((StarGenParticle*)p);
    finalparticles.push_back(vec);
    v[0] = (double)p->GetVx();
    v[1] = (double)p->GetVy();
    v[2] = (double)p->GetVz();

    if(vec->Pt() < mSeed)continue;
    seeds.push_back(vec);
  }
...
//_______________________________________________________________
float StDijetFilter::dR(StarGenParticle* p1,StarGenParticle* p2) const
{
  float dphi = fabs(atan(p1->GetPy()/p1->GetPx()) - acos(p2->GetPy()/p2->GetPx()));
  float deta = atan(p1->GetPz()/sqrt((p1->GetPx()*p1->GetPx())+(p1->GetPy()*p1->GetPy()))) - atan(p2->GetPz()/sqrt((p2->GetPx()*p2->GetPx())+(p2->GetPy()*p2->GetPy())));
  float pi = acos(-1.0);
  if(dphi > pi)dphi = 2*pi - dphi;
  return sqrt(pow(deta,2) + pow(dphi,2));
}
//_______________________________________________________________
float StDijetFilter::dR(StarGenParticle* p1,JetFourVec* v) const
{
  float dphi = fabs(atan(p1->GetPy()/p1->GetPx()) - v->Phi());
  float deta = atan(p1->GetPz()/sqrt((p1->GetPx()*p1->GetPx())+(p1->GetPy()*p1->GetPy()))) - v->Eta();
  float pi = acos(-1.0);
  if(dphi > pi)dphi = 2*pi - dphi;
  return sqrt(pow(deta,2) + pow(dphi,2));
}
//_______________________________________________________________
float StDijetFilter::dR(JetFourVec* v,StarGenParticle* p) const
{
  return dR(p,v);
}
//_______________________________________________________________
...
//_______________________________________________________________
JetFourVec::JetFourVec(StarGenParticle* g)
{
  px = g->GetPx();
  py = g->GetPy();
  pz = g->GetPz();
