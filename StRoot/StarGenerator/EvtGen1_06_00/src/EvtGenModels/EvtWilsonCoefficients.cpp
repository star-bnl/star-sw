...
void EvtWilsonCoefficients::SetRenormalizationScheme(std::string scheme){
  if(scheme=="NDR") m_ksi=0;
    ...
    EvtGenReport(EVTGEN_ERROR,"EvtGen") << "ERROR: EvtWilsonCoefficients knows only NDR and HV schemes !" << std::endl;
    ::abort();
  }
}
...
// calculate Lambda matching alphaS using simple iterative method
  ...
  for(i=0;i<maxstep && (difference=fabs(alphaS(mu,n_f,Lambda)-alpha))>=epsilon;i++){
    EvtGenReport(EVTGEN_DEBUG,"EvtGen") << " Difference of alpha_S from " << alpha << " is " << difference << " at Lambda = " << Lambda << std::endl;
  ...
  if(difference>=epsilon){
    EvtGenReport(EVTGEN_ERROR,"EvtGen") << " ERROR: Did not converge Lambda for alpha_s = " << alpha << " , difference " << difference << " >= " << epsilon << " after " << i << " steps !" << std::endl;
    ::abort();
    return -1;
  }else{
    EvtGenReport(EVTGEN_INFO,"EvtGen") << " For alpha_s = " << alphaS(mu,n_f,Lambda) << " was found Lambda = " << Lambda << std::endl;
    return Lambda;
  }
}
...
void EvtWilsonCoefficients::CalculateAllCoefficients(){
  ...
  EvtGenReport(EVTGEN_INFO,"EvtGen") << " +---------------------------------------" << std::endl;
  EvtGenReport(EVTGEN_INFO,"EvtGen") << " | Table of Wilson coeficients:" << std::endl;
  EvtGenReport(EVTGEN_INFO,"EvtGen") << " +---------------------------------------" << std::endl;
  ...
  EvtGenReport(EVTGEN_INFO,"EvtGen") << " | C7eff0 =  " << m_C7eff0 << std::endl;
  EvtGenReport(EVTGEN_INFO,"EvtGen") << " | C8     =  " << m_C8 << std::endl;
  EvtGenReport(EVTGEN_INFO,"EvtGen") << " | C8eff0 =  " << m_C8eff0 << std::endl;
  EvtGenReport(EVTGEN_INFO,"EvtGen") << " | C9     =  " << m_C9 << std::endl;
  EvtGenReport(EVTGEN_INFO,"EvtGen") << " | C10    =  " << m_C10 << std::endl;
  EvtGenReport(EVTGEN_INFO,"EvtGen") << " +---------------------------------------" << std::endl;
  EvtGenReport(EVTGEN_INFO,"EvtGen") << " | Other constants:" << std::endl;
  EvtGenReport(EVTGEN_INFO,"EvtGen") << " +---------------------------------------" << std::endl;
  EvtGenReport(EVTGEN_INFO,"EvtGen") << " | Scale = " << m_mu << " GeV" << std::endl;
  EvtGenReport(EVTGEN_INFO,"EvtGen") << " | Number of effective flavors = " << m_n_f << std::endl;
  EvtGenReport(EVTGEN_INFO,"EvtGen") << " | Corresponding to aS(M_Z)" << "=" << m_alphaMZ << " Lambda = " << m_Lambda << " GeV" << std::endl;
  EvtGenReport(EVTGEN_INFO,"EvtGen") << " | Strong coupling constant = " << m_alphaS << std::endl;
  EvtGenReport(EVTGEN_INFO,"EvtGen") << " | Electromagnetic constant = 1/" << m_ialpha << std::endl;
  EvtGenReport(EVTGEN_INFO,"EvtGen") << " | Top mass = " << m_M_t << " GeV" << std::endl;
  EvtGenReport(EVTGEN_INFO,"EvtGen") << " | W-boson mass = " << m_M_W << " GeV" << std::endl;
  EvtGenReport(EVTGEN_INFO,"EvtGen") << " | Z-boson mass = " << m_M_Z << " GeV" << std::endl;
  EvtGenReport(EVTGEN_INFO,"EvtGen") << " | Sinus squared of Weinberg angle = " << m_sin2W << std::endl;
  EvtGenReport(EVTGEN_INFO,"EvtGen") << " +---------------------------------------" << std::endl;
  EvtGenReport(EVTGEN_DEBUG,"EvtGen") << " | Intermediate functions:" << std::endl;
  EvtGenReport(EVTGEN_DEBUG,"EvtGen") << " +---------------------------------------" << std::endl;
  ...
  EvtGenReport(EVTGEN_DEBUG,"EvtGen") << " | C9~  = " << m_C9tilda << std::endl;
  EvtGenReport(EVTGEN_DEBUG,"EvtGen") << " | C10~ = " << m_C10tilda << std::endl;
