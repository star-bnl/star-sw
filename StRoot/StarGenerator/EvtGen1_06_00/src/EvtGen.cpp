...
EvtGen::EvtGen(const char* const decayName,
  ...
  if (randomEngine==0){
    ...
    EvtGenReport(EVTGEN_INFO,"EvtGen") <<"No random engine given in "
  ...
  EvtGenReport(EVTGEN_INFO,"EvtGen") << "Main decay file name  :"<<decayName<<endl;
  EvtGenReport(EVTGEN_INFO,"EvtGen") << "PDT table file name   :"<<pdtTableName<<endl;
  ...
  EvtGenReport(EVTGEN_INFO,"EvtGen") << "Mixing type integer set to "<<_mixingType<<endl;
...
void EvtGen::readUDecay(const char* const uDecayName, bool useXml){
  ...
  if ( uDecayName[0] == 0) {
    EvtGenReport(EVTGEN_INFO,"EvtGen") << "Is not reading a user decay file!"<<endl;
  }
  else{  
    ...
    if (indec) {
      ...
      EvtGenReport(EVTGEN_INFO,"EvtGen") << "Can not find UDECAY file '"
...
void EvtGen::generateDecay(EvtParticle *p){
  ...
  do{
    ...
    if ( times==10000) {
      EvtGenReport(EVTGEN_ERROR,"EvtGen") << "Your event has been rejected 10000 times!"<<endl;
