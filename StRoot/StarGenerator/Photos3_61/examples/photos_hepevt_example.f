...
C     PREPARE AN EVENT in HEPEVT
C     TWO TEST EVENTS HAVE BEEN PROVIDED WITH THIS EXAMPLE
      CALL SIMPLE_EE_TO_Z_TO_TAUTAU_EVENT
c      CALL EVEN_SIMPLER_Z_TO_EE_EVENT
...
C      CALL PHOTOS_PROCESS_BRANCH(4)
      ...
      SUBROUTINE SIMPLE_EE_TO_Z_TO_TAUTAU_EVENT
...
C     ---------------------------------------------------------------------
     ...
     $                    1.3956753909587860D-01,  5,  0,  0, 0)
      ...
      SUBROUTINE EVEN_SIMPLER_Z_TO_EE_EVENT
C serve as an exaple; to be replaced with event generated by user 
      REAL*8 AMELL

      AMELL = 0.0005111;

C     CREATE SIMPLE EVENT: Z -> e+ e-
C     ---------------------------------------------------------------------
      CALL ADD_PARTICLE( 23, 2,  0.000000000000000D+00,  0.000000000000000D+00,
     $      -4.821915360414504D+02, 4.900549668552968D+02, 9.118760000000000D+01, 0, 0, 2, 3)
      CALL ADD_PARTICLE( 11, 1, -3.018114335125028D+01, -1.204071905454172D+01,
     $      -7.717282429699088D+01, 8.373485020774415D+01, 5.109989200000000D-04, 1, 0, 0, 0)
      CALL ADD_PARTICLE(-11, 1,  3.018114335125028D+01,  1.204071905454172D+01,
     $      -4.050187117444596D+02, 4.063201166475526D+02, 5.109989200000000D-04, 1, 0, 0, 0) 
      END
