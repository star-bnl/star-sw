...
c 19    typevt=0                      !no interaction
c      if(jdiff.eq.0.or.jdiff.gt.10)then                     !ND (no rap-gaps)
c       typevt=1
c      elseif(jdiff.eq.8.or.jdiff.eq.10.or.jdiff.eq.3.or.jdiff.eq.4
c     *.or.jdiff.eq.5)then                                   !DD + (CD+LMD)
c       typevt=2
c      elseif(jdiff.eq.1.or.jdiff.eq.6)then                  !SD pro
c       typevt=4
c      elseif(jdiff.eq.2.or.jdiff.eq.7)then                  !SD tar
c       typevt=-4
c      elseif(jdiff.eq.9)then                                !CD
c       typevt=3
c      else
c       stop'problem with typevt!'
c      endif
c This is the test for RHICf study to seperate the double and central!!!! (09/May/2026) Seunghwan
c Above comments line is the origin code

19    typevt=0                      !no interaction
      if(jdiff.eq.0.or.jdiff.gt.10)then                     !ND (no rap-gaps)
       typevt=1
      elseif(jdiff.eq.8.or.jdiff.eq.3.or.jdiff.eq.4
     *.or.jdiff.eq.5)then                                   !DD 
       typevt=2
