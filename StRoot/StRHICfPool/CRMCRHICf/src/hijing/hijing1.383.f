...
C     Version 1.36
c
c     Modification Oct. 8, 1998. In hijing, log(rangen()) occasionally
c     causes overfloat. It is modified to log(max(rangen(),1.0e-20)).
c
c
...
c
        if(IHNT2(1).EQ.2) then
           rnd1=max(rangen(),1.0e-20)
           rnd2=max(rangen(),1.0e-20)
           rnd3=max(rangen(),1.0e-20)
...
C        print*,"KT:",KT
C        print*,YT(1,KT),YT(2,KT),YT(3,KT)
...
C
        BB=SQRT(BMIN**2+rangen()*(BMAX**2-BMIN**2))
        PHI=2.0*HIPR1(40)*rangen()
        BBX=BB*COS(PHI)
        BBY=BB*SIN(PHI)
        HINT1(19)=BB
        HINT1(20)=PHI
...
C
        IF(IHPR2(3).NE.0) THEN
           NHARD=1+INT(rangen()*(NCOLT-1)+0.5)
...
C
        IF(IHPR2(9).EQ.1) THEN
                NMINI=1+INT(rangen()*(NCOLT-1)+0.5)
...
C                                already been produced here
           XR1=-ALOG(EXP(-TTRIG)+rangen()*(1.0-EXP(-TTRIG)))
 106           NJET=NJET+1
           XR1=XR1-ALOG(max(rangen(),1.0e-20))
           IF(XR1.LT.TTRIG) GO TO 106
           XR=0.0
 107           NJET=NJET+1
           XR=XR-ALOG(max(rangen(),1.0e-20))
...
110        XR=-ALOG(EXP(-TT)+rangen()*(1.0-EXP(-TT)))
111        NJET=NJET+1
        XR=XR-ALOG(max(rangen(),1.0e-20))
...
C
                ...
                DO 390 I=N_ST,N
                   ...
                   K(I,4)=N_STR
                   NATT=NATT+1
c                   print *,I,K(I,1),K(I,2)
                   KATT(NATT,1)=K(I,2)
                   KATT(NATT,2)=NFTP
                   KATT(NATT,4)=K(I,1)
                   IF(K(I,3).EQ.0) THEN
                      KATT(NATT,3)=0
                   ELSEIF(K(K(I,3),2).EQ.IDSTR) THEN
                      KATT(NATT,3)=0
                   ELSE
                      KATT(NATT,3)=NATT-I+K(I,3)+N_STR-K(K(I,3),4)
                   ENDIF
...
580                CONTINUE
                N=N+NTJ(JTP)
        ENDIF
        IF(IHPR2(1).GT.0.AND.rangen().LE.HIDAT(3)) THEN
             HIDAT20=HIDAT(2)
             HIPR150=HIPR1(5)
...
C
     ...
     &                /(1.0+EXP(-2.0*(VINT(47)-HIDAT(1))))
        I_INIRAD=0
        IF(rangen().LE.PINIRAD) I_INIRAD=1
...
C
        ...
        SAVE INI,I_LAST,XSEC0,COEF0,MINT44,MINT45
C
        IHNT2(11)=JP
        IHNT2(12)=JT
...
C                        the pt kick is the pt from hard-collision.
        I_SNG=0
        IF(IHPR2(13).NE.0 .AND. rangen().LE.HIDAT(4)) I_SNG=1
...
C               ********decite whether to have single-diffractive
        IF(IHPR2(5).EQ.0) THEN
                PKC=HIPR1(2)*SQRT(-ALOG(1.0-rangen()
     ...
     &               -rangen()*(EXP(-HIPR1(20)**2/HIPR1(2)**2)-
     &               EXP(-PKCMX**2/HIPR1(2)**2))))
C
        IF(I_SNG.EQ.1) PKC=0.65*SQRT(
     &                -ALOG(1.0-rangen()*(1.0-EXP(-PKCMX**2/0.65**2))))
C                        ********select PT kick
30        PHI0=2.0*HIPR1(40)*rangen()
        PKC11=PKC*SIN(PHI0)
        PKC12=PKC*COS(PHI0)
        PKC21=-PKC11
        PKC22=-PKC12
...
C                        single-diffr
              IF(rangen().GT.0.5.OR.(NFT(JT,5).GT.2.OR.
     &                      NTJ(JT).NE.0.OR.NFT(JT,10).NE.0)) THEN
                 D1=DPN
                 D2=DTX
                 NFP3=NFP(JP,3)
...
c*** this is identical to the statement following the next ELSE IF
     ...
     &       .AND.rangen().GT.0.5).OR.(NPJ(JP).EQ.0
     &        .AND.NTJ(JT).NE.0)).AND.NFP(JP,5).LE.2) THEN
              D1=DPN
              D2=DTX
              NFP3=NFP(JP,3)
...
C       so that pt=p1+p2
     ...
     &                        ABS(NFT(JT,1)*NFT(JT,2)).LT.100) THEN
                KICKDIT=0
        ENDIF
        IF((KICKDIP.EQ.0.AND.rangen().LT.0.5)
     &     .OR.(KICKDIP.NE.0.AND.rangen()
...
C                                ********pt kick for proj
        PT(JT,1)=PT11-PKC21
        PT(JT,2)=PT12-PKC22
        IF((KICKDIT.EQ.0.AND.rangen().LT.0.5)
     &     .OR.(KICKDIT.NE.0.AND.rangen()
...
C
     ...
     &               -rangen()*(EXP(-HIPR1(8)**2/HIPR1(2)**2)-
     &               EXP(-PTMAX**2/HIPR1(2)**2))))

        ELSE
                PT=HIPR1(2)*SQRT(-ALOG(1.0-rangen()*
...
C        This generate random number between XMIN and XMAX
        ...
        RX=RR(I,JMIN)+(RR(I,JMAX)-RR(I,JMIN))*rangen()
        JL=0
        JU=202
10        IF(JU-JL.GT.1) THEN
           JM=(JU+JL)/2
...
C
C
        SUBROUTINE PARTONHI(F,X1,X2,QQ)
        IMPLICIT REAL*8(A-H,O-Z)
        REAL HIPR1(100),HINT1(100)
        COMMON/HIPARNT/HIPR1,IHPR2(50),HINT1,IHNT2(50)
        SAVE  /HIPARNT/
