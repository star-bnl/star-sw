...
C... Form meson, with spin and flavour mixing for diagonal states.
     ...
     &    INT(RMIX+PARF(IMIX)))+KFLS
        ...
        ENDIF
        IF(KMUL.EQ.2.OR.KMUL.EQ.3) KF=KF+SIGN(10000,KF)
        IF(KMUL.EQ.4) KF=KF+SIGN(20000,KF)
...
C.. Form baryon. Distinguish Lambda- and Sigmalike baryons.
        ...
        ENDIF
        KF=SIGN(1000*KFLD+100*KFLE+10*KFLF+KFLS,KFL1)
        IF(KSIG.EQ.0) KF=SIGN(1000*KFLD+100*KFLF+10*KFLE+KFLS,KFL1)
      ENDIF
      RETURN
...
C...Reconstruct flavour of produced quark/diquark.
      IF(KTAB3.LE.6) THEN
        KFL3A=KTAB3
        KFL3B=0
        KFL3=SIGN(KFL3A,KFL1*(2*KTAB1-13))
      ELSE
        KFL3A=1
     ...
     &  KFL3+2
        KFL3=SIGN(KFL3,KFL1*(13-2*KTAB1))
      ENDIF
...
C...Reconstruct baryon code.
      ELSE
        ...
        KFLF=MIN(KFLA,KFLB,KFLC)
        KFLE=KFLA+KFLB+KFLC-KFLD-KFLF
        IF(KTABS.EQ.0) KF=SIGN(1000*KFLD+100*KFLF+10*KFLE+2,KFL1)
        IF(KTABS.GE.1) KF=SIGN(1000*KFLD+100*KFLE+10*KFLF+2*KTABS,KFL1)
      ENDIF
