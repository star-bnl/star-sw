...
C...and UED particle code (5 000 000 + id)
      ...
      DIMENSION IUEDEQ(KKFLMA)
C unvar      DIMENSION IUEDEQ(KKFLMA),MUED(2)
...
C...t -> ~t + ~chi_i0, i = 1, 2, 3 or 4.
            ...
            IF(SHR.GT.PMNCHI+PMSTOP) THEN
              IZ=I-9
              DO 130 IK=1,4
                ZMIXC(IZ,IK)=CMPLX(ZMIX(IZ,IK),ZMIXI(IZ,IK))
  130         CONTINUE
              AL=SHR*CONJG(ZMIXC(IZ,4))/(2.0D0*PMAS(24,1)*SINB)
     ...
     &        SMZ(IZ)*4D0*SHR*DBLE(FL*CONJG(FR)))/SH
              IF(KFLR.GT.0) THEN
...
C***Note that widths are not given in dimensional quantities here.
        DO 190 I=1,MDCY(KC,3)
          ...
          RM1=PMAS(ABS(KFDP(IDC,1)),1)**2/SH
          RM2=PMAS(ABS(KFDP(IDC,2)),1)**2/SH
...
C...Charged H loops: loop integral and charges.
  ...
  260       CONTINUE
          ...
          ELSEIF(I.LE.17) THEN
C...h0 -> Z0 + Z0, W+ + W-
            PM1=PMAS(ABS(KFDP(IDC,1)),1)
            PG1=PMAS(ABS(KFDP(IDC,1)),2)
...
C...pi_tc -> f + f'.
          FCOF=1D0
          IF(ABS(KFDP(IDC,1)).LT.10) FCOF=3D0*RADC
C...pi_tc+ -> W b b~
...
C...H_L++/--:
        ...
        DO 510 I=1,MDCY(KC,3)
          ...
          IF(I.LE.6) THEN
C...H_L++/-- -> l+/- + l'+/-
            FCOF=PARP(180+3*((ABS(KFDP(IDC,1))-11)/2)+
     &      (ABS(KFDP(IDC,2))-9)/2)**2
