...
C...Local arrays, character variables and data.
     ...
     &CHTMP*16,CHIN(2)*12,STATE(-1:5)*4,CHKIN(21)*18,DISGA(2)*28,
...
C...Off-shell branchings.
     ...
     &      PMAS(KC,1),0D0,0D0,STATE(MDCY(KC,1)),0D0
            DO 120 J=1,MDCY(KC,3)
              IDC=J+MDCY(KC,2)-1
              NGP1=0
              IF(ABS(KFDP(IDC,1)).LE.20) NGP1=
     &        (MOD(ABS(KFDP(IDC,1)),10)+1)/2
              NGP2=0
              IF(ABS(KFDP(IDC,2)).LE.20) NGP2=
     &        (MOD(ABS(KFDP(IDC,2)),10)+1)/2
...
C unvar 8300 FORMAT(1X,'I',50X,'I',11X,'I',5X,'I')
 8500 FORMAT(1X/ 1X/
