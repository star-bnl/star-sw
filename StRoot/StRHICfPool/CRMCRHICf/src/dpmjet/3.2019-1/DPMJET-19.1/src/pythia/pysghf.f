...
C...Local arrays
C unvar      DIMENSION WDTP(0:400),WDTE(0:400,0:5)
...
C...f + q -> f' + Q
          ...
          DO 130 I=MMIN1,MMAX1
            ...
            DO 120 J=MMIN2,MMAX2
              ...
              IF(I*J.GT.0.AND.MOD(ABS(I+J),2).EQ.0) GOTO 120
              IF(I*J.LT.0.AND.MOD(ABS(I+J),2).EQ.1) GOTO 120
              IF(ABS(I).LT.MINT(55).AND.MOD(ABS(I+MINT(55)),2).EQ.1)
     &        THEN
                NCHN=NCHN+1
                ISIG(NCHN,1)=I
                ISIG(NCHN,2)=J
     ...
     &          (ABS(I)+1)/2)*VINT(180+J)
                IF(MOD(MINT(55),2).EQ.1) FACCKM=VCKM(ABS(I)/2,
     ...
     &            WIDS(MINT(55),3)
              ...
              ENDIF
              IF(ABS(J).LT.MINT(55).AND.MOD(ABS(J+MINT(55)),2).EQ.1)
     &        THEN
                NCHN=NCHN+1
                ISIG(NCHN,1)=I
                ISIG(NCHN,2)=J
                ISIG(NCHN,3)=2
                IF(MOD(MINT(55),2).EQ.0) FACCKM=VCKM(MINT(55)/2,
     &          (ABS(J)+1)/2)*VINT(180+I)
                IF(MOD(MINT(55),2).EQ.1) FACCKM=VCKM(ABS(J)/2,
...
C...g + gamma -> Q + Qbar
          ...
          THQ=-0.5D0*SH*(1D0-BE34*CTH)
          UHQ=-0.5D0*SH*(1D0+BE34*CTH)
          FACQQ=COMFAC*AS*AEM*(KCHG(ABS(MINT(55)),1)/3D0)**2*
...
C...gamma + gamma -> F + Fbar (heavy fermion, quark or lepton)
          ...
          THQ=-0.5D0*SH*(1D0-BE34*CTH)
          UHQ=-0.5D0*SH*(1D0+BE34*CTH)
          FACFF=COMFAC*AEM**2*(KCHG(ABS(MINT(56)),1)/3D0)**4*2D0*
     ...
     &    SQMAVG*SH**2*(SH-2D0*SQMAVG))/(THQ*UHQ)**2
          IF(ABS(MINT(56)).LT.10) FACFF=3D0*FACFF
          IF(ABS(MINT(56)).LT.10.AND.MSTP(35).GE.1)
...
C...(recalculate kinematics for massless partons).
          ...
          DO 2442 I=MMINA,MMAXA
            IF(I.EQ.0.OR.ABS(I).GT.MSTP(58)) GOTO 2442
            DO 2441 ISDE=1,2
...
C...(recalculate kinematics for massless partons).
          ...
          DO 2444 I=MMINA,MMAXA
            IF(I.EQ.0.OR.ABS(I).GT.MSTP(58)) GOTO 2444
            DO 2443 ISDE=1,2
...
C...(recalculate kinematics for massless partons).
          ...
          DO 2446 I=MMINA,MMAXA
            IF(I.EQ.0.OR.ABS(I).GT.MSTP(58)) GOTO 2446
            DO 2445 ISDE=1,2
...
C...q + g -> q + QQ~[3P01]
     ...
     &            (TH-3D0*SQMQQ)**2*(SH2+UH2)/(SQMQQR*TH*UHSH2**2)
          ...
          DO 2452 I=MMINA,MMAXA
            IF(I.EQ.0.OR.ABS(I).GT.MSTP(58)) GOTO 2452
            DO 2451 ISDE=1,2
...
C...q + g -> q + QQ~[3P11]
     ...
     &              +C4*(EL1K11*EL2K21+EL1K21*EL2K11)/2D0
          ...
          DO 2454 I=MMINA,MMAXA
            IF(I.EQ.0.OR.ABS(I).GT.MSTP(58)) GOTO 2454
            DO 2453 ISDE=1,2
...
C...q + g -> q + QQ~[3P21]
     ...
     &              +(C9+C0)*(EL1K11*EL2K21)**2)
          ...
          DO 2456 I=MMINA,MMAXA
            IF(I.EQ.0.OR.ABS(I).GT.MSTP(58)) GOTO 2456
            DO 2455 ISDE=1,2
