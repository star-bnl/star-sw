...
C...NO RIGHT-HANDED NEUTRINOS
          IF (((IMASS.EQ.2).AND.((ABS(KFR(JRES)).EQ.12).OR
     &         .(ABS(KFR(JRES)).EQ.14).OR.(ABS(KFR(JRES)).EQ.16))).OR
     &         .KFR(JRES).EQ.0) GOTO 100
          RES(IRES,1) = PMAS(PYCOMP(IMASS*KSUSY1+ABS(KFR(JRES))),1)
          RES(IRES,2) = PMAS(PYCOMP(IMASS*KSUSY1+ABS(KFR(JRES))),2)
          INTRES(IRES,1) = ABS(KFR(JRES))
          INTRES(IRES,2) = IMASS
