...
C...alpha_strong for jet rate. Initial value for y cut.
        ...
        IF(ABS(MSTJ(101)).LE.1.OR.(MSTJ(109).EQ.0.AND.MSTJ(111).EQ.0))
...
C...Parametrization of second order three-jet cross-section.
        IF(ABS(MSTJ(101)).LE.1.OR.MSTJ(101).EQ.3.OR.MSTJ(109).EQ.2.OR.
     &  CUT.GE.0.25D0) THEN
          PARJ(153)=0D0
...
C...Shift in second order three-jet cross-section with optimized Q^2.
        IF(MSTJ(111).EQ.1.AND.ABS(MSTJ(101)).GE.2.AND.MSTJ(101).NE.3
...
C...Parametrization of second order four-jet cross-section.
        IF(ABS(MSTJ(101)).LE.1.OR.CUT.GE.0.125D0) THEN
          PARJ(154)=0D0
        ELSE
          CT=LOG(1D0/CUT-5D0)
