...
C...f + fbar -> gamma*/Z0
          ...
          DO 100 I=MMINA,MMAXA
            ...
            VI=AI-4D0*EI*XWV
            HI0=HP0
            IF(ABS(I).LE.10) HI0=HI0*FACA/3D0
            HI1=HP1
            IF(ABS(I).LE.10) HI1=HI1*FACA/3D0
            NCHN=NCHN+1
            ISIG(NCHN,1)=I
            ISIG(NCHN,2)=-I
            ISIG(NCHN,3)=1
...
C...f + fbar' -> W+/-
     ...
     &        GOTO 110
              KCHW=(KCHG(IA,1)*SIGN(1,I)+KCHG(JA,1)*SIGN(1,J))/3
              HI=HP*2D0
...
C...gamma, gamma/Z interference and Z couplings to final fermion pairs
     ...
     &      IMDM=1
            ...
            ELSEIF(I.LE.16) THEN
              ...
              VF=AF-4D0*EF*XWV
            ENDIF
            RM1=PMAS(ABS(KFDP(IDC,1)),1)**2/SQM4
  ...
  220     CONTINUE
          ...
          DO 230 I=1,MIN(16,MDCY(23,3))
            ...
            ELSEIF(I.LE.16) THEN
              ...
              VF=AF-4D0*EF*XWV
            ENDIF
            RM1=PMAS(ABS(KFDP(IDC,1)),1)**2/SQM3
            IF(4D0*RM1.LT.1D0) THEN
              FCOF=1D0
...
C...Loop over allowed flavours
          DO 310 I=MMINA,MMAXA
            ...
            FCOI=1D0
            IF(ABS(I).LE.10) FCOI=FACA/3D0
            IF(MSTP(50).LE.0.OR.ABS(I).LE.10) THEN
              IF(AI.LT.0D0) THEN
...
C...Loop over flavours; consider full gamma/Z structure
          DO 340 I=MMINA,MMAXA
            IF(I.EQ.0.OR.ABS(I).GT.MSTP(58)) GOTO 340
            EI=KCHG(ABS(I),1)/3D0
            AI=SIGN(1D0,EI)
            VI=AI-4D0*EI*XWV
...
C...Propagators: as simulated in PYOFSH and as desired
          ...
          DO 360 I=MMINA,MMAXA
            IF(I.EQ.0.OR.ABS(I).GT.MSTP(58)) GOTO 360
            IA=ABS(I)
            KCHW=SIGN(1,KCHG(IA,1)*SIGN(1,I))
            WIDSC=(WDTE(0,1)+WDTE(0,(5-KCHW)/2)+WDTE(0,4))/WDTP(0)
            DO 350 ISDE=1,2
...
C...Loop over flavours; consider full gamma/Z structure
     ...
     &      (VI**2+AI**2)*HFZZ)/HBW4
            FZQD=MAX(PMAS(ABS(I),1)**2*SQM4,FZQDTM)
            DO 380 ISDE=1,2
