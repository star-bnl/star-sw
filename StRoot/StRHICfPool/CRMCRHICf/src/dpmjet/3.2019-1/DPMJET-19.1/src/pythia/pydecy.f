...
C...If tau -> tau + gamma then add gamma energy and loop.
  ...
  140       CONTINUE
        ...
        ELSEIF(ABS(K(IMTAU,2)).GT.100) THEN
...
C...but can be reconstructed by adding neutrino momentum.
          KFORIG=-SIGN(24,K(ITAU,2))
          IORIG=0
          DO 160 II=K(IMTAU,4),K(IMTAU,5)
            IF(K(II,2)*SIGN(1,K(ITAU,2)).EQ.-16) THEN
              DO 150 J=1,4
...
C.. Stick to started popcorn system, else pick side at random
  ...
  330   JT=2
        ...
        IF(JT.EQ.4.AND.SIGN(1,KFL1(1)*(10-ABS(KFL1(1))))*
     &  SIGN(1,KFL1(JT)*(10-ABS(KFL1(JT)))).GT.0) JT=3
        IF(JT.EQ.3) JT2=2
        IF(JT.EQ.4) JT3=2
...
C...Check invariant mass of W jets. May give one particle or start over.
  600 IF((MMAT.EQ.42.OR.MMAT.EQ.43.OR.MMAT.EQ.44.OR.MMAT.EQ.48)
     &.AND.ABS(K(N+1,2)).LT.10) THEN
        ...
        CALL PYKFDI(K(N+1,2),-SIGN(KFLDUM,K(N+1,2)),KFLDMP,KF1)
        CALL PYKFDI(K(N+2,2),-SIGN(KFLDUM,K(N+2,2)),KFLDMP,KF2)
...
C...Phase space decay of partons from W decay.
  610 IF((MMAT.EQ.42.OR.MMAT.EQ.48).AND.ABS(K(N+1,2)).LT.10) THEN
        KFLO(1)=K(N+1,2)
        KFLO(2)=K(N+2,2)
        K(N+1,1)=K(N+3,1)
        K(N+1,2)=K(N+3,2)
...
C...Set up for parton shower evolution from jets.
     ...
     &  ABS(K(NSAV+1,2)).LE.10.AND.ABS(K(NSAV+2,2)).LE.10) THEN
        K(NSAV+1,1)=3
        K(NSAV+2,1)=3
        K(NSAV+1,4)=MSTU(5)*(NSAV+2)
        ...
        MSTJ(92)=NSAV+1
      ELSEIF(MSTJ(23).GE.1.AND.(MMAT.EQ.32.OR.MMAT.EQ.44).AND.
     &  ABS(K(NSAV+1,2)).LE.20.AND.ABS(K(NSAV+2,2)).LE.20) THEN
        MSTJ(92)=NSAV+1
      ELSEIF(MSTJ(23).GE.1.AND.MMAT.EQ.33.AND.ABS(K(NSAV+2,2)).EQ.21)
     &  THEN
        K(NSAV+1,1)=3
        K(NSAV+2,1)=3
        K(NSAV+3,1)=3
        KCP=PYCOMP(K(NSAV+1,2))
        KQP=KCHG(KCP,2)*SIGN(1,K(NSAV+1,2))
        JCON=4
        IF(KQP.LT.0) JCON=5
