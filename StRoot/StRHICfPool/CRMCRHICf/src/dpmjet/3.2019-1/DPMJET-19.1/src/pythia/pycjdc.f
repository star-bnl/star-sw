...
C...Local variables
     ...
     &XMZ,XMZ2,AXMJ,AXMI
      ...
      DATA SR2/1.414213562373095048801688724210D+00/
      DATA PI/3.141592653589793238462643383279D+00/
      DATA PREC/1D-2/
...
C...1 OR 2 DEPENDING ON CHARGINO TYPE
      ...
      DO 110 I=1,2
        DO 100 J=1,2
          VMIXC(J,I)=CMPLX(VMIX(J,I),VMIXI(J,I))
          UMIXC(J,I)=CMPLX(UMIX(J,I),UMIXI(J,I))
  100   CONTINUE
  110 CONTINUE
...
C...CHI_2+ -> CHI_1+ + Z0
      IF(AXMI.GE.AXMJ+XMZ) THEN
        LKNT=LKNT+1
        IJ=1
        OLPP=-VMIXC(IJ,1)*CONJG(VMIXC(IX,1))-
     &  VMIXC(IJ,2)*CONJG(VMIXC(IX,2))/2D0
        ORPP=-UMIXC(IX,1)*CONJG(UMIXC(IJ,1))-
     &  UMIXC(IX,2)*CONJG(UMIXC(IJ,2))/2D0
        GX2=ABS(OLPP)**2+ABS(ORPP)**2
        GLR=DBLE(OLPP*CONJG(ORPP))
...
C...CHARGED LEPTONS
     ...
     &  VMIXC(IJ,2)*CONJG(VMIXC(IX,2))/2D0
        ORPP=-UMIXC(IX,1)*CONJG(UMIXC(IJ,1))-
     &  UMIXC(IX,2)*CONJG(UMIXC(IJ,2))/2D0
        CXC(1)=CMPLX((T3I-XW*EI)/XW/XW1)*ORPP
        CXC(2)=CMPLX(0D0,0D0)
        CXC(3)=CMPLX((T3I-XW*EI)/XW/XW1)*OLPP
        CXC(4)=-VMIXC(IJ,1)*CONJG(VMIXC(IX,1))*CMPLX(T3I/XW)
        CXC(5)=-CMPLX(EI/XW1)*ORPP
        CXC(6)=CMPLX(0D0,0D0)
        CXC(7)=-CMPLX(EI/XW1)*OLPP
        CXC(8)=CMPLX(0D0,0D0)
...
C...NEUTRINOS
        ...
        CXC(1)=CMPLX((T3I-XW*EI)/XW/XW1)*ORPP
        CXC(3)=CMPLX((T3I-XW*EI)/XW/XW1)*OLPP
        CXC(4)=-UMIXC(IJ,1)*CONJG(UMIXC(IX,1))*CMPLX(T3I/XW)
        CXC(5)=-CMPLX(EI/XW1)*ORPP
        CXC(7)=-CMPLX(EI/XW1)*OLPP
        IF( XXC(5).LT.AXMI ) THEN
          XXC(5)=1D6
...
C...NO 3-BODY OPTION
        IF(AXMI.GE.AXMJ+XMH) THEN
          ...
          OLPP=(VMIXC(2,1)*CONJG(UMIXC(1,2))*EH(IH) -
     &    VMIXC(2,2)*CONJG(UMIXC(1,1))*DH(IH))/SR2
          ORPP=(CONJG(VMIXC(1,1))*UMIXC(2,2)*EH(IH) -
     &    CONJG(VMIXC(1,2))*UMIXC(2,1)*DH(IH))/SR2
          XMK=XMJ*ETAH(IH)
          GX2=ABS(OLPP)**2+ABS(ORPP)**2
          GLR=DBLE(OLPP*CONJG(ORPP))
...
C...CHI+_I -> CHI0_J + W+
      DO 220 IJ=1,4
        ...
        IF(AXMI.GE.AXMJ+XMW) THEN
          LKNT=LKNT+1
          DO 180 I=1,4
            ZMIXC(IJ,I)=CMPLX(ZMIX(IJ,I),ZMIXI(IJ,I))
  180     CONTINUE
          CXC(1)=(CONJG(ZMIXC(IJ,2))*VMIXC(IX,1)-
     &    CONJG(ZMIXC(IJ,4))*VMIXC(IX,2)/SR2)
          CXC(3)=(ZMIXC(IJ,2)*CONJG(UMIXC(IX,1))+
     &    ZMIXC(IJ,3)*CONJG(UMIXC(IX,2))/SR2)
          GX2=ABS(CXC(1))**2+ABS(CXC(3))**2
          GLR=DBLE(CXC(1)*CONJG(CXC(3)))
...
C...LEPTONS
  ...
  190     CONTINUE
          CXC(1)=(CONJG(ZMIXC(IJ,2))*VMIXC(IX,1)-
     &    CONJG(ZMIXC(IJ,4))*VMIXC(IX,2)/SR2)/SR2
          CXC(3)=(ZMIXC(IJ,2)*CONJG(UMIXC(IX,1))+
     &    ZMIXC(IJ,3)*CONJG(UMIXC(IX,2))/SR2)/SR2
          CXC(5)=CMPLX(0D0,0D0)
          CXC(7)=CMPLX(0D0,0D0)
          IA=11
          JA=12
          ...
          CXC(2)=VMIXC(IX,1)*CONJG(ZMIXC(IJ,1)*(EJ-T3J)*
     &    TANW+ZMIXC(IJ,2)*T3J)/SR2
          CXC(4)=-CONJG(UMIXC(IX,1))*(
...
C...CHI+_I -> CHI0_J + H+
      DO 230 IJ=1,4
        ...
        IF(AXMI.GE.AXMJ+XMHP) THEN
          LKNT=LKNT+1
          OLPP=CBETA*(ZMIXC(IJ,4)*CONJG(VMIXC(IX,1))+(ZMIXC(IJ,2)+
     &    ZMIXC(IJ,1)*TANW)*CONJG(VMIXC(IX,2))/SR2)
          ORPP=SBETA*(CONJG(ZMIXC(IJ,3))*UMIXC(IX,1)-
     &    (CONJG(ZMIXC(IJ,2))+CONJG(ZMIXC(IJ,1))*TANW)*
     &    UMIXC(IX,2)/SR2)
          GX2=ABS(OLPP)**2+ABS(ORPP)**2
          GLR=DBLE(OLPP*CONJG(ORPP))
...
C...~U_L D
     ...
     &    (ABS(CA)**2+ABS(CB)**2)+4D0*DBLE(CA*CONJG(CB))*XMF*XMI)
          IDLAM(LKNT,3)=0
          IF(MOD(J,2).EQ.0) THEN
            IDLAM(LKNT,1)=-KF1
            IDLAM(LKNT,2)=J
...
C...A 2-BODY -- 2-BODY CHAIN
      ...
      IF(AXMI.GE.XMJ) THEN
        ...
        XXC(9)=1D6
        XXC(10)=0D0
        OLPP=CMPLX(COS(RMSS(32)),SIN(RMSS(32)))
        ORPP=CONJG(OLPP)
        CXC(1)=CMPLX(0D0,0D0)
        CXC(3)=CMPLX(0D0,0D0)
        CXC(5)=CMPLX(0D0,0D0)
        ...
        CXC(4)=-CONJG(VMIXC(IX,1))*ORPP/SR2
        CXC(6)=CMPLX(0D0,0D0)
        CXC(8)=CMPLX(0D0,0D0)
        IF(XXC(5).LT.AXMI) THEN
          XXC(5)=1D6
