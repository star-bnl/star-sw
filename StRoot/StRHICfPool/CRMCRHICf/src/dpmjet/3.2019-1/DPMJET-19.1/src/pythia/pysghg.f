...
C...Unitarize partial wave amplitudes with Pade or K-matrix method
        ...
        ELSE
          A00U=(A00L+DBLE(A004))/(1D0-CMPLX(0.D0,A00L+DBLE(A004)))
          A20U=(A20L+DBLE(A204))/(1D0-CMPLX(0.D0,A20L+DBLE(A204)))
          A11U=(A11L+DBLE(A114))/(1D0-CMPLX(0.D0,A11L+DBLE(A114)))
        ENDIF
      ENDIF
...
C...W+ + W- -> h0
          ...
          DO 140 I=MMIN1,MMAX1
            ...
            EI=SIGN(1D0,DBLE(I))*KCHG(ABS(I),1)
            DO 130 J=MMIN2,MMAX2
              IF(J.EQ.0.OR.KFAC(2,J).EQ.0) GOTO 130
              EJ=SIGN(1D0,DBLE(J))*KCHG(ABS(J),1)
...
C...Strongly interacting Z_L/W_L model of Dobado, Herrero, Terron
     ...
     &      ABS(A20U+3D0*A11U*DBLE(CTH))**2
          ...
          DO 270 I=MMIN1,MMAX1
            ...
            VI=AI-4D0*EI*XWV
            AVI=AI**2+VI**2
            KCHWI=SIGN(1,KCHG(ABS(I),1)*SIGN(1,I))
            DO 260 J=MMIN2,MMAX2
              ...
              VJ=AI-4D0*EJ*XWV
              AVJ=AJ**2+VJ**2
              KCHWJ=SIGN(1,KCHG(ABS(J),1)*SIGN(1,J))
              NCHN=NCHN+1
              ISIG(NCHN,1)=I
              ISIG(NCHN,2)=J
              ISIG(NCHN,3)=1
...
C...Calculate loop contributions for intermediate gamma* and Z0
          CIGTOT=CMPLX(0D0,0D0)
          CIZTOT=CMPLX(0D0,0D0)
          JMAX=3*MSTP(1)+1
          DO 370 J=1,JMAX
            ...
            BABI=1D0/(BALP-BBET)
            IF(BALP.LT.1D0) THEN
              F0ALP=CMPLX(DBLE(ASIN(SQRT(BALP))),0D0)
              F1ALP=F0ALP**2
            ELSE
              F0ALP=CMPLX(DBLE(LOG(SQRT(BALP)+SQRT(BALP-1D0))),
     &        -DBLE(0.5D0*PARU(1)))
              F1ALP=-F0ALP**2
            ENDIF
            F2ALP=DBLE(SQRT(ABS(BALP-1D0)/BALP))*F0ALP
            IF(BBET.LT.1D0) THEN
              F0BET=CMPLX(DBLE(ASIN(SQRT(BBET))),0D0)
              F1BET=F0BET**2
            ELSE
              F0BET=CMPLX(DBLE(LOG(SQRT(BBET)+SQRT(BBET-1D0))),
     &        -DBLE(0.5D0*PARU(1)))
              F1BET=-F0BET**2
            ENDIF
  ...
  370     CONTINUE
          ...
          CIZTOT=CIZTOT*DBLE(XWC)/CMPLX(DBLE(SH-SQMZ),DBLE(GMMZ))
...
C...f + fbar' -> H+/-
     ...
     &        GOTO 520
              ...
              KCHHC=(KCHG(IA,1)*SIGN(1,I)+KCHG(JA,1)*SIGN(1,J))/3
...
C...H propagator: as simulated in PYOFSH and as desired
     ...
     &      (SQMHC-SQMQ-SH)/SH)
            KCHHC=SIGN(1,KCHG(IA,1)*SIGN(1,I))
            DO 540 ISDE=1,2
