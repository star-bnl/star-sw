...
C...(If the latter, also event will be picked here.)
        IF(MINT(111).GE.11.AND.ABS(IDWTUP).EQ.2.AND.LOOP.GE.2) THEN
          CALL UPEVNT
          CALL PYUPRE
        ELSEIF(MINT(111).GE.11.AND.ABS(IDWTUP).GE.3) THEN
          CALL UPEVNT
          CALL PYUPRE
          ISUB=0
  110     ISUB=ISUB+1
...
C...Find elastic slope and lower limit diffractive slope.
        IHA=MAX(2,ABS(MINT(103))/110)
        IF(IHA.GE.5) IHA=1
        IHB=MAX(2,ABS(MINT(104))/110)
        IF(IHB.GE.5) IHB=1
        IF(ISUB.EQ.91) THEN
...
C...Extract cross section event weight.
        IF(ABS(IDWTUP).EQ.1.OR.ABS(IDWTUP).EQ.4) THEN
          SIGS=1D-9*XWGTUP
        ELSE
          SIGS=1D-9*XSECUP(KFPR(ISUB,1))
        ENDIF
        IF(ABS(IDWTUP).GE.1.AND.ABS(IDWTUP).LE.3) THEN
...
C...cross-section used in weighting.
     ...
     &      VINT(22),VINT(23),VINT(26)
      ...
      ELSEIF(VIOL.GT.VINT(108)) THEN
        ...
        IF(VIOL.GT.1D0) THEN
          ...
          IF(ISTSB.EQ.11.AND.(ABS(IDWTUP).EQ.1.OR.ABS(IDWTUP).EQ.2))
     &    THEN
     ...
     &        XMAXUP(KFPR(ISUB,1))
            ENDIF
          ENDIF
          IF(ISTSB.NE.11.OR.ABS(IDWTUP).EQ.1) THEN
...
C...Calculate x value of photon for parton inside photon inside e.
      DO 350 JT=1,2
        ...
        IF(ABS(MINT(14+JT)).LE.8.OR.MINT(14+JT).EQ.21) MSPLI=MSPLI+1
        IF(MSPLI.EQ.2) THEN
          KFLH=MINT(14+JT)
          XHRD=VINT(140+JT)
          Q2HRD=VINT(54)
