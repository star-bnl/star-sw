...
C...Boost copied system to CM frame. Find CM energy and sum flavours.
  ...
  130 CONTINUE
      DO 140 I=NSAV+1,NSAV+NJET
        ...
        KFA=ABS(K(I,2))
        IF(KFA.LE.3) THEN
          NFI(KFA)=NFI(KFA)+SIGN(1,K(I,2))
        ELSEIF(KFA.GT.1000) THEN
          KFLA=MOD(KFA/1000,10)
          KFLB=MOD(KFA/100,10)
          IF(KFLA.LE.3) NFI(KFLA)=NFI(KFLA)+SIGN(1,K(I,2))
          IF(KFLB.LE.3) NFI(KFLB)=NFI(KFLB)+SIGN(1,K(I,2))
        ENDIF
  140 CONTINUE
...
C...New hadron. Generate flavour and hadron species.
  ...
  200     CALL PYKFDI(KFL1,0,KFL2,K(I,2))
          ...
          IF(IRANK.EQ.1.AND.ABS(KFL1).LE.10.AND.ABS(KFL2).GT.10) THEN
...
C...Find hadron mass. Generate four-momentum.
          ...
          MZSAV=0
          IF(ABS(KFL1).GE.4.AND.ABS(KFL1).LE.8.AND.MSTU(90).LT.8) THEN
            MZSAV=1
            MSTU(90)=MSTU(90)+1
            MSTU(90+MSTU(90))=I
            PARU(90+MSTU(90))=Z
...
C...Subtract off produced hadron flavours, finished if zero.
      DO 240 I=NSAV+NJET+1,N
        ...
        KFLC=MOD(KFA/10,10)
        IF(KFLA.EQ.0) THEN
          IF(KFLB.LE.3) NFL(KFLB)=NFL(KFLB)-SIGN(1,K(I,2))*(-1)**KFLB
          IF(KFLC.LE.3) NFL(KFLC)=NFL(KFLC)+SIGN(1,K(I,2))*(-1)**KFLB
        ELSE
          IF(KFLA.LE.3) NFL(KFLA)=NFL(KFLA)-SIGN(1,K(I,2))
          IF(KFLB.LE.3) NFL(KFLB)=NFL(KFLB)-SIGN(1,K(I,2))
          IF(KFLC.LE.3) NFL(KFLC)=NFL(KFLC)-SIGN(1,K(I,2))
        ENDIF
  240 CONTINUE
      NREQ=(ABS(NFL(1))+ABS(NFL(2))+ABS(NFL(3))-ABS(NFL(1)+
     &NFL(2)+NFL(3)))/2+ABS(NFL(1)+NFL(2)+NFL(3))/3
...
C...Take away flavour of low-momentum particles until enough freedom.
  ...
  260 CONTINUE
      ...
      IF(KFLA.EQ.0) THEN
        ISGN=SIGN(1,K(IREM,2))*(-1)**KFLB
        IF(KFLB.LE.3) NFL(KFLB)=NFL(KFLB)+ISGN
        IF(KFLC.LE.3) NFL(KFLC)=NFL(KFLC)-ISGN
      ELSE
        IF(KFLA.LE.3) NFL(KFLA)=NFL(KFLA)+SIGN(1,K(IREM,2))
        IF(KFLB.LE.3) NFL(KFLB)=NFL(KFLB)+SIGN(1,K(IREM,2))
        IF(KFLC.LE.3) NFL(KFLC)=NFL(KFLC)+SIGN(1,K(IREM,2))
      ENDIF
      NREM=NREM+1
      NREQ=(ABS(NFL(1))+ABS(NFL(2))+ABS(NFL(3))-ABS(NFL(1)+
     &NFL(2)+NFL(3)))/2+ABS(NFL(1)+NFL(2)+NFL(3))/3
...
C...Find combination of existing and new flavours for hadron.
  280 NFET=2
      ...
      IF(ABS(NFL(1))+ABS(NFL(2))+ABS(NFL(3)).EQ.0) NFET=0
      DO 290 J=1,NFET
        IFET(J)=1+(ABS(NFL(1))+ABS(NFL(2))+ABS(NFL(3)))*PYR(0)
        KFLF(J)=SIGN(1,NFL(1))
        IF(IFET(J).GT.ABS(NFL(1))) KFLF(J)=SIGN(2,NFL(2))
        IF(IFET(J).GT.ABS(NFL(1))+ABS(NFL(2))) KFLF(J)=SIGN(3,NFL(3))
  290 CONTINUE
     ...
     &.LT.0.OR.KFLF(1)*(NFL(1)+NFL(2)+NFL(3)).LT.0)) GOTO 280
      ...
      IF(NFET.EQ.1) KFLF(2)=SIGN(1+INT((2D0+PARJ(2))*PYR(0)),-KFLF(1))
      IF(NFET.LE.2) KFLF(3)=0
      IF(KFLF(3).NE.0) THEN
        KFLFC=SIGN(1000*MAX(ABS(KFLF(1)),ABS(KFLF(3)))+
     &  100*MIN(ABS(KFLF(1)),ABS(KFLF(3)))+1,KFLF(1))
        IF(KFLF(1).EQ.KFLF(3).OR.(1D0+3D0*PARJ(4))*PYR(0).GT.1D0)
     &  KFLFC=KFLFC+SIGN(2,KFLFC)
      ELSE
        KFLFC=KFLF(1)
      ENDIF
      ...
      DO 300 J=1,MAX(2,NFET)
        NFL(ABS(KFLF(J)))=NFL(ABS(KFLF(J)))-SIGN(1,KFLF(J))
  300 CONTINUE
