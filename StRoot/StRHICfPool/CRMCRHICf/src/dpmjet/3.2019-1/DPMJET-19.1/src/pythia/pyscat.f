...
C...Choose new quark/lepton flavour for relevant annihilation graphs
  ...
  200   CONTINUE
     ...
     &        OR.ISUB.EQ.316) THEN
          KFLF=5
          MINT(2)=MINT(2)-4
        ELSEIF(ISUB.EQ.382.AND.ITCM(5).EQ.1.AND.ABS(MINT(15)).LE.2
     &  .AND.ABS(KFLF).GE.3) THEN
...
C...f + fbar' -> W+/-
          KCH1=KCHG(ABS(MINT(15)),1)*SIGN(1,MINT(15))
          KCH2=KCHG(ABS(MINT(16)),1)*SIGN(1,MINT(16))
          KFRES=SIGN(24,KCH1+KCH2)
...
C...f + g -> f + g; th = (p(f)-p(f))**2
          ...
          IF(MINT(15).NE.21) KCS=SIGN(1,MINT(15))
          IF(MINT(16).NE.21) KCS=SIGN(1,MINT(16))
...
C...f + g -> f' + W+/-; th = (p(f)-p(f'))**2; choose flavour f'
          ...
          I=MINT(14+JS)
          IA=ABS(I)
          MINT(23-JS)=SIGN(24,KCHG(IA,1)*I)
          RVCKM=VINT(180+I)*PYR(0)
          DO 290 J=1,MSTP(1)
            IB=2*J-1+MOD(IA,2)
            IPM=(5-SIGN(1,I))/2
...
C...q + gamma -> q' + pi+/-; th=(p(q)-p(q'))**2
          ...
          I=MINT(14+JS)
          IA=ABS(I)
          MINT(23-JS)=SIGN(211,KCHG(IA,1)*I)
          IB=3-IA
          MINT(20+JS)=SIGN(IB,I)
          KCC=22
        ENDIF
...
C...f + q -> f' + Q; th = (p(f) - p(f'))**2
  ...
  410       KFANEW=KFANEW+2
     ...
     &        VCKM(KFANEW/2,(KFAOLD+1)/2)
          ...
          IF(MINT(2).EQ.1) THEN
            MINT(21)=SIGN(MINT(55),MINT(15))
            MINT(22)=SIGN(KFANEW,MINT(16))
          ELSE
            MINT(21)=SIGN(KFANEW,MINT(15))
            MINT(22)=SIGN(MINT(55),MINT(16))
            JS=2
          ENDIF
          KCC=22
...
C...g + g -> Q + Qbar + h0
          KCS=(-1)**INT(1.5D0+PYR(0))
          MINT(21)=SIGN(KFPR(ISUBSV,2),KCS)
          MINT(22)=-MINT(21)
...
C...q + qbar -> Q + Qbar + h0
          MINT(21)=SIGN(KFPR(ISUBSV,2),MINT(15))
          MINT(22)=-MINT(21)
          KCC=4
          KFRES=KFHIGG
...
C...f + fbar' -> W'+/-
          KCH1=KCHG(ABS(MINT(15)),1)*SIGN(1,MINT(15))
          KCH2=KCHG(ABS(MINT(16)),1)*SIGN(1,MINT(16))
          KFRES=SIGN(34,KCH1+KCH2)
 
        ELSEIF(ISUB.EQ.143) THEN
C...f + fbar' -> H+/-
          KCH1=KCHG(ABS(MINT(15)),1)*SIGN(1,MINT(15))
          KCH2=KCHG(ABS(MINT(16)),1)*SIGN(1,MINT(16))
          KFRES=SIGN(37,KCH1+KCH2)
 
        ELSEIF(ISUB.EQ.144) THEN
C...f + fbar' -> R
          KFRES=SIGN(41,MINT(15)+MINT(16))
 
        ELSEIF(ISUB.EQ.145) THEN
C...q + l -> LQ (leptoquark)
          IF(ABS(MINT(16)).LE.8) JS=2
          KFRES=SIGN(42,MINT(14+JS))
          KCC=28+JS
          KCS=SIGN(1,MINT(14+JS))
...
C...e + gamma -> e* (excited lepton)
          IF(MINT(15).EQ.22) JS=2
          KFRES=SIGN(KFPR(ISUB,1),MINT(14+JS))
          KCC=22
...
C...f + g -> f' + H+/-; th = (p(f)-p(f'))**2
          ...
          I=MINT(14+JS)
          IA=ABS(I)
          MINT(23-JS)=SIGN(37,KCHG(IA,1)*I)
...
C...q + g -> LQ + lbar; LQ=leptoquark; th=(p(q)-p(LQ))^2
          IF(MINT(15).EQ.21) JS=2
          MINT(20+JS)=SIGN(42,MINT(14+JS))
          KFLQL=KFDP(MDCY(42,2),2)
          MINT(23-JS)=-SIGN(KFLQL,MINT(14+JS))
          KCC=15+JS
          KCS=SIGN(1,MINT(14+JS))
...
C...q + qbar -> LQ + LQbar; LQ=leptoquark; th=(p(q)-p(LQ))**2
          MINT(21)=SIGN(42,MINT(15))
          MINT(22)=-MINT(21)
          KCC=4
 
        ELSEIF(ISUB.EQ.165) THEN
C...q + qbar -> l- + l+; th=(p(q)-p(l-))**2
          MINT(21)=SIGN(KFPR(ISUB,1),MINT(15))
          MINT(22)=-MINT(21)
 
        ELSEIF(ISUB.EQ.166) THEN
C...q + qbar' -> l + nu; th=(p(u)-p(nu))**2 or (p(ubar)-p(nubar))**2
          IF(MOD(MINT(15),2).EQ.0) THEN
            MINT(21)=SIGN(KFPR(ISUB,1)+1,MINT(15))
            MINT(22)=SIGN(KFPR(ISUB,1),MINT(16))
          ELSE
            MINT(21)=SIGN(KFPR(ISUB,1),MINT(15))
            MINT(22)=SIGN(KFPR(ISUB,1)+1,MINT(16))
          ENDIF
...
C...q + q' -> q" + q* (excited quark)
          ...
          JS=MINT(2)
          MINT(20+JS)=SIGN(KFQSTR,MINT(14+JS))
          IF(ABS(MINT(15)).NE.KFQEXC.AND.ABS(MINT(16)).NE.KFQEXC)
     &    MINT(23-JS)=SIGN(KFQEXC,MINT(17-JS))
          KCC=22
          JS=3-JS
...
C...f + fbar' -> rho_tc+/-
          KCH1=KCHG(ABS(MINT(15)),1)*SIGN(1,MINT(15))
          KCH2=KCHG(ABS(MINT(16)),1)*SIGN(1,MINT(16))
          KFRES=SIGN(KTECHN+213,KCH1+KCH2)
...
CMRENNA++
      ELSEIF(ISUB.LE.215) THEN
        IF(ISUB.EQ.201) THEN
C...f + fbar -> ~e_L + ~e_Lbar
          MINT(21)=SIGN(KSUSY1+11,KCS)
          MINT(22)=-MINT(21)
 
        ELSEIF(ISUB.EQ.202) THEN
C...f + fbar -> ~e_R + ~e_Rbar
          MINT(21)=SIGN(KSUSY2+11,KCS)
          MINT(22)=-MINT(21)
...
C...f + fbar -> ~mu_L + ~mu_Lbar
          MINT(21)=SIGN(KSUSY1+13,KCS)
          MINT(22)=-MINT(21)
 
        ELSEIF(ISUB.EQ.205) THEN
C...f + fbar -> ~mu_R + ~mu_Rbar
          MINT(21)=SIGN(KSUSY2+13,KCS)
          MINT(22)=-MINT(21)
...
C...f + fbar -> ~tau_1 + ~tau_1bar
          MINT(21)=SIGN(KSUSY1+15,KCS)
          MINT(22)=-MINT(21)
 
        ELSEIF(ISUB.EQ.208) THEN
C...f + fbar -> ~tau_2 + ~tau_2bar
          MINT(21)=SIGN(KSUSY2+15,KCS)
          MINT(22)=-MINT(21)
...
C...q + qbar' -> ~l_L + ~nulbar; th arbitrary
          KCH1=KCHG(ABS(MINT(15)),1)*SIGN(1,MINT(15))
          KCH2=KCHG(ABS(MINT(16)),1)*SIGN(1,MINT(16))
          MINT(21)=-SIGN(KFPR(ISUB,1),KCH1+KCH2)
          MINT(22)=SIGN(KFPR(ISUB,2),KCH1+KCH2)
 
        ELSEIF(ISUB.EQ.211) THEN
C...q + qbar'-> ~tau_1 + ~nutaubar; th arbitrary
          KCH1=KCHG(ABS(MINT(15)),1)*SIGN(1,MINT(15))
          KCH2=KCHG(ABS(MINT(16)),1)*SIGN(1,MINT(16))
          MINT(21)=-SIGN(KSUSY1+15,KCH1+KCH2)
          MINT(22)=SIGN(KSUSY1+16,KCH1+KCH2)
 
        ELSEIF(ISUB.EQ.212) THEN
C...q + qbar'-> ~tau_2 + ~nutaubar; th arbitrary
          KCH1=KCHG(ABS(MINT(15)),1)*SIGN(1,MINT(15))
          KCH2=KCHG(ABS(MINT(16)),1)*SIGN(1,MINT(16))
          MINT(21)=-SIGN(KSUSY2+15,KCH1+KCH2)
          MINT(22)=SIGN(KSUSY1+16,KCH1+KCH2)
 
        ELSEIF(ISUB.EQ.213) THEN
C...f + fbar -> ~nul + ~nulbar
          MINT(21)=SIGN(KFPR(ISUB,1),KCS)
          MINT(22)=-MINT(21)
 
        ELSEIF(ISUB.EQ.214) THEN
C...f + fbar -> ~nutau + ~nutaubar
          MINT(21)=SIGN(KSUSY1+16,KCS)
          MINT(22)=-MINT(21)
        ENDIF
...
C...th=(p(q)-p(chi+))**2 or (p(qbar)-p(chi-))**2
          KCH1=KCHG(ABS(MINT(15)),1)*SIGN(1,MINT(15))
          MINT(21)=SIGN(KSUSY1+24,KCH1)
          MINT(22)=-MINT(21)
 
        ELSEIF(ISUB.EQ.227) THEN
C...f + fbar -> ~chi+-2 + ~chi-+2
          KCH1=KCHG(ABS(MINT(15)),1)*SIGN(1,MINT(15))
          MINT(21)=SIGN(KSUSY1+37,KCH1)
          MINT(22)=-MINT(21)
...
C...if 15=qbar, 16=q and js=2, chi2+ + chi1-, th=(q-chi1-)**2
          ...
          IF(MINT(2).EQ.1) THEN
            MINT(21)= SIGN(KSUSY1+24,KCH1)
            MINT(22)= -SIGN(KSUSY1+37,KCH1)
c            IF(KCH2.EQ.0) JS=2
          ELSE
            MINT(21)= SIGN(KSUSY1+37,KCH1)
            MINT(22)= -SIGN(KSUSY1+24,KCH1)
            JS=2
c            IF(KCH2.EQ.1) JS=2
          ENDIF
...
C...CHECK THIS
          ...
          MINT(20+JS)=KSUSY1+22
          MINT(23-JS)=SIGN(KSUSY1+24,KCH1+KCH2)
...
C...q + qbar' -> ~chi01 + ~chi+-2
          ...
          IF(MOD(MINT(15),2).EQ.0) JS=2
          MINT(20+JS)=KSUSY1+22
          MINT(23-JS)=SIGN(KSUSY1+37,KCH1+KCH2)
...
C...qj + g -> ~qk_L + ~chi+-1
          ...
          I=MINT(14+JS)
          IA=ABS(I)
          MINT(23-JS)=SIGN(KSUSY1+24,KCHG(IA,1)*I)
          IB=-IA+INT((IA+1)/2)*4-1
          MINT(20+JS)=SIGN(KSUSY1+IB,I)
          KCC=15+JS
          KCS=SIGN(1,MINT(14+JS))
...
C...qj + g -> ~qk_L + ~chi+-2
          ...
          IB=-IA+INT((IA+1)/2)*4-1
          MINT(20+JS)=SIGN(KSUSY1+IB,I)
          MINT(23-JS)=SIGN(KSUSY1+37,KCHG(IA,1)*I)
          KCC=15+JS
          KCS=SIGN(1,MINT(14+JS))
...
C...f + fbar -> ~t_1 + ~t_1bar; th = (p(q)-p(sq))**2
          ...
          MINT(21)=ISGN*SIGN(KFPR(ISUB,1),KCS)
          MINT(22)=-MINT(21)
...
C...f + fbar -> ~t_1 + ~t_2bar; th = (p(q)-p(sq))**2
     ...
     &    (KCS.LT.0.AND.MINT(2).EQ.2)) THEN
            MINT(21)=SIGN(KFPR(ISUB,1),KCS)
            MINT(22)=-SIGN(KFPR(ISUB,2),KCS)
          ELSE
            JS=2
            MINT(21)=SIGN(KFPR(ISUB,2),KCS)
            MINT(22)=-SIGN(KFPR(ISUB,1),KCS)
          ENDIF
C...Correct color combination
          IF(MINT(43).EQ.4) KCC=4
...
C...qi + qj -> ~qi_L + ~qj_L
          KCC=MINT(2)
          IF(MINT(15)*MINT(16).LT.0) KCC=KCC+2
          MINT(21)=SIGN(KSUSY1+ABS(MINT(15)),MINT(15))
          MINT(22)=SIGN(KSUSY1+ABS(MINT(16)),MINT(16))
 
        ELSEIF(ISUB.EQ.272.OR.ISUB.EQ.282.OR.ISUB.EQ.292) THEN
C...qi + qj -> ~qi_R + ~qj_R
          KCC=MINT(2)
          IF(MINT(15)*MINT(16).LT.0) KCC=KCC+2
          MINT(21)=SIGN(KSUSY2+ABS(MINT(15)),MINT(15))
          MINT(22)=SIGN(KSUSY2+ABS(MINT(16)),MINT(16))
 
        ELSEIF(ISUB.EQ.273.OR.ISUB.EQ.283.OR.ISUB.EQ.293) THEN
C...qi + qj -> ~qi_L + ~qj_R
          MINT(21)=SIGN(KFPR(ISUB,1),MINT(15))
          MINT(22)=SIGN(KFPR(ISUB,2),MINT(16))
          KCC=MINT(2)
          IF(MINT(15)*MINT(16).LT.0) KCC=KCC+2
...
C...The two channels have the same cross section
          ...
          IF(MINT(15).NE.21)THEN
            KCS=SIGN(1,MINT(15))
            MUED(2)=472
            MUED(1)=KCS*(KKFLMI+ABS(MINT(15)))
            MINT(22)=IUEDEQ(472)
            MINT(21)=KCS*IUEDEQ(KKFLMI+ABS(MINT(15)))
          ENDIF
          IF(MINT(16).NE.21)THEN
            KCS=SIGN(1,MINT(16))
            MUED(2)=KCS*(KKFLMI+ABS(MINT(16)))
            MUED(1)=472
            MINT(22)=KCS*IUEDEQ(KKFLMI+ABS(MINT(16)))
            MINT(21)=IUEDEQ(472)
          ENDIF
...
C...The two channels have the same cross section
          ...
          IF(MINT(15).EQ.MINT(16))THEN
            MUED(1)=SIGN(1,MINT(15))*(KKFLMI+ABS(MINT(15)))
            MUED(2)=MINT(21)
            MINT(21)=SIGN(1,MINT(15))*IUEDEQ(KKFLMI+ABS(MINT(15)))
            MINT(22)=MINT(21)
          ELSE
            MUED(1)=SIGN(1,MINT(15))*(KKFLMI+ABS(MINT(15)))
            MUED(2)=SIGN(1,MINT(16))*(KKFLMI+ABS(MINT(16)))
            MINT(21)=SIGN(1,MINT(15))*IUEDEQ(KKFLMI+ABS(MINT(15)))
            MINT(22)=SIGN(1,MINT(16))*IUEDEQ(KKFLMI+ABS(MINT(16)))
          ENDIF
          IF(MINT(15)*MINT(16).LT.0) KCC=KCC+2        
...
C...The two channels have the same cross section
          ...
          IF(XFLAOUT.LE.0.2)THEN
            MUED(1)=SIGN(1,KCS)*(KKFLMI+1)
            MINT(21)=SIGN(1,KCS)*IUEDEQ(KKFLMI+1)
          ELSEIF(XFLAOUT.LE.0.4)THEN
            MUED(1)=SIGN(1,KCS)*(KKFLMI+2)
            MINT(21)=SIGN(1,KCS)*IUEDEQ(KKFLMI+2)
          ELSEIF(XFLAOUT.LE.0.6)THEN
            MUED(1)=SIGN(1,KCS)*(KKFLMI+3)
            MINT(21)=SIGN(1,KCS)*IUEDEQ(KKFLMI+3)
          ELSEIF(XFLAOUT.LE.0.8)THEN
            MUED(1)=SIGN(1,KCS)*(KKFLMI+4)
            MINT(21)=SIGN(1,KCS)*IUEDEQ(KKFLMI+4)
          ELSE
            MUED(1)=SIGN(1,KCS)*(KKFLMI+5)
            MINT(21)=SIGN(1,KCS)*IUEDEQ(KKFLMI+5)
          ENDIF
          MINT(22)=-MINT(21)
          MUED(2)=-MUED(1)
          KCC=MINT(2)+10
...
C...q + qbar'    -> q*_D + q*_S_bar'
          MUED(1)=SIGN(1,MINT(15))*(456+ABS(MINT(15)))
          MUED(2)=SIGN(1,MINT(16))*(450+ABS(MINT(16)))
          MINT(21)=SIGN(1,MINT(15))*IUEDEQ(456+ABS(MINT(15)))
          MINT(22)=SIGN(1,MINT(16))*IUEDEQ(450+ABS(MINT(16)))
          KCC=MINT(2)+2
 ...
 433      CONTINUE
          ...
          DO I=1,IUED(3)-1
            ...
            IF(XFLAOUT.LE.FLAVV)THEN                  
              MUED(1)=SIGN(1,MINT(15))*(KKFLMI+IOKFLA(I))
              MINT(21)=SIGN(1,MINT(15))*IUEDEQ(KKFLMI+IOKFLA(I))
              GOTO 435
            ENDIF
          ENDDO
 435      CONTINUE
          IF(ABS(MUED(1)).LT.451.AND.ABS(MUED(1)).GT.462)THEN
...
C...l + l -> H_L++/--, H_R++/--
          KCH1=KCHG(ABS(MINT(15)),1)*SIGN(1,MINT(15))
          KCH2=KCHG(ABS(MINT(16)),1)*SIGN(1,MINT(16))
          KFRES=SIGN(KFPR(ISUB,1),KCH1+KCH2)
 
        ELSEIF(ISUB.GE.343.AND.ISUB.LE.348) THEN
C...l + gamma -> l' + H++/--; th=(p(l)-p(H))**2
          IF(MINT(15).EQ.22) JS=2
          MINT(20+JS)=SIGN(KFPR(ISUB,1),-MINT(14+JS))
          MINT(23-JS)=SIGN(KFPR(ISUB,2),-MINT(14+JS))
          KCC=22
...
C...as inner process).
  ...
  450     CONTINUE
          KCC=22
          KFRES=SIGN(KFPR(ISUB,1),MINT(15))
...
C...f + fbar -> charged+ charged- technicolor
          ...
          MINT(21)=SIGN(KFPR(ISUB,1),KSW)
          MINT(22)=-SIGN(KFPR(ISUB,2),KSW)
...
C...f + fbar' -> neutral charged technicolor
          ...
          KCH2=KCHG(ABS(MINT(16)),1)*SIGN(1,MINT(16))
          IF(MINT(15)*(KCH1+KCH2).LT.0) JS=2
          MINT(23-JS)=SIGN(KFPR(ISUB,IC),KCH1+KCH2)
...
C...f + fbar' -> charged neutral technicolor
          ...
          KCH2=KCHG(ABS(MINT(16)),1)*SIGN(1,MINT(16))
          IF(MINT(15)*(KCH1+KCH2).GT.0) JS=2
          MINT(20+JS)=SIGN(KFPR(ISUB,IC),KCH1+KCH2)
...
C...g + g -> t + b + H+/-
          ...
          MINT(22)=SIGN(5,-KCS)
          KCC=11+INT(0.5D0+PYR(0))
          KFRES=SIGN(KFHIGG,-KCS)
 
        ELSEIF(ISUB.EQ.402) THEN
C...q + qbar -> t + b + H+/-
          KFL=(-1)**INT(1.5D0+PYR(0))
          MINT(21)=SIGN(INT(6.+.5*KFL),KCS)
          MINT(22)=SIGN(INT(6.-.5*KFL),-KCS)
          KCC=4
          KFRES=SIGN(KFHIGG,-KFL*KCS)
        ENDIF
...
C...QUARKONIA---
     ...
     $      CALL PYERRM(21,"No decays for resonance in Generic 2->2")
          KCI1=PYCOMP(MINT(15))
          KCI2=PYCOMP(MINT(16))
          ICOL1=SIGN(KCHG(KCI1,2),MINT(15))
          ICOL2=SIGN(KCHG(KCI2,2),MINT(16))
          KFF1=KFPR(ISUB,1)
          KFF2=KFPR(ISUB,2)
          KCF1=PYCOMP(KFF1)
          KCF2=PYCOMP(KFF2)
...
C...matrix element?
          IF(ISUB.EQ.402.AND.ABS(MINT(21)).EQ.5) JTA=3-JT
          IF(ABS(K(I,2)).LE.22) THEN
            P(I,5)=PYMASS(K(I,2))
          ELSE
  ...
  570   CONTINUE
     ...
     &  VINT(213)*(SHPR-P(IPU5,4))*SQRT(SQL12))/(2D0*PMS12)
        ...
        IF(ISUB.EQ.402.AND.ABS(MINT(21)).EQ.5) THEN
C...t and b in opposide order in event list as compared to
C...matrix element
...
C...Z0 + Z0 -> h0, W+ + W- -> h0: store Higgs and outgoing partons
        ...
        DO 580 JT=1,2
          ...
          K(IZW,1)=21
          K(IZW,2)=23
          IF(ISUB.EQ.8) K(IZW,2)=SIGN(24,PYCHGE(MINT(14+JT)))
          K(IZW,3)=IZW-2
          P(IZW,1)=-P(I,1)
          P(IZW,2)=-P(I,2)
...
C...Z0 and W+/- scattering: store bosons and outgoing partons
        ...
        DO 600 JT=1,2
          ...
          IF(MINT(14+JT).EQ.MINT(20+JT)) THEN
            K(IZW,2)=23
          ELSE
            K(IZW,2)=SIGN(24,PYCHGE(MINT(14+JT))-PYCHGE(MINT(20+JT)))
          ENDIF
          K(IZW,3)=IZW-2
          P(IZW,1)=-P(I,1)
          P(IZW,2)=-P(I,2)
          ...
          IF(ABS(K(IPU,2)).LE.10.OR.K(IPU,2).EQ.21) THEN
