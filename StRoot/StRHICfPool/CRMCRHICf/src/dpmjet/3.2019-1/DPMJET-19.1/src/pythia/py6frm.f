...
C...Check which fermion pairs are quarks and which leptons.
      ...
      IF(ABS(K(I3,2)).LT.10.AND.ABS(K(I4,2)).LT.10) THEN
        IQL34=1
      ELSEIF(ABS(K(I3,2)).GT.10.AND.ABS(K(I4,2)).GT.10) THEN
        IQL34=2
      ELSE
        CALL PYERRM(16,'(PY6FRM:) second fermion pair inconsistent')
      ENDIF
      IF(ABS(K(I5,2)).LT.10.AND.ABS(K(I6,2)).LT.10) THEN
        IQL56=1
      ELSEIF(ABS(K(I5,2)).GT.10.AND.ABS(K(I6,2)).GT.10) THEN
        IQL56=2
      ELSE
...
C...If t+tbar: loop over two top systems.
        DO 200 IT1=1,2
          ...
          ELSE
            ...
            IW2=I6
          ENDIF
          IF(ABS(K(IBO,2)).NE.5) CALL PYERRM(6,
