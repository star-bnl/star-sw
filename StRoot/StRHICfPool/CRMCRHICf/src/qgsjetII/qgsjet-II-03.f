...
C         S. Ostapchenko, Nucl.Phys.Proc.Suppl. 151 (2006) 143         C
C----------------------------------------------------------------------C
...
C                 Version qgsjet-II-03 (for CONEX)                     C
C=======================================================================
...
c-----------------------------------------------------------------------------
     ...
     *,qfan(51,11,11,6,120)
      ...
      version = 203.                                       !used to link with nexus
...
c Pomeron-hadron vertex
      fp(2)=(1.d0-qnorm2)*qggamfun(real(3.d0-alpd+ahl(2)))
     */qggamfun(real(1.d0+ahl(2)))/qggamfun(real(2.d0-alpd))
      do icz=1,3,2
       fp(icz)=qggamfun(real(3.d0-alpd+ahl(icz)))/qggamfun(real(2.d0
     * -alpd))/qggamfun(real(1.d0+ahl(icz)))*(1.d0-qnorm1)
      enddo
c check normalization
     ...
     *           ,form='unformatted')
       ...
       if (lzmaUse.ne.0) then
         ...
         call LzmaFillArray(qppdi,size(qppdi))
         call LzmaFillArray(qlegi,size(qlegi))
         call LzmaFillArray(qfanu,size(qfanu))
         call LzmaFillArray(qfan,size(qfan))
         call LzmaFillArray(qpomu,size(qpomu))
         call LzmaFillArray(qpom1,size(qpom1))
         ...
         call LzmaFillArray(qpomi0,size(qpomi0))
     ...
     *       ,qpomu,qpom1,qpomr,qlegc,qpomc,qpomi,qpomi0,gsect,fsud,qrt
      ...
      elseif(.not.producetables)then
        write(moniou,*) "Missing QGSDAT-II-03 file !"        
...
c integrated cut Pomeron eikonals
     ...
     * *qggamfun(real(1.d0-alpd+dels))**2*qggamfun(real(1.d0+ahl(icz)))
     * *qggamfun(real(1.d0+ahl(2)))/qggamfun(real(2.d0-alpd+dels
     * +ahl(icz)))/qggamfun(real(2.d0-alpd+dels+ahl(2)))
...
c radius
          rnuc(2)=1.19*float(ia(2))**(1./3.)-1.38*float(ia(2))**(-1./3.)
...
c Nuclear cross sections
      ...
      elseif(.not.producetables)then
        write(moniou,*) "Missing sectnu-II-03 file !"        
        write(moniou,*) "Please correct the defined path ",
     &"or force production ..."
        stop
       ...
       do iia2=1,6
        ...
        qgasect(ie,iia1,iia2)=log(gprod)
       enddo
       enddo
       enddo
       if(ifII03ncs.ne.2)then
...
c206   format(2x,'qgaini: new and old cross sections'
c     *,' for ordered ladder:'/4x,e10.3,3x,e10.3)
c207   format(2x,'qgaini: new and old cross sections'
c     *,' for general ladder:'/4x,e10.3,3x,e10.3)
c208   format(2x,'qgaini: hard cross sections finished')
...
c214   format(2x,'qgaini: integrated cut Pomeron leg eikonals')
...
c------------------------------------------------------------------------
      ...
      integer debug
      parameter(iapmax=208)
ctp   dimension vpau0(iapmax),vtau0(iapmax),vpau(iapmax),vtau(iapmax)
      dimension vpau0(iapmax),vtau0(iapmax)
     *,vpac(iapmax),vtac(iapmax)
ctp  *,vpac(iapmax),vtac(iapmax),vpac0(iapmax),vtac0(iapmax)
      common /qgarr1/  ia(2),icz,icp
      common /qgarr2/  scm,wp0,wm0
      common /qgarr7/  xa(iapmax,3),xb(iapmax,3),b
      common /qgarr16/ cc(2,3),iddp(iapmax),iddt(iapmax)
...
c-----------------------------------------------------------------------------
      implicit double precision (a-h,o-z)
      integer debug
      parameter(iapmax=208,npbmax=1000,npnmax=1000,npmax=5000
     *,legmax=900)                                              !so030206
...
c nuclear configurations
      ...
      do ii=1,ia(1)               !used to link with nexus
      do i=1,3
       xan(ii,i)=xa(ii,i)
      enddo
      enddo
...
ctp define collision type
      ...
      if(lqa(1).gt.0.and.lqb(1).gt.0)then  !ND (at least one cut P attached)
        typevt=1
     ...
     * .or.(iwp(1).eq.2.and.iwt(1).eq.2))then !DD
        typevt=2                           
       elseif(lqa(1).gt.0.or.iwp(1).eq.2)then  !SD
        typevt=4
       elseif(lqb(1).gt.0.or.iwt(1).eq.2)then  !SD
        typevt=-4  
       else                                 !CD
        typevt=3
       endif
...
c216   format(2x,'qgconf: ',i3,'-th process in the block contains'
c     *,' multi-Pomeron vertex')
c217   format(2x,'qgconf: ',i3,'-th process in the block has a'
c     *,' rap. gap with the proj.')
c218   format(2x,'qgconf: ',i3,'-th process in the block has a'
c     *,' rap. gap with the targ.')
...
c-----------------------------------------------------------------------
      implicit double precision (a-h,o-z)
      integer debug
      parameter(iapmax=208,npmax=5000,levmax=20,legmax=900)      !so030206
...
c normalization of rejection function
     ...
     ***(1.d0+ahl(2)))**(1.d0+dels-alpd))
      ...
      if(gb0.le.0.d0.or.gbcor10.le.0.d0.or.gbcor20.le.0.d0)then
       if(debug.ge.1)write (moniou,202)               !so030206
       iret=1
       goto 21
      endif
...
1     continue
      xpomr=scm**(-qgran(b10))               !proposed LC momentum for 3P-vertex
      gbcor1=(1.d0-(1.d0-(1.d0-xpomr)**(1.d0+ahl(icz)))
     ***(1.d0+dels-alpd))
      gbcor2=(1.d0-(1.d0-(1.d0-1.d0/xpomr/scm)
     ***(1.d0+ahl(2)))**(1.d0+dels-alpd))
      if(qgran(b10).gt.gbcor1*gbcor2*(gbcor1+gbcor2)
...
c vv9: 0 targ. legs and >1 proj. legs with "holes" (proj. diffr.)
     ...
     *-(vpac(ip)-vpac0(ip))*exp(-sumup))
      ...
      elseif(vv9.gt.0.d0)then
       ...
       if(debug.ge.1)write (moniou,220)vvt               !so030206
       iret=1
       goto 21
      endif
...
3      ntry=ntry+1
       ...
       if(ip.eq.ia(1).or.ntry.gt.100)then
        ...
        do i=1,nppr0
         if(qgran(b10).lt.vpac0(ip)/vpac(ip))then
          itypr0(i)=0
         else
          npprh0=npprh0+1
          itypr0(i)=1
        ...
        do ipp=ip+1,ia(1)
         ...
         if(ninc.ne.0)then
          ...
          do i=nppr0-ninc+1,nppr0
           if(qgran(b10).lt.vpac0(ipp)/vpac(ipp))then
            itypr0(i)=0
           else
            npprh0=npprh0+1
            itypr0(i)=1
...
4      ntry=ntry+1
       ...
       if(it.eq.ia(2).or.ntry.gt.100)then
        ...
        do i=1,nptg0
         if(qgran(b10).lt.vtac0(it)/vtac(it))then
          itytg0(i)=0
         else
          nptgh0=nptgh0+1
          itytg0(i)=1
        ...
        do itt=it+1,ia(2)
         ...
         if(ninc.ne.0)then
          ...
          do i=nptg0-ninc+1,nptg0
           if(qgran(b10).lt.vtac0(itt)/vtac(itt))then
            itytg0(i)=0
           else
            nptgh0=nptgh0+1
            itytg0(i)=1
...
6      ntry=ntry+1
     ...
     * -(vpac(ip)-vpac0(ip)+v1p-v1p0)*exp(-sumup)*(1.d0-exp(-sumup)))
      ...
      nptg=0
      npin=0
      if((jt.eq.5.or.jt.eq.2.and.npprh0.lt.nppr0).and.qgran(b10)
     ...
     *.or.jt.eq.2.and.npprh0.eq.nppr0.and.qgran(b10)
     *.lt.(vtac(it)-vtac0(it)+v1t-v1t0)*(exp(-sumut)*(1.d0-exp(-sumut))
...
7      xpomr1=xpomr/(xpomr*scm)**qgran(b10)
     ...
     * +qgpin0(xpomr/xpomr1,vvx)/sigs)*.7d0
      ...
      if((jt.eq.8.or.jt.eq.3.and.nptgh0.lt.nptg0).and.qgran(b10)
     ...
     *.or.jt.eq.3.and.nptgh0.eq.nptg0.and.qgran(b10)
     *.lt.(vpac(ip)-vpac0(ip)+v1p-v1p0)*(exp(-sumup)*(1.d0-exp(-sumup))
...
8      xpomr1=xpomr**qgran(b10)
     ...
     * **(1.d0+dels-alpd)
       do in=1,nprin
        n=0
22      xpomr1=xpomr**qgran(b10)
        gbcor1=1.d0-(1.d0-(1.d0-xpomr1)**(1.d0+ahl(icz)))
     *  **(1.d0+dels-alpd)
        if(qgran(b10).gt.gbcor1/gbcor10)goto 22
...
23    continue
     ...
     * **(1.d0+ahl(2)))**(1.d0+dels-alpd)
       do in=1,ntgin
        n=0
24      xpomr1=xpomr/(xpomr*scm)**qgran(b10)
        gbcor2=1.d0-(1.d0-(1.d0-1.d0/xpomr1/scm)
     *  **(1.d0+ahl(2)))**(1.d0+dels-alpd)
        if(qgran(b10).gt.gbcor2/gbcor20)goto 24
...
25    continue
     ...
     *.and.qgran(b10).lt.(v1p0*exp(-2.d0*sumup)
...
10     ii(m)=ii(m)+1                         !next cut fan in the vertex
     ...
     *  ,1.d0-exp(-sumut),iddp(ip1),icz,5))
       ...
       if(itypom.eq.0.and.qgran(b10).lt.v1p0/vpac0(ipp)
     ...
     *   -(vpac(ipp)-vpac0(ipp))*exp(-sumup))*(1.d0-vvxt)**2
        ...
        if(v3pt0.le.0.d0)then
         if(debug.ge.1)write (moniou,221)v3pt0               !so030206
         iret=1
         goto 21
        endif
...
11      xpomm(m+1)=xpomm(m)**qgran(b10)
        gbcor1=1.d0-(1.d0-(1.d0-xpomm(m+1))**(1.d0+ahl(icz)))
     *  **(1.d0+dels-alpd)
        if(qgran(b10).gt.gbcor1/gbcor10)goto 11
...
14      ntry=ntry+1
        ...
        if(ipp.eq.ia(1).or.ntry.gt.100)then
         ...
         do ip1=ipp+1,ia(1)
          ...
          if(ninc.ne.0)then
           ...
           do i=nppm(m+1)-ninc+1,nppm(m+1)
            if(qgran(b10).lt.vpac0(ip1)/vpac(ip1))then
             itypm(i,m+1)=0
            else
             npprh=npprh+1
             itypm(i,m+1)=1
        ...
        if(itypom.eq.0.and.npprh.eq.nppm(m+1).and.qgran(b10)
...
15    continue
     ...
     *.and.qgran(b10).lt.(v1t0*exp(-2.d0*sumut)
...
17     ii(m)=ii(m)+1                           !next cut fan in the vertex
     ...
     *  ,iddt(it1),2,5))
       ...
       if(itypom.eq.0.and.qgran(b10).lt.v1t0/vtac0(itt)            !single leg
     * .or.m.eq.levmax)then
        nptg=nptg+1
...
18      xpomm(m+1)=xpomm(m)/(xpomm(m)*scm)**qgran(b10)
        gbcor2=1.d0-(1.d0-(1.d0-1.d0/xpomm(m+1)/scm)
     *  **(1.d0+ahl(2)))**(1.d0+dels-alpd)
        if(qgran(b10).gt.gbcor2/gbcor20)goto 18
...
20        ntry=ntry+1
        ...
        if(itt.eq.ia(2).or.ntry.gt.100)then
         ...
         do it1=itt+1,ia(2)
          ...
          if(ninc.ne.0)then
           ...
           do i=nppm(m+1)-ninc+1,nppm(m+1)
            if(qgran(b10).lt.vtac0(it1)/vtac(it1))then
             itypm(i,m+1)=0
            else
             nptgh=nptgh+1
             itypm(i,m+1)=1
        ...
        if(itypom.eq.0.and.nptgh.eq.nppm(m+1).and.qgran(b10)
...
220   format(2x,'qg3pdf: vvt<0!',2x,'vvt=',e10.3)          !so030206
221   format(2x,'qg3pdf: v3pt0<0!',2x,'v3pt0=',e10.3)      !so030206
      return
      end
...
c-----------------------------------------------------------------------------
      ...
      integer debug
      parameter(iapmax=208,npbmax=1000,npnmax=1000,npmax=5000
     *,legmax=900,njmax=50000)                                  !so030206
     ...
     *     ,ic11,ic12,ic22,ic21,0,0)             !string hadronization
        ...
        if(nqs(npb).ne.0)then
         do n=1,nqs(npb)                         !loop over single Pomerons
          ...
          aks=qgran(b10)*(wsoft+wgg+wqg+wgq+wqq)
...
c to assure an efficient procedure
     ...
     *    .or.delf-alpd.lt.0.d0.and.alfl.le.0.d0)then
           up=1.d0-qgran(b10)**(1.d0/(1.d0-alpd+delf))
          ...
          elseif(delf-alpd.lt.0.d0.and.alfl.gt.0.d0)then
           up=-log(1.d0-qgran(b10)*(1.d0-exp(-alfl*(1.d0-alpd+delf))))
     *     /alfl/(1.d0-alpd+delf)
...
3     continue
      ...
      if(jini.eq.1)then                   !general ladder
       ...
       sjb=0.d0
       if(qgran(b10).lt.sj1/sj)then       !change to one-way ordered ladder
        jj=3-jj
        sj=sj1
...
c define color connections for the 2nd emitted jet
     ...
     * .and.i.ne.nvb.or.iqq.eq.3.and.i.ne.nva.and.i.ne.nvb)then
      ...
      enddo
c      if(ebal(1).gt.1.d-5)write (*,*)'ebal',ebal,iqq,jqq,nva,nvb,nj
...
1       zuk=qgran(b10)*cr1(jj)-1.d0
        if(zuk)2,2,3
2       tt=rnuc(jj)/wsnuc(jj)*(qgran(b10)**.3333d0-1.d0)
        goto 6
...
ctp      if(debug.ge.3)write (moniou,201)ep
      qgnrm=(ep(1)-ep(2))*(ep(1)+ep(2))-ep(3)**2-ep(4)**2

ctp      if(debug.ge.4)write (moniou,202)qgnrm
ctp201   format(2x,'qgnrm - invariant mass for ep=',4(e10.3,1x))
ctp202   format(2x,'qgnrm=',e10.3)
      return
      end
...
c---------------------------------------------------------------------------
      ...
      elseif(icz.eq.2)then
       ic1=int(1.3333d0+qgran(b10))   !valence quark type (for proton)
       if(ic1.eq.1)then
        ich1=int(qgran(b10)+.5d0)+2   !leading nucleon type
        ic2=1-ich1
...
c-----------------------------------------------------------------------------
      ...
      if(jexpr.gt.0)then                                 !so161205
       if(qgran(b10).gt.(1.d0-wex(icz))**jexpr)jexip=1   !so161205
      endif                                              !so161205
      ...
      if(qgran(b10).lt..5d0)then
       if(jexip.eq.1)then
        if(ddmax1.gt.ddmin1)then
         dmass1=ddmin1*(1.d0-qgran(b10)*(1.d0-(ddmax1/ddmin1)
     *   **(alpd-1.d0)))**(1.d0/(alpd-1.d0))
       ...
       if(jexit.eq.1)then
        ...
        if(ddmax2.gt.ddmin2)then
         dmass2=ddmin2*(1.d0-qgran(b10)*(1.d0-(ddmax2/ddmin2)
     *   **(alpd-1.d0)))**(1.d0/(alpd-1.d0))
        else
         if(iabs(izt).ge.7)then
          dmass2=dmmin(2)
...
c-----------------------------------------------------------------------------
      ...
      integer debug
      character *2 tyq
ctp      dimension wp(2),ic(2),ept(4),ep(4),ep1(4),ey(3),ey0(3)
...
1     sww=dsqrt(ww)
     ...
     *iab.eq.5.and.sww.gt.restm+am0+amc+wwm)then
       if(iab.le.2)then
        if(sww.gt.restm+2.d0*amn.and.qgran(b10).lt.dc(1))then
...
c-----------------------------------------------------------------------
      ...
      do i=1,4
       ep(i)=ep0(i)
      enddo
c      call qgtran(ep,ey0,1)    !no boost
...
1      xp=.5d0*qgran(b10)**(2.d0/(1.d0-alpd))
       if(qgran(b10).gt.(2.d0*(1.d0-xp))**(-.5d0*(1.d0+alpd)))goto 1
       wp1=wpi*xp
...
2      xm=.5d0*qgran(b10)**(2.d0/(1.d0-alpd))
       if(qgran(b10).gt.(2.d0*(1.d0-xm))**(-.5d0*(1.d0+alpd)))goto 2
       wm1=wmi*xm
...
201   format(2x,'qgcrossc - nucleus-nucleus interaction cross sections,'
     *,' N of iter.:',i5)
202   format(2x,'qgcrossc: gtot=',e10.3,2x,'gprod=',e10.3,2x,'gabs='
     *,e10.3/4x,'gdd=',e10.3,2x,'gqel=',e10.3,2x,'gcoh=',e10.3)
      return
      end
...
c-------------------------------------------------------------------------------
      ...
      if(iap.eq.1)then
       ...
       if(iat.eq.14)then
        do i=1,3
        do m=1,3
         qgsect=qgsect+qgasect(je+i-1,ja+m-1,5)*wk(i)*wa(m)
        enddo
        enddo
       elseif(iat.eq.40)then
        do i=1,3
        do m=1,3
         qgsect=qgsect+qgasect(je+i-1,ja+m-1,6)*wk(i)*wa(m)
        enddo
        enddo
       else
        do i=1,3
        do m=1,3
        do l=1,3
         qgsect=qgsect+qgasect(je+i-1,ja+m-1,jb+l-1)*wk(i)*wa(m)*wb(l)
        enddo
        enddo
        enddo
       endif
