...
int StPmtSignal::getAdc(int nPhotoElectrons, simulatorVersion version) {
    ...
    LOG_DEBUG << Form("hit has photoElectronGain = (%f/%d) ", nElectrons, nPhotoElectrons);
    if (nPhotoElectrons > 0) {LOG_DEBUG << Form("= %e", static_cast<float>(nElectrons/nPhotoElectrons)) << endm;}
    else                     {LOG_DEBUG << endm;}
