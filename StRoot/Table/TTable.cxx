...
void TTable::AsString(void *buf, EColumnType type, Int_t width,std::ostream &out) const
{
  ...
  //
   ...
   switch (type) {
      case kFloat:
         out << std::dec  << setw(width) << setprecision(width-3) << *(float *)buf;
         break;
      case kInt:
         out << std::dec  <<  setw(width) << *(int *)buf;
         break;
      case kLong:
         out << std::dec  << setw(width) << *(Long_t *)buf;
         break;
      case kShort:
         out << std::dec  << setw(width) << *(short *)buf;
         break;
      case kDouble:
         out << std::dec  << setw(width) << setprecision(width-3) << *(double *)buf;
         break;
      case kUInt:
         out << std::dec  << setw(width) << *(unsigned int *)buf;
         break;
      case kULong:
         out << std::dec  << setw(width) << *(ULong_t *)buf;
         break;
      case kUShort:
...
//             else              std::cout << "\t" << setw(strlen((*member).fColumnName)) << " ";
...
//             std::cout << std::endl;
		...
		if (indexOffset == arrayLength - 1) {
		  std::cout << "\t " << setw(strlen((*member).fColumnName));
		  for (Int_t i=0;i<dim;i++) {
		    std::cout << "["<<std::dec<<arrayLayout[i]<<"]";
		  }
		...
		if (indexOffset) std::cout << "\t " << setw(strlen((*member).fColumnName));
		for (Int_t i=0;i<dim;i++) {
		  std::cout << "["<<std::dec<<arrayLayout[i]<<"]";
		}
		LineOld = Line.str();
