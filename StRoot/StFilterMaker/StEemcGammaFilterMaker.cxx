...
Int_t StEemcGammaFilterMaker::Init()
{
    ...
    if (mEemcSamplingFraction <= 0) {
	LOG_ERROR << "EEMC sampling fraction is incorrect (" << mEemcSamplingFraction << ")!" << endm;
    }
    ...
    if (!mFilterMode)
      LOG_INFO << "Init() : Running the TEST mode (accepting all events). Set mFilterMode=1 to actually reject events in BFC"<< endm;
...
//     }
    ...
    if (this->getUseDbParams()) { // get parameter values from the database
