...
proc genericMacroFile {} {
    return {void doEstruct( const char* fileListFile,
            ...
            cent->setCentralities(mbBins,7);
            const  double ptCut[] = { 0.15, 0.5, 2.0};
            const  double ptMultCut[] = { 5, 156, 911 };
            cent->setPts(ptCut,3,ptMultCut,3);

            // Histogram centrality/pt selection so we can include them
            // in the data histogram files.
            TH1D *cenClass = new TH1D("cenClass","cenClass",8,-1.5,7.5);
            TH1D *impactDefs   = new TH1D("impactDefs","impactDefs",7,0.5,7.5);
            for (int i=0;i<7;i++) {
...
set path $env(MYDATA)/Data/auau130/minbias
...
set f   [open $path/scripts/auau130MinBiasData[pid].xml w]
...
catch {exec star-submit auau130MinBiasData[pid].xml} result
