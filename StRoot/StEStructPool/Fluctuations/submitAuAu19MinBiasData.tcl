...
#triggerWord,0,0	       # triggerWord cut
primaryVertexZ,-40,40      # primary vertex cut
centrality,1,700           # centrality cut
numTracks,1,700            # number of tracks per event passing track cuts
# ******************************************** 
# ******************************************** 
# *************** Track Cuts ***************** 
# *** format = variable,minvalue,maxvalue  *** 
...
proc genericMacroFile {} {
    return {void doEstruct( const char* fileListFile,
            ...
            // CANNOT FIND STAR STANDARD CENTRALITY CUTS
            // Following line is using all tracks passing cuts.
            const double mbBins[] = { 7, 47, 123, 350, 600 };
            cent->setCentralities(mbBins,5);
            const  double ptCut[] = { 0.15, 0.5, 2.0};
            const  double ptMultCut[] = { 7, 123, 600 };
            cent->setPts(ptCut,3,ptMultCut,3);

            // Histogram centrality/pt selection so we can include them
            // in the data histogram files.
            TH1D *cenClass = new TH1D("cenClass","cenClass",6,-1.5,5.5);
            TH1D *impactDefs   = new TH1D("impactDefs","impactDefs",5,0.5,5.5);
            for (int i=0;i<4;i++) {
...
<input URL="catalog:star.bnl.gov?production=P02ge,trgsetupname=minBias22GeVZDC,filetype=daq_reco_MuDst,tpc=1,ftpc=1,sanity=1,filename!~zerobias,storage!=HPSS" singleCopy="true" preferStorage="local" nFiles="all" />
...
set path $env(MYDATA)/Data/auau19/minbias
...
set f   [open $path/scripts/auau19MinBiasData[pid].xml w]
...
catch {exec star-submit auau19MinBiasData[pid].xml} result
