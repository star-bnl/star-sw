...
triggerWord,AuAu62GeVMinBias2004    	# triggerWord cut
primaryVertexZ,-30,30       		# primary vertex cut
...
proc genericMacroFile {} {
    return {void doEstruct( const char* fileListFile,
            ...
            // const double mbBins[] = { 9, 20, 38, 65, 102, 154, 222, 313, 373, 600 };
            // Following line is using all tracks passing cuts.
            const double mbBins[] = { 7, 15, 30, 55, 91, 141, 207, 294, 405, 549, 642, 900 };
            cent->setCentralities(mbBins,12);
            const  double ptCut[] = { 0.15, 0.5, 2.0};
            const  double ptMultCut[] = { 7, 141, 900 };
            cent->setPts(ptCut,3,ptMultCut,3);

            // Histogram centrality/pt selection so we can include them
            // in the data histogram files.
            TH1D *cenClass = new TH1D("cenClass","cenClass",13,-1.5,9.5);
            TH1D *impactDefs   = new TH1D("impactDefs","impactDefs",12,0.5,10.5);
            for (int i=0;i<12;i++) {
...
<input URL="catalog:star.bnl.gov?production=P04id,trgsetupname=production62GeV,filetype=daq_reco_MuDst,tpc=1,sanity=1,filename!~zerobias,storage!=HPSS" singleCopy="true" preferStorage="local" nFiles="all" />
<input URL="catalog:star.bnl.gov?production=P04ie,trgsetupname=production62GeV,filetype=daq_reco_MuDst,tpc=1,sanity=1,filename!~zerobias,storage!=HPSS" singleCopy="true" preferStorage="local" nFiles="all" />
...
set path $env(MYDATA)/Data/auau62/minbias
...
set f   [open $path/scripts/auau62MinBiasData[pid].xml w]
...
catch {exec star-submit auau62MinBiasData[pid].xml} result
