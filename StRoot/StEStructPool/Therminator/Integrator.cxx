...
Integrator::ReadMultInteg(ParticleDB *aDB)
{
  ...
  TString trIntName = sRPInstance->getPar("InputDirSHARE");
  trIntName += "/fintegrandmax_";
  trIntName += tHash;
  trIntName += ".txt";

  tFileIn = new ifstream(trIntName);
  if ((tFileIn) && (tFileIn->is_open())) {
    PRINT_MESSAGE("Reading Max Integrand values from " << trIntName.Data());
    ...
    PRINT_MESSAGE("Max Integrand file " << trIntName.Data() << " not found. Generating...");
    tFileOut = new ofstream(trIntName.Data());
  ...
  TString trMultName = sRPInstance->getPar("InputDirSHARE");
  trMultName += "/fmultiplicity_";
  trMultName += tHash;
  trMultName += ".txt";
  tFileIn = new ifstream(trMultName);
  if ((tFileIn) && (tFileIn->is_open())) {
    PRINT_MESSAGE("Reading Multiplicities from " << trMultName.Data());
  }
  else {
    PRINT_MESSAGE("Multiplicities file " << trMultName.Data() << " not found. Generating...");
    tFileOut = new ofstream(trMultName.Data());
