...
//
// example invocation:
// .x doRichTof.C(10,"some_directory/some_dst_file.xdf")
//
// example ROOT file invocation:
// .x doRichTof.C(10,"some_directory/some_dst_file.root")
//
// example multi-ROOT file invocation:
// .x doRichTof.C(9999,"some_directory/*.dst.root")
//
//////////////////////////////////////////////////////////////////////////////
//
...
void doRichTof()
{
    cout << "Usage: doRichTof.C(2)  // work with default event.root file" << endl;
    cout << "       doRichTof.C(startEvent,nEvents,\"path/somefile.event.root\")" << endl;
    cout << "       doRichTof.C(nEvents,\"path/*.event.root\")" << endl;
    cout << "       doRichTof.C(nEvents,\"path/file.dst.root\",\"evout\") //Write out StEvent" << endl;	
    cout << "       doRichTof.C(nEvents,\"path/file.dst.root\",\"display\") //EventDispay" << endl;	
    cout << "       doRichTof.C(nEvents,\"path/file.dst.root\",\"dbon\") //DB on" << endl;	
}
//		ProtoTypes

void doRichTof(Int_t nEvents, const Char_t ** fileList, const Char_t *qaflag =0);
...
void doRichTof(Int_t startEvent, Int_t nEventsQQ, const Char_t **fileList, const Char_t *qaflag)
{
  ...
  cout <<  endl << endl <<" doRichTof -  input # events = " << nEvents << endl;
  Int_t ilist=0;
  while(fileList[ilist]){ 
      cout << " doRichTof -  input fileList = " << fileList[ilist] << endl;
      ilist++; 
    }
  cout << " doRichTof -  input qaflag   = " << qaflag << endl;
...
//    StAnalysisMaker *analysisMaker = new StAnalysisMaker("analysis");
    ...
    strangeDst->SetNoKeep(); // Erase micro-DST after each event 

    StRichTofMuDstMaker *richTof = new StRichTofMuDstMaker("richTof");
...
void doRichTof(const Int_t startEvent, const Int_t nEvents, const Char_t *file, const Char_t *qaflag)
{
    printf("*file = %s\n",file);
    ...
    doRichTof(startEvent,nEvents,fileListQQ,qaflag);
}
//--------------------------------------------------------------------------
void doRichTof(const Int_t nEvents, const Char_t *file, const Char_t *qaflag)
{
    doRichTof(1,nEvents,file,qaflag);
}

//--------------------------------------------------------------------------
void doRichTof(const Int_t nEvents, const Char_t *path,const Char_t *file, const Char_t *qaflag, int flag)
{
    TString F;
    ...
    doRichTof(1,nEvents,F.Data(),opt.Data());
}

//--------------------------------------------------------------------------
void doRichTof(Int_t nEvents, const Char_t **fileList, const Char_t *qaflag)
{ doRichTof(1,nEvents,fileList,qaflag); }
...
// now unpack BfcStatus table in doRichTof so people can see if maker errors exist
//
...
// put Victor's code to write out .event.root file from doRichTof.C in here with a flag to turn off and on - now we can get rid of doRichTofOut.C ... don't have to keep up 2 sets of macros
//
...
// make TurnDisplay macro available for doRichTof
//
