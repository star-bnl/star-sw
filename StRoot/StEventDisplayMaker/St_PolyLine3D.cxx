...
// $Id: St_PolyLine3D.cxx,v 1.4 2007/02/20 22:29:14 fine Exp $ 
// ***********************************************************************
...
//*-*                      ================================
  fPoints = new St_Points3D(points);
}
      ...
      , Option_t *)
{
   ...
   Int_t size=Size();
   out<<"pline3D = new St_PolyLine3D("<<fPoints->GetN()<<","<<quote<<GetOption()<<quote<<");"<<endl;
 
   // SaveLineAttributes(out,"pline3D",1,1,1);
 
   if (size > 0) {
    for (Int_t i=0;i<size;i++) {
       out<<"   pline3D->SetPoint("<<i<<","<<fPoints->GetX(i)<<","<<fPoints->GetY(i)<<","<<fPoints->GetZ(i)<<");"<<endl;
    }
   }
...
//*-*                  ====================================
   if (fPoints) fPoints->SetPoints(n,p,option);
}
...
// Revision 1.4  2007/02/20 22:29:14  fine
