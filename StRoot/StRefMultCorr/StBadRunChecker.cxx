...
| "run19"  | "col"         | "19.6"    | "auau"  |
*/
StBadRunChecker::StBadRunChecker(TString Run,TString CollisionMode, TString RunEnergy,
                                 TString Species) {
  Run.ToLower();
  RunEnergy.ToLower();
...
void StBadRunChecker::readBadRunsFromHeaderFile() {
  switch(mRunIndex) {
    case 0:
      ...
      std::cout << "read in bad run list for " << mRun << " " << mColMode << " " 
                << mSpecies << " " << mEnergy << " GeV. " << nBadRun_run19_col_19p6_auau 
                << " bad runs in total." << endl;
      break;
      /*
      case 1:
          for(int i=0;i<2;i++){
      ...
      */
      //Add more runs here
      default:
        std::cout << mRun << " " << mColMode << " " << mSpecies 
                  << " " << mEnergy << " GeV doesn't exist or the bad run list is not available yet.Exit!" << std::endl;
        exit(0);
  }
}
...
Bool_t StBadRunChecker::isBadRunSubSys(const Int_t RunId,TString mS) {
  ...
  if (RunId<mRunRange[0]||RunId>mRunRange[1]) {
    std::cout << "Warning: " << RunId << " is not in " << mRun << " " << mColMode << " " 
              << mSpecies << " " << mEnergy << "GeV. Exit!" << std::endl;
    exit(0);
  }
  mS.ToLower(); 
  std::vector<Int_t>::iterator iter = std::find(mBadRun_all.begin(), mBadRun_all.end(), RunId);
  ...
  //return ( iter != mBadRun.end() ) ;
    ...
    for(int iname=0;iname<12;iname++){
      ...
      if(mS.Contains(mSubSysName[iname])){
        std::cout << "Check if this run is bad for: " << mSubSysName[iname] << std::endl;
