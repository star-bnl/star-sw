...
Knowing if a track is a {\sc tpt} track or an {\sc ittf} track is done by quering {\tt StTrack->fitting\-Me\-thod()}, and checking if it's equal to or different than a specific value that signs the {\sc ittf} tracks. This value is stored in the member-variable {\tt ITTFflag}.
...
\begin{maliste}
\item Bfield is assigned 0 in the constructor : no need because it's being assigned $1.10^{-10}$ afterwards.
...
\item Comment ``Secondary cut" in the V0Finder should be replaced with ``{\em Primary and} secondary", for those cuts are used (as written in the DB files themselves) for both primary and secondary V0's.
