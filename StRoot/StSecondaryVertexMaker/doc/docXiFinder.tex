...
Before the loops, tables called {\tt ptrks}, {\tt ntrks}, etc..., are filled in the function {\tt StV0FinderMa\-ker::Pre\-pare()}, and the values used afterwards are those that are stored in these tables, in order to improve the speed of the code. The XiFinder doesn't use all of them yet... but will.
...
{\footnotesize\bf CUT \hspace{-30pt} CUT} if one track doesn't point away from Pvx \\
...
\item DcaTrackToPvx : same as above, see also the next paragraph
\item Decay length : the calculated V0 decay length has to be $>$ {\tt DB->dlen}
...
As said previously, the like-sign method in the XiFinder consists in finding candidates build with one of the daughters being the antiparticle of the expected daughter. So we find and store $\Lambda + \pi^-$ (resp. $K^-$ for the $\Omega$) and $\overline{\Lambda} + \pi^+$ (resp. $K^+$).
...
\item Symmetry : a track is transformed into its symmetric with respect to the $(xOy)$ plane,
\item Rotating + symmetry : doing both transofmations together is equivalent to taking the symmetric of the track with respect to the primary vertex.
\end{maliste}
...
\item {\tt kRotatingUseSymmetry} = 2 : the bachelor tracks are ``symmetrised" with respect to the bending plane,
...
In the code, all rotating methods calculations are achieved in one shot, thanks to the pre-definition of a couple of interesting variables which I describe below. In the code, once the bachelor helix is moved, nothing else is changed by the use of a rotating-like method. So what is done is simply the booking (and then usage) of a StHelixModel called {\tt bachGeom} from both the initial parameters of the helix and the ``interesting variables", whose value is set at the beginning of the XiFinder, before the loop on the bachelor tracks.
...
The combination of the values in this table and the transformations listed just above it give the mathematical transformations listed in the table p. \pageref{MathsRotating}. This avoids a check of the rotating choice by an if-loop inside the for-loop on the tracks, and thus time-consuming jumps in the code.
