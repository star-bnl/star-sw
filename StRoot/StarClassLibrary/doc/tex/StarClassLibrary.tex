...
%
% $Id: StarClassLibrary.tex,v 1.1 1999/02/17 12:37:56 ullrich Exp $
%
...
\begin{center}
  ...
  {\LARGE $ $Date: 1999/02/17 12:37:56 $ $}  % replaced by cvs with current revision  
  \vfill
\end{center}
\cleardoublepage
\end{titlepage}
...
\textit{The following is of general interest for software developer only:}

\index{Standard Template Library}
\index {STL} etc.  It was also felt that additional functionality specific
It is divided up in five directories:
\name{./include}, \name{./src}, \name{./lib}, \name{./examples}, and
\name{./doc}.
...
the \comp{examples} directory in the library itself.  Web based documentation,
    and \name{./doc/html} for a class browser.
\item[\name{./include}] contains the different header files
    (extension \name{.h} and \name{.hh}),
\item[\name{./src}] the referring source code (extension \name{.cc}),
These developments have been made in consultation with the CERN LHC++
    demonstrate most the features of every classes.
\item[\comp{./lib/\$STAR\_SYS}] contains the shared and/or
    archive library for the platform identified by \comp{\$STAR\_SYS}.
...
There are two Makefiles in the top directory \name{StarClassLibrary}
\vspace{-11pt}
\begin{description}
\item[\name{GNUmakefile}] is the general makefile used to compile and install
    the SCL and its documentation
\item[\name{GNUmakefile.architecture}] defines the macros and compiler flags
    for each individual supported platform.
\end{description}

The \name{./examples} directory contains a separate makefile which is not
invoked when installing the SCL. It must be run manually. 
It also used the flags defined in \name{GNUmakefile.architecture}.

All makefiles are especially written for the GNU make utility (gmake) and rely on
...
Note that these makefiles do not contain any hard-wired file names;
the dependencies are rather created on-the-fly.


\section{Installing the SCL}  \index{Installing SCL} \label{instal}

  \end{itemize}
and is therefore present in the actual STAR software releases.\\
If you want to install the SCL locally on your machine you should
continue to read the following three sections which explains how to
obtain, compile and install the code.
  \begin{itemize}
\subsection{Getting the source code} 

The {\em StarClassLibrary} is now under {\bf CVS} control at BNL.  It can
...
Further tests on AIX are not foreseen unless there is sufficient user demand. \index{Irix} \index{AIX}
    \comp{cvs checkout StarClassLibrary}
\item HP-UX 10.20: \index{HP-UX}
    \item egcs 1.0.2 (g++) or higher. 
\subsection{Compilation}  \index{Compiling SCL} \label{compile}

In order to create the library and (optionally) the referring examples
first {\bf set the correct system flag}. The flag is stored in the environment
variable \comp{STAR\_SYS}. If you are using the official STAR login scripts
it should be already defined. Note, however, that the SCL is not yet supported
...
\begin{itemize}
  \item set the correct system flag:\\
    ...
    \item \comp{sun4x\_56}
    \end{itemize}
  \item \comp{cd StarClassLibrary}
  \item \comp{gmake}  \\ creates the library
  \item \comp{gmake doc} \\ makes the documentation and the class browser 
\end{itemize}
...
installation) \index{egcs} \index{gcc} by defining \comp{STAR\_SYS=i386\_linux2}.  Note also
that ``unsupported platform'' does not necessarily mean that the SCL will not
...
If the SCL is installed properly they should be defined
...
\item[\comp{SCL\_WITHOUT\_ROOT}:] makes the SCL without ant reference to ROOT classes.
    If not set several classes will inherit from the ROOT base class TObject which
    causes the SCL to depend an essentially all of the ROOT class library.\index{ROOT}
    \name{GNUmakefile}
...
Use \comp{gmake doc} to craete all available SCL documentation.
It consists of the {\em User Guide and
\end{description}    
\end{description}
...
\begin{table}[htb]
  ...
  and constants are still defined to be of type:\\ \comp{static const HepDouble}. \\ \\
...
int main()
{
    ...
    \comp{NOTE:} The user must link with \texttt{packlib}.\footnote{The
      process is somewhat more complicated for SUN.}  This is
    set up in the \\ \comp{GNUmakefile.architecture} file according
    to the {\bf CERN\_ROOT}\label{CERN_ROOT} environmental variable
    set in the STAR login scripts.
...
\item[Public Member\\ Functions]
    It is possible to set and access the data members within the class
    via the names associated with a space-time vector:
    Given a position \args{s} in space, returns the path length of the helix
