...
public:
    ...
    void Set (double A, double Z, double Dens, double x0, double p,const TGeoMaterial *mate,double curv=0);
    void Set (const TGeoMaterial *mate,                   double p,double curv=0);
///???    void Set (double p);
    void Add(double len);
  ...
  double PLoss(double p=0.) 	const;	
  double dPLossdP0(double p) 	const;	
  double TotLen() 		const	{return fTotLen;}	
  double Perp() const;		// space deviation normal to track in xy plane
  double Dang() const;		// Phi   deviation 
