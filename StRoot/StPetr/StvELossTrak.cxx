...
void StvELossTrak::Set(double A, double Z, double dens, double x0, double p
                      ,const TGeoMaterial *mate, double curv)
{
  fdEdX=0;
  fCurv = curv;
  if (p<=0) p = fP[1];
...
//_____________________________________________________________________________
void StvELossTrak::Set(const TGeoMaterial *mate,double p,double curv)
{
    Set(mate->GetA(),mate->GetZ(),mate->GetDensity(),mate->GetRadLen(),p,mate,curv);
}
//______________________________________________________________________________
//_____________________________________________________________________________
...
assert(mat);
    Set(aux.fA, aux.fZ, aux.fDens, aux.fX0, fP[1],mat,fCurv);
  }

}
//_____________________________________________________________________________
double StvELossTrak::Perp() const
{
// C == Curvature
// k == dC/dl
// l == track length 
...
//  Dang = (Ploss/P)*C*L/2
// ============================
   return (PLoss()/P())*fCurv*TotLen()*TotLen()/6;
}
//_____________________________________________________________________________
double StvELossTrak::Dang() const
{
   return (PLoss()/P())*fCurv*TotLen()/2;
}
//_____________________________________________________________________________
...
//_____________________________________________________________________________
double StvELossTrak::PLoss(double pp) const 
{
  if (gNoEloss) return 0;
  double p = (pp)? pp: 0.5*(fP[0]+fP[1]);
  double ep = (fM/p)*(fM/p);
...
{"Vacuum",	16,	1e-16,		 1e-16,	1e-16,		1e16   }};
  ...
  StvELossTrak *el = new StvELossTrak;
  ...
  for (int imat=0;imat<15;imat++) {
    ...
    for (int jk=0;jk<2;jk++) {
      ...
      el->Set(mat,p,curv);
      el->Add(len);
      printf("%s \tLen=%g \tP=%g \tPloss=%g \tTheta2=%g Perp=%g Dang=%g\n",
      mat->GetName(),el->TotLen(),el->P(),el->PLoss(p),el->GetTheta2(),el->Perp(),el->Dang());
      if (jk) printf("\n");
      len = -len;
    }
    delete mat;
