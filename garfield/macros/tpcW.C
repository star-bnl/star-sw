...
void tpc(TString IO = "Inner1170", Int_t nEvents = 1) {
  ...
  //  gas->SetFieldGrid(100.,1e3, 20, true, BField, BField, 1, Angle, Angle, 1);
  //  gas->SetFieldGrid(100.,1e3, 20, true, BField, BField, 1, Angle, Angle, 1);
  ...
  const Double_t yC       = -10;     // pseudo cathode
  ...
  Double_t dzBox = 20;
  Double_t dxBox = 0.5 * (nRep - 1) * perSens + 1.0;
  Double_t dyBox = -yC;
  SolidBox* box = new SolidBox(0., 0., 0., dxBox, dyBox, dzBox);
  ...
  comp->SetPeriodicityX(perSens);
  // Add the anode wires.
  for (Int_t i = 0; i <= nRep; ++i) {
    Double_t x = perSens * ((2*i - nRep + 1)/2);
    comp->AddWire((i - 2.) * perSens, ySens, dSens, vAnode, "s");
  }
  Int_t nc = TMath::Nint(aWpitch/cWpitch);
  // Add the cathode wires.
  for (Int_t i = 0; i < nRep*nc; ++i) {
  ...
  TCanvas *cc = 0; // new TCanvas("Cell","Cell");
  ...
  TCanvas *cd = 0; // new TCanvas("Drift","Drift");
  ViewDrift* vd = new ViewDrift();
  vd->SetCanvas(cd);
#if 0
  vd->SetArea(-nRep * perSens / 2., -yPad, -2.,           
	       nRep * perSens / 2.,  yPad,   2.); 
#else
  vd->SetArea(-10, -10, -10, 10, 10, 10);
#endif
