...
//________________________________________________________________________________
void tpcCF4(Int_t nEvents = 0, const Char_t *OutName = "GL.root") {
 ...
 //aval->EnableNullCollisionSteps();
  ...
  for (Int_t i = nEvents; i--;) {
	...
	continue;
      ...
      const std::vector<DriftLineRKF::step> &path_i = driftline_i->m_path;
      UInt_t nI = path_i.size();
      for (auto step : path_i) {
	//	ion_status->Fill(step.status);
	//	ion_status->Fill(step.m_status);
	//	if (step.status) continue;
        X = step.x; //(step.xf + step.xi)/2;
        Y = step.y; // (step.yf + step.yi)/2;
        T = step.t; //(step.tf + step.ti)/2;
	//        dX = step.xf - step.xi;
	//        dY = step.yf - step.yi;
        L = 1; // TMath::Sqrt(dX*dX + dY*dY);
	xy_ion->Fill(X,Y);
	xy_ionL->Fill(X,Y,L);
