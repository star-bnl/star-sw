...
void AtomPhotoAbsCS::get_escape_particles(
  ...
  if (main_n <= 0) {
    ...
    const double en1 = get_threshold(nshell) - hdist - 2 * get_threshold(n_min);
    el_energy.push_back(en);
  ...
  if (main_n_largest - main_n < 2) {
    ...
    double en1 = get_threshold(nshell) - hdist - 2 * get_threshold(n_min);
    el_energy.push_back(en);
    if (en1 >= 0.0) el_energy.push_back(en1);
    return;
  }
  ...
  if (en1 > 0.) {
    ...
    const double en2 = get_threshold(n_chosen) - 2 * get_threshold(n_min);
    if (en2 > 0.) {
...
int MolecPhotoAbsCS::get_total_Z() const {
  ...
  const long q = qatom_ps.size();
  int s = 0;
  for (long n = 0; n < q; n++) s += atom[n]->get_Z();
  return s;
}
