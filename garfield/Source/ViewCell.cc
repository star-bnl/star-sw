...
ViewCellWire::ViewCellWire(const double x, const double y, const double z,
                           ...
                           const int type)
    : TObject(), x0(x), y0(y), z0(z), r(10.e-4), l(50.), wireType(type) {
...
TBuffer3D& ViewCellWire::GetBuffer(bool& ok, const bool debug) {
...
ViewCellPlane::ViewCellPlane(const double center, const bool vert,
                             const double size)
    : TObject(), planeCenter(center), isVertical(vert), planeSize(10.) {
...
TBuffer3D& ViewCellPlane::GetBuffer(bool& ok, const bool debug) {
...
ViewCellTube::ViewCellTube(const double x, const double y, const double z,
    ...
    : TObject(), x0(x), y0(y), z0(z), r(1.), n(0) {
...
TBuffer3D& ViewCellTube::GetBuffer(bool& ok, const bool debug) {

  if (n == 0) return GetBufferCylinder(ok, debug);
  return GetBufferPolygon(ok, debug);
}

TBuffer3D& ViewCellTube::GetBufferCylinder(bool& ok, const bool debug) {
...
TBuffer3D& ViewCellTube::GetBufferPolygon(bool& ok, const bool debug) {
  ...
  if (debug) {
    ...
    for (int i = 0; i < 4 * n; ++i) {
      std::cout << "      " << i << "      " << tube.fPols[6 * i + 2]
                << "      " << tube.fPols[6 * i + 3] << "      "
                << tube.fPols[6 * i + 4] << "      " << tube.fPols[6 * i + 5]
                << "\n";
    }
  }
...
void ViewCell::SetArea(double xmin, double ymin, double zmin, double xmax,
...
void ViewCell::SetArea() { hasUserArea = false; }
...
void ViewCell::Draw(Option_t* option) {
...
void ViewCell::PlotLine(const double x0, const double y0, const double x1,
                        const double y1) {
