...
bool ComponentAnalyticField::CellCheck() {
  ...
  for (unsigned int i = 0; i < m_nWires; ++i) {
    ...
    for (unsigned int j = i + 1; j < m_nWires; ++j) {
      ...
      std::cerr << m_className << "::CellCheck:\n    Wires " 
                << m_w[i].type << " at (" << xprti << ", " << yprti << ") and "
                << m_w[j].type << " at (" << xprtj << ", " << yprtj 
  ...
  for (int i = m_nWires; i--;) {
    const double rw = 0.5 * m_w[i].d;
    if (setx) {
      m_xmin = std::min(m_xmin, m_w[i].x - rw);
      m_xmax = std::max(m_xmax, m_w[i].x + rw);
    } else {
      m_xmin = m_w[i].x - rw;
      m_xmax = m_w[i].x + rw;
      setx = true;
    ...
    if (sety) {
      m_ymin = std::min(m_ymin, m_w[i].y - rw);
      m_ymax = std::max(m_ymax, m_w[i].y + rw);
    } else {
      m_ymin = m_w[i].y - rw;
      m_ymax = m_w[i].y + rw;
      sety = true;
    ...
    if (setz) {
      m_zmin = std::min(m_zmin, -0.5 * m_w[i].u);
      m_zmax = std::max(m_zmax, +0.5 * m_w[i].u);
    } else {
      m_zmin = -0.5 * m_w[i].u;
      m_zmax = +0.5 * m_w[i].u;
      setz = true;
...
void ComponentAnalyticField::ConformalMap(const std::complex<double>& z,
  ...
  if (z == 0.) {
    ...
    double arot = -TwoPi *
                  ...
                  m_ntube;
    const std::complex<double> zz = z * std::complex<double>(cos(arot), sin(arot));
    // Expand in a series.
...
void ComponentAnalyticField::WfieldStripZ(const double xpos, const double ypos,
  ...
  switch (ip) {
    case 0:
      xw = -ypos + 0.5 * (theStrip.smin + theStrip.smax);
      yw = xpos - m_coplan[ip];
      break;
    case 1:
      xw = ypos - 0.5 * (theStrip.smin + theStrip.smax);
      ...
      break;
    case 2:
      xw = xpos - 0.5 * (theStrip.smin + theStrip.smax);
      yw = ypos - m_coplan[ip];
      break;
    case 3:
      xw = -xpos + 0.5 * (theStrip.smin + theStrip.smax);
  ...
  const double w = 0.5 * fabs(theStrip.smax - theStrip.smin);
...
void ComponentAnalyticField::WfieldStripXy(const double xpos, const double ypos,
  ...
  switch (ip) {
    case 0:
      xw = -zpos + 0.5 * (theStrip.smin + theStrip.smax);
      yw = xpos - m_coplan[ip];
      break;
    case 1:
      xw = zpos - 0.5 * (theStrip.smin + theStrip.smax);
