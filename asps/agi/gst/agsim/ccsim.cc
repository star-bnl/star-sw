...
/*CMZ :  2.00/00 20/10/99  16.07.04  by  Pavel Nevski*/
/*CMZ :  1.40/05 26/08/98  20.14.15  by  Pavel Nevski*/
/*CMZ :  1.40/05 24/08/98  09.45.12  by  Pavel Nevski*/
...
{/* just avoid stupid warning */ return 1;}
...
#ifdef CRAIG
// will work with the newest dsl sometime
  tDs=NULL;
  pDs=dDs=NULL;
...
{  tdmDataset*   tDs  = NULL;
   ...
   if (!ds) { printf (" tdm_zero_all: no path to %s \n",path); return 0; }
#ifdef CRAIG
   // recursive clean-up
   return tdm_clear_allR(ds);
#else
   // original pavel's code - up to 20 levels
   l=0; dd[0]=ds; mm[0]=-1;
   for (;ds;)
   {
     if (ds->tid)
...
extern "C" int tdm_clear_allR (DS_DATASET_T  *pDataset)
{
   ...
   else { dsSetTableRowCount(pDataset,0); }
   return 1;
}
...
*                                                                            *
\****************************************************************************/
extern "C" int dsFindTab (DS_DATASET_T **ppEntry, DS_DATASET_T* pDataset,
                                                                    char* spec)
{
    int i,same;
...
/****************************************************************************\
*         ami_call_ is analog of a STAF-standard ami_module_call,            *
*         but it by-passes table type checking against modile idl            *
*                                                                            *
*         amin_module_call is really a STAF-compliant procedure.             *
\****************************************************************************/
...
#endif
{
  ...
  for (i=0;i<*n;i++)
   {  t[i]=tdm->findTable(tables+i*lt);   if (!t[i]) return 0;
      ...
      if (gcflag_>1) printf(" table found %20s \n",h[i]->name);
   }
   status = ami_pamSwitch(myRank, myPamFtn, h, d);
   /* return row counter from table header to staf wrapper */
...
extern "C" void type_of_call ami_module_register_ (char* name, int n)
{
   ...
   if (!ami)  return;   /* staf not started */
