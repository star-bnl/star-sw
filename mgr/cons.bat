...
rem $Id: cons.bat,v 1.1 1999/07/16 17:59:10 fisyak Exp $
...
# $Id: cons.bat,v 1.1 1999/07/16 17:59:10 fisyak Exp $
...
$cons_history =
  q(
    ...
    Add -q flag to be quiet about Installs and Removals. - <rv@fore.com>
    More Robust Consign mechanism. - <knight@baldmt.com>
    InstallAs method to install a file as another - <rv@fore.com>
    Fix Repository to include appropriate library paths. - <rv@fore.com>
    Fix SourcePath to return correct repository path. - <rv@fore.com>
    Win32 Fix to work with generic perl. - <v-ronaar@Exchange.Microsoft.com>
);
...
sub Repository {
	...
	next if ! $d;
	chop $d;
	push(@param::rpath, $dir::top->lookupdir($dir))
	    if -d $d && $d ne $my_dir;
	push @INC, $d;
    }
}
...
Any of the above pseudo variables may be followed immediately by C<:d> or
C<:f>, to indicate the directory or file associated with the
name. Continuing with the above example, C<%<:f> would expand to C<foo bar
baz>, and C<%>:d> would expand to C<test>.
...
The C<cons> command is always invoked from the root of the build tree. A
...
directory). The command is invoked as follows:
