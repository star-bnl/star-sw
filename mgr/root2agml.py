...
def main():
    ...
    #optparser.add_option("--topvolume", help="Renames the top volume, which will be placed w/in the cave",          dest="topvolume",   default="AGML" )
    ...
    ROOT.gROOT.ProcessLine('TGeoManager::Import("%s");'%opts.filename)

    geom = ROOT.gGeoManager
#   top  = geom.GetTopVolume()

    # Rename the top volume
    #top.SetName( opts.topvolume )
    
    ## if ( opts.topvolume != None ):
    ##     top = geom.FindVolumeFast( opts.topvolume )
    ...
    for index,vol in enumerate(list_of_volumes):
        name = vol.GetName()
        ...
        try:
            ...
            dict_of_volume_names[name] = index        
