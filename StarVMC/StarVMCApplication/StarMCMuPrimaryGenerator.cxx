...
StarMCMuPrimaryGenerator::StarMCMuPrimaryGenerator(const Char_t *HbFile, TDataSet *Data) : 
  StarMCPrimaryGenerator(), fMuFileName(HbFile), fMuTree(0), fMuFile(0), 
  fEntry(-1), fnEntries(0), fHbtId(999), fData(Data) 
{
  fMuFile = new TFile(fMuFileName.Data());
  assert(fMuFile);
  cout << "StarMCMuPrimaryGenerator open " << fMuFileName << endl;
  if (Debug()) fMuFile->ls();
}
//_____________________________________________________________________________
void StarMCMuPrimaryGenerator::GeneratePrimary(const TVector3& origin) {     
#if 0
...
void StarMCMuPrimaryGenerator::GeneratePrimaries() {// generate primaries from MuOOK NTuple
  ...
  Int_t toBeDone = 1; 
  if (! fMuTree) {
    fMuTree = (TTree*) fMuFile->Get(Form("h%i",fHbtId));
    assert(fMuTree);
    fMuTree->SetBranchAddress("itrac",&event.itrac);
    fMuTree->SetBranchAddress("istat",&event.p.isthep);
    fMuTree->SetBranchAddress("ipdg" ,&event.p.idhep);
    fMuTree->SetBranchAddress("moth1",&event.p.jmohep[0]);
    fMuTree->SetBranchAddress("moth2",&event.p.jmohep[1]);
    fMuTree->SetBranchAddress("idau1",&event.p.jdahep[0]);
    fMuTree->SetBranchAddress("idau2",&event.p.jdahep[1]);
    fMuTree->SetBranchAddress("Pxyz" ,&event.p.phep[0]);
    fMuTree->SetBranchAddress("ener" ,&event.p.phep[3]);
    fMuTree->SetBranchAddress("mass" ,&event.p.phep[4]);
    fMuTree->SetBranchAddress("Vxyz" ,&event.p.vhep[0]);
    fMuTree->SetBranchAddress("Vtime",&event.p.vhep[3]);
    fnEntries = fMuTree->GetEntriesFast();
    fEntry = 0;
  }
  if (fData) {
  ...
  for (; fEntry<fnEntries;fEntry++) {
    Long64_t ientry = fMuTree->LoadTree(fEntry);
    if (ientry < 0) break;
    nb = fMuTree->GetEntry(fEntry);   nbytes += nb;
    if (Debug()) {
#if 0
	...
	if (Debug())
	  cout << "StarMCMuPrimaryGenerator::GeneratePrimaries\t" << event.p.idhep << " is not in TDatabasePDG. skip it." << endl;
      }
      NPrim++;
