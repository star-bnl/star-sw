...
void AgMLDbMatrix::operator()( const char* table, const int row, double matrix[4][4] )
{
  ...
  if ( ttable )
    {
      LOG_INFO << "  [AgML 2.1] Apply DB Matrix: " << tablename.Data() << " [" << row << "]" << endm;
      //#ifdef VERBOSE
      ...
      matrix[3][2] = 0;
      matrix[3][3] = 1;
      LOG_DEBUG << Form("  [AgML 2.1]  %8.5f %8.5f %8.5f | %8.5f mm",t->r00, t->r01, t->r02,10* t->t0) << endm;
      LOG_DEBUG << Form("  [AgML 2.1]  %8.5f %8.5f %8.5f | %8.5f mm",t->r10, t->r11, t->r12,10* t->t1) << endm;
      LOG_DEBUG << Form("  [AgML 2.1]  %8.5f %8.5f %8.5f | %8.5f mm",t->r20, t->r21, t->r22,10* t->t2) << endm;
    }
  else  // no DB table.  Warn and return 1.
    {
      LOG_WARN << "  [AgML 2.1] Missing DB Matrix: " << tablename.Data() << " [" << row << "] (apply unit matrix)" << endm;
      matrix[0][0] = 1;
      matrix[0][1] = 0;
      matrix[0][2] = 0;
      matrix[0][3] = 0;
