...
my $day0 = Math::BigInt->new(timelocal(0,0,0,31,11,112));
...
my $year = 2013;
if ($year =~ /2013/) {
  $database = "onl15.starp.bnl.gov:3502";
  $database_runlog = "onl15.starp.bnl.gov:3501";
  $database_physics = "onl15.starp.bnl.gov:3606";
}
if ($year =~ /2006/) {
...
if ($dbp) {
  my $query_physicson = "select fill_yellow,unix_timestamp(declaredTime) from monitor where code=520 and fill_yellow>17200";
  ...
  my $query_physicsoff = "select fill_yellow,unix_timestamp(declaredTime) from monitor where code=521 and fill_yellow>17200";
...
while (<IN>) {
	...
	print STDERR "No bunches for $pat, fill $fill.  Taking 100\n";
	$bunchesbyfill{$fill} = 100;
    }
}
...
#my $crosssection = 2.61e9; #ZDC from van der Meer scan, early March 2012
my $crosssection=2.81e9; #ZDC from van der Meer scan, Apr. 8
...
# # #      my $an2 = $n2/(1.-$an);
      ...
      my $nbunches = $bunchesbyfill{$fill};
       unless ($nbunches) {
 	  print STDERR "No bunches for fill $fill.  Taking 100.\n";
 	  $nbunches = 100.;
       }
...
#	  $crosssection=5.48e7;
#      }
# Deal with difference of rich and scalers
#      $singlescorr *= (0.997-2.767e-7*$n12+1.996e-13*$n12*$n12);
...
# Kludge: tweak by TCU Live/live
  ...
  printf HTML qq(<tr><td> after correction by average TCULive/Live: %5.3f </td></tr> \n),$totlivefrac/($sumsamp/$totalon);
