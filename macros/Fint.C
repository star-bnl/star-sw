/* $Id: Fint.C,v 1.3 2012/08/23 19:16:00 fisyak Exp $ 
   $Log: Fint.C,v $
...
//________________________________________________________________________________
TVector3 Fint (Int_t narg, Double_t *arg, Int_t *nent, Double_t *ent, TVector3 *table) {
  if (narg < 1 || narg > 5) return TVector3();
  Int_t index[32];
  Double_t weight[32];
  Int_t knots = FintKnots(narg,arg,nent,ent,index,weight);
  TVector3 val(0,0,0);
