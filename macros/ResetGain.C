...
void ResetGain(const Char_t *tabNam = "Calibrations/tpc/tpcGain",Int_t date = 20050312, Int_t time = 0){ 
  if (dbMk == 0) Load();
  dbMk->SetDebug(2);
  ...
  if (table) {
    ...
    cout << "==============================================" << endl;
    St_tpcGain *tpcGain = ((St_tpcGain *)table);
    tpcGain_st *g = tpcGain->GetTable();
    int row = 0;
    for (int sector = 0; sector < Nrows; sector++,g++) {
      memset(g->Gain[row],0,182*sizeof(Float_t));
    }
    if (Nrows > 10) Nrows = 10;
    table->Print(0,Nrows);
    TString name(gSystem->BaseName(tabNam));
    name += Form(".%8i.%06i.new.root",t[0].GetDate(),t[0].GetTime());
