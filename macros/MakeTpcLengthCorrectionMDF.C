...
/*
  ...
  root.exe TPoints*UG*.root  MakeTpcLengthCorrectionMDF.C
 ...
 dir TpcZCorrectionB.20*.C | grep GeV | grep fixed | sed 's/TpcZCorrectionB/TpcLengthCorrectionMDF/g' | awk '{print "ln -s TpcLengthCorrectionMDF.FXT.C "$9}'
 dir TpcZCorrectionB.20*.C | grep GeV | grep -v fixed | sed 's/TpcZCorrectionB/TpcLengthCorrectionMDF/g' | awk '{print "ln -s TpcLengthCorrectionMDF.COL.C "$9}'
 */
#ifndef __CINT__
#include <stdlib.h>
#include <string.h>
...
void  MakeTpcLengthCorrectionMDF(Int_t date = 0, Int_t time = 0){
  TFile *fIn[3] = {0};
  ...
  for (Int_t l = 0; l < 3; l++) {
	...
	if (h) {
	  //	  out << "  //" << gDirectory->GetName() << "; MakeTpcLengthCorrectionMDF1(\"" << histN[m] << "\",5,1,20);" << endl;
	  //	  MakeTpcLengthCorrectionMDF1(histN[m],  5,type,20); 
	  h2mdf(histN[m],  5, 1,20); 
	  PrintRow();
	}
      }
