// $Id: EveDis.C,v 1.1 2011/10/07 21:41:44 fisyak Exp $
...
void AddHits(const StEvent *event,EStuDraw3DEvent trackHitsOnly, StTrackType type) {
   ...
   if (trackHitsOnly != kUnusedHitsOnly) {
      ...
      if (type == tpt) {// Seed_t 
	vector<Seed_t> &seeds = *StiTpcSeedFinder::GetSeeds();
	UInt_t NS = seeds.size();
	for (UInt_t s  = 0; s < NS; s++) {
	  Seed_t &aSeed = *&seeds[s];
	  UInt_t NH = aSeed.vhit.size();
	  std::vector<float> hitPoints;
	  for (UInt_t ih = 0; ih < NH; ih++) {
	    SeedHit_t *vhit = aSeed.vhit[ih];
	    StiHit *ht = vhit->hit;
	    if (! ht) continue;
	    const StMeasuredPoint *shit = ht->stHit();
	    if (! shit) continue;
	    hitPoints.push_back( shit->position().x());
	    hitPoints.push_back( shit->position().y());
	    hitPoints.push_back( shit->position().z());
	  }
	  std::vector<float>::iterator xyz = hitPoints.begin();
	  Color_t trackColor = 1;	  
	  Points(hitPoints.size()/3,&*xyz,trackColor,sty,siz);
	  if (trackHitsOnly == kUsedHits) SetModel(0);
	}
      } else {
