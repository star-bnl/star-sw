...
void ppbarM2() {
  ...
  m2p->SetStats(0);
  m2p->Fit("pol1");
  TF1 *pol1 = (TF1*) m2p->GetListOfFunctions()->FindObject("pol1");
  if (pol1) l->AddEntry(m2p,Form("p  : %6.4f + %6.4f log_{10}P",pol1->GetParameter(0), pol1->GetParameter(1)));
  else      l->AddEntry(m2p,"p");
#if 1
  ...
  m2pbar->SetStats(0);
  m2pbar->Fit("pol1");
  pol1 = (TF1*) m2pbar->GetListOfFunctions()->FindObject("pol1");
  if (pol1) l->AddEntry(m2pbar,Form("pbar: %6.4f + %6.4f log_{10}P",pol1->GetParameter(0), pol1->GetParameter(1)));
  else      l->AddEntry(m2pbar,"pbar");
