...
void pratio(Int_t mult1=1, Int_t mult2=9,
	    Int_t   iy1=7, Int_t iy2=8, // |eta| < 0.6
...
// 	  Double_t cont   = Csum + CKpT;
// 	  Double_t EK     = EKpT*EKpT;
// 	  Double_t EP     = 0;
  ...
  for (int i=1; i<=nx; i++) {
    Double_t sN = sumN->GetBinContent(i);
    Double_t sP = sumP->GetBinContent(i);
    Double_t r =  sN/sP;
    Double_t eP = sumP->GetBinError(i)/sP;
    Double_t eN = sumN->GetBinError(i)/sN;
    cout << i << "\tsN:" << sN << "+/-" << eN << "\tsP:" << sP << "+/-" << eP;
    eP *= eP;  
    eN *= eN;
  ...
  c1->SetLogx(1);
  TF1 *pol = new TF1("pol","pol1",0.4,0.8);
