...
using namespace std;
#include "Fint.C"
Double_t fint(Int_t *narg, Float_t *arg,Int_t *nent,Float_t *ent, Float_t *table) {
#if 1
  return Fint(*narg,arg,nent,ent,table);
#else
...
/* Subroutine */ 
Bool_t e104p(Int_t *narg, Float_t *arg, Int_t *nent, Float_t *
	ent, Float_t *table, Float_t *tref)
{
    /* Initialized data */
    ...
    /* Format strings */
    //    static char fmt_10[] = "(/\002 *** ARITHMETIC ERROR ***\002,2i8,1x,e12.3"	    ",3x,5e12.3)";
    ...
    /* Builtin functions */
    //    Int_t i_nint(Float_t *), s_wsfe(cilist *), do_fio(Int_t *, char *,     ftnlen), e_wsfe(void);
    ...
    static Float_t test, relpr = 1e-7;
...
/* #include "kernnumt/sysdat.inc" */
    ...
    res = (r__1 = *tref - test, TMath::Abs(r__1));
    r__1 = res / relpr;
    cout << "args[";
    for (int i = 1; i <= *narg; i++) {cout << arg[i]; if (i < *narg) cout << ","; else cout << "] = ";}
    cout << "test = " << test << " tref = " << *tref << " res = " << res << " r__1 = " << r__1 << endl;
    irel = TMath::Nint(r__1);
    Bool_t ok = irel <= margin;
    if (! (ok)) {
#if 0
	s_wsfe(&io___46);
...
/* L110: */
		    ...
		    okt = e104p(&narg, arg, nent, &ent[1], &table[1], &tref);
		    ok = ok && okt;
		    i__2 = ny;
		    for (ky = 1; ky <= i__2; ++ky) {
			narg = 2;
