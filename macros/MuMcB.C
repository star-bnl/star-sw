/* 
   root.exe lMuDst.C MuMcB.C+
   root.exe lMuDst.C MuMcB.root
>> .L MuMcB.C+;  Init(0); DrawEff(); DrawQA(); // FPE_OFF
*/
//#define __DEVT__ 
...
#endif
/* 
   Structure: histsT[gp][type][particle][pm][x][i] => 
    Tracks:
      Global:
                    Mc: k        - All Mc tracks
                        All : p
			      (+)/(-) :s => histsT[gp][t][p][pm][x][i]   x = 0 vs No. fit and No. bad points, x = 1 vs Eta and pT  
                                                                         i variable type
			Pion
                    McHit       - Reconstractable Mc Tracks
                    Reco     - 
		    Clone
                    Ghost
                    Lost
...
enum TrackMatchType {
  ...
  kAll = 0, kPion, kPartypeT,                                             // switch between All and pion
  kPositive = 0, kNegative, kTotalSigns,                                 // switch between charges
  kTotalQA = 15, kTotalQAll,                                             // no. of plots for Global and Primary tracks
...
TH2F *McRcHit = 0;
TFile *fOut = 0;
TH3F *histsT[kTotalT][kTotalMatchType][kPartypeT][kTotalSigns][kVariables][kTotalQAll] = {0}; // [charge][
...
void Init(const Char_t *outfName="Out.root") {
  ...
  for (Int_t tv = 0; tv < 1; tv++) {
    if (tv != 0) conitnue; // Tracks
    const Char_t *TitleTrType[kTotalT] = {"Global Tracks", "Primary Tracks"};
    for (Int_t gp = kGlobal; k < kTotalT; k++) {
      const  PlotName_t plotNameMatch[kTotalMatchType] = {
	{kMcTk,    "Mc",    "Mc tracks All"},
	{kMcHitTk, "Hit",   Form("Mc tracks which have >= %i Mc Hits",MinNoMcTpcHits)},
	...
	for (Int_t p = 0; p < kPartypeT; p++) {
	  ...
	  for (Int_t pm = kPositive; pm < kTotalSigns; pm++) {
	    ...
	    for (Int_t x = 0; x < kVariables; x++) {
	      //                /Global Tracks/Mc/All/(+)/NoHits
	      TString dir(Form("/%s/%s/%s/%s/%s",TitleTrType[gp],plotNameMatch[type].Name,ParticleType[p],TitleCharge[s],VarSet[x]));
	      const VarName_t plotVar[kTotalQA] = {           //no.fit                      no.bad          
		...
		{"Phi",       "Phi",                                 0,   0.           0,  0,   0,    0,   0,   0,  120,  -180,   180, 1}
	      };
		...
		if (opt) {
		  gDirectory->cd(dir);
		  histsT[gp][type][particle][pm][i] = gDirectory->Get(plotVar[i]);
		  continue;
		}
		...
		if (i != kTotalQA) {
		  if (! x) {// No.Hits
		    histsT[gp][type][particle][pm][x][i] = new TH3F(plotGlVar[i].Name,
								    Form("%s for %s %s %s %s %s", plotVar[i].Title,
									 TitleTrType[gp],plotNameMatch[type].Name,ParticleType[p],TitleCharge[s],VarSet[x]),
								    noFit-9, 9.5, noFit + 0.5, 11,-0.5, 10.5,
								    plotGlVar[i].nz, plotGlVar[i].zmin, plotGlVar[i].zmax);
		    histsT[gp][type][particle][pm][x][i]->GetXaxis()->SetTitle("No. of Fit Points");
		    histsT[gp][type][particle][pm][x][i]->GetYaxis()->SetTitle("No. of Bad Points");
		  } else {
		    ...
		    histsT[gp][type][particle][pm][x][i]->GetXaxis()->SetTitle("  #eta");
		    histsT[gp][type][particle][pm][x][i]->GetYaxis()->SetTitle("pT/|q| (GeV/c)");
		    delete [] zBins;
		  }
		  histsT[gp][type][particle][pm][x][i]->GetZaxis()->SetTitle(plotGlVar[i].Title);
		  histsT[gp][type][particle][pm][x][i]->SetMarkerColor(pm+1); 
		  histsT[gp][type][particle][pm][x][i]->SetLineColor(pm+1); 
		}
	      }
	    }
	  }
...
void FillQAGl(const StMuTrack *gTrack = 0, const StMuMcTrack *mcTrack = 0, const StDcaGeometry *dcaG = 0, const StMuMcVertex *mcVertex = 0) {
  ...
  for (Int_t i = 0; i < kTotalQAGl; i++) {
    histsQA[s][kGlobal][i]->Fill(gTrack->nHitsFit(), gTrack->nHitsFit()*(100.-gTrack->qaTruth())/100., x[i]);
    histsK[0][s][kGlobal][i]->Fill(mcTrack->Pxyz().pseudoRapidity(), mcTrack->Pxyz().perp(), x[i]);
    if (mcTrack->GePid() == 8 || mcTrack->GePid() == 9)
      histsK[1][s][kGlobal][i]->Fill(mcTrack->Pxyz().pseudoRapidity(), mcTrack->Pxyz().perp(), x[i]);
  }
...
void FillQAPr(const StMuTrack *pTrack = 0, const StMuMcTrack *mcTrack = 0, const StMuPrimaryTrackCovariance *cov = 0) 
{
  ...
  for (Int_t i = 0; i < kTotalQAPr; i++) {
    histsQA[s][kPrimary][i]->Fill(pTrack->nHitsFit(), pTrack->nHitsFit()*(100.-pTrack->qaTruth())/100., x[i]);
    histsK[0][s][kPrimary][i]->Fill(mcTrack->Pxyz().pseudoRapidity(), mcTrack->Pxyz().perp(), x[i]);
    if (mcTrack->GePid() == 8 || mcTrack->GePid() == 9)
      histsK[1][s][kPrimary][i]->Fill(mcTrack->Pxyz().pseudoRapidity(), mcTrack->Pxyz().perp(), x[i]);
  }
}
//________________________________________________________________________________
...
void DrawQA(Int_t kk = -1, Int_t ii = -1, Int_t ll = -2) {// versus Nhits
  ...
  for (Int_t k = k1; k < k2; k++) {
    Int_t N = kTotalQAGl;
    if (k == kPrimary) N = kTotalQAPr;
    cout << "k = " << k << "\t" << TitleTrType[k] << endl;
    Int_t i1 = 0, i2 = N;
    if (ii >= 0) {i1 = ii; i2 = ii + 1;}
    for (Int_t i = i1; i < i2; i++) {
      cout << "i = " << i << "\t";
      if (k == kPrimary) cout << plotPrVar[i].Name;
...
void DrawEff(Double_t ymax=1.0, Double_t pTmin = -1, Int_t animate=0) {// Efficiencies
  ...
  for (Int_t k = kGlobal; k < kTotalT; k++) {
...
//________________________________________________________________________________
void MuMcB(Long64_t nevent = 999999,
  ...
  for (Long64_t ev = 0; ev < nevent; ev++) {
    ...
    map<Int_t,Int_t> &Gl2Pr = mu->IdGlobalId2IdPrimaryTrack(); // map global to primary track Ids from vertex with idTruth == 1
    multimap<Int_t,Int_t> &Mc2RcTracks = mu->IdMc2IdRcTracks(); // map between global and Mc tracks from primary Mc vertex
    // =============  Build map between  Rc and Mc vertices 
    multimap<Int_t,Int_t> &Mc2RcVertices = mu->Id2RcVxR(); // Reconstructable !
    for (Int_t l = 0; l < mu->numberOfPrimaryVertices(); l++) {
    ...
    for (Int_t m = 0; m < NoMuMcTracks; m++) {
      StMuMcTrack *mcTrack = mu->MCtrack(m);
      if (! mcTrack) continue;
	...
	if (! idMcTrack) break;
	StMuMcTrack *mcTrackP = mu->MCtrack(idMcTrack-1);
	IdVx = mcTrackP->IdVx();
	if (! IdVx) break;
      }
      if (IdVx != 1) continue;
	...
	if (mcTrack->No_tpc_hit() >= MinNoMcTpcHits) GiD[3]->Fill(mcTrack->GePid());
      ...
      TrackMatchType s = kPositive;
      if (mcTrack->Charge() < 0) s = kNegative;
      hists[s][kGlobal][kMcTk]->Fill(TMath::RadToDeg()*mcTrack->Pxyz().phi(), mcTrack->Pxyz().pseudoRapidity(),mcTrack->Pxyz().perp());
      hists[s][kPrimary][kMcTk]->Fill(TMath::RadToDeg()*mcTrack->Pxyz().phi(), mcTrack->Pxyz().pseudoRapidity(),mcTrack->Pxyz().perp());
      if (mcTrack->No_tpc_hit() < MinNoMcTpcHits) continue; 
      hists[s][kGlobal][kMcHitTk]->Fill(TMath::RadToDeg()*mcTrack->Pxyz().phi(), mcTrack->Pxyz().pseudoRapidity(),mcTrack->Pxyz().perp());
      if (IdVx == 1) hists[s][kPrimary][kMcHitTk]->Fill(TMath::RadToDeg()*mcTrack->Pxyz().phi(), mcTrack->Pxyz().pseudoRapidity(),mcTrack->Pxyz().perp());
      TrackMcType kType = TrackType(mcTrack,Mc2RcTracks);
      Int_t l = kMcHitTk;
      switch (kType) {
      case kRecoTk: l = kRecoTk  ; break;
      case kLostTk:    l = kLostTk ; break;
      case kCloneTk:   l = kCloneTk; break;
      default:       l = kMcHitTk;   break;
      }
      hists[s][kGlobal][l]->Fill(TMath::RadToDeg()*mcTrack->Pxyz().phi(), mcTrack->Pxyz().pseudoRapidity(),mcTrack->Pxyz().perp());
      hists[s][kPrimary][l]->Fill(TMath::RadToDeg()*mcTrack->Pxyz().phi(), mcTrack->Pxyz().pseudoRapidity(),mcTrack->Pxyz().perp());
      if (l == kRecoTk) {
	Int_t Id = mcTrack->Id()-1;
    ...
    // check for ghosts
    for (Int_t kg = 0; kg < mu->numberOfGlobalTracks(); kg++) {
      StMuTrack *gTrack = mu->globalTracks(kg);
      if ( ! AcceptGhost(gTrack)) continue;
      if ( gTrack->idTruth()) continue;
      TrackMatchType s = kPositive;
      if (gTrack->charge() < 0) s = kNegative;
      hists[s][kGlobal][kGhostTk]->Fill(TMath::RadToDeg()*gTrack->phi(),gTrack->eta(),(gTrack->charge()*gTrack->pt()));
    }
    for (Int_t l = 0; l < mu->numberOfPrimaryVertices(); l++) {
      StMuPrimaryVertex *Vtx = (StMuPrimaryVertex *) PrimaryVertices->UncheckedAt(l);
	...
	hists[s][kPrimary][kGhostTk]->Fill(TMath::RadToDeg()*pTrack->phi(),pTrack->eta(),pTrack->pt());
      }
    }
#if 0
	...
	if (particle) cout << "Parent Track:" << *particle << endl;
      ...
      if (m) {
	StMuMcTrack *mcTrack = mu->MCtrack(m-1);
	if (! mcTrack) continue;
    ...
    for (Int_t k = 0; k < NoKFTracks; k++) {
      ...
      if (particle->IdTruth()) {
	StMuMcTrack *mcTrack = mu->MCtrack(particle->IdTruth()-1);
