...
/*
  root.exe -q -b xyPad3C*.root Chain.C 'MakeTpcPadCorrectionMDFitP.C+(tChain,7,20,20210129,5)' >& MakeTpcPadCorrectionMDF.log
 */
#ifndef __CINT__
#include <stdlib.h>
#include <string.h>
...
//by  b_branchname->GetEntry(ientry); //read only this branch
  ...
  fit->SetName(Form("MDF_%i_%i",sector,IO));
...
void MakeTpcPadCorrectionMDFitP(TChain *tChain, Int_t max=7, Int_t maxTerm = 20, Int_t date = 20190201, Int_t time = 709){
  if (! tChain) return;
  FitP t(tChain);
  TFile *fOut = new TFile("MakeTpcPadCorrectionMDFitP.root","recreae");
  TString cOut =  Form("TpcPadCorrectionMDF.%8i.%06i.C",date,time);
