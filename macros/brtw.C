...
Double_t total(Double_t *x, Double_t *par) {
  ...
  if (pq > 0) {
	...
	if (L > -1) {
	  L = s;
	  signal += TMath::Exp(sL)*BreitWignerF(x[0],par[3*s+1],par[3*s+2]);
	} else {
    ...
    if (par[3*NoSignals] > -90) {
      ...
      Double_t bglog = par[3*NoSignals]*TMath::Log(pq) + pq*sum;
...
void brtw(TH1F *hist, Double_t MMin=0.3, Double_t MMax = 1.3, Double_t m1 = mpi, Double_t m2 = mpi, Int_t l = 0, Bool_t baryon = kFALSE) {
  ...
  for (Int_t k = 0; k < 3; k++) {// total, signal, background
    f[k] = (TF1 *) gROOT->GetListOfFunctions()->FindObject(FuncNames[k]);
    if (f[k]) continue;
    f[k] = new TF1(FuncNames[k],total,MMin,MMax, NoParameters);
    f[k]->SetNpx(400);
    if (k == 0) { // Total
      for (Int_t s = 0; s < NoSignals; s++) {
  ...
  draw = kTRUE;
#if 0
  TH1F *h = SubstracF(hist,Total,"r");
  cFit->cd(2);
  h->Draw();
#endif
  Total->GetParameters(params);
  Signal->SetParameters(params);
  Signal->SetParameter(3*NoSignals,-99);
  Signal->Draw("same");
  if (savepad) savepad->cd();
}
//________________________________________________________________________________
void K0BW(const Char_t *histN = "/Particles/KFParticlesFinder/Particles/Ks/Parameters") {
