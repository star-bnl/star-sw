...
//________________________________________________________________________________
#ifndef __StvPull__
void Pulls(ULong64_t NoEvents = 99999999, const Char_t *files = "./MuDst/*tags.root", Int_t opt=0) 
#else  /* __StvPull__ */
void PullsV(ULong64_t NoEvents = 99999999, const Char_t *files = "./MuDst/*tags.root", Int_t opt=0) 
#endif /* __StvPull__ */
{
#ifndef __StvPull__
...
//________________________________________________________________________________
void DrawPulls(const Char_t *histName="pullZPNvsZR", const Char_t *projection = "zx", Double_t ymin = 1.2, Double_t ymax = -0.2) {
  ...
  while ( (f = (TFile *) next()) ) { 
    ...
    TH2 *h2 = (TH2 *) f->Get(Form("%s_%s",hist->GetName(),projection));
    if (! h2) h2 = (TH2 *) hist->Project3D(projection);
    if (nf == 1) {c1->SetName(h2->GetName()); c1->SetTitle(h2->GetTitle());}
    h2->SetStats(0);
    if (ymin < ymax) {
      TH1F *frame = pad->DrawFrame(h2->GetXaxis()->GetXmin(), TMath::Max(ymin,h2->GetYaxis()->GetXmin()), 
				   h2->GetXaxis()->GetXmax(), TMath::Min(ymax,h2->GetYaxis()->GetXmax()));
      frame->SetTitle(h2->GetTitle());
      frame->SetXTitle(h2->GetXaxis()->GetTitle());
      frame->SetYTitle(h2->GetYaxis()->GetTitle());
      h2->Draw("colzsame");
    } else {
      h2->Draw("colz");
    }
    ...
    if (! mu) {
      h2->FitSlicesY();
      mu = (TH1 *) f->Get(Form("%s_1",h2->GetName()));
    }
    if (mu) mu->Draw("same");
