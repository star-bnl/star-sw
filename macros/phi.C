...
#endif
TFile  *outfile = 0;
void phi(Int_t nevents=10000, TChain* fChain=0)
{
  if (!fChain) return;
  ...
  for (Int_t jentry=0; jentry<nentries;jentry++) {
	...
	pT = pl4.Pt();
	hists[0][m]->Fill(y,pT,M);
	hists[0][10]->Fill(y,pT,M);
	if (pTrack->PidFitdEdxLength() < 60. ||
	    lTrack->PidFitdEdxLength() < 60.
	    ) continue;
	if (pTrack->PidKaon() < 20.0) continue; 
	Int_t ok2 = frac->Set(pPicoEvent,lTrack);
	if (lTrack->PidKaon() < 20.0) continue;
	hists[1][m]->Fill(y,pT,M);
	hists[1][10]->Fill(y,pT,M);
      } 
    }
