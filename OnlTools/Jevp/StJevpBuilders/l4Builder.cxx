...
void l4Builder::initialize(int argc, char *argv[])
{
  ...
  sprintf(dEdxMeanFiles[0],"%s/OnlTools/Jevp/StJevpBuilders/dedx/dedx_mean_Electron",Currentdir);
  sprintf(dEdxMeanFiles[1],"%s/OnlTools/Jevp/StJevpBuilders/dedx/dedx_mean_Pion",Currentdir);
  sprintf(dEdxMeanFiles[2],"%s/OnlTools/Jevp/StJevpBuilders/dedx/dedx_mean_Kaon",Currentdir);
  sprintf(dEdxMeanFiles[3],"%s/OnlTools/Jevp/StJevpBuilders/dedx/dedx_mean_Proton",Currentdir);
  sprintf(dEdxMeanFiles[4],"%s/OnlTools/Jevp/StJevpBuilders/dedx/dedx_mean_Deuteron",Currentdir);
  sprintf(dEdxMeanFiles[5],"%s/OnlTools/Jevp/StJevpBuilders/dedx/dedx_mean_Triton",Currentdir);
  sprintf(dEdxMeanFiles[6],"%s/OnlTools/Jevp/StJevpBuilders/dedx/dedx_mean_He3",Currentdir);
  sprintf(dEdxMeanFiles[7],"%s/OnlTools/Jevp/StJevpBuilders/dedx/dedx_mean_He4",Currentdir);
  inputDedx();

  ifstream inData;
  string paraName;
  inData.open("./inputPara.dat");
  if(inData.good()) {
    while(!inData.eof()) {
      inData>>paraName;
  ...
  if(switch_upc == 1) cout<<"HltPlots(include UPC) OK."<<endl;
  else cout<<"HltPlots(no UPC) OK."<<endl;
  ...
  fTheoDedx_K_pos = new TF1("TheoDedx_K_pos", funcDedx_K_pos, 0.05, 5., 0);
  fTheoDedx_K_neg = new TF1("TheoDedx_K_neg", funcDedx_K_neg, -5., -0.05, 0);
...
void l4Builder::startrun(daqReader *rdr) 
{
  ...
  runnumber = rdr->run;
  if (switch_upc==1) for(int i=0;i<98;i++) getPlotByIndex(i)->getHisto(0)->histo->Reset();
  else for(int i=0;i<62;i++) getPlotByIndex(i)->getHisto(0)->histo->Reset();
...
//   sprintf(num,"%i",runnumber);
  ...
  for(int i=0; i<20000; i++) {
    sprintf(inum, "./output/%i.dat", tmpRunNum);
...
//   hHFM_dEdx_UPC->Write();
//   hDiElectronInvMassFullRange_UPC->Write();
//   hDiElectronInvMassFullRangeBG_UPC->Write();
//   hDiMuonInvMassFullRange->Write();
//   hDiMuonInvMassFullRangeBG->Write();
//   hDiMuonInvMassTpxCut->Write();
//   hDiMuonInvMassTpxCutBG->Write();
//   hDiPionInvMassFullRange->Write();
...
//   //***********************************************************************************************  
  ...
  for(u_int i=0 ; i < hlt_gt->nGlobalTracks ; i++) {
	...
	if(daqID & upc){
	  hdEdx_UPC->Fill(p*q,dedx) ; // fhor HF reference
	}
      }
    }//loop of tracks
...
//     cout<<Daughter1dedx<<endl;
    ...
    if(Daughter2EmcSN >= 0) {
      ...
      Daughter2_PE_ratio = Daughter2p/Daughter2TowerEnergy ; 
      Daughter2_EP_ratio = Daughter2TowerEnergy/Daughter2p  ;
//       cout<<"Daughter2_EP_ratio: "<<Daughter2_EP_ratio<<endl;
...
//   addPlot(HltPlots[index]);
  ...
  //	HltPlots[index]->optstat = 0;
  //	HltPlots[index]->setDrawOpts("colz");
  //	hTofprimaryMult = new TH2F("BB_TofMult_primaryMult","BB_TofMult_primaryMult",2200,0,2200,2200,0,2200);
  //	ph = new PlotHisto();
  //	ph->histo = hTofprimaryMult;
  //	HltPlots[index]->addHisto(ph);
  //	addPlot(HltPlots[index]);
