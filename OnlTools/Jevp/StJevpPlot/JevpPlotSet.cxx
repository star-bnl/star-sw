...
int JevpPlotSet::addPlot(JevpPlot *hist)
{
  ...
  hist->GetPlotName();   // Needed because it sets the plotname and thus the hash as well!
  PCP;
...
{
    PCP;
    JevpPlot *p = (JevpPlot *)plots.FindObject(name);
    PCP;

    //LOG("JEFF", "[%s] name: %s plot: %p", getPlotSetName(), name, p);
...
void JevpPlotSet::_event(daqReader *rdr)
{
    ...
    double tm = processingTimer->record_time();
    processingTime += tm;
