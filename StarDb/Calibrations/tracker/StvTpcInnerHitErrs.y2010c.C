...
memset(&row,0,tableSet->GetRowSize());
    row.yErr	 = 0.0006155811; // Intrinsic resolution, padrow or Y direction		;
    row.zErr	 = 0.004761928; // Intrinsic resolution, z direction			;
    row.thkDet	 =     1.3225; // detector thickness , not fitted			;
    row.yyDiff	 = 0.004700078; // Diffusion in XY direction				;
    row.zzDiff	 = 0.005376144; // Diffusion in Z direction  				;
    row.yFact	 =  0.5308722; // Error factor in Y-direction 			;
    row.zFact	 =  0.5356543; // Error factor in Z-direction 			;
