...
TDataSet *CreateTable() { 
  ...
  row.tauCI                 =   0;  
  row.tauCO                 =   0;  
  row.SigmaJitterTI         = 1.05*0.38; // 0.0; //0.52; //1.5*0.4317/4.5;// 0.4317;// 0.25;//ad  0.0;// b for Tpx inner 
  row.SigmaJitterTO         = 1.05*0.39; // 0.0; //0.52; //1.5*0.4300/5.;// 0.4300;// E: 0.4801;//0.25;//ad  0.0;// b for Tpx outer 
  row.SigmaJitterXI         = 0.0; //0.1027785/2.;// 0.1027785; // P: 0.1353*1.05/1.10; //O: 0.1353*1.05;// N: 0.1353; // C:0.;
  row.SigmaJitterXO         = 0.0; //0.107525/2.; // 0.107525;  // P: 0.1472*1.05/1.03; //O: 0.1472*1.05;// N: 0.1472; // C:0.;
  ...
  row.transverseDiffusion   = 0.987*0.02218*TMath::Sqrt(1 + row.OmegaTau*row.OmegaTau) ; // Magboltz
  ...
  row.T0offsetI  =  0.0803082                      -0.145027 + 5.16612524755828417e-02; // ~/work/Tpc/TpcRS/2019/TpcRS_19GeV; ((TF1 *) TI_1->GetListOfFunctions()->FindObject("pol2"))->GetParameter(0)
  row.T0offsetO  = 0.5*(-6.04314e-02-3.44168e-02)-0.00930199 + 5.77291104065420849e-02;// ((TF1 *) TO_1->GetListOfFunctions()->FindObject("pol2"))->GetParameter(0)
