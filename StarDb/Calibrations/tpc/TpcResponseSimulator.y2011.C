// $Id: TpcResponseSimulator.y2011.C,v 1.3 2011/10/11 19:09:23 fisyak Exp $
...
TDataSet *CreateTable() { 
  ...
  row.SigmaJitterTO         = 0.4801;//0.25;//ad  0.0;// b for Tpx outer 
  row.SigmaJitterXI         = 0.1353; // C:0.;
  row.SigmaJitterXO         = 0.1472; // C:0.;
  row.longitudinalDiffusion = 0.0370;// 0.0232;//K  0.0370;J // cm/sqrt(cm)   ; // 0.0232; from Laser Fit
  row.transverseDiffusion   = 0.06336;//cm/sqrt(cm) TpcRS_2011_pp500LowLum_B *1.1585; 
...
#else
    /* WE */  2.39250e-01, -3.33329e-03,  // Inner _B
    /* WE */  1.50611e-01,  7.26005e-04   // Outer
#endif
  };
  ...
  row.T0offset   = 0.50 + 1.65431e-01 -  3.45247e-01; // TpcT->Draw("fRcHit.mMcl_t/64-3.0*0.055*Frequency-fMcHit.mMcl_t:fMcHit.mPosition.mX3>>T(210,-210,210,100,-2,3)","fNoMcHit==1&&fNoRcHit==1&&fRcHit.mQuality>90","colz")
  tableSet->AddAt(&row);
  // ----------------- end of code ---------------
