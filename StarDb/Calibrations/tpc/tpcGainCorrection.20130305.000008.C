TDataSet *CreateTable() { 
  ...
  row.nrows      =        nrows; // VoltageCGFRunXIII08pp500p1
  row.idx        =            1; //  FitP->Draw("mu:y-1390>>O(6,-110,10)","(i&&j&&i>13&&abs(mu)<0.4)/(dmu*dmu)","profg")
  row.npar       =            5; // 
  row.min        =         -120;
  row.max        =          100;
  row.a[0]       = TMath::Log(1310)       +3.94933e-04-6.94781e-03; // from tsspar gain at nominal 1390 V + corrections
  row.a[1]       = 10.211e-3 -7.77617e-04 +3.68859e-05-6.26536e-04; //
  row.a[2]       =            9.01092e-06 -2.70508e-06-6.68034e-06; //
  row.a[3]       =			  -6.01231e-08; //
  row.a[4]       =			  -2.15159e-10; //
