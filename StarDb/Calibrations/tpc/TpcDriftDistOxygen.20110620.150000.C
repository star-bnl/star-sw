TDataSet *CreateTable() { 
  ...
  row.a[0]	 =  1.97223e-02; // -6.2709e-03;//  3.58294622853787268e+03; // mean 
  row.a[1]	 = -5.44731e-06; //  1.7502e-6;// slope from Blair   1/( O2 in ppm., cm ) 
  row.a[2]       =  2.68906e-10; // 
  tableSet->AddAt(&row);
  memset(&row,0,tableSet->GetRowSize());
  row.npar       =      3;       // 
  row.a[0]	 =  8.07956e-02; //  4.20540160413917147e+03;// Inner
  row.a[1]	 = -1.83671e-05; // slope from Blair   1/( O2 in ppm., cm ) 
  row.a[2]       =  7.54449e-10; 
  tableSet->AddAt(&row);
  // ----------------- end of code ---------------
