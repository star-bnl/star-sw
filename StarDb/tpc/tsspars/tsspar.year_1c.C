...
memset(&row,0,tableSet->GetRowSize());
 memcpy(&row.fileout,"\x04\x08o\xfc\x00",5);// output file for pixel data (none->table) 
    ...
    row.gain_in	 =       2540; // gas gain:inner sector (dimensionless) ;
    row.gain_out	 =       1160; // gas gain:outer sector (dimensionless) ;
    ...
    row.threshold	 =          4; // adc threshold (adc counts but a real num ;
    ...
    row.wire_coupling_in	 =        0.5; // wire-to-pad coupling inner sector ;
    row.wire_coupling_out	 =        0.5; // wire-to-pad coupling outer sector ;
