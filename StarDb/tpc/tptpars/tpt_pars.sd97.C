...
memset(&row,0,tableSet->GetRowSize());
    ...
    row.nhol	 =          3; // allowed gap size ;
    row.nzslic	 =         20; // # of slices( even) along the z axis. ;
    row.pass_on	 =          1; // Pass on or off ;
    row.delrlm	 =          5; // limit on the extrapolation in bend plane ;
    row.delzlm	 =          2; // limit on the extrapolation ;
    row.drlm1	 =          1; // tolerance on the 1 step in the bend plan ;
    row.drmax	 =          2; // max road width in x,y ;
    row.drplim	 =          1; // Limit on the linear extrap. in bend plan ;
    row.drstr	 =        0.8; // road width in st dev  in the bend plane ;
    row.dzlm1	 =        0.4; // tolerance on the first step ;
    row.dzmax	 =          2; // maximum road width in z ;
    row.dzplim	 =          3; // Limit for linear extrapolation in z ;
    row.dzstr	 =        0.8; // road width in st dev in z ;
    row.mxweig	 =         10; // max # of hits/channel for the starter ;
    row.prob[0]	 =         10; // probability cut ;
    row.prob[1]	 =         10;
...
memset(&row,0,tableSet->GetRowSize());
    ...
    row.irow_seg_end	 =          1; // row to end segment formation (no begin) ;
    row.mctest	 =          0; // switch for the MC testing ;
    row.merge	 =          1; // Switch for segment merging ;
    row.minfit	 =          4; // minimum number of polongs on a track ;
    row.mxtry	 =          4; // max # of attempts to fit ;
    row.nhol	 =          5; // allowed gap size ;
    row.nzslic	 =          4; // # of slices( even) along the z axis. ;
    row.pass_on	 =          0; // Pass on or off ;
